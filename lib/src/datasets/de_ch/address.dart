import 'package:imitatio/src/datasets/models/address.dart';

class DeChAddressData extends AddressDataset {
  @override
  List<String> get cities {
    return const [
      "Aarau",
      "Aarberg",
      "Aarburg",
      "Adliswil",
      "Aesch",
      "Affoltern am Albis",
      "Agno",
      "Aigle",
      "Allschwil",
      "Altdorf",
      "Altstätten",
      "Amriswil",
      "Appenzell",
      "Arbon",
      "Arlesheim",
      "Arosa",
      "Arth",
      "Ascona",
      "Aubonne",
      "Avenches",
      "Baar",
      "Bad Zurzach",
      "Baden",
      "Basel",
      "Bassersdorf",
      "Bellinzona",
      "Belp",
      "Bern",
      "Beromünster",
      "Biasca",
      "Biel",
      "Biel/Bienne",
      "Bienne",
      "Binningen",
      "Birsfelden",
      "Bischofszell",
      "Boudry",
      "Bourg-Saint-Pierre",
      "Bremgarten",
      "Brig-Glis",
      "Brugg",
      "Buchs",
      "Bulle",
      "Burgdorf",
      "Bussigny",
      "Bülach",
      "Büren a.A.",
      "Carouge",
      "Cham",
      "Chiasso",
      "Chur",
      "Châtel-Saint-Denis",
      "Chêne-Bougeries",
      "Conthey",
      "Coppet",
      "Cossonay",
      "Crissier",
      "Croglio",
      "Cudrefin",
      "Cully",
      "Davos*",
      "Delémont",
      "Diessenhofen",
      "Dietikon",
      "Dübendorf",
      "Ebikon",
      "Ecublens",
      "Eglisau",
      "Einsiedeln",
      "Elgg",
      "Emmen",
      "Erlach",
      "Estavayer-le-Lac",
      "Flawil",
      "Frauenfeld",
      "Freienbach",
      "Fribourg",
      "Geneva",
      "Gland",
      "Glarus",
      "Gordola",
      "Gossau",
      "Grandcour",
      "Grandson",
      "Greifensee",
      "Grenchen",
      "Gruyères",
      "Grüningen",
      "Herisau",
      "Hermance",
      "Hinwil",
      "Horgen",
      "Horw",
      "Huttwil",
      "Ilanz",
      "Illnau-Effretikon",
      "Interlaken",
      "Ittigen",
      "Kaiserstuhl",
      "Klingnau",
      "Kloten",
      "Kreuzlingen",
      "Kriens",
      "Köniz",
      "Küsnacht",
      "La Chaux-de-Fonds",
      "La Neuveville",
      "La Sarraz",
      "La Tour-de-Peilz",
      "La Tour-de-Trême",
      "Lachen",
      "Lancy",
      "Langenthal",
      "Laufen",
      "Laufenburg",
      "Laupen",
      "Lausanne",
      "Le Grand-Saconnex",
      "Le Landeron",
      "Le Locle",
      "Lenzburg",
      "Les Clées",
      "Leuk",
      "Lichtensteig",
      "Liestal",
      "Locarno",
      "Losone",
      "Lucerne",
      "Lugano",
      "Lutry",
      "Lyss",
      "Maienfeld",
      "Martigny",
      "Meilen",
      "Mellingen",
      "Mendrisio",
      "Meyrin",
      "Monthey",
      "Montreux",
      "Morcote",
      "Morges",
      "Moudon",
      "Moutier",
      "Muri bei Bern",
      "Murten",
      "Muttenz",
      "Männedorf",
      "Möhlin",
      "Münchenbuchsee",
      "Münchenstein",
      "Münsingen",
      "Neuchâtel",
      "Neuhausen am Rheinfall",
      "Neunkirch",
      "Nidau",
      "Nyon",
      "Oberwil",
      "Oftringen",
      "Olten",
      "Onex",
      "Opfikon",
      "Orbe",
      "Orsières",
      "Ostermundigen",
      "Payerne",
      "Peseux",
      "Pfäffikon",
      "Plan-les-Ouates",
      "Porrentruy",
      "Pratteln",
      "Prilly",
      "Pully",
      "Rapperswil-Jona",
      "Regensberg",
      "Regensdorf",
      "Reinach",
      "Renens",
      "Rheinau",
      "Rheineck",
      "Rheinfelden",
      "Richterswil",
      "Riehen",
      "Risch",
      "Riva San Vitale",
      "Rolle",
      "Romainmôtier",
      "Romanshorn",
      "Romont (FR)",
      "Rorschach",
      "Rue",
      "Rüti",
      "Saillon",
      "Saint-Maurice",
      "Saint-Prex",
      "Saint-Ursanne",
      "Sargans",
      "Sarnen",
      "Schaffhausen",
      "Schlieren",
      "Schwyz",
      "Sembrancher",
      "Sempach",
      "Sierre",
      "Sion",
      "Solothurn",
      "Spiez",
      "Splügen",
      "Spreitenbach",
      "St. Gallen",
      "St. Moritz",
      "Stans",
      "Steckborn",
      "Steffisburg",
      "Stein am Rhein",
      "Steinhausen",
      "Stäfa",
      "Suhr",
      "Sursee",
      "Thalwil",
      "Thun",
      "Thusis",
      "Thônex",
      "Unterseen",
      "Urdorf",
      "Uster",
      "Uznach",
      "Uzwil",
      "Val-de-Travers",
      "Valangin",
      "Vernier",
      "Versoix",
      "Vevey",
      "Veyrier",
      "Villars-sur-Glâne",
      "Villeneuve",
      "Visp",
      "Volketswil",
      "Waldenburg",
      "Walenstadt",
      "Wallisellen",
      "Wangen an der Aare",
      "Weinfelden",
      "Werdenberg",
      "Wettingen",
      "Wetzikon",
      "Wiedlisbach",
      "Wil",
      "Willisau",
      "Winterthur",
      "Wohlen",
      "Wädenswil",
      "Yverdon-les-Bains",
      "Zermatt",
      "Zofingen",
      "Zollikofen",
      "Zollikon",
      "Zug",
      "Zürich",
      "Échallens",
    ];
  }

  @override
  List<String> get continents {
    return const [
      "Afrika",
      "Antarktis",
      "Asien",
      "Australien",
      "Europa",
      "Nordamerika",
      "Südamerika",
    ];
  }

  @override
  List<String> get countries {
    return const [
      "Afghanistan",
      "Albanien",
      "Algerien",
      "Amerikanisch-Ozeanien",
      "Amerikanisch-Samoa",
      "Amerikanische Jungferninseln",
      "Andorra",
      "Angola",
      "Anguilla",
      "Antarktis",
      "Antigua und Barbuda",
      "Argentinien",
      "Armenien",
      "Aruba",
      "Ascension",
      "Aserbaidschan",
      "Australien",
      "Bahamas",
      "Bahrain",
      "Bangladesch",
      "Barbados",
      "Belarus",
      "Belgien",
      "Belize",
      "Benin",
      "Bermuda",
      "Bhutan",
      "Bolivien",
      "Bosnien und Herzegowina",
      "Botsuana",
      "Brasilien",
      "Britische Jungferninseln",
      "Britisches Territorium im Indischen Ozean",
      "Brunei Darussalam",
      "Bulgarien",
      "Burkina Faso",
      "Burundi",
      "Ceuta und Melilla",
      "Chile",
      "China",
      "Cookinseln",
      "Costa Rica",
      "Curaçao",
      "Côte d’Ivoire",
      "Demokratische Volksrepublik Korea",
      "Deutschland",
      "Diego Garcia",
      "Dominica",
      "Dominikanische Republik",
      "Dschibuti",
      "Dänemark",
      "Ecuador",
      "El Salvador",
      "Eritrea",
      "Estland",
      "Falklandinseln",
      "Fidschi",
      "Finnland",
      "Frankreich",
      "Französisch-Guayana",
      "Französisch-Polynesien",
      "Französische Süd- und Antarktisgebiete",
      "Färöer",
      "Gabun",
      "Gambia",
      "Georgien",
      "Ghana",
      "Gibraltar",
      "Grenada",
      "Griechenland",
      "Grönland",
      "Guadeloupe",
      "Guam",
      "Guatemala",
      "Guernsey",
      "Guinea",
      "Guinea-Bissau",
      "Guyana",
      "Haiti",
      "Honduras",
      "Indien",
      "Indonesien",
      "Irak",
      "Iran",
      "Irland",
      "Island",
      "Isle of Man",
      "Israel",
      "Italien",
      "Jamaika",
      "Japan",
      "Jemen",
      "Jersey",
      "Jordanien",
      "Kaimaninseln",
      "Kambodscha",
      "Kamerun",
      "Kanada",
      "Kanarische Inseln",
      "Kap Verde",
      "Karibische Niederlande",
      "Kasachstan",
      "Katar",
      "Kenia",
      "Kirgisistan",
      "Kiribati",
      "Kokosinseln",
      "Kolumbien",
      "Komoren",
      "Kongo-Brazzaville",
      "Kongo-Kinshasa",
      "Kosovo",
      "Kroatien",
      "Kuba",
      "Kuwait",
      "Laos",
      "Lesotho",
      "Lettland",
      "Libanon",
      "Liberia",
      "Libyen",
      "Liechtenstein",
      "Litauen",
      "Luxemburg",
      "Madagaskar",
      "Malawi",
      "Malaysia",
      "Malediven",
      "Mali",
      "Malta",
      "Marokko",
      "Marshallinseln",
      "Martinique",
      "Mauretanien",
      "Mauritius",
      "Mayotte",
      "Mazedonien",
      "Mexiko",
      "Mikronesien",
      "Monaco",
      "Mongolei",
      "Montenegro",
      "Montserrat",
      "Mosambik",
      "Myanmar",
      "Namibia",
      "Nauru",
      "Nepal",
      "Neukaledonien",
      "Neuseeland",
      "Nicaragua",
      "Niederlande",
      "Niger",
      "Nigeria",
      "Niue",
      "Norfolkinsel",
      "Norwegen",
      "Nördliche Marianen",
      "Oman",
      "Pakistan",
      "Palau",
      "Palästinensische Autonomiegebiete",
      "Panama",
      "Papua-Neuguinea",
      "Paraguay",
      "Peru",
      "Philippinen",
      "Pitcairninseln",
      "Polen",
      "Portugal",
      "Puerto Rico",
      "Republik Korea",
      "Republik Moldau",
      "Ruanda",
      "Rumänien",
      "Russische Föderation",
      "Réunion",
      "Salomonen",
      "Sambia",
      "Samoa",
      "San Marino",
      "Saudi-Arabien",
      "Schweden",
      "Schweiz",
      "Senegal",
      "Serbien",
      "Seychellen",
      "Sierra Leone",
      "Simbabwe",
      "Singapur",
      "Sint Maarten",
      "Slowakei",
      "Slowenien",
      "Somalia",
      "Sonderverwaltungsregion Macau",
      "Sonderverwaltungszone Hongkong",
      "Spanien",
      "Sri Lanka",
      "St. Barthélemy",
      "St. Helena",
      "St. Kitts und Nevis",
      "St. Lucia",
      "St. Martin",
      "St. Pierre und Miquelon",
      "St. Vincent und die Grenadinen",
      "Sudan",
      "Suriname",
      "Svalbard und Jan Mayen",
      "Swasiland",
      "Syrien",
      "São Tomé und Príncipe",
      "Südafrika",
      "Südgeorgien und die Südlichen Sandwichinseln",
      "Südsudan",
      "Tadschikistan",
      "Taiwan",
      "Tansania",
      "Thailand",
      "Timor-Leste",
      "Togo",
      "Tokelau",
      "Tonga",
      "Trinidad und Tobago",
      "Tristan da Cunha",
      "Tschad",
      "Tschechische Republik",
      "Tunesien",
      "Turkmenistan",
      "Turks- und Caicosinseln",
      "Tuvalu",
      "Türkei",
      "Uganda",
      "Ukraine",
      "Ungarn",
      "Uruguay",
      "Usbekistan",
      "Vanuatu",
      "Vatikanstadt",
      "Venezuela",
      "Vereinigte Arabische Emirate",
      "Vereinigte Staaten",
      "Vereinigtes Königreich",
      "Vietnam",
      "Wallis und Futuna",
      "Weihnachtsinsel",
      "Westsahara",
      "Zentralafrikanische Republik",
      "Zypern",
      "Ägypten",
      "Äquatorialguinea",
      "Äthiopien",
      "Åland-Inseln",
      "Österreich",
    ];
  }

  @override
  String formatAddress({
    required String suffix,
    required String name,
    required int number,
  }) {
    return '$name $number';
  }

  @override
  String get postalCodeFormat => '####';

  @override
  List<String> states({required bool isAbbr}) {
    return switch (isAbbr) {
      true => const [
          "CH-AG",
          "CH-AI",
          "CH-AR",
          "CH-BE",
          "CH-BL",
          "CH-BS",
          "CH-FR",
          "CH-GE",
          "CH-GL",
          "CH-GR",
          "CH-JU",
          "CH-LU",
          "CH-NE",
          "CH-NW",
          "CH-OW",
          "CH-SG",
          "CH-SH",
          "CH-SO",
          "CH-SZ",
          "CH-TG",
          "CH-TI",
          "CH-UR",
          "CH-VD",
          "CH-VS",
          "CH-ZG",
          "CH-ZH",
        ],
      false => const [
          "Aargau",
          "Appenzell Ausserrhoden",
          "Appenzell Innerrhoden",
          "Basel-Landschaft",
          "Basel-Stadt",
          "Bern",
          "Freiburg",
          "Genf",
          "Glarus",
          "Graubünden",
          "Jura",
          "Kanton",
          "Luzern",
          "Neuenburg",
          "Nidwalden",
          "Obwalden",
          "Schaffhausen",
          "Schwyz",
          "Solothurn",
          "St.Gallen",
          "Tessin",
          "Thurgau",
          "Uri",
          "Waadt",
          "Wallis",
          "Zug",
          "Zürich",
        ],
    };
  }

  @override
  List<String> streets({required bool isSuffix}) {
    return switch (isSuffix) {
      true => const [],
      false => const [
          "Aarbergergasse",
          "Aargauerstrasse",
          "Abeggweg",
          "Abendweg",
          "Ackermannstrasse",
          "Ackersteinstrasse",
          "Ackerstrasse",
          "Adlisbergbachweg",
          "Adlisbergstrasse",
          "Adolf-Lüchinger-Strasse",
          "Aegertenstrasse",
          "Aegertenweg",
          "Aehrenweg",
          "Aemmerliweg",
          "Aemtlerstrasse",
          "Affolternstrasse",
          "Agleistrasse",
          "Agnes-Robmann-Weg",
          "Agnesstrasse",
          "Ahornstrasse",
          "Akazienstrasse",
          "Albert-Näf-Platz",
          "Albert-Schneider-Weg",
          "Albertstrasse",
          "Albin-Zollinger-Platz",
          "Albisgütliweg",
          "Albisriederplatz",
          "Albisriederstrasse",
          "Albisriederweg",
          "Albisstrasse",
          "Albulastrasse",
          "Alderstrasse",
          "Alemannensteig",
          "Alfred-Escher-Strasse",
          "Alfred-Strebel-Weg",
          "Algierstrasse",
          "Allenmoosstrasse",
          "Allenwiesliweg",
          "Allmannstrasse",
          "Allmendsteg",
          "Allmendstrasse",
          "Alte Feldeggstrasse",
          "Alte Gockhauserstrasse",
          "Alte Kalchbühlstrasse",
          "Alte Mühlackerstrasse",
          "Alte Regensdorferstrasse",
          "Alte Zürichstrasse",
          "Altenhofstrasse",
          "Alter Kirchenweg",
          "Althoossteig",
          "Althoosstrasse",
          "Altstetterplatz",
          "Altstetterstrasse",
          "Altweg",
          "Altwiesenstrasse",
          "Am Bach",
          "Am Börtli",
          "Am Giessen",
          "Am Glattbogen",
          "Am Guggenberg",
          "Am Holbrig",
          "Am Hönggerberg",
          "Am Katzenbach",
          "Am Luchsgraben",
          "Am Oeschbrig",
          "Am Rank",
          "Am Schanzengraben",
          "Am Suteracher",
          "Am Sägertenbach",
          "Am Wasser",
          "Am Wettingertobel",
          "Amazonenstrasse",
          "Amazonenweg",
          "Ameisenweg",
          "Ampèresteg",
          "Ampèrestrasse",
          "Amselsteig",
          "Amthausgasse",
          "An der Specki",
          "Andreaspark",
          "Andreasstrasse",
          "Anemonenstrasse",
          "Angelikaweg",
          "Angererstrasse",
          "Ankengasse",
          "Ankenweid",
          "Ankerstrasse",
          "Anna-Heer-Strasse",
          "Annemarie-Schwarzenbach-Weg",
          "Anny-Klawa-Platz",
          "Anton-Higi-Strasse",
          "Anwandstrasse",
          "Apfelbaumstrasse",
          "Apollostrasse",
          "Appenzellerstrasse",
          "Appenzellerweg",
          "Aprikosenstrasse",
          "Arbentalstrasse",
          "Arbenzstrasse",
          "Arianestrasse",
          "Armin-Bollinger-Weg",
          "Arminstrasse",
          "Arnikaweg",
          "Arnold-Kübler-Strasse",
          "Arosasteig",
          "Arosastrasse",
          "Arterstrasse",
          "Arthur-Rohn-Strasse",
          "Arvenweg",
          "Aspholzstrasse",
          "Aspweg",
          "Asternweg",
          "Asylstrasse",
          "Attenhoferstrasse",
          "Aubrigstrasse",
          "Aubruggstrasse",
          "Aubruggweg",
          "Auf der Egg",
          "Auf der Mauer",
          "August-Forel-Strasse",
          "Augustinergasse",
          "Augustinerhof",
          "Auhofstrasse",
          "Aurorastrasse",
          "Ausserdorfstrasse",
          "Ausstellungsstrasse",
          "Austrasse",
          "Auwiesenstrasse",
          "Auzelg",
          "Azurstrasse",
          "Baalweg",
          "Bachmannweg",
          "Bachmattstrasse",
          "Bachmattweg",
          "Bachofnerstrasse",
          "Bachstrasse",
          "Bachtelweg",
          "Bachtobelstrasse",
          "Bachtobelweg",
          "Bachwiesenstrasse",
          "Bachwiesenweg",
          "Badenerstrasse",
          "Badergasse",
          "Badstrasse",
          "Badweg",
          "Bahnhaldenstrasse",
          "Bahnhofbrücke",
          "Bahnhofpassage",
          "Bahnhofplatz",
          "Bahnhofquai",
          "Bahnhofstrasse",
          "Bahnweg",
          "Balberstrasse",
          "Balderngasse",
          "Balgriststrasse",
          "Balgristweg",
          "Ballonstrasse",
          "Bannholzstrasse",
          "Banzwiesenstrasse",
          "Baschligplatz",
          "Baslerstrasse",
          "Basteigasse",
          "Basteiplatz",
          "Batteriesteig",
          "Batteriestrasse",
          "Bauhallengasse",
          "Bauherrenstrasse",
          "Baumackerstrasse",
          "Baumgartnerstrasse",
          "Baumgasse",
          "Baumhaldenstrasse",
          "Baumschulweg",
          "Baurstrasse",
          "Beatengasse",
          "Beatenplatz",
          "Beckenhofstrasse",
          "Beckhammer",
          "Bederstrasse",
          "Beerirain",
          "Beethovenstrasse",
          "Begonienstrasse",
          "Bellariarain",
          "Bellariastrasse",
          "Bellerivestrasse",
          "Bellevueplatz",
          "Bellikersteig",
          "Belsitostrasse",
          "Benedikt-Fontana-Weg",
          "Bergacker",
          "Bergellersteig",
          "Bergellerstrasse",
          "Berghaldenstrasse",
          "Bergheimstrasse",
          "Bergholzweg",
          "Bergliweg",
          "Bergmannweg",
          "Bergstrasse",
          "Bergweg",
          "Bergwiesen",
          "Berneggweg",
          "Bernerstrasse",
          "Bernerstrasse Nord",
          "Bernerstrasse Süd",
          "Bernhard-Jaeggi-Weg",
          "Berninaplatz",
          "Berninastrasse",
          "Bertastrasse",
          "Besenrainstrasse",
          "Besenrainweg",
          "Beustweg",
          "Biberlinsburgweg",
          "Biberlinstrasse",
          "Biberlinweg",
          "Bienenstrasse",
          "Billeterstrasse",
          "Billoweg",
          "Billrothstrasse",
          "Billrothweg",
          "Binderweg",
          "Binzallee",
          "Binzmühlestrasse",
          "Binzring",
          "Binzstrasse",
          "Binzwiesenstrasse",
          "Bionstrasse",
          "Birchdörfli",
          "Bircher-Benner-Platz",
          "Birchplatz",
          "Birchsteg",
          "Birchstrasse",
          "Birkenweg",
          "Birmensdorferstrasse",
          "Birnbaumstrasse",
          "Blaufahnenstrasse",
          "Blaumeisliweg",
          "Blauäcker",
          "Bleicherweg",
          "Bleulerstrasse",
          "Blumenfeldstrasse",
          "Blumenweg",
          "Bluntschlisteig",
          "Bläsistrasse",
          "Blümlisalpstrasse",
          "Blütenstrasse",
          "Bockhornstrasse",
          "Bocklerstrasse",
          "Bodenacker",
          "Bodmerstrasse",
          "Bogenweg",
          "Bolistrasse",
          "Bolleystrasse",
          "Bombachhalde",
          "Bombachsteig",
          "Bombachstrasse",
          "Borrweg",
          "Brahmsstrasse",
          "Brandholzstrasse",
          "Brandschenkesteig",
          "Brandschenkestrasse",
          "Brandweg",
          "Brauerstrasse",
          "Breitenackerstrasse",
          "Breitenlooweg",
          "Breitensteinstrasse",
          "Breitingerstrasse",
          "Breitlooweg",
          "Breitweg",
          "Bremgartnerstrasse",
          "Brinerstrasse",
          "Bristenstrasse",
          "Brotgasse",
          "Brown-Boveri-Strasse",
          "Bruchstrasse",
          "Bruderholzweg",
          "Bruderwies",
          "Bruderwiesweg",
          "Bruggerweg",
          "Brugghölzliweg",
          "Brunaubrücke",
          "Brunaustrasse",
          "Bruneggweg",
          "Brunnackerweg",
          "Brunnenhofstrasse",
          "Brunnenhofweg",
          "Brunngasse",
          "Brunnwiesenstrasse",
          "Brüderhofweg",
          "Brüggliäcker",
          "Brühlweg",
          "Brünneliacker",
          "Brünneliweg",
          "Brüttenweg",
          "Bubenbergstrasse",
          "Bucheggplatz",
          "Bucheggstrasse",
          "Buchenbaumweg",
          "Buchenrainstrasse",
          "Buchenrainweg",
          "Buchensteig",
          "Buchenweg",
          "Buchfinkenstrasse",
          "Buchholzrain",
          "Buchholzstrasse",
          "Buchhölzlistrasse",
          "Buchhölzliweg",
          "Buchlernstrasse",
          "Buchmattweg",
          "Buchwiesen",
          "Buchwiesenweg",
          "Buchzelgstrasse",
          "Buchzelgweg",
          "Buckhauserstrasse",
          "Buhnrain",
          "Buhnstrasse",
          "Bullingerhof",
          "Bullingerplatz",
          "Bullingerstrasse",
          "Bungertweg",
          "Burenweg",
          "Burgsteig",
          "Burgstrasse",
          "Burgweg",
          "Burriweg",
          "Burstwiesenstrasse",
          "Buschbergsteig",
          "Buschbergstrasse",
          "Butzenstrasse",
          "Bächlerstrasse",
          "Bächlerweg",
          "Bächtoldstrasse",
          "Bäckerstrasse",
          "Bändlistrasse",
          "Bändliweg",
          "Bärenbohlstrasse",
          "Bärenbrüggli",
          "Bärengasse",
          "Bäulisteig",
          "Bäulistrasse",
          "Béatrice-von-Wattenwyl-Haus",
          "Böcklinstrasse",
          "Börsenstrasse",
          "Büchnerstrasse",
          "Bühleggweg",
          "Bühlholzweg",
          "Bühlstrasse",
          "Bühlwiesenstrasse",
          "Bührlestrasse",
          "Bülachhof",
          "Bülachstrasse",
          "Bürglistrasse",
          "Bürkliplatz",
          "Büttenweg",
          "Calandastrasse",
          "Campanellaweg",
          "Carl-Schröter-Strasse",
          "Carl-Spitteler-Strasse",
          "Carl-Wehrli-Weg",
          "Carmenstrasse",
          "Caroline-Farner-Weg",
          "Caspar-Wüst-Strasse",
          "Central",
          "Ceresstrasse",
          "Chaletweg",
          "Chelleweg",
          "Chillesteig",
          "Chorgasse",
          "Chorherrenweg",
          "Chriesiweg",
          "Chutzenstrasse",
          "Claridenstrasse",
          "Clausiussteig",
          "Clausiusstrasse",
          "Conrad-Ferdinand-Meyer-Strasse",
          "Cordelia-Guggenheim-Weg",
          "Corrodisteig",
          "Corrodistrasse",
          "Cramerstrasse",
          "Culmannstrasse",
          "Cyklamenweg",
          "Cäcilienstrasse",
          "Cäsar-Ritz-Strasse",
          "Dachslernstrasse",
          "Dachslernweg",
          "Dachsschleife",
          "Dahliastrasse",
          "Damhirschweg",
          "Dammsteg",
          "Dammstrasse",
          "Dammweg",
          "Dangelstrasse",
          "Dangelweg",
          "David-Hess-Weg",
          "Degenriedstrasse",
          "Delphinstrasse",
          "Dennlerstrasse",
          "Denzlerstrasse",
          "Denzlerweg",
          "Diabasweg",
          "Dialogweg",
          "Dianastrasse",
          "Dienerhölzliweg",
          "Dienerstrasse",
          "Dietzingerstrasse",
          "Diggelmannstrasse",
          "Distelweg",
          "Dohlenweg",
          "Dolderstrasse",
          "Doldertal",
          "Dora-Staudinger-Strasse",
          "Dorfbachweg",
          "Dorflindenstrasse",
          "Dorfstrasse",
          "Drahtschmidlisteg",
          "Drahtschmidlisteig",
          "Drahtzugsteig",
          "Drahtzugstrasse",
          "Drehergasse",
          "Dreikönigbrücke",
          "Dreikönigstrasse",
          "Dreispitz",
          "Dreiwiesenstrasse",
          "Drosselstrasse",
          "Drusbergstrasse",
          "Dubsstrasse",
          "Dufourstrasse",
          "Dunantstrasse",
          "Dunkelhölzlifussweg",
          "Dunkelhölzlistrasse",
          "Dunkelhölzliweg",
          "Duttweilerbrücke",
          "Duttweilerstrasse",
          "Döltschihalde",
          "Döltschiweg",
          "Dörflistrasse",
          "Dübendorfstrasse",
          "Ebelstrasse",
          "Edelweissstrasse",
          "Edenstrasse",
          "Edisonstrasse",
          "Eduard-Imhof-Strasse",
          "Effingerstrasse",
          "Eggbühlstrasse",
          "Eggenschwilerweg",
          "Egglenweg",
          "Eggpromenade",
          "Eggweg",
          "Eglistrasse",
          "Eibenstrasse",
          "Eichacker",
          "Eichbühlstrasse",
          "Eichhalde",
          "Eichhaldenweg",
          "Eichholzweg",
          "Eichhörnliweg",
          "Eichrainstrasse",
          "Eichstrasse",
          "Eidechsenweg",
          "Eidmattstrasse",
          "Eierbrechtstrasse",
          "Eigenheimstrasse",
          "Eigenstrasse",
          "Eigenwasenstrasse",
          "Einfangstrasse",
          "Einsteinbrücke",
          "Einsteinstrasse",
          "Eisenbahnerstrasse",
          "Eisengasse",
          "Eisfeldstrasse",
          "Eisgasse",
          "Eisvogelweg",
          "Ekkehardsteig",
          "Ekkehardstrasse",
          "Eleonorenstrasse",
          "Elfenweg",
          "Elias-Canetti-Strasse",
          "Elisabethenstrasse",
          "Ellen-Widmann-Weg",
          "Elsastrasse",
          "Else-Lasker-Schüler-Weg",
          "Else-Züblin-Strasse",
          "Else-Züblin-Weg",
          "Elsässergasse",
          "Emil-Klöti-Strasse",
          "Emil-Oprecht-Platz",
          "Emil-Oprecht-Strasse",
          "Emil-Rütti-Weg",
          "Emil-Spillmann-Weg",
          "Emilie-Kempin-Spyri-Weg",
          "Engadinerweg",
          "Engelbertstrasse",
          "Engelstrasse",
          "Engimattstrasse",
          "Englischviertelstrasse",
          "Engstringerweg",
          "Engweg",
          "Entlisbergstrasse",
          "Entlisbergweg",
          "Enzenbühlstrasse",
          "Enzenried",
          "Enzianweg",
          "Erchenbühlstrasse",
          "Erchenbühlweg",
          "Erdbrustweg",
          "Erika-Mann-Strasse",
          "Erikastrasse",
          "Erismannhof",
          "Erismannstrasse",
          "Erlachstrasse",
          "Erlenstrasse",
          "Erlenweg",
          "Erligatterweg",
          "Ernastrasse",
          "Ernst-Nobs-Platz",
          "Ernst-Zöbeli-Strasse",
          "Ernst-Zöbeli-Weg",
          "Eschenhaustrasse",
          "Eschenholzweg",
          "Eschenweg",
          "Escher-Wyss-Platz",
          "Eschergutweg",
          "Eschwiesenstrasse",
          "Espenhofweg",
          "Ettenfeldstrasse",
          "Etzelsteig",
          "Etzelstrasse",
          "Etzelweg",
          "Eugen-Huber-Strasse",
          "Eulenweg",
          "Europaallee",
          "Europabrücke",
          "Eyhof",
          "FIFA-Strasse",
          "Fabrikstrasse",
          "Falkenstrasse",
          "Falkenweg",
          "Farbhofstrasse",
          "Farbhofweg",
          "Farenweg",
          "Federnstrasse",
          "Fehrenstrasse",
          "Feilengasse",
          "Feldblumenstrasse",
          "Feldblumenweg",
          "Feldeggstrasse",
          "Feldenmoosstrasse",
          "Feldgütliweg",
          "Feldstrasse",
          "Felix-und-Regula-Platz",
          "Fellenbergstrasse",
          "Fellenbergweg",
          "Felsberg",
          "Felsenkellerweg",
          "Felsenrainstrasse",
          "Felsenrainweg",
          "Felsenstrasse",
          "Fennergasse",
          "Ferdinand-Hodler-Strasse",
          "Fernsehstrasse",
          "Feuerweg",
          "Feusisbergli",
          "Fichtenstrasse",
          "Fierzgasse",
          "Finkenrain",
          "Finslerstrasse",
          "Fischerweg",
          "Fliederstrasse",
          "Flobotstrasse",
          "Florastrasse",
          "Florhofgasse",
          "Flurstrasse",
          "Flössergasse",
          "Flüelastrasse",
          "Flühgasse",
          "Forchstrasse",
          "Forellenweg",
          "Forrenweidstrasse",
          "Forstersteig",
          "Forsterstrasse",
          "Forsthausstrasse",
          "Forsthausweg",
          "Fortunagasse",
          "Frankengasse",
          "Frankentalerstrasse",
          "Frankentalerweg",
          "Franklinplatz",
          "Franklinstrasse",
          "Franzosenweg",
          "Frauenbrünnelistrasse",
          "Frauenklinikstrasse",
          "Frauentalweg",
          "Fraumünsterstrasse",
          "Frechenmätteliweg",
          "Freieckgasse",
          "Freiensteinstrasse",
          "Freiestrasse",
          "Freigutstrasse",
          "Freihofstrasse",
          "Freilagerstrasse",
          "Freilagerweg",
          "Freischützgasse",
          "Freudenbergstrasse",
          "Freudwilerweg",
          "Freyastrasse",
          "Friedackerstrasse",
          "Friedaustrasse",
          "Friedensgasse",
          "Friedheimstrasse",
          "Friedhofstrasse",
          "Friedrich-Glauser-Gasse",
          "Friedrichstrasse",
          "Friesenberghalde",
          "Friesenbergstrasse",
          "Friesenburgweg",
          "Friesstrasse",
          "Fritschistrasse",
          "Fritz-Fleiner-Weg",
          "Fritz-Heeb-Weg",
          "Frohalpstrasse",
          "Frohburgstrasse",
          "Frohbühlstrasse",
          "Fronwaldstrasse",
          "Fronwaldweg",
          "Froschaugasse",
          "Frymannstrasse",
          "Fröbelstrasse",
          "Fröhlichstrasse",
          "Fuchsiastrasse",
          "Fuchspass",
          "Funkackerstrasse",
          "Funkwiesenstrasse",
          "Furkastrasse",
          "Furttalstrasse",
          "Färberstrasse",
          "Föhrenstrasse",
          "Förrlibuckstrasse",
          "Fürstweg",
          "Füsslistrasse",
          "Gablerstrasse",
          "Gagliardiweg",
          "Gallusstrasse",
          "Gamperstrasse",
          "Gartenhofstrasse",
          "Gartenstrasse",
          "Gartenweg",
          "Gasometerstrasse",
          "Gasstrasse",
          "Gattikerstrasse",
          "Gaugerstrasse",
          "Geerenweg",
          "Geeringsteig",
          "Geeringstrasse",
          "Geerlibuchweg",
          "Gehrenholz",
          "Gehrenholzstrasse",
          "Geibelstrasse",
          "Geigergasse",
          "Geissbergweg",
          "Gellertstrasse",
          "Gemeindestrasse",
          "General-Guisan-Quai",
          "General-Wille-Strasse",
          "Genferstrasse",
          "Georg-Baumberger-Weg",
          "Georg-Kempf-Strasse",
          "Georgengasse",
          "Geranienstrasse",
          "Gerbergasse",
          "Gerechtigkeitsgasse",
          "Gerhardstrasse",
          "Germaniastrasse",
          "Geroldrampe",
          "Geroldstrasse",
          "Gerstenstrasse",
          "Gertrud-Kurz-Strasse",
          "Gertrudplatz",
          "Gertrudstrasse",
          "Gessnerallee",
          "Gessnerbrücke",
          "Gfellstrasse",
          "Giacomettistrasse",
          "Giacomettiweg",
          "Giblenstrasse",
          "Giblenweg",
          "Giebeleich",
          "Giessereistrasse",
          "Giesshübelstrasse",
          "Gimpelweg",
          "Ginsterstrasse",
          "Ginsterweg",
          "Girhaldenstrasse",
          "Girhaldenweg",
          "Gladbachstrasse",
          "Gladiolenweg",
          "Glasmalergasse",
          "Glattstegweg",
          "Glatttalstrasse",
          "Glattwiesenstrasse",
          "Glaubtensteig",
          "Glaubtenstrasse",
          "Gleisbogen",
          "Gletscherstrasse",
          "Glimmerweg",
          "Glockengasse",
          "Gloriasteig",
          "Gloriastrasse",
          "Glärnischstrasse",
          "Glättlistrasse",
          "Gmeimeriweg",
          "Goethestrasse",
          "Goetzstrasse",
          "Goldackerweg",
          "Goldauerstrasse",
          "Goldbrunnenplatz",
          "Goldbrunnenstrasse",
          "Goldregenweg",
          "Gorwiden",
          "Gottfried-Keller-Strasse",
          "Gotthardstrasse",
          "Gotthelfstrasse",
          "Gottlieb-Kramer-Steig",
          "Gottlieb-Kramer-Weg",
          "Grabenweg",
          "Grabenwies",
          "Granitweg",
          "Gratstrasse",
          "Gratweg",
          "Graue Gasse",
          "Grebelackerstrasse",
          "Greblerweg",
          "Greifenseestrasse",
          "Grenzsteig",
          "Grenzwiesweg",
          "Gretenweg",
          "Griesernweg",
          "Grimselstrasse",
          "Grossackerstrasse",
          "Grossalbis",
          "Grossmannstrasse",
          "Grossmünsterplatz",
          "Grossrieglenweg",
          "Grosswiesenstrasse",
          "Grubenackerstrasse",
          "Grubenstrasse",
          "Grundstrasse",
          "Gräbligasse",
          "Grünauring",
          "Grünaustrasse",
          "Grüngasse",
          "Grünhaldenstrasse",
          "Grünmattstrasse",
          "Grünwaldstrasse",
          "Grünwaldweg",
          "Grütholzstrasse",
          "Grütlistrasse",
          "Grütstrasse",
          "Gstalderweg",
          "Gsteigstrasse",
          "Gubelhangstrasse",
          "Gubelplatz",
          "Gubelstrasse",
          "Gubristweg",
          "Gugelweg",
          "Guggachstrasse",
          "Guggerweg",
          "Gugolzstrasse",
          "Gujerstrasse",
          "Guldinerweg",
          "Gustav-Gull-Platz",
          "Gustav-Heinrich-Weg",
          "Gutenbergstrasse",
          "Guthirtstrasse",
          "Gutstrasse",
          "Gwandensteig",
          "Gämsenstrasse",
          "Gänziloobrücke",
          "Gänzilooweg",
          "Gärtnerstrasse",
          "Gässli",
          "Güetliweg",
          "Habsburgstrasse",
          "Hadlaubsteig",
          "Hadlaubstrasse",
          "Hafnerstrasse",
          "Hagenbuchrain",
          "Hagenholzstrasse",
          "Hagwiesenweg",
          "Hainerweg",
          "Haldenbachstrasse",
          "Haldeneggsteig",
          "Haldenstrasse",
          "Hallenstrasse",
          "Hallerweg",
          "Hallwylplatz",
          "Hallwylstrasse",
          "Hambergersteig",
          "Hammersteig",
          "Hammerstrasse",
          "Hanfrose",
          "Hangelstrasse",
          "Hans-Huber-Strasse",
          "Hanslinweg",
          "Hardaupark",
          "Hardaustrasse",
          "Hardauweg",
          "Hardbrücke",
          "Hardeggsteg",
          "Hardeggstrasse",
          "Hardeggweg",
          "Hardgutstrasse",
          "Hardhof",
          "Hardplatz",
          "Hardstrasse",
          "Hardturmsteg",
          "Hardturmstrasse",
          "Hardturmweg",
          "Haselholzweg",
          "Haselmausweg",
          "Haselweg",
          "Hasenrain",
          "Haslerstrasse",
          "Haspelstrasse",
          "Hauerweg",
          "Hauhölzliweg",
          "Haumesserstrasse",
          "Hauriweg",
          "Hauserstrasse",
          "Hauswiesenstrasse",
          "Hauswiesenweg",
          "Hausäcker",
          "Hechtplatz",
          "Hedwigsteig",
          "Hedwigstrasse",
          "Heerenschürlistrasse",
          "Heerenwiesen",
          "Hegarstrasse",
          "Hegenmatt",
          "Hegianwandweg",
          "Hegibachplatz",
          "Hegibachstrasse",
          "Hegnauweg",
          "Heideggerweg",
          "Heidegraben",
          "Heidi-Abel-Weg",
          "Heidwiesen",
          "Heilighüsli",
          "Heimatstrasse",
          "Heimplatz",
          "Heimstrasse",
          "Heinrich-Bosshardt-Strasse",
          "Heinrich-Federer-Strasse",
          "Heinrich-Gretler-Weg",
          "Heinrich-Wolff-Strasse",
          "Heinrichstrasse",
          "Heiristeg",
          "Heizenholz",
          "Helen-Keller-Strasse",
          "Helenastrasse",
          "Heliosstrasse",
          "Hellmutstrasse",
          "Hellrainweg",
          "Helvetiaplatz",
          "Herbartstrasse",
          "Herbstweg",
          "Herdernstrasse",
          "Herman-Greulich-Strasse",
          "Hermetschloobrücke",
          "Hermetschloostrasse",
          "Herostrasse",
          "Herrenbergstrasse",
          "Herrengasse",
          "Herrligstrasse",
          "Herrligweg",
          "Hertensteinstrasse",
          "Herterbrücke",
          "Hertersteg",
          "Herzogenmühlestrasse",
          "Herzogstrasse",
          "Hesenlooweg",
          "Hessenweidweg",
          "Heubeeriweg",
          "Heuelsteig",
          "Heuelstrasse",
          "Heugabelweg",
          "Hildastrasse",
          "Himmelsleiterli",
          "Himmeristrasse",
          "Himmeriweg",
          "Hinterbergstrasse",
          "Hinterhagenweg",
          "Hintermeisterhof",
          "Hirschengasse",
          "Hirschengraben",
          "Hirschenplatz",
          "Hirschgartnerweg",
          "Hirschwiesenstrasse",
          "Hirslanderbergstrasse",
          "Hirslandersteig",
          "Hirslanderstrasse",
          "Hirtenweg",
          "Hirzelstrasse",
          "Hirzenbachstrasse",
          "Hirzenbachweg",
          "Hitzigweg",
          "Hochfarbstrasse",
          "Hochstrasse",
          "Hofackerstrasse",
          "Hoffeld",
          "Hoffnungsstrasse",
          "Hoffnungsweg",
          "Hofhölzliweg",
          "Hofstrasse",
          "Hofwiesenstrasse",
          "Hohe Promenade",
          "Hohenbühlstrasse",
          "Hohenklingenallee",
          "Hohenklingensteig",
          "Hohenklingenstrasse",
          "Hohensteinstrasse",
          "Hohensteinweg",
          "Hohlstrasse",
          "Hohmoos",
          "Holbeinstrasse",
          "Holbrigstrasse",
          "Holderbachweg",
          "Holunderhof",
          "Holunderweg",
          "Holzgasse",
          "Holzhofweg",
          "Holzmatt",
          "Holzwiesweg",
          "Honeggerweg",
          "Honigstrasse",
          "Honrainweg",
          "Hopfenstrasse",
          "Horensteinstrasse",
          "Horgenweg",
          "Hornbachstrasse",
          "Horneggstrasse",
          "Hornergasse",
          "Hortensienstrasse",
          "Hottingerbergstrasse",
          "Hottingerplatz",
          "Hottingerstrasse",
          "Hotzesteig",
          "Hotzestrasse",
          "Hubeggstrasse",
          "Hubenstrasse",
          "Huberholzweg",
          "Huberwiesenstrasse",
          "Huebweg",
          "Hufgasse",
          "Hugo-Koblet-Weg",
          "Hugostrasse",
          "Hungerbergstrasse",
          "Hurdäckerstrasse",
          "Hurdäckerweg",
          "Huttensteig",
          "Huttenstrasse",
          "Häderlihof",
          "Hädrichstrasse",
          "Hägelerweg",
          "Häldeliweg",
          "Häringsplatz",
          "Häringstrasse",
          "Hätzlergasse",
          "Höcklerbrücke",
          "Höcklerstrasse",
          "Höcklerweg",
          "Höfliweg",
          "Höhenring",
          "Höhensteig",
          "Höhenweg",
          "Hölderlinsteig",
          "Hölderlinstrasse",
          "Hönggerbergring",
          "Hönggerrampe",
          "Hönggerstrasse",
          "Hörnlistrasse",
          "Höschgasse",
          "Hügelstrasse",
          "Hürlimannplatz",
          "Hürstholzstrasse",
          "Hürstholzweg",
          "Hürstringstrasse",
          "Hürststrasse",
          "Hüslibachstrasse",
          "Hüttenkopfstrasse",
          "Hüttenrainstrasse",
          "Hüttisstrasse",
          "Ida-Bindschedler-Strasse",
          "Idaplatz",
          "Idastrasse",
          "Ilanzhofweg",
          "Ilgenstrasse",
          "Im Altried",
          "Im Bergdörfli",
          "Im Birkenhof",
          "Im Brächli",
          "Im Eisernen Zeit",
          "Im Glockenacker",
          "Im Grossried",
          "Im Grund",
          "Im Hagacker",
          "Im Hagenbrünneli",
          "Im Hau",
          "Im Hegi",
          "Im Heimgärtli",
          "Im Heuried",
          "Im Hofgarten",
          "Im Holzerhurd",
          "Im Hummel",
          "Im Isengrind",
          "Im Jungen Berg",
          "Im Klösterli",
          "Im Kratz",
          "Im Laubegg",
          "Im Maas",
          "Im Mittelleimbach",
          "Im Raindörfli",
          "Im Rehsprung",
          "Im Rossweidli",
          "Im Sack",
          "Im Schaber",
          "Im Schellenberg",
          "Im Schilf",
          "Im St. Blasienhof",
          "Im Stadtgraben",
          "Im Stelzenacker",
          "Im Struppen",
          "Im Sträler",
          "Im Stückler",
          "Im Sydefädeli",
          "Im Tannegg",
          "Im Tiergarten",
          "Im Trichtisal",
          "Im Waidegg",
          "Im Walder",
          "Im Wiesenbeetli",
          "Im Wingert",
          "Im Wyl",
          "Im alten Brunnen",
          "Im oberen Boden",
          "Imbisbühlhalde",
          "Imbisbühlsteig",
          "Imbisbühlstrasse",
          "Imbisbühlweg",
          "Imfeldsteig",
          "Imfeldstrasse",
          "Immenweg",
          "In Böden",
          "In Büngerten",
          "In Gassen",
          "In den Rütenen",
          "In der Breiti",
          "In der Ey",
          "In der Hub",
          "In der Looren",
          "In der Schüpf",
          "In der Sommerau",
          "In der Wässeri",
          "Inselhofstrasse",
          "Irchelstrasse",
          "Irisstrasse",
          "Irringersteig",
          "Islerstrasse",
          "Jacob-Burckhardt-Strasse",
          "Jakob-Fügli-Strasse",
          "Jakob-Peter-Weg",
          "James-Joyce-Strasse",
          "Jenatschstrasse",
          "Joachim-Hefti-Weg",
          "Johannastrasse",
          "Johannes-Ott-Weg",
          "Johannesgasse",
          "Jonas-Furrer-Strasse",
          "Josefstrasse",
          "Jucheggstrasse",
          "Juchstrasse",
          "Juchweg",
          "Judith-Gessner-Platz",
          "Juliastrasse",
          "Jungholzhof",
          "Jungholzstrasse",
          "Jungholzweg",
          "Jungstrasse",
          "Junkerngasse",
          "Jupitersteig",
          "Jupiterstrasse",
          "Jägergasse",
          "Kalandergasse",
          "Kalanderplatz",
          "Kalchbühlstrasse",
          "Kalchbühlweg",
          "Kalchtarenweg",
          "Kalkbreitestrasse",
          "Kaminfegergasse",
          "Kanalstrasse",
          "Kanonengasse",
          "Kantonsschulstrasse",
          "Kantstrasse",
          "Kanzleistrasse",
          "Kapfsteig",
          "Kapfstrasse",
          "Kappelergasse",
          "Kappelerhof",
          "Kappeliholzstrasse",
          "Kappelistrasse",
          "Kappenbühlstrasse",
          "Kappenbühlweg",
          "Karl-Bürkli-Strasse",
          "Karl-Schmid-Strasse",
          "Karl-Stauffer-Strasse",
          "Karlstrasse",
          "Karstlernstrasse",
          "Kartausstrasse",
          "Kasernenstrasse",
          "Kasinostrasse",
          "Kaspar-Winkler-Weg",
          "Katharinenweg",
          "Katzenbachstrasse",
          "Katzenbachweg",
          "Katzenrütifussweg",
          "Katzenschwanzstrasse",
          "Katzenschwanzweg",
          "Katzenseestrasse",
          "Katzentischstrasse",
          "Kehlhofstrasse",
          "Kelchweg",
          "Kellerweg",
          "Keltenstrasse",
          "Kempfhofsteig",
          "Kempfhofweg",
          "Kempterstrasse",
          "Kenngottweg",
          "Kernstrasse",
          "Kettberg",
          "Kiefernweg",
          "Kienastenwiesweg",
          "Kieselgasse",
          "Kilchbergsteig",
          "Kilchbergstrasse",
          "Kinkelstrasse",
          "Kirchbühlweg",
          "Kirchenackerweg",
          "Kirchenfeld",
          "Kirchenfeldweg",
          "Kirchenweg",
          "Kirchgasse",
          "Klarastrasse",
          "Klausstrasse",
          "Klebestrasse",
          "Kleeweidstrasse",
          "Kleinalbis",
          "Kleinbühlstrasse",
          "Kleinertstrasse",
          "Kleinhölzliweg",
          "Kleinjoggsteig",
          "Kleinstrasse",
          "Klingenstrasse",
          "Klopstockstrasse",
          "Klosbachstrasse",
          "Kloster-Fahr-Weg",
          "Klosterfeldweg",
          "Klosterholzweg",
          "Klosterweg",
          "Klusdörfli",
          "Kluseggstrasse",
          "Klusplatz",
          "Klusstrasse",
          "Klusweg",
          "Knüslistrasse",
          "Kochergasse",
          "Kochstrasse",
          "Kohlenplatzweg",
          "Kolbenacker",
          "Kolbenhofereggstrasse",
          "Kolbenhofstrasse",
          "Kolibriweg",
          "Konkordiastrasse",
          "Konrad-Ilg-Strasse",
          "Konradstrasse",
          "Kornamtsweg",
          "Korneliusstrasse",
          "Kornhausbrücke",
          "Kornhausstrasse",
          "Kosakenweg",
          "Kraftstrasse",
          "Kramgasse",
          "Kranzweg",
          "Krattenturmstrasse",
          "Krautgartengasse",
          "Krebsgasse",
          "Kreuzbühlstrasse",
          "Kreuzplatz",
          "Kreuzstrasse",
          "Kreuzwiesen",
          "Kripfstrasse",
          "Krokusweg",
          "Kronenstrasse",
          "Kronwiesenstrasse",
          "Kronwiesenweg",
          "Kruggasse",
          "Krummweg",
          "Krähbühlsteig",
          "Krähbühlstrasse",
          "Krähbühlweg",
          "Kräuelgasse",
          "Krönleinstrasse",
          "Kugelfangweg",
          "Kunzweg",
          "Kurfirstensteig",
          "Kurfirstenstrasse",
          "Kurhausstrasse",
          "Kurt-Früh-Weg",
          "Kurt-Guggenheim-Strasse",
          "Kurt-Hirschfeld-Weg",
          "Kurvenstrasse",
          "Kurzgasse",
          "Kuttelgasse",
          "Kyburgstrasse",
          "Käferholzstrasse",
          "Kämbelgasse",
          "Känzelistrasse",
          "Känzeliweg",
          "Käshaldenstrasse",
          "Käshaldenweg",
          "Köchlistrasse",
          "Köhlerstrasse",
          "Köllikerstrasse",
          "Köngengasse",
          "Körnerstrasse",
          "Köschenrütistrasse",
          "Köschenrütiweg",
          "Kügeliloostrasse",
          "Kühriedweg",
          "Kühweidweg",
          "Küngenmatt",
          "Künstlergasse",
          "Künzlistrasse",
          "Kürbergsteig",
          "Kürbergstrasse",
          "Kürbergweg",
          "Kürbsensteig",
          "Lachenacker",
          "Lachenzelgstrasse",
          "Lagerstrasse",
          "Lamprechtweg",
          "Landenbergstrasse",
          "Landhusweg",
          "Landisstrasse",
          "Landoltstrasse",
          "Langackerstrasse",
          "Langensteinenstrasse",
          "Langfachweg",
          "Langfurren",
          "Langgrütstrasse",
          "Langgrütweg",
          "Langhagweg",
          "Langmattweg",
          "Langmauerstrasse",
          "Langstrasse",
          "Langweid",
          "Langwiesstrasse",
          "Laternengasse",
          "Laubiweg",
          "Laufferweg",
          "Laurenzgasse",
          "Lavaterstrasse",
          "Lavendelweg",
          "Lebristweg",
          "Leebernweg",
          "Lehensteig",
          "Lehenstrasse",
          "Lehfrauenweg",
          "Leimbachstrasse",
          "Leimgasse",
          "Leimgrübelstrasse",
          "Lenggfussweg",
          "Lengghalde",
          "Lenggstrasse",
          "Leonhard-Ragaz-Weg",
          "Leonhardshalde",
          "Leonhardstrasse",
          "Leonhardstreppe",
          "Lerchenberg",
          "Lerchenhalde",
          "Lerchenrain",
          "Lerchenstrasse",
          "Lessingstrasse",
          "Lettenfussweg",
          "Lettenholzstrasse",
          "Lettensteg",
          "Lettenstrasse",
          "Lettenviadukt",
          "Letzigraben",
          "Letzistrasse",
          "Letziweg",
          "Leuengasse",
          "Leutholdstrasse",
          "Leutschenbachstrasse",
          "Libellenweg",
          "Lichtstrasse",
          "Liebensteinstrasse",
          "Liebiseggweg",
          "Liebwiesweg",
          "Ligusterstrasse",
          "Lilienstrasse",
          "Limmatplatz",
          "Limmatquai",
          "Limmatstrasse",
          "Limmattalstrasse",
          "Lindenbachstrasse",
          "Lindenhof",
          "Lindenhofstrasse",
          "Lindenplatz",
          "Lindenstrasse",
          "Lindenweg",
          "Lindenzelgweg",
          "Lintheschergasse",
          "Linthescherhof",
          "Lochbrunnenweg",
          "Lochenweg",
          "Lochmannstrasse",
          "Lommisweg",
          "Loogartenstrasse",
          "Loorenbachweg",
          "Loorengutweg",
          "Loorenhalde",
          "Loorenkopfstrasse",
          "Loorenkopfweg",
          "Loorenrain",
          "Loorenrank",
          "Loorenstrasse",
          "Loorentorweg",
          "Louis-Braille-Strasse",
          "Louis-Häfliger-Park",
          "Luchsweg",
          "Luchswiesenstrasse",
          "Luchswiesenweg",
          "Luckenholzweg",
          "Luegete",
          "Luegislandstrasse",
          "Luggwegstrasse",
          "Luisenstrasse",
          "Lunastrasse",
          "Lureiweg",
          "Lutherstrasse",
          "Lux-Guyer-Weg",
          "Lybensteinweg",
          "Lydia-Welti-Escher-Hof",
          "Lyrenweg",
          "Lägernstrasse",
          "Läufebachweg",
          "Läufeweg",
          "Löwenplatz",
          "Löwensteg",
          "Löwenstrasse",
          "Maaghof",
          "Maagplatz",
          "Magdalenenstrasse",
          "Magnolienstrasse",
          "Magnusstrasse",
          "Maienburgweg",
          "Maienstrasse",
          "Maienweg",
          "Mainaustrasse",
          "Malergasse",
          "Malojaweg",
          "Malvenstrasse",
          "Malzstrasse",
          "Maneggbrücke",
          "Maneggpromenade",
          "Maneggstrasse",
          "Maneggweg",
          "Manesseplatz",
          "Manessestrasse",
          "Mannshäuliweg",
          "Mantelgasse",
          "Marbachweg",
          "Marchwartstrasse",
          "Marderstrasse",
          "Margaretenweg",
          "Margrit-Rainer-Strasse",
          "Marie-Baum-Platz",
          "Marie-Curie-Platz",
          "Marie-Heim-Vögtlin-Weg",
          "Marienstrasse",
          "Marktgasse",
          "Markusstrasse",
          "Marmorgasse",
          "Marsstrasse",
          "Marstallweg",
          "Martastrasse",
          "Martinsrütistrasse",
          "Martinstrasse",
          "Mascha-Kaléko-Weg",
          "Maschinenstrasse",
          "Massholderweg",
          "Massénastrasse",
          "Mathysweg",
          "Mattackerstrasse",
          "Mattengasse",
          "Mattenhof",
          "Mattensteg",
          "Max-Bill-Platz",
          "Max-Frisch-Platz",
          "Max-Högger-Strasse",
          "Medikerweg",
          "Meientalstrasse",
          "Meier-Bosshard-Strasse",
          "Meierhofplatz",
          "Meierholzstrasse",
          "Meierweg",
          "Meierwiesenstrasse",
          "Meierwiesenweg",
          "Meiliweg",
          "Meinrad-Lienert-Strasse",
          "Meinradstrasse",
          "Meisenweg",
          "Meret-Oppenheim-Weg",
          "Merkurplatz",
          "Merkurstrasse",
          "Metzgergasse",
          "Metzgerweg",
          "Micafilstrasse",
          "Michael-Maggi-Strasse",
          "Michelstrasse",
          "Milchbuckstrasse",
          "Militärbrücke",
          "Militärstrasse",
          "Mimosenstrasse",
          "Minervastrasse",
          "Mirabellenstrasse",
          "Mittagweg",
          "Mittelbergsteig",
          "Mittelstrasse",
          "Mittelwaldstrasse",
          "Molkenstrasse",
          "Mommsenstrasse",
          "Monikastrasse",
          "Moosacker",
          "Moosbergerweg",
          "Moosgutstrasse",
          "Moosholzstrasse",
          "Moosstrasse",
          "Morgartenstrasse",
          "Morgentalstrasse",
          "Morgenweg",
          "Moränenstrasse",
          "Motorenstrasse",
          "Moussonstrasse",
          "Muggenbühlerhölzliweg",
          "Muggenbühlstrasse",
          "Murhaldenweg",
          "Murwiesenstrasse",
          "Murwiesenweg",
          "Museumstrasse",
          "Mutschellenstrasse",
          "Mythenquai",
          "Möcklistrasse",
          "Möhrlistrasse",
          "Mösliweg",
          "Mötteliweg",
          "Mööslistrasse",
          "Mühlackerstrasse",
          "Mühlebachstrasse",
          "Mühlegasse",
          "Mühlehalde",
          "Mühlehaldensteig",
          "Mühlesteg",
          "Mühleweg",
          "Mühlezelgstrasse",
          "Müllerstrasse",
          "Münchhaldenstrasse",
          "Münchsteig",
          "Münsterbrücke",
          "Münstergasse",
          "Münsterhof",
          "Münzplatz",
          "Mürtschenstrasse",
          "Müseliweg",
          "Nagelfluhweg",
          "Naglerwiesenstrasse",
          "Nansenstrasse",
          "Napfgasse",
          "Naphtastrasse",
          "Narzissenstrasse",
          "Nebelbachstrasse",
          "Neeserweg",
          "Nelkenstrasse",
          "Neptunstrasse",
          "Nettie-Sutro-Strasse",
          "Neudorfstrasse",
          "Neue Hard",
          "Neuengasse",
          "Neufrankengasse",
          "Neugasse",
          "Neugutstrasse",
          "Neuhausstrasse",
          "Neumarkt",
          "Neumühlequai",
          "Neumünsterallee",
          "Neumünsterstrasse",
          "Neunbrunnenstrasse",
          "Neustadtgasse",
          "Neuwiesenstrasse",
          "Nidelbadstrasse",
          "Niederdorfstrasse",
          "Niederhofenrain",
          "Nietengasse",
          "Niklaus-Meienberg-Weg",
          "Niklausstrasse",
          "Norastrasse",
          "Nordbrücke",
          "Nordheimstrasse",
          "Nordsteig",
          "Nordstrasse",
          "Notzenschürlistrasse",
          "Nussbaumstrasse",
          "Nussgasse",
          "Nydeggstalden",
          "Näfgasse",
          "Nägelihof",
          "Nägelistrasse",
          "Nötzlistrasse",
          "Nürenbergstrasse",
          "Nürenbergweg",
          "Nüschelersteg",
          "Nüschelerstrasse",
          "Oberdorfstrasse",
          "Obere Bläsistrasse",
          "Obere Waidstrasse",
          "Obere Weiherstrasse",
          "Obere Zäune",
          "Oberer Bodenweg",
          "Oberer Gloriasteig",
          "Oberer Heuelsteig",
          "Oberer Kalberhauweg",
          "Oberer Selnauweg",
          "Oberholzstrasse",
          "Oberleestrasse",
          "Oberscheunenweg",
          "Oberwiesenstrasse",
          "Obmannamtsgasse",
          "Obstgartensteig",
          "Obstgartenstrasse",
          "Obsthaldenstrasse",
          "Obsthaldenweg",
          "Oerlikerpark",
          "Oerlikonerstrasse",
          "Oetenbacherholz",
          "Oetenbacherholzstrasse",
          "Oetenbachgasse",
          "Oetlisbergstrasse",
          "Oetlisbergweg",
          "Ohmstrasse",
          "Okenstrasse",
          "Oleanderstrasse",
          "Olgastrasse",
          "Olgaweg",
          "Olivengasse",
          "Opfikonstrasse",
          "Opfikonweg",
          "Orellistrasse",
          "Orelliweg",
          "Oscar-Bonomo-Weg",
          "Oskar-Bider-Strasse",
          "Ostbühlstrasse",
          "Othmarstrasse",
          "Ottenbergstrasse",
          "Ottenbühlweg",
          "Ottenweg",
          "Ottikerstrasse",
          "Ottikerweg",
          "Ottilienstrasse",
          "Otto-C.-Bänninger-Weg",
          "Otto-Jaag-Strasse",
          "Otto-Lang-Weg",
          "Otto-Schütz-Weg",
          "Ottostrasse",
          "Owenweg",
          "Panoramaweg",
          "Panweg",
          "Pappelstrasse",
          "Paradeplatz",
          "Paradiesstrasse",
          "Parkring",
          "Paul-Clairmont-Strasse",
          "Paul-Grüninger-Weg",
          "Paul-Pflüger-Strasse",
          "Paulstrasse",
          "Pelikanplatz",
          "Pelikanstrasse",
          "Pestalozzianlage",
          "Pestalozzistrasse",
          "Peterstobelstrasse",
          "Peterstobelweg",
          "Pfadiweg",
          "Pfaffhauserweg",
          "Pfalzgasse",
          "Pfarrhausstrasse",
          "Pfarrweg",
          "Pfingstweid",
          "Pfingstweidbrücke",
          "Pfingstweidstrasse",
          "Pfirsichstrasse",
          "Pflanzschulstrasse",
          "Pflaumenweg",
          "Pflugstrasse",
          "Physikstrasse",
          "Phönixweg",
          "Piazza Cella",
          "Pilatusstrasse",
          "Pilgerweg",
          "Plattenholzweg",
          "Plattenstrasse",
          "Plenterweg",
          "Polenweg",
          "Postbrücke",
          "Postgasse",
          "Poststrasse",
          "Predigergasse",
          "Predigerplatz",
          "Preyergasse",
          "Primelstrasse",
          "Probsteistrasse",
          "Probusweg",
          "Promenadengasse",
          "Püntstrasse",
          "Quaibrücke",
          "Quarzstrasse",
          "Quellenstrasse",
          "Querstrasse",
          "Quittenstrasse",
          "Radgasse",
          "Rainfussweg",
          "Rainstrasse",
          "Raintobelweg",
          "Rankstrasse",
          "Rathausbrücke",
          "Rathausgasse",
          "Rautihalde",
          "Rautistrasse",
          "Rebbergsteig",
          "Rebbergstrasse",
          "Rebeggstrasse",
          "Rebeggweg",
          "Rebenstrasse",
          "Rebenweg",
          "Rebgasse",
          "Rebhaldenstrasse",
          "Rebhügelstrasse",
          "Rebhüsliweg",
          "Rebstockweg",
          "Reckenholzstrasse",
          "Reckenholzweg",
          "Redingstrasse",
          "Regensbergstrasse",
          "Regensdorferstrasse",
          "Regina-Kägi-Hof",
          "Regina-Kägi-Strasse",
          "Reginastrasse",
          "Regulastrasse",
          "Rehalpstrasse",
          "Rehgässchen",
          "Reinacherstrasse",
          "Reinhardstrasse",
          "Reinhold-Frei-Strasse",
          "Reishauerstrasse",
          "Reitergasse",
          "Reitweg",
          "Rellstabstrasse",
          "Rellstabweg",
          "Remisenstrasse",
          "Renggersteig",
          "Renggerstrasse",
          "Rennweg",
          "Resedastrasse",
          "Reservoirweg",
          "Restelbergstrasse",
          "Ricarda-Huch-Strasse",
          "Richard-Kissling-Weg",
          "Richard-Wagner-Strasse",
          "Rickenstrasse",
          "Riedackerstrasse",
          "Riedenhaldensteig",
          "Riedenhaldenstrasse",
          "Riedenholzstrasse",
          "Riedenholzweg",
          "Riedgrabenweg",
          "Riedhoferrain",
          "Riedhofstrasse",
          "Riedhofweg",
          "Riedmattstrasse",
          "Riedtlistrasse",
          "Riedweg",
          "Riesbachstrasse",
          "Rieterplatz",
          "Rieterstrasse",
          "Rigiplatz",
          "Rigistrasse",
          "Rindermarkt",
          "Ringstrasse",
          "Rislingstrasse",
          "Risweg",
          "Ritterstrasse",
          "Robert-Maillart-Strasse",
          "Robert-Seidel-Hof",
          "Robert-Walser-Gasse",
          "Rodungsweg",
          "Roggenstrasse",
          "Rolandstrasse",
          "Rolf-Balsiger-Strasse",
          "Rollengasse",
          "Rolliweg",
          "Rosa-Bloch-Weg",
          "Rosenbühlstrasse",
          "Rosengartenbrücke",
          "Rosengartenstrasse",
          "Rosengasse",
          "Rosenhof",
          "Rosmarinweg",
          "Rossackerstrasse",
          "Rossbergstrasse",
          "Rosshalde",
          "Rosshauweg",
          "Roswiesenstrasse",
          "Rotachstrasse",
          "Rotbuchstrasse",
          "Rothstrasse",
          "Rotwandstrasse",
          "Rousseaustrasse",
          "Ruchackerstrasse",
          "Rudenzweg",
          "Rudolf-Brun-Brücke",
          "Rudolf-Hägi-Strasse",
          "Rudolfstrasse",
          "Ruedi-Walter-Strasse",
          "Ruggernweg",
          "Ruhestrasse",
          "Rumpelhaldensteig",
          "Rumpelhaldenweg",
          "Rumpumpsteig",
          "Russenbrünnelistrasse",
          "Russenweg",
          "Räffelstrasse",
          "Rämistrasse",
          "Rässlerweg",
          "Räuberweg",
          "Röhrenweg",
          "Römergasse",
          "Römerhofplatz",
          "Römerholzstrasse",
          "Röntgenplatz",
          "Röntgenstrasse",
          "Röschibachsteig",
          "Röschibachstrasse",
          "Röschibachweg",
          "Röslibrunnenweg",
          "Röslistrasse",
          "Rössligasse",
          "Rötelsteig",
          "Rötelstrasse",
          "Rötelterrasse",
          "Rübenweg",
          "Rückgasse",
          "Rüdenplatz",
          "Rüdigerstrasse",
          "Rümlangstrasse",
          "Rüteliweg",
          "Rütihofstrasse",
          "Rütihofweg",
          "Rütistrasse",
          "Rütiweg",
          "Rütiwiesweg",
          "Rütschistrasse",
          "Saatlenfussweg",
          "Saatlenstrasse",
          "Saatlenzelg",
          "Sackzelg",
          "Sagentobelbachweg",
          "Sagentobelstrasse",
          "Sagentobelweg",
          "Salersteig",
          "Salerstrasse",
          "Sallenbachstrasse",
          "Salomon-Vögelin-Strasse",
          "Salvatorstrasse",
          "Salzweg",
          "Samariterstrasse",
          "Sandacker",
          "Sandstrasse",
          "Saumackerstrasse",
          "Saumstrasse",
          "Schaffhauserplatz",
          "Schaffhauserstrasse",
          "Schafmattstrasse",
          "Schafmattweg",
          "Schanzackerstrasse",
          "Schanzeneggstrasse",
          "Schanzengasse",
          "Schanzengrabenpromenade",
          "Schauenbergstrasse",
          "Schaufelbergerstrasse",
          "Schauplatzgasse",
          "Scheffelstrasse",
          "Scheibenrain",
          "Scheideggstrasse",
          "Scheitergasse",
          "Scherrstrasse",
          "Scheuchzerstrasse",
          "Schienengasse",
          "Schienhutgasse",
          "Schiffbauplatz",
          "Schiffbaustrasse",
          "Schifflände",
          "Schiffländeplatz",
          "Schillerstrasse",
          "Schimmelstrasse",
          "Schindlersteig",
          "Schindlerstrasse",
          "Schipfe",
          "Schlatterweg",
          "Schlehdornweg",
          "Schleifergasse",
          "Schlierenberg",
          "Schlossergasse",
          "Schlossgasse",
          "Schluhenweg",
          "Schlösslistrasse",
          "Schlössliweg",
          "Schlüsselgasse",
          "Schmelzbergstrasse",
          "Schmidgasse",
          "Schmidhofgasse",
          "Schneckenmannstrasse",
          "Schneebelistrasse",
          "Schneebeliweg",
          "Schneeglöggliweg",
          "Schneggengasse",
          "Schoeckstrasse",
          "Schoffelgasse",
          "Schreberweg",
          "Schreinerstrasse",
          "Schrennengasse",
          "Schubertstrasse",
          "Schulerweg",
          "Schulhausstrasse",
          "Schulsteig",
          "Schulstrasse",
          "Schumacherweg",
          "Schuppisstrasse",
          "Schwalbenweg",
          "Schwamendingenstrasse",
          "Schwamendingerplatz",
          "Schwandenacker",
          "Schwandenholzstrasse",
          "Schwandenwiesen",
          "Schwanengasse",
          "Schwarzbächlistrasse",
          "Schwarzenbachweg",
          "Schweighofstrasse",
          "Schweigmatt",
          "Schweizergasse",
          "Schweizerhofgasse",
          "Schwellistrasse",
          "Schwendenholzweg",
          "Schwendenweg",
          "Schwertstrasse",
          "Schwertweg",
          "Schwingerstrasse",
          "Schächenstrasse",
          "Schäferweg",
          "Schäppistrasse",
          "Schäppiweg",
          "Schäracher",
          "Schärenfeld",
          "Schärenmoosstrasse",
          "Schärrergasse",
          "Schönauring",
          "Schönauweg",
          "Schönberggasse",
          "Schönbühlstrasse",
          "Schöneggplatz",
          "Schöneggstrasse",
          "Schöneichstrasse",
          "Schönleinstrasse",
          "Schöntalstrasse",
          "Schörlistrasse",
          "Schörliweg",
          "Schürbungert",
          "Schürgistrasse",
          "Schürliweg",
          "Schürwies",
          "Schützengasse",
          "Schützenmatt",
          "Schützenrain",
          "Schützeweg",
          "Sechseläutenplatz",
          "Seebacherstrasse",
          "Seebahnstrasse",
          "Seeblickstrasse",
          "Seeblickweg",
          "Seefeldquai",
          "Seefeldstrasse",
          "Seegartenstrasse",
          "Seehofstrasse",
          "Seeholzweg",
          "Seerosenstrasse",
          "Seestrasse",
          "Seewiesenstrasse",
          "Segantinisteig",
          "Segantinistrasse",
          "Segetenweg",
          "Segnesstrasse",
          "Seidengasse",
          "Seilergraben",
          "Selnaubrücke",
          "Selnaustrasse",
          "Seminarstrasse",
          "Sempacherstrasse",
          "Sempersteig",
          "Sennhauserweg",
          "Serpentinweg",
          "Sieberstrasse",
          "Siegfriedstrasse",
          "Siewerdtstrasse",
          "Signaustrasse",
          "Sihlamtsstrasse",
          "Sihlberg",
          "Sihlbrücke",
          "Sihlfeldstrasse",
          "Sihlhallenstrasse",
          "Sihlhölzlibrücke",
          "Sihlhölzlistrasse",
          "Sihlporte",
          "Sihlpromenade",
          "Sihlquai",
          "Sihlrainstrasse",
          "Sihlstrasse",
          "Sihlweidstrasse",
          "Sillerweg",
          "Sillerwies",
          "Silvrettaweg",
          "Simmlersteig",
          "Singlistrasse",
          "Siriusstrasse",
          "Soldanellastrasse",
          "Solisweg",
          "Sommerauweg",
          "Sonderistrasse",
          "Sonneggsteig",
          "Sonneggstrasse",
          "Sonnenberghölzliweg",
          "Sonnenbergstrasse",
          "Sonnenrain",
          "Sonnhaldenstrasse",
          "Sonntagsteig",
          "Soodstrasse",
          "Sophie-Albrecht-Weg",
          "Sophie-Taeuber-Strasse",
          "Sophienstrasse",
          "Spalierweg",
          "Spanweidstrasse",
          "Spatenstrasse",
          "Spechtweg",
          "Speerstrasse",
          "Speicherweg",
          "Spelteriniweg",
          "Sperletweg",
          "Spiegelgasse",
          "Spiegelhofstrasse",
          "Spielweg",
          "Spielwiesenstrasse",
          "Spillmannweg",
          "Spindelstrasse",
          "Spirgartenstrasse",
          "Spiserstrasse",
          "Spitalbachweg",
          "Spitalgasse",
          "Spitzackerstrasse",
          "Splügenstrasse",
          "Sportweg",
          "Sprecherstrasse",
          "Sprensenbühlstrasse",
          "Spulenweg",
          "Spyriplatz",
          "Spyristeig",
          "Spyristrasse",
          "Spöndlistrasse",
          "St. Annagasse",
          "St. Jakobstrasse",
          "St. Moritzstrasse",
          "St. Peterhofstatt",
          "St. Peterstrasse",
          "St. Urbangasse",
          "Stadelhoferplatz",
          "Stadelhoferstrasse",
          "Stadthausquai",
          "Stadtweg",
          "Staffelhof",
          "Staffelstrasse",
          "Stampfenbachplatz",
          "Stampfenbachstrasse",
          "Stampfenbrunnenstrasse",
          "Stapferstrasse",
          "Starengasse",
          "Stationsstrasse",
          "Staubstrasse",
          "Staudenbühlstrasse",
          "Staudenbühlweg",
          "Staudenweg",
          "Stauffacherbrücke",
          "Stauffacherquai",
          "Stauffacherstrasse",
          "Steffenstrasse",
          "Stegengasse",
          "Steigholzweg",
          "Steinackerweg",
          "Steinbockgasse",
          "Steinbrüchelstrasse",
          "Steinentischstrasse",
          "Steinfelsplatz",
          "Steinhaldenstrasse",
          "Steinhausweg",
          "Steinkluppenweg",
          "Steinmühlegasse",
          "Steinmühleplatz",
          "Steinstrasse",
          "Steinwiesenweg",
          "Steinwiesplatz",
          "Steinwiesstrasse",
          "Stelzenstrasse",
          "Stephan-à-Porta-Weg",
          "Sternenstrasse",
          "Sternwartstrasse",
          "Stettbacherrain",
          "Stettbachstrasse",
          "Stettbachweg",
          "Stierenriedweg",
          "Stierliweg",
          "Stiglenstrasse",
          "Stockenwiesweg",
          "Stockerstrasse",
          "Stodolastrasse",
          "Stoffelstrasse",
          "Stolzestrasse",
          "Storchengasse",
          "Stotzstrasse",
          "Stotzweg",
          "Strandbadweg",
          "Strassburgstrasse",
          "Strehlgasse",
          "Streitholzstrasse",
          "Streulistrasse",
          "Strickhofstrasse",
          "Studackerstrasse",
          "Stäblistrasse",
          "Stöckenackerstrasse",
          "Stöckengasse",
          "Stöckenhaldenweg",
          "Stöckentobelstrasse",
          "Stöckentobelweg",
          "Stöckenweg",
          "Stüdliweg",
          "Stüssihofstatt",
          "Stüssistrasse",
          "Sumatrasteig",
          "Sumatrastrasse",
          "Sunnige Hof",
          "Susanna-Gossweiler-Platz",
          "Susenbergstrasse",
          "Sustenstrasse",
          "Sustenweg",
          "Synagogengasse",
          "Sädleneggweg",
          "Sädlenweg",
          "Säntisstrasse",
          "Südstrasse",
          "Süsslerenstrasse",
          "Süsslerenweg",
          "Talacker",
          "Talbächliweg",
          "Talchernsteig",
          "Talchernstrasse",
          "Talstrasse",
          "Talweg",
          "Talwiesenstrasse",
          "Tannenrain",
          "Tannenrauchstrasse",
          "Tannenstrasse",
          "Technoparkstrasse",
          "Tellstrasse",
          "Tessinerplatz",
          "Teufwiesenstrasse",
          "Theaterstrasse",
          "Therese-Giehse-Strasse",
          "Thermengasse",
          "Thomasweg",
          "Thujastrasse",
          "Thurgauerstrasse",
          "Thurwiesenstrasse",
          "Tiefenhöfe",
          "Tischlerweg",
          "Titlisstrasse",
          "Tièchestrasse",
          "Tobeleggstrasse",
          "Tobeleggweg",
          "Tobelhofstrasse",
          "Tobelholzweg",
          "Tobelsteig",
          "Toblerplatz",
          "Toblerstrasse",
          "Torgasse",
          "Traktorenstrasse",
          "Tramstrasse",
          "Traubenstrasse",
          "Traugottstrasse",
          "Treichlerstrasse",
          "Trichtenhausenfussweg",
          "Trichtenhausenstrasse",
          "Trichtisalweg",
          "Triemlifussweg",
          "Triemlihalde",
          "Triemlistrasse",
          "Trittligasse",
          "Trottenstrasse",
          "Tuggenerstrasse",
          "Tuggenerweg",
          "Tulpenstrasse",
          "Tulpenweg",
          "Tunnelstrasse",
          "Turbinenplatz",
          "Turbinenstrasse",
          "Turnersteig",
          "Turnerstrasse",
          "Tuschgenweg",
          "Töbeliweg",
          "Tödistrasse",
          "Töpferstrasse",
          "Tüffenwies",
          "Ueberlandpark",
          "Ueberlandstrasse",
          "Uetliberghalde",
          "Uetlibergstrasse",
          "Uetlibergweg",
          "Uhlandstrasse",
          "Ulmbergstrasse",
          "Ulmenweg",
          "Ulrichstrasse",
          "Unionstrasse",
          "Universitätstrasse",
          "Untere Zäune",
          "Unterer Pfaffhauserweg",
          "Unterer Selnauweg",
          "Unterfeldstrasse",
          "Untergraben",
          "Unterholzstrasse",
          "Unterholzweg",
          "Untermoosstrasse",
          "Unterwerkstrasse",
          "Uraniastrasse",
          "Urselweg",
          "Usteribrücke",
          "Usteristrasse",
          "Utobrücke",
          "Utoplatz",
          "Utoquai",
          "Varlinweg",
          "Veilchenstrasse",
          "Venusstrasse",
          "Verena-Conzett-Strasse",
          "Verenastrasse",
          "Verrucanoweg",
          "Vetterliweg",
          "Viaduktstrasse",
          "Viktoriastrasse",
          "Viktoriaweg",
          "Vogelsangstrasse",
          "Vogtsrain",
          "Volkmarstrasse",
          "Voltastrasse",
          "Vorderberg",
          "Vorhaldenstrasse",
          "Vulkanplatz",
          "Vulkanstrasse",
          "Waaggasse",
          "Wachtelstrasse",
          "Wachterweg",
          "Waffenplatzstrasse",
          "Wagnergasse",
          "Waidbadstrasse",
          "Waidbergweg",
          "Waidfussweg",
          "Waidhofweg",
          "Waidmattweg",
          "Waidstrasse",
          "Waisenhausstrasse",
          "Walchebrücke",
          "Walcheplatz",
          "Walchestrasse",
          "Walchetor",
          "Walchetreppe",
          "Waldgartenweg",
          "Waldhausstrasse",
          "Waldhüslistrasse",
          "Waldkauzweg",
          "Waldmannstrasse",
          "Waldmeisterweg",
          "Waldschulweg",
          "Waldstrasse",
          "Wallisackerweg",
          "Wallisellenstrasse",
          "Waltersbachstrasse",
          "Waltersteig",
          "Wannenholzstrasse",
          "Wannenweg",
          "Wannerstrasse",
          "Wartauweg",
          "Wartstrasse",
          "Wasersteig",
          "Waserstrasse",
          "Wasserschöpfi",
          "Wasserwerkstrasse",
          "Wattstrasse",
          "Weberstrasse",
          "Wechselwiesenstrasse",
          "Wegackerstrasse",
          "Weggengasse",
          "Wehntalerstrasse",
          "Wehrenbachhalde",
          "Wehrlisteig",
          "Weibelackerstrasse",
          "Weibelackerweg",
          "Weichengasse",
          "Weidmannstrasse",
          "Weiherholzstrasse",
          "Weihersteig",
          "Weinbergfussweg",
          "Weinbergstrasse",
          "Weineggstrasse",
          "Weineggweg",
          "Weingartenweg",
          "Weingasse",
          "Weinplatz",
          "Weisshau",
          "Weite Gasse",
          "Weitlingweg",
          "Weizenstrasse",
          "Welchogasse",
          "Weltistrasse",
          "Wengistrasse",
          "Werdgartengasse",
          "Werdgässchen",
          "Werdhölzlistrasse",
          "Werdinsel",
          "Werdmühleplatz",
          "Werdmühlestrasse",
          "Werdplatz",
          "Werdstrasse",
          "Werikonweg",
          "Werkgasse",
          "Wernerstrasse",
          "Westbühlstrasse",
          "Weststrasse",
          "Wetlistrasse",
          "Wettingerwies",
          "Wettsteinstrasse",
          "Wibichstrasse",
          "Wickenweg",
          "Widdergasse",
          "Widmerstrasse",
          "Widumweg",
          "Wiedingsteig",
          "Wiedingstrasse",
          "Wiesendangerstrasse",
          "Wiesenstrasse",
          "Wieslergasse",
          "Wiesliacher",
          "Wildbachstrasse",
          "Wildenstrasse",
          "Wildenweg",
          "Wildfangweg",
          "Wilfriedstrasse",
          "Wilhelmstrasse",
          "Winkelriedstrasse",
          "Winkelstrasse",
          "Winkelwiese",
          "Winterthurerstrasse",
          "Winzerhalde",
          "Winzerstrasse",
          "Wipkingerbrücke",
          "Wipkingerpark",
          "Wipkingerplatz",
          "Wipkingerweg",
          "Wirzenweid",
          "Wissmannstrasse",
          "Witellikerstrasse",
          "Witikonerstrasse",
          "Wohllebgasse",
          "Wohnstadion Kirchenacker",
          "Wolfbachstrasse",
          "Wolfbachtobelweg",
          "Wolfgang-Pauli-Strasse",
          "Wolfgrimweg",
          "Wolfswinkel",
          "Woloweg",
          "Wonnebergstrasse",
          "Wotanstrasse",
          "Wuhrstrasse",
          "Wunderlistrasse",
          "Wydenstrasse",
          "Wydlerweg",
          "Wydäckerring",
          "Wyssenbühlsteig",
          "Wyssgasse",
          "Wäldlistrasse",
          "Wässerlingweg",
          "Wühre",
          "Würzgrabenstrasse",
          "Würzwies",
          "Zahnradstrasse",
          "Zanggerweg",
          "Zapfensteig",
          "Zederstrasse",
          "Zehnderweg",
          "Zehntenhausplatz",
          "Zehntenhausstrasse",
          "Zeisigweg",
          "Zelghalde",
          "Zelglistrasse",
          "Zelgmatt",
          "Zelgstrasse",
          "Zelgwiesenstrasse",
          "Zellerstrasse",
          "Zeltweg",
          "Zentralstrasse",
          "Zeppelinstrasse",
          "Zeughausgasse",
          "Zeughausstrasse",
          "Zeugwartgasse",
          "Zeunersteig",
          "Zeunerstrasse",
          "Ziegeleiweg",
          "Ziegelhüttenstrasse",
          "Ziegelstrasse",
          "Zielackerstrasse",
          "Zielweg",
          "Zimmergasse",
          "Zimmerlistrasse",
          "Zinistrasse",
          "Zinnengasse",
          "Zollbrücke",
          "Zollikerstrasse",
          "Zollingerweg",
          "Zollstrasse",
          "Zooweg",
          "Zschokkestrasse",
          "Zum Sillerblick",
          "Zur frohen Aussicht",
          "Zurlindenstrasse",
          "Zweiackerstrasse",
          "Zweierplatz",
          "Zweierstrasse",
          "Zwielplatz",
          "Zwingliplatz",
          "Zwinglistrasse",
          "Zwirnerhalde",
          "Zwirnerstrasse",
          "Zwischenbächen",
          "Zwyssigstrasse",
          "Zwängiweg",
          "Zypressenstrasse",
          "Zähringerplatz",
          "Zähringerstrasse",
          "Zöllystrasse",
          "Züghusplatz",
          "Zürichbergstrasse",
          "Zürichholzstrasse",
        ],
    };
  }
}
