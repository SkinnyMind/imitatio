import 'package:imitatio/src/datasets/models/finance.dart';

class ArDzFinanceData extends FinanceDataset {
  @override
  List<String> get banks {
    return const [
      "بنك الجزائر الخارجي",
      "البنك الوطني الجزائري",
      "بنك الفلاحة والتنمية الريفية",
      "بنك التنمية المحلية",
      "القرض الشعبي الجزائري",
      "الصندوق الوطني للتوفير والاحتياط",
      "بنك البركة الجزائري",
      "المؤسسة المصرفية العربية - الجزائر",
      "ناتيكسيس - الجزائر",
      "سوسيتي جينيرال - الجزائر",
      "سيتي بنك - الجزائر",
      "البنك العربي - الجزائر",
      "بي ان بي باريباس - الجزاير",
      "ترست بنك - الجزائر",
      "بنك الخليج - الجزائر",
      "بنك الإسكان للتجارة والتمويل - الجزائر",
      "فرانس-بنك الجزاير",
      "بنك السلام - الجزائر",
      "اش اس بي سي - الجزائر",
      "البنك الوطني للإسكان",
    ];
  }

  @override
  List<String> get companies {
    return const [
      "الجزائرية للمياه",
      "(شركة المياه و التطهير للجزائر (سيال",
      "المؤسسة العمومية للتلفزيون",
      "المؤسسة الوطنية لعتاد الأشغال العمومية",
      "سوناطراك",
      "إتراج",
      "اتصالات الجزائر",
      "الإذاعة الجزائرية",
      "البث الإذاعي والتلفزي الجزائري",
      "ياليدين",
      "الجزائرية للطرق السيارة",
      "الخطوط الجوية الجزائرية",
      "الديوان الوطني الجزائري للسياحة",
      "الشركة الجزائرية لصناعة المروحيات",
      "الشركة الوطنية للعربات الصناعية",
      "الشركة الوطنية للنقل بالسكك الحديدية",
      "الشركة الوطنية للهندسة المدنية والبناء",
      "المؤسسة الوطنية للصناعات الإلكترونية",
      "المؤسسة الوطنية للصناعات الكهرومنزلية",
      "سوقرال",
      "النقل البحري الجزائري",
      "الوكالة الجزائرية للإشعاع الثقافي",
      "الوكالة الوطنية للدراسات و متابعة إنجاز الاستثمارات في السكك الحديدية",
      "بريد الجزائر",
      "ترامواي الجزائر",
      "تيليفيريك",
      "سونلغاز",
      "سيرتا",
      "شركة الجرارات الجزائرية",
      "سيترام",
      "صيدال",
      "طيران الطاسيلي",
      "مترو الجزائر",
      "قناة الجزائر الدولية",
      "مترو وهران",
      "مجموعة كوسيدار",
      "مدار القابضة",
      "مرسيدس بنز الجزائر",
      "مصنع سيتال",
      "موبيليس",
      "هيبروك للنقل البحري",
      "وجهات الخطوط الجوية الجزائرية",
      "إريس",
      "إفري",
      "اش بي تكنولوجي",
      "الباسو سوناطراك للغاز الطبيعي المسال",
      "السلام إلكترونيك",
      "المعهد الجزائري للبترول",
      "المعهد الجزائري للتقييس",
      "جيون إلكترونيك",
      "حمود بوعلام",
      "دار البرزخ للنشر",
      "سيفيتال",
      "فاغور براند",
      "كابريس",
      "كالغاز",
      "كوبرا إلكترونيكس",
      "كوندور",
      "مايكروسوفت الجزائر",
      "مجمع أمودا للهندسة",
      "مصنع كريباور",
      "نومهيد",
      "ورود",
      "يسير",
    ];
  }

  @override
  List<String> companyTypes({required bool isAbbr}) {
    return switch (isAbbr) {
      true => const ["شركة", "مجمع"],
      false => const ["شركة", "مجمع"],
    };
  }

  @override
  String get currencyCode => "DZD";

  @override
  String get currencySymbol => 'دج';
}
