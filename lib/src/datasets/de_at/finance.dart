// It's ok to have long strings in that file
// ignore_for_file: lines_longer_than_80_chars

import 'package:imitatio/src/datasets/models/finance.dart';

class DeAtFinanceData extends FinanceDataset {
  @override
  List<String> get banks {
    return const [
      "Erste Group Bank AG",
      "Raiffeisen Bank International AG",
      "UniCredit Bank Austria AG",
      "BAWAG Group AG",
      "Oesterreichische Nationalbank",
      "Raiffeisenlandesbank Oberösterreich AG",
      "Raiffeisenlandesbank Niederösterreich-Wien AG",
      "Raiffeisen-Landesbank Tirol AG",
      "Raiffeisen-Landesbank Steiermark AG",
      "Erste Bank der oesterreichischen Sparkassen AG",
      "BKS Bank AG",
      "Hypo Vorarlberg Bank AG",
      "Raiffeisenlandesbank Kärnten - Rechenzentrum und Servicegesellschaft mbH",
      "Bank für Tirol und Vorarlberg AG",
      "Hypo Noe Gruppe Bank AG",
      "R-QUADRAT Bank AG",
      "Raiffeisen Bank Burgenland regGenmbH",
      "HYPO Oberösterreich Vermögensverwaltung AG",
      "Schoellerbank AG",
      "Hyposwiss Privatbank AG",
      "DenizBank AG",
      "Oberbank AG",
      "Bankhaus Schelhammer & Schattera AG",
      "Investkredit Bank AG",
      "UniCredit Bank Austria AG",
      "Hypo Steiermark AG",
      "Raiffeisenbankengruppe Kärnten",
      "Hypo Alpe-Adria-Bank International AG",
      "Hypo Salzburg Bank AG",
      "Allgemeine Sparkasse Oberösterreich Bankaktiengesellschaft",
      "Volksbank Wien AG",
      "BTV VIER LÄNDER BANK",
      "Bankhaus Carl Spängler & Co. Aktiengesellschaft",
      "Bankhaus Krentschker & Co. AG",
      "Hypo Investmentbank AG",
      "Porsche Bank AG",
      "Easybank AG",
      "Merkur Bank KGaA",
      "Donau Bank AG",
      "Schwarzach Wertpapier AG",
      "bank99",
      "Addiko Bank AG",
      "Bankhaus Jungholz",
      "FondsBank Austria GmbH",
      "BKS Bank AG Croatia Branch",
      "VakifBank International AG",
      "Yapi Kredi Bank Austria",
      "Hypo Tirol Bank AG",
      "Concordia Banking Software",
      "Bankhaus Spängler AG",
      "HYPO-BANK BURGENLAND AG",
      "Erste Bank Hungary Zrt.",
      "Kärntner Sparkasse Aktiengesellschaft",
      "Hypo Oberösterreich Holding AG",
      "CRIF GmbH",
      "Coface Austria Kreditversicherungsmakler GmbH",
      "UniCredit Leasing Austria GmbH",
      "Creditreform Rating Aktiengesellschaft",
      "FactoringBank Gesellschaft m.b.H.",
      "Hyposwiss Advisors AG",
    ];
  }

  @override
  List<String> get companies {
    return const [
      "AB Effectenbeteiligungen",
      "AGRANA Beteiligungs",
      "ANMATHE Beteiligungs",
      "AT&S Austria Technologie & Systemtechnik",
      "Andritz",
      "Athos Immobilien",
      "Austria Email",
      "Austriamicrosystems",
      "BDI – BioEnergy International",
      "BEKO Holding",
      "BKS Bank",
      "BWT",
      "Bank für Kärnten und Steiermark",
      "Bank für Tirol und Vorarlberg",
      "Bene",
      "Binder+Co",
      "Brain Force Holding",
      "Burgenland Holding",
      "Böhler-Uddeholm",
      "C-Quadrat Investment",
      "C.A.T. Oil",
      "CA Immobilien Anlagen",
      "CA Immobilien International",
      "CPI Immobilien",
      "CPI Thes Immobilien",
      "Capexit II CEE Private Equity Invest",
      "Christ Water Technology",
      "Constantia Packaging",
      "Do & Co Restaurants & Catering",
      "ECO Business-Immobilien",
      "EMTS Technologie",
      "ETV Holding",
      "EVN",
      "EYBL International",
      "Erste Bank der österreichischen Sparkassen",
      "Erste Group Bank",
      "Euromarketing i.A.",
      "Fabasoft",
      "Feratel Media Technologies",
      "Flughafen Wien",
      "Frauenthal Holding",
      "Generali Holding Vienna",
      "Gericom",
      "Gurktaler Aktiengesellschaft",
      "HTA Beteiligungs-Invest",
      "HTI High Tech Industries",
      "Hirsch Servo",
      "Hutter & Schrantz",
      "IPO Board.Net",
      "Immofinanz",
      "Imperial Hotels Austria",
      "Inku",
      "Josef Manner & Comp.",
      "KTM Power Sports",
      "Kapsch Trafficcom",
      "Kreco Realitäten",
      "L.A.I. Beteiligungs-Investment",
      "Lenzing",
      "Linz Textil Holding",
      "Management Trust Holding",
      "Maschinenfabrik Heid",
      "Mayr-Melnhof Karton",
      "Miba",
      "OMV",
      "Oberbank",
      "Ottakringer Brauerei",
      "PLA Eurologistics",
      "Palfinger",
      "Pankl & Hoffmann",
      "Pankl Racing Systems",
      "Plaut",
      "Polytec Holding",
      "Porr",
      "Prime Site Immobilien",
      "Private Equity Performance Beteiligungs",
      "RHI",
      "Raiffeisen International Bank+Holding",
      "Rath",
      "Rosenbauer International",
      "S&T System Integration & Technology Distribution",
      "SLAV Handel, Vertretung und Beteiligung",
      "STRABAG SE",
      "SW Umwelttechnik Stoiser & Wolschner",
      "Sanochemia Pharmazeutika",
      "Schlumberger",
      "Schoeller-Bleckmann Oilfield Equipment",
      "Semperit Holding",
      "Sparkassen-Immobilienanlagen",
      "Stadlauer Malzfabrik",
      "TG Holding",
      "Teak Holz International",
      "Telekom Austria",
      "UBM Realitätenentwicklung",
      "UNIQA Versicherungen",
      "Unternehmens Invest",
      "Update software",
      "Vorarlberger Landes- und Hypothekenbank",
      "Warimpex Finanz- und Beteiligungs",
      "Wiener Privatbank Immobilieninvest",
      "Wiener Städtische Allgemeine Versicherung Vienna Insurance Group",
      "Wienerberger",
      "Wolford",
      "Zumtobel",
      "bwin Interactive Entertainment",
      "conwert Immobilien Invest SE",
      "Österreichische Elektrizitätswirtschafts",
      "Österreichische Post",
      "Österreichische Volksbanken",
    ];
  }

  @override
  List<String> companyTypes({required bool isAbbr}) {
    return switch (isAbbr) {
      true => const [
        "AG",
        "GesbR",
        "GmbH",
        "GmbH & Co. KG",
        "KEG",
        "KG",
        "OEG",
        "OG",
      ],
      false => const [
        "Aktiengesellschaft",
        "Gesellschaft des bürgerlichen Rechts",
        "Gesellschaft mit beschränkter Haftung",
        "Kommanditerwerbsgesellschaft",
        "Offene Erwerbsgesellschaft",
      ],
    };
  }

  @override
  String get currencyCode => "EUR";

  @override
  String get currencySymbol => "€";
}
