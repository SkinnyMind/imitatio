import 'package:imitatio/src/datasets/models/person.dart';
import 'package:imitatio/src/enums.dart';

class FaPersonData extends PersonDataset {
  @override
  List<String> get academicDegrees {
    return const ["لیسانس", "فوق لیسانس", "دکتری"];
  }

  @override
  List<String> get genders {
    return const ["مذکر", "مؤنث", "دیگر"];
  }

  @override
  List<String> get languages {
    return const [
      "آدیغه",
      "آستوری",
      "آسی",
      "آفریکانس",
      "آلبانیایی",
      "آلمانی",
      "آمهاری",
      "آذربایجانی",
      "اردو",
      "ارمنی",
      "ازبکی",
      "اسپانیایی",
      "اسپرانتو",
      "استونیایی",
      "اسلواکیایی",
      "اسلوونیایی",
      "انگلیسی",
      "اندونزیایی",
      "اوکراینی",
      "اویغوری",
      "ایتالیایی",
      "ایرلندی",
      "ایسلندی",
      "باسکی",
      "بختیاری",
      "بلاروسی",
      "بلغاری",
      "بلوچی",
      "بنگالی",
      "بوسنیایی",
      "براهویی",
      "پرتغالی",
      "پشتو",
      "تاتاری",
      "تاتی",
      "تالشی",
      "تاگالوگ",
      "تایلندی",
      "ترکی",
      "ترکی",
      "ترکمنی",
      "جاوه‌ای",
      "چکی",
      "چواش",
      "چینی",
      "دانمارکی",
      "روسی",
      "رومانیایی",
      "ژاپنی",
      "ساکسونی",
      "سانسکریت",
      "سومالی",
      "سوئدی",
      "سواحیلی",
      "سیسیلی",
      "سینهالی",
      "صربی",
      "عبری",
      "عربی",
      "فارسی",
      "فرانسوی",
      "فریزی",
      "فنلاندی",
      "قرقیزی",
      "قزاقی",
      "کاتالان",
      "کردی",
      "کردی",
      "کردی",
      "کرواتی",
      "کره‌ای",
      "گیلکی",
      "گِیلیک",
      "گرجی",
      "گالیسی",
      "لاتین",
      "لتلندی",
      "لری",
      "لوکزامبورگی",
      "لهستانی",
      "لیتوانیایی",
      "لکی",
      "ماراتی",
      "مازندرانی",
      "مالایی",
      "ماندارین",
      "مجاری",
      "مغولی",
      "مین",
      "لاری",
      "نپالی",
      "نروژی",
      "نروژی",
      "والونی",
      "ولزی",
      "ویتنامی",
      "هلندی",
      "هندی",
      "یونانی",
      "یاقوتی",
    ];
  }

  @override
  List<String> names(Gender gender) {
    return switch (gender) {
      Gender.female => const [
        "آبگینه",
        "آبنوس",
        "آتنا",
        "آتوسا",
        "اختر",
        "آذر",
        "آذرافروز",
        "آذرگون",
        "آزرمیدخت",
        "آذرنوش",
        "آذین",
        "آرا",
        "آرتمیس",
        "آرزو",
        "آرمینا",
        "آرمینه",
        "آرمیتا",
        "آریانا",
        "آریادخت",
        "آریانوش",
        "آرینا",
        "آزاده",
        "آسا",
        "آسانا",
        "آستیاژ",
        "آفتاب",
        "آفرین",
        "آگرین",
        "آلا",
        "آلاء",
        "آلاله",
        "آناهیتا",
        "آندریا",
        "آنوشا",
        "آوا",
        "آوید",
        "آویده",
        "آویزه",
        "آویسا",
        "آوین",
        "آوینه",
        "آهو",
        "ابریشم",
        "اختر",
        "ارغوان",
        "اروشا",
        "استر",
        "افروز",
        "افروغ",
        "افسانه",
        "افسر",
        "افسون",
        "افشان",
        "افشار",
        "آرتمیس",
        "اومتی",
        "امتیس",
        "امیتیس",
        "اناهید",
        "اندیشه",
        "انوشه",
        "ایران",
        "ایرمن",
        "ایران بانو",
        "ایراندخت",
        "بانو",
        "بنفشه",
        "بهار",
        "بهاران",
        "بهاره",
        "بهارک",
        "بهدیس",
        "بهدخت",
        "بهرخ",
        "بهشید",
        "بهناز",
        "بهنوش",
        "بوسه",
        "بیتا",
        "باران",
        "پانیذ",
        "پائیزه",
        "پارمیس",
        "پدیده",
        "پرتو",
        "پردیس",
        "پرستو",
        "پرند",
        "پرنیا",
        "پرنیان",
        "پروانه",
        "پروین",
        "پری",
        "پریا",
        "پریچهر",
        "پریدخت",
        "پریرو",
        "پریزاد",
        "پریسا",
        "پریشاد",
        "پریناز",
        "پریوش",
        "پگاه",
        "پوپک",
        "پوران",
        "پوران",
        "پوراندخت",
        "پوری",
        "پونه",
        "پژهان",
        "پیمانه",
        "پرنیان",
        "پارمیدا",
        "پادینا",
        "تابنده",
        "ترانه",
        "ترسا",
        "ترمه",
        "ترنه",
        "تهمینه",
        "تیشتر",
        "تیر",
        "ترنگ",
        "توراندخت",
        "تینا",
        "ته‌لار",
        "ته‌نیا",
        "تریفه",
        "تیروژ",
        "جوانه",
        "جوان",
        "چکامه",
        "خجسته",
        "خورشید",
        "خوروش",
        "دانه",
        "دری",
        "دریا",
        "دلارا",
        "دلارام",
        "دلبر",
        "دلکش",
        "دلوسه",
        "دوکا",
        "دینا",
        "رامش",
        "راشر",
        "رامینا",
        "رسا",
        "رها",
        "رهام",
        "روان",
        "رود",
        "رودابه",
        "روجا",
        "روشنک",
        "روناک",
        "رؤیا",
        "رکسانا",
        "روژینا",
        "رازان",
        "ریزان",
        "زری",
        "زرین",
        "زرین‌دخت",
        "زویا",
        "زیبا",
        "زینا",
        "زیور",
        "زایه‌له",
        "ژاله",
        "ژرفا",
        "ژیلا",
        "ژینر",
        "ژینا",
        "سارک",
        "سارینا",
        "ساغر",
        "ساناز",
        "سایه",
        "سپیده",
        "ستاره",
        "ستوده",
        "سرگل",
        "سروناز",
        "سمانه",
        "سودابه",
        "سوری",
        "سوزان",
        "سوسن",
        "سوگل",
        "سوگند",
        "سیما",
        "سیمین",
        "شاپرک",
        "شاپری",
        "شادان",
        "شادی",
        "شانا",
        "شانار",
        "شایسته",
        "شبره",
        "شبنم",
        "شرمینه",
        "شکیبا",
        "شمیلا",
        "شمیم",
        "شهپر",
        "شهربانو",
        "شهرخ",
        "شهربانو",
        "شهرزاد",
        "شهرناز",
        "شهره",
        "شهناز",
        "شهین",
        "شوکا",
        "شقایق",
        "شکوفه",
        "شکوه",
        "شکیلا",
        "شیدا",
        "شیده",
        "شیرین",
        "شیرین‌بانو",
        "شیفته",
        "شیلا",
        "شیما",
        "شیوا",
        "شورانگیز",
        "غمزه",
        "غنچه",
        "فاختک",
        "فرانک",
        "فرشته",
        "فروزنده",
        "فروغ",
        "فرنیا",
        "فریبا",
        "فریما",
        "فرین",
        "فریناز",
        "فوژان",
        "فیروزه",
        "فریماه",
        "فرنوش",
        "قشنگ",
        "کتایون",
        "کمند",
        "کرانه",
        "کشور",
        "کیان",
        "کیهانه",
        "گردآفرید",
        "گلاره",
        "گلاوژ",
        "گل‌افروز",
        "گل‌اندام",
        "گلبانو",
        "گلبهار",
        "گلابتون",
        "گلبیز",
        "گلپا",
        "گلپری",
        "گلپونه",
        "گلتاج",
        "گلتن",
        "گلرخ",
        "گلرو",
        "گلشن",
        "گلشید",
        "گلشیفته",
        "گلمیس",
        "گلنار",
        "گلناز",
        "گلنوش",
        "گل‌ورچین",
        "گلی",
        "گوهر",
        "گوهرشاد",
        "گیتا",
        "گیتی",
        "گیسو",
        "گیلا",
        "گیلان",
        "لادن",
        "لاله",
        "لاله‌رخ",
        "مازون",
        "مامک",
        "مانا",
        "ماندان",
        "ماندانا",
        "مانلی",
        "ماه‌پری",
        "ماهتو",
        "ماه‌چهر",
        "ماهدخت",
        "ماه‌برزین",
        "ماهنوش",
        "ماه‌جهان",
        "ماهزاد",
        "ماهک",
        "مرجان",
        "مرجانه",
        "مروارید",
        "مژده",
        "مژگان",
        "مسل",
        "مستانه",
        "مشکان‌دخت",
        "مشکین‌دخت",
        "مونا",
        "منیژه",
        "مهتا",
        "مهتاب",
        "مهدخت",
        "مهدیس",
        "مهرآذر",
        "مهرا",
        "مهراد",
        "مهراب",
        "مهرانگیز",
        "مهرانه",
        "مهربان",
        "مهرخ",
        "مهرریز",
        "مهرسا",
        "مهروا",
        "مهرک",
        "مهری",
        "مهزاد",
        "مهسا",
        "مهستی",
        "مهشید",
        "مهکامه",
        "مهناز",
        "مهنوش",
        "مهکامه",
        "مهین",
        "موژان",
        "میترا",
        "میخک",
        "میگل",
        "مینا",
        "مینو",
        "نازآفرین",
        "نازگل",
        "نازنین",
        "نازیلا",
        "ناژیلا",
        "ناژین",
        "ناهید",
        "نرگس",
        "نرمین",
        "نسترن",
        "نسرین",
        "نگار",
        "نگین",
        "نهال",
        "نوا",
        "نوش‌آفرین",
        "نوشزاد",
        "نوشین",
        "نیلوفر",
        "نیکو",
        "نیکی",
        "نیکسا",
        "نیشا",
        "نیوشا",
        "ویس",
        "ویدا",
        "هستی",
        "هما",
        "هنگامه",
        "هوروش",
        "یارا",
        "یاس",
        "یاسمن",
        "یاسمین",
        "یگانه",
        "یکتا",
        "یسنا",
        "یوتاب",
      ],
      Gender.male => const [
        "آبتین",
        "آتش",
        "آترین",
        "آتورپات",
        "آذرخش",
        "آذرمه",
        "آذین",
        "آرتین",
        "آرام",
        "آراز",
        "آراد",
        "آرتا",
        "آرتان",
        "آرش",
        "آرشام",
        "آرمان",
        "آرمین",
        "آروین",
        "آریا",
        "آریابان",
        "آریانا",
        "آرین",
        "آریوبرزن",
        "آزاد",
        "آزرمگان",
        "آساد",
        "آستیاژ",
        "آستیاگ",
        "آلان",
        "آوه",
        "آویژه",
        "آیین‌گشسب",
        "آریو",
        "آریافر",
        "آریامن",
        "آریامنش",
        "آریامهر",
        "آریان",
        "اخگر",
        "ارخشا",
        "ارد",
        "اردشیر",
        "اردلان",
        "اردوان",
        "ارژنگ",
        "ارس",
        "ارشا",
        "آرشام",
        "ارشک",
        "ارشیا",
        "ارمایل",
        "اروند",
        "اژدر",
        "اسفندیار",
        "اشک",
        "اشکان",
        "اشکبوس",
        "افراسیاب",
        "افشار",
        "افشین",
        "البرز",
        "الوند",
        "امید",
        "انوش",
        "انوشیروان",
        "اورمزد",
        "اورنگ",
        "ایرج",
        "ایشتوویگو",
        "بابک",
        "باربد",
        "بامداد",
        "بامین",
        "بامشاد",
        "بردیا",
        "بزرگمهر",
        "به‌آیین",
        "بهبد",
        "بهرام",
        "بهمن",
        "بهمنش",
        "بیژن",
        "بهروز",
        "بهراد",
        "بهرنگ",
        "بهزاد",
        "بهداد",
        "بهشاد",
        "بهنام",
        "برسام",
        "برزو",
        "برنا",
        "برزن",
        "برومند",
        "برزین",
        "بهداد",
        "تیرداد",
        "تهماسب",
        "تهمتن",
        "تهمورث",
        "تورج",
        "تابان",
        "تیریژ",
        "جاماسب",
        "جاوید",
        "جمشید",
        "جهانگیر",
        "جهان‌بخش",
        "جهانشاه",
        "جهاندار",
        "جهان",
        "چنگیز",
        "چه‌کو",
        "چیا",
        "چیاکو",
        "چومان",
        "خداداد",
        "خدابخش",
        "خدایار",
        "خسرو",
        "خشایار",
        "خوبیار",
        "خورشید",
        "خوناس",
        "خناو",
        "دارا",
        "دادویه",
        "دادمهر",
        "دامون",
        "دانوش",
        "دانو",
        "داراب",
        "داریوش",
        "دیاکو",
        "رادین",
        "رامبد",
        "رامین",
        "رامتین",
        "رخشان",
        "رستان",
        "روزبه",
        "رادمان",
        "رامان",
        "رها",
        "رهام",
        "راد",
        "رستم",
        "ریونیز",
        "زاب",
        "زال",
        "زامیاد",
        "‍‍‍‍‍زاوش",
        "زرتشت",
        "زردشت",
        "زرمهر",
        "زند",
        "زوبین",
        "ژوبین",
        "سالار",
        "ساسان",
        "سامان",
        "سام",
        "سروش",
        "سپهر",
        "سپنتا",
        "سورن",
        "سورنا",
        "سوشیانت",
        "سوشیانس",
        "سهراب",
        "سهند",
        "سیامک",
        "سیاوش",
        "سیروس",
        "سینا",
        "شادمهر",
        "شروین",
        "شاپور",
        "شایا",
        "شایان",
        "شاهرخ",
        "شاهین",
        "شباهنگ",
        "شهرام",
        "شاهکام",
        "شهراد",
        "شهاب",
        "شهرداد",
        "شهریار",
        "شهروز",
        "شیرزاد",
        "شیرنگ",
        "شهیار",
        "شهبد",
        "شهنام",
        "شوپه",
        "شیریزدان",
        "شاهو",
        "شیروان",
        "شه‌مال",
        "شه‌مزین",
        "شیرکو",
        "شورش",
        "فرزان",
        "فرسا",
        "فرامرز",
        "فرزاد",
        "فرزام",
        "فرزین",
        "فردین",
        "فرداد",
        "فرود",
        "فرورتیش",
        "فرمان",
        "فرهنگ",
        "فریبرز",
        "فرناد",
        "فرهاد",
        "فرنام",
        "فرهام",
        "فرهود",
        "فروهر",
        "فیروز",
        "فرشاد",
        "فرشید",
        "فرخ",
        "فرخزاد",
        "فربد",
        "فهام",
        "فریا",
        "کوروش",
        "کارن",
        "کامبیز",
        "کامران",
        "کامشاد",
        "کامیار",
        "کاوه",
        "کاووس",
        "کنارنگ",
        "کورس",
        "کورش",
        "کیانوش",
        "کیارش",
        "کیاوش",
        "کیا",
        "کیان",
        "کیوس",
        "کوشا",
        "کیخسرو",
        "کیقباد",
        "کیکاووس",
        "کیوان",
        "کیومرث",
        "کاردو",
        "کاروان",
        "کاروخ",
        "کازیوه",
        "کارزان",
        "کاژاو",
        "کوماس",
        "گرگین",
        "گستهم",
        "گیو",
        "گودرز",
        "گرسیوز",
        "گرشاسب",
        "گشتاسب",
        "گلینوش",
        "گوران",
        "گووه‌ند",
        "لهراسب",
        "لاس",
        "لاوچاک",
        "لیزان",
        "مهدیار",
        "مازیار",
        "ماکان",
        "مانشت",
        "مانی",
        "ماهان",
        "مرداس",
        "مهرگان",
        "مرداویج",
        "مزدک",
        "منوچهر",
        "مهبد",
        "مهداد",
        "مهرداد",
        "مهرزاد",
        "مهرشاد",
        "مهران",
        "مهزاد",
        "مهراب",
        "مهرشاد",
        "مهزیار",
        "مهیار",
        "مهرنگ",
        "میلاد",
        "مه‌ریوان",
        "میلان",
        "میران",
        "نرمن",
        "نریمان",
        "نکیسا",
        "نوید",
        "نیما",
        "نیوشا",
        "نوژن",
        "نوذر",
        "نیاسا",
        "ناسیکو",
        "نسکو",
        "نه‌به‌ز",
        "نه‌سره‌و",
        "نه‌هروز",
        "نه‌وشیروان",
        "نه‌که‌روز",
        "نچیروان",
        "ونداد",
        "وریا",
        "وندیداد",
        "وهومن",
        "هامون",
        "هرمز",
        "هورمزد",
        "هورموند",
        "هومن",
        "هوشنگ",
        "هوتن",
        "هومان",
        "هوشیار",
        "هورداد",
        "یاور",
        "یزدگرد",
        "یزدان",
        "یادگار",
      ],
    };
  }

  @override
  List<String> nationalities(Gender gender) {
    return switch (gender) {
      _ => const [
        "افغان",
        "آلبانیایی",
        "الجزایری",
        "اهل آندورا",
        "اهل آنگولا",
        "آرژانتینی",
        "ارمنی",
        "استرالیایی",
        "اتریشی",
        "آذربایجانی",
        "اهل باهاما",
        "بحرینی",
        "بنگلادشی",
        "اهل باربادوس",
        "بلاروسی",
        "بلژیکی",
        "اهل بلیز",
        "اهل بنین",
        "بوتانی",
        "بولیویایی",
        "بوسنیایی",
        "اهل بوتسوانا",
        "برزیلی",
        "اهل بونئی",
        "بلغاری",
        "اهل بورکینیا فاسو",
        "اهل بروندی",
        "کامبوجی",
        "کامرونی",
        "کانادایی",
        "اهل چاد",
        "شیلیایی",
        "چینی",
        "کلمبیایی",
        "کنگویی",
        "کاستاریکایی",
        "کروات",
        "کوبایی",
        "قبرسی",
        "اهل جمهوری چک",
        "دانمارکی",
        "اهل جیبوتی",
        "اهل دومینیکا",
        "اهل جمهوری دومنیکن",
        "اهل اکوادور",
        "مصری",
        "اهل السالوادور",
        "انگلیسی",
        "ایتره ای",
        "استونیایی",
        "اتیوپیایی",
        "اهل فیجی",
        "فنلاندی",
        "فرانسوی",
        "گابنی",
        "گامبیایی",
        "گرجی",
        "آلمانی",
        "غنایی",
        "یونانی",
        "اهل گرانادا",
        "گواتمالایی",
        "اهل گینه",
        "اهل گویان",
        "اهل هایتی",
        "هندوراسی",
        "مجاری",
        "ایسلندی",
        "هندی",
        "اندونزیایی",
        "ایرانی",
        "عراقی",
        "ایرلندی",
        "ایتالیایی",
        "جامائیکایی",
        "ژاپنی",
        "اردنی",
        "قزاق",
        "کنیایی",
        "کویتی",
        "لائوسی",
        "لیتوانیایی",
        "لبنانی",
        "لیبریایی",
        "لیبیایی",
        "لیتوانیایی",
        "مقدونیه ای",
        "اهل ماداگاسکار",
        "اهل مالاوی",
        "مالزیایی",
        "مالدیوی",
        "اهل مالی",
        "اهل مالت",
        "موریتانیایی",
        "اهل موریس",
        "مکزیکی",
        "اهل موناکو",
        "مغول",
        "مونتنگرویی",
        "مراکشی",
        "موزامبیکی",
        "برمه ای",
        "نامیبیایی",
        "نپالی",
        "هلندی",
        "نیوزلندی",
        "نیکاراگوئه ای",
        "اهل نیجر",
        "نیجریه ای",
        "کره ای",
        "نروژی",
        "عمانی",
        "پاکستانی",
        "فلستینی",
        "پانامایی",
        "اهل گینه نو",
        "پاراگوئه ای",
        "پرویی",
        "فیلیپینی",
        "لهستانی",
        "پرتغالی",
        "قطری",
        "رومانیایی",
        "روس",
        "رواندایی",
        "سعودی",
        "سنگالی",
        "صرب",
        "اهل سیشل",
        "اهل سیرالئون",
        "سنگاپور",
        "اسلواک",
        "اسلوونیایی",
        "سومالیایی",
        "آفریقایی",
        "کره ای",
        "اسپانیایی",
        "سریلانکایی",
        "سودانی",
        "سرینامی",
        "اهل سوازیلند",
        "سوئدی",
        "سوئیسی",
        "سوری",
        "تایوانی",
        "تاجیک",
        "تانزانیایی",
        "تایلندی",
        "توگویی",
        "اهل ترینیداد و توباگو",
        "تونسی",
        "ترک",
        "ترکمن",
        "اهل توالو",
        "اوگاندایی",
        "اکراینی",
        "اماراتی",
        "آمریکایی",
        "اروگوئه ای",
        "ازبک",
        "اهل وانوواتو",
        "ونزوئلایی",
        "ویتنامی",
        "یمنی",
        "زامبیایی",
        "زیمباوه ای",
      ],
    };
  }

  @override
  List<String> get occupations {
    return const [
      "پزشک خانواده",
      "جراح",
      "متخصص قلب",
      "روان پزشک",
      "دکتر پوست",
      "پزشک زنان",
      "پزشک متخصص زایمان",
      "پرتوشناس، رادیولوژیست",
      "روانشناس",
      "معنی",
      "رئیس دانشکده",
      "معلم",
      "استاد دانشگاه",
      "معلم خصوصی",
      "معلم ریاضی",
      "شاگرد خصوصی",
      "معنی",
      "محقق",
      "ریاضی دان",
      "شیمی دان",
      "گیاه شناس",
      "تاریخ دان",
      "جغرافی دان",
      "روان شناس",
      "جامعه شناس",
      "فیلسوف",
      "معنی",
      "نقاش",
      "معمار",
      "رهبر (ارکستر)",
      "خواننده",
      "تهیه کننده",
      "نویسنده",
      "مولف",
      "نمایش نامه نویس",
      "گزارشگر",
      "طراح",
      "طراح گرافیک",
      "معنی",
      "مهندس",
      "تعمیر کار برق",
      "نجار",
      "نقاش",
      "سرایدار",
      "خدمتکار (زن)",
      "معنی",
      "مهندس پرواز",
      "راننده",
      "راننده تاکسی",
      "ملوان",
      "معنی",
      "وکیل",
      "افسر پلیس",
      "مامور ترافیک",
      "نگهبان، پاسدار",
      "رئیس زندان",
      "معنی",
      "آشپز",
      "معنی",
      "متصدی کامپیوتر",
      "توسع دهنده وب",
      "برنامه نویس اندروید",
      "معنی",
      "فوتبالیست",
      "کشتی گیر",
      "کمک داور",
      "معنی",
      "خیاط",
      "پستچی",
      "سیاستمدار",
      "مترجم",
      "کتابدار",
      "معدنچی",
      "ماهیگیر",
      "جنگلبان",
      "آرایشگر",
      "کشیش",
      "معنی",
      "دکتر",
      "پزشک عمومی",
      "چشم پزشک",
      "پزشک اطفال",
      "متخصص بی هوشی",
      "متخصص غدد",
      "متخصص اعصاب",
      "جراح پلاستیک",
      "روانکاو",
      "معنی",
      "مدیر مدرسه",
      "پروفسور",
      "معلم مدرسه",
      "استاد دانشگاه",
      "مشاور",
      "دانش آموز",
      "معنی",
      "دانشمند",
      "مخترع",
      "فیزیک دان",
      "زیست شناس",
      "جانور شناس",
      "باستان شناس",
      "زمین شناس",
      "اقتصاد دان",
      "زبان شناس",
      "معنی",
      "هنرمند",
      "مجسمه ساز، پیکر تراش",
      "آهنگساز",
      "موسیقی دان",
      "کارگردان",
      "فیلم بردار",
      "شاعر",
      "ناشر",
      "روزنامه نگار",
      "عکاس",
      "طراح لباس",
      "معنی",
      "کارگر ساختمان",
      "مکانیک، تعمیر کار",
      "جوشکار",
      "لوله کش",
      "تعمیر کار",
      "نظافت چی زن",
      "معنی",
      "خلبان",
      "مهماندار پرواز",
      "راننده (خصوصی)",
      "راننده اتوبوس",
      "معنی",
      "قاضی",
      "مشاور قانونی",
      "مامور پلیس",
      "کارآگاه",
      "محافظ شخصی",
      "نگهبان زندان",
      "معنی",
      "سرآشپز",
      "معنی",
      "برنامه نویس کامپیوتر",
      "متخصص نرم افزار",
      "برنامه نویس وب",
      "طراح وب",
      "معنی",
      "مربی",
      "تیرانداز",
      "داور",
      "روزنامه نگار",
      "معنی",
      "آتش نشان",
      "خیاط زن",
      "مدل",
      "ویراستار",
      "مترجم",
      "جواهر فروش",
      "کشاورز",
      "صیاد",
      "باغبان",
      "مشاور",
    ];
  }

  @override
  List<String> get politicalViews {
    return const [
      "بی تفاوت",
      "آنارشیسم",
      "لیبرال",
      "کمونیست",
      "در حد متوسط",
      "سوسیالیست",
      "محافظه کار",
      "طرفدار ازادی فردی",
    ];
  }

  @override
  List<String> surnames(Gender gender) {
    return switch (gender) {
      _ => const [
        "آدینه",
        "آریان‌پور",
        "آژند",
        "آشتیانی",
        "آشنا",
        "آشوری",
        "آقاجری",
        "آهنگر",
        "آهنگری",
        "آهی",
        "آیتی",
        "ابتکار",
        "ابریشمی",
        "ابطحی",
        "ابوذر",
        "ادب",
        "ارسباران",
        "استادی",
        "اشتری",
        "اشراقی",
        "اشکوری",
        "اصفهانی",
        "اصلانی",
        "اعتبار",
        "اعتماد",
        "اعلم",
        "افخم",
        "افخمی",
        "اللهیاری",
        "الهام",
        "امانت",
        "امانی",
        "امیری",
        "امین‌زاده",
        "انتظامی",
        "انوار",
        "انوری",
        "انوشه",
        "اوستا",
        "ایمانی",
        "باستانی",
        "باطنی",
        "باغچه‌بان",
        "باهنر",
        "بحرینی",
        "بخاری",
        "بختیار",
        "بختیاری",
        "بدخشانی",
        "برزویی",
        "بزرگ‌نیا",
        "بهاور",
        "بهبهانی",
        "بهشتی",
        "بیگی",
        "پارسا",
        "پارسی",
        "پازارگاد",
        "پازوکی",
        "پایا",
        "پایدار",
        "پایور",
        "پستا",
        "پناهنده",
        "پناهی",
        "پناهیان",
        "پورناظری",
        "پورنگ",
        "پیران",
        "پیرحیاتی",
        "پیوندی",
        "تابش",
        "تبریزی",
        "ترکاشوند",
        "ترکان",
        "توسلی",
        "توفیق",
        "توفیقی",
        "توکل",
        "توکلیان",
        "تهرانی",
        "ثابتی",
        "جعفریان",
        "جمادی",
        "جنتی",
        "جهانبگلو",
        "جهانگیری",
        "جهانی",
        "چاوشی",
        "چگنی",
        "چلبی",
        "چنگیزی",
        "حائری",
        "حجتی",
        "حسابی",
        "حقانی",
        "حقیقی",
        "حکمت",
        "حکمی",
        "حکیمی",
        "حیاتی",
        "خاتمی",
        "خامنه‌ای",
        "خاموشی",
        "خداپناهی",
        "خدایی",
        "خراسانی",
        "خرم‌آبادی",
        "خسروپناه",
        "خمسه",
        "خوئینی",
        "خوئینی‌ها",
        "خیابانی",
        "داد",
        "دانایی‌فر",
        "دانایی‌فرد",
        "داودی",
        "داور",
        "دباغ",
        "درگاهی",
        "دری",
        "دستغیب",
        "دهقان",
        "دیباج",
        "ذاکری",
        "راسخ",
        "راوندی",
        "رجایی",
        "رحماندوست",
        "رحمانیان",
        "رستمی",
        "رسولی",
        "رفیعی",
        "رنجبر",
        "رنگرز",
        "روحانی",
        "روزبه",
        "رهنما",
        "زارع",
        "زالی",
        "زرشناس",
        "زمردیان",
        "زهرایی",
        "زین‌الدین",
        "ساعی",
        "سبحانی",
        "سبزواری",
        "سپه‌وند",
        "ستاری",
        "سحاب",
        "سحابی",
        "سراج",
        "سرشار",
        "سرمد",
        "سروستانی",
        "سروش",
        "سلامت",
        "سیف",
        "سیف‌زاده",
        "شادمهر",
        "شاملو",
        "شاه‌حسینی",
        "شاهی",
        "شبستری",
        "شجاعی",
        "شرع‌پسند",
        "شرف",
        "شریعتمداری",
        "شریعتی",
        "شریف",
        "شریفیان",
        "شفا",
        "شهیدی",
        "شیخ‌الاسلامی",
        "شیرازی",
        "شیرمحمدی",
        "شیروانی",
        "صانعی",
        "صباغ",
        "صدر",
        "صدیق",
        "صدیقی",
        "صغیری",
        "صفوی",
        "ضابطی",
        "طالب‌زاده",
        "طالقانی",
        "طباطبائی",
        "طبیب‌زاده",
        "طریقت",
        "ظریف",
        "عارف",
        "عاشوری",
        "عالی",
        "عبادی",
        "عبدالکریمی",
        "عبدالملکی",
        "عراقی",
        "عزیزی",
        "عصار",
        "عقیلی",
        "علم",
        "علم‌الهدی",
        "علی عسگری",
        "علی‌آبادی",
        "علیا",
        "علی‌پور",
        "علی‌زمانی",
        "عنایت",
        "غضنفری",
        "غنی",
        "فارسی",
        "فاطمی",
        "فانی",
        "فتاحی",
        "فرامرزی",
        "فرج",
        "فرشیدورد",
        "فرمانفرمائیان",
        "فروتن",
        "فرهنگ",
        "فریاد",
        "فنایی",
        "فنی‌زاده",
        "فولادوند",
        "فهمیده",
        "قاضی",
        "قانعی",
        "قانونی",
        "قمیشی",
        "قنبری",
        "قهرمان",
        "قهرمانی",
        "قهرمانیان",
        "قهستانی",
        "کاکاوند",
        "کامکار",
        "کاملی",
        "کاویانی",
        "کدیور",
        "کردبچه",
        "کرمانی",
        "کریمی",
        "کلباسی",
        "کمالی",
        "کوشکی",
        "کهنمویی",
        "کیان",
        "کیانی",
        "گل محمدی",
        "گلپایگانی",
        "گنجی",
        "لاجوردی",
        "لاچینی",
        "لاهوتی",
        "لنکرانی",
        "لوکس",
        "مجاهد",
        "مجتبایی",
        "مجتبوی",
        "مجتهد شبستری",
        "مجتهدی",
        "مجرد",
        "محجوب",
        "محجوبی",
        "محدثی",
        "محمدرضایی",
        "محمدی",
        "مددی",
        "مرادخانی",
        "مرتضوی",
        "مستوفی",
        "مشا",
        "مصاحب",
        "مصباح",
        "مصباح‌زاده",
        "مطهری",
        "مظفر",
        "معارف",
        "معروف",
        "معین",
        "مفتاح",
        "مقدم",
        "ملایری",
        "ملک",
        "ملکیان",
        "منوچهری",
        "موحد",
        "موسوی",
        "موسویان",
        "مهاجرانی",
        "مهدی‌پور",
        "میرباقری",
        "میرزاده",
        "میرسپاسی",
        "میزبانی",
        "ناظری",
        "نامور",
        "نجفی",
        "ندوشن",
        "نراقی",
        "نعمت‌زاده",
        "نقدی",
        "نقیب‌زاده",
        "نواب",
        "نوبخت",
        "نوبختی",
        "نهاوندی",
        "نیشابوری",
        "واثقی",
        "واعظ",
        "واعظ‌زاده",
        "واعظی",
        "وکیلی",
        "هاشمی",
        "هاشمی رفسنجانی",
        "هاشمیان",
        "هامون",
        "هدایت",
        "هراتی",
        "هروی",
        "همایون",
        "همت",
        "همدانی",
        "هوشیار",
        "هومن",
        "یاحقی",
        "یادگار",
        "یثربی",
        "یلدا",
        "ضرغام",
      ],
    };
  }

  @override
  List<String> get telephoneFormats {
    return const [
      "+98-###-########",
      "+98-912-########",
      "###-########",
      "+98-41-3###-####",
      "061-####-####",
      "+98-61-####-####",
      "026-####-####",
      "314-26-####-####",
    ];
  }

  @override
  List<String> titles({required Gender gender, required TitleType titleType}) {
    return switch (gender) {
      Gender.female =>
        titleType == TitleType.academic
            ? const ["دکتر", "پروفسور", "مهندس", "کارشناس ارشد مدیریت"]
            : const ["خانم", "بانو", "دوشیزه"],
      Gender.male =>
        titleType == TitleType.academic
            ? const ["دکتر", "پروفسور", "مهندس", "کارشناس ارشد مدیریت"]
            : const ["آقای", "استاد"],
    };
  }

  @override
  List<String> get universities {
    return const [
      "علوم تحقیقات و فناوری",
      "بهداشت، درمان و آموزش پزشکی",
      "دانشگاه فرهنگیان",
      "تربیت معلم",
      "وزارت آموزش و پرورش",
      "دانشگاه جامع علمی کاربردی",
      "وزارت علوم",
      "دانشگاه تهران",
      "دانشگاه علم و صنعت",
      "دانشگاه صنعتی امیرکبیر",
      "دانشگاه فردوسی مشهد",
      "دانشگاه محقق اردبیلی",
      "دانشگاه بین‌المللی امام خمینی قزوین",
      "دانشگاه زنجان",
      "دانشگاه سمنان",
      "دانشگاه لرستان",
      "دانشگاه قم",
      "دانشگاه آیت‌الله‌العظمی بروجردی",
      "دانشگاه آیت‌الله‌العظمی حائری میبد",
      "دانشگاه اراک",
      "دانشگاه اردکان",
      "دانشگاه ارومیه",
      "دانشگاه اصفهان",
      "دانشگاه الزهرا",
      "دانشگاه ایلام",
      "دانشگاه بجنورد",
      "دانشگاه بناب",
      "دانشگاه بوعلی سینا همدان",
      "دانشگاه بیرجند",
      "دانشگاه بین‌المللی امام خمینی قزوین",
      "دانشگاه تبریز",
      "دانشگاه تخصصی فناوری‌های نوین آمل",
      "دانشگاه تربت حیدریه",
      "دانشگاه تربیت دبیر شهید رجایی تهران",
      "دانشگاه تربیت مدرس",
      "دانشگاه تفرش",
      "دانشگاه تهران",
      "دانشگاه جامع علمی کاربردی",
      "دانشگاه جیرفت",
      "دانشگاه جهرم",
      "دانشگاه حضرت نرجس رفسنجان",
      "دانشگاه حکیم سبزواری",
      "دانشگاه خلیج فارس بوشهر",
      "دانشگاه خوارزمی تهران",
      "دانشگاه دامغان",
      "دانشگاه دخترانه حضرت معصومه قم",
      "دانشگاه دریانوردی و علوم دریایی چابهار",
      "دانشگاه رازی کرمانشاه",
      "دانشگاه زابل",
      "دانشگاه زنجان",
      "دانشگاه سلمان فارسی کازرون",
      "دانشگاه سید جمال‌الدین اسدآبادی",
      "دانشگاه سمنان",
      "دانشگاه سیستان و بلوچستان",
      "دانشگاه شهرضا",
      "دانشگاه شاهد",
      "دانشگاه شهرکرد",
      "دانشگاه شهید باهنر کرمان",
      "دانشگاه تحصیلات تکمیلی صنعتی کرمان",
      "دانشگاه شهید بهشتی",
      "دانشگاه شهید چمران اهواز",
      "دانشگاه شهید مدنی آذربایجان",
      "دانشگاه شیراز",
      "دانشگاه صنعت آب و برق",
      "دانشگاه صنعتی اراک",
      "دانشگاه صنعتی ارومیه",
      "دانشگاه صنعتی اصفهان",
      "دانشگاه صنعتی امیرکبیر",
      "دانشگاه صنعتی نوشیروانی بابل",
      "دانشگاه صنعتی بیرجند",
      "دانشگاه صنعتی جندی‌شاپور دزفول",
      "دانشگاه صنعتی خاتم‌الانبیا",
      "دانشگاه صنعتی خواجه نصیرالدین طوسی",
      "دانشگاه صنعتی سهند",
      "دانشگاه صنعتی سیرجان",
      "دانشگاه صنعتی شاهرود",
      "دانشگاه صنعتی شریف",
      "دانشگاه صنعتی شیراز",
      "دانشگاه صنعتی قم",
      "دانشگاه صنعتی کرمانشاه",
      "دانشگاه صنعتی همدان",
      "دانشگاه صنعتی هویزه",
      "دانشگاه علامه طباطبایی",
      "دانشگاه علم و صنعت ایران",
      "دانشگاه علم و فناوری مازندران",
      "دانشگاه علوم اقتصادی",
      "دانشگاه علوم دریایی خرمشهر",
      "دانشگاه علوم کشاورزی و منابع طبیعی رامین",
      "دانشگاه علوم کشاورزی و منابع طبیعی ساری",
      "دانشگاه علوم کشاورزی و منابع طبیعی گرگان",
      "دانشگاه فردوسی مشهد",
      "دانشگاه فرزانگان سمنان",
      "دانشگاه فسا",
      "دانشگاه فنی و حرفه‌ای",
      "دانشگاه فنی ومهندسی بوئین زهرا",
      "دانشگاه قم",
      "دانشگاه کاشان",
      "دانشگاه کردستان",
      "دانشگاه کوثر بجنورد",
      "دانشگاه گرمسار",
      "دانشگاه گلستان",
      "دانشگاه گنبد کاووس",
      "دانشگاه گیلان",
      "دانشگاه لرستان",
      "دانشگاه مازندران",
      "دانشگاه محقق اردبیلی",
      "دانشگاه مراغه",
      "دانشگاه ملایر",
      "دانشگاه مهندسی علوم و فناوری‌های نوین گلبهار",
      "دانشگاه مهندسی فناوری‌های نوین قوچان",
      "دانشگاه نیشابور",
      "دانشگاه ولایت ایرانشهر",
      "دانشگاه ولی عصر رفسنجان",
      "دانشگاه هرمزگان",
      "دانشگاه هنر اسلامی تبریز",
      "دانشگاه هنر اصفهان",
      "دانشگاه هنر تهران",
      "دانشگاه یاسوج",
      "دانشگاه یزد",
      "مؤسسه آموزش عالی راهبرد شمال رشت",
      "دانشکده ریاضی و کامپیوتر خوانسار",
      "دانشکده فنی و مهندسی گلپایگان",
      "دانشکده کشاورزی شیروان",
      "دانشکده کشاورزی و دامپروری تربت جام",
      "دانشکده کشاورزی و منابع طبیعی داراب",
      "مجتمع آموزش عالی اسفراین",
      "مجتمع آموزش عالی بم",
      "مجتمع آموزش عالی سراوان",
      "مجتمع آموزش عالی لارستان",
      "مجتمع آموزش عالی گناباد",
      "مرکز آموزش عالی استهبان",
      "مرکز آموزش عالی اقلید",
      "مرکز آموزش عالی کاشمر",
      "مرکز آموزش عالی لامرد",
      "مرکز آموزش عالی محلات",
      "مرکز آموزش عالی ممسنی",
      "مرکز تحصیلات تکمیلی در علوم پایه زنجان",
      "دانشگاه فنی و حرفه‌ای",
      "دانشکده فنی شهید شمسی پور",
      "دانشکده فنی پسرانه خوارزمی ملایر",
      "دانشکده فنی شهید بهشتی کرج",
      "دانشکده فنی پسرانه خوارزمی ملایر",
      "دانشکده فنی و حرفه‌ای دختران دکتر شریعتی",
      "دانشکده فنی و حرفه‌ای کوثر گنبدکاووس",
      "دانشکده فنی شهید شمسی پور",
      "دانشکده فنی شهیدجباریان همدان",
      "دانشکده فنی شهید صدوقی یزد",
      "دانشکده فنی و حرفه‌ای پسران شهرکرد",
      "آموزشکده فنی پسران قم",
      "دانشکده فنی امام علی (ع) کردکوی",
      "دانشکده فنی امام محمد باقر ساری",
      "دانشکده فنی شماره ۲ ساری",
      "دانشکده فنی امیرکبیر اراک",
      "دانشکده فنی انقلاب اسلامی",
      "دانشکده فنی دارالفنون بجنورد",
      "دانشکده فنی ولیعصر (عج) تهران",
      "دانشکده فنی شماره یک تبریز",
      "دانشکده فنی شهید چمران کرمان",
      "دانشکده فنی شهید باهنر شیراز",
      "دانشکده فنی . حرفه‌ای الزهرا(س) مشهد",
      "دانشکده فنی سروش اصفهان",
      "دانشکده فنی شهید مهاجر",
      "آموزشکده فنی و حرفه‌ای پسران خمین",
      "دانشکده فنی پسران نجف آباد",
      "دانشکده فنی پسران میانه",
      "وزارت بهداشت، درمان و آموزش پزشکی",
      "دانشگاه علوم پزشکی نیشابور",
      "دانشگاه علوم پزشکی ارتش",
      "دانشگاه علوم پزشکی اراک",
      "دانشگاه علوم پزشکی اردبیل",
      "دانشگاه علوم پزشکی ارومیه",
      "دانشگاه علوم پزشکی اصفهان",
      "دانشگاه علوم پزشکی ایران",
      "دانشگاه علوم پزشکی ایلام",
      "دانشگاه علوم پزشکی بابل",
      "دانشگاه علوم پزشکی بقیةالله",
      "دانشگاه علوم پزشکی بم",
      "دانشگاه علوم پزشکی هرمزگان",
      "دانشگاه علوم پزشکی بوشهر",
      "دانشگاه علوم پزشکی بیرجند",
      "دانشگاه علوم پزشکی تبریز",
      "دانشگاه علوم پزشکی تربت حیدریه",
      "دانشگاه علوم پزشکی تهران",
      "دانشگاه علوم پزشکی جندی‌شاپور اهواز",
      "دانشگاه علوم پزشکی جهرم",
      "دانشگاه علوم پزشکی جیرفت",
      "دانشگاه علوم پزشکی رفسنجان",
      "دانشگاه علوم پزشکی زابل",
      "دانشگاه علوم پزشکی زاهدان",
      "دانشگاه علوم پزشکی زنجان",
      "دانشگاه علوم پزشکی سمنان",
      "دانشگاه علوم پزشکی شاهرود",
      "دانشگاه علوم پزشکی شهرکرد",
      "دانشگاه علوم پزشکی شهید بهشتی",
      "دانشگاه علوم پزشکی شیراز",
      "دانشگاه علوم پزشکی فسا",
      "دانشگاه علوم پزشکی قزوین",
      "دانشگاه علوم پزشکی قم",
      "دانشگاه علوم پزشکی کاشان",
      "دانشگاه علوم پزشکی کردستان",
      "دانشگاه علوم پزشکی کرمان",
      "دانشگاه علوم پزشکی کرمانشاه",
      "دانشگاه علوم پزشکی گلستان",
      "دانشگاه علوم پزشکی گیلان",
      "دانشگاه علوم پزشکی مازندران",
      "دانشگاه علوم پزشکی مراغه",
      "دانشگاه علوم پزشکی مشهد",
      "دانشگاه علوم پزشکی گناباد",
      "دانشگاه علوم پزشکی همدان",
      "دانشگاه علوم پزشکی یاسوج",
      "دانشگاه علوم پزشکی یزد",
      "دانشگاه علوم بهزیستی و توانبخشی",
      "دانشکده علوم پزشکی اراک",
      "دانشکده علوم پزشکی ارتش",
      "دانشکده علوم پزشکی اردبیل",
      "دانشکده علوم پزشکی ارومیه",
      "دانشکده علوم پزشکی اصفهان",
      "دانشکده علوم پزشکی ایلام",
      "دانشکده پیراپزشکی نهاوند",
      "دانشکده علوم پزشکی گراش(فارس)",
      "دانشگاه علوم پزشکی دزفول",
      "دانشکده علوم پزشکی سبزوار",
      "دانشکده علوم پزشکی لرستان",
      "دانشکده پرستاری آمل",
      "شعبه بین‌المللی دانشگاه علوم پزشکی زاهدان (چابهار)",
      "مرکز آموزش عالی علوم پزشکی ممسنی",
      "آموزش و پرورش",
      "دانشگاه فرهنگیان",
      "شورای عالی انقلاب فرهنگی",
      "تربیت معلم",
      "محمود مهرمحمدی",
      "دانشگاه افسری امام علی",
      "دانشگاه اقلید",
      "دانشگاه امام صادق",
      "دانشگاه جامع امام حسین",
      "دانشکده پست و مخابرات ایران",
      "دانشگاه شهید مطهری",
      "دانشکده صدا و سیما",
      "دانشگاه صنایع و معادن ایران",
      "دانشگاه صنعت نفت",
      "دانشگاه صنعتی اردبیل",
      "دانشگاه صنعتی سیستان و بلوچستان",
      "دانشگاه صنعتی مالک اشتر",
      "دانشگاه علوم انتظامی امین",
      "دانشکده علوم سیاسی و، فرهنگ و ارتباطات شهید محلاتی (ره)",
      "دانشکده علوم قضایی و خدمات اداری",
      "دانشگاه علوم کشاورزی و منابع طبیعی ساری",
      "دانشگاه علوم وفنون فارابی",
      "دانشگاه مذاهب اسلامی",
      "دانشگاه هوایی شهید ستاری",
      "دانشکده فنی شهید شمسی پور",
      "آموزشکده آمار و انفورماتیک",
      "آموزشکده فنی پسران اراک",
      "آموزشکده فنی پسران علی آبادکتول",
      "آموزشکده فنی پسران گلپایگان",
      "آموزشکده فنی علی ابن ابیطالب بم",
      "آموزشکده فنی مهندسی ایلام",
      "آموزشکده فنی پسران دزفول (دکتر عصاریان)",
      "آموزشکده محیط زیست کرج",
      "دانشکده اطلاعات",
      "دانشکده تربیت دبیر فنی صومعه سرا",
      "دانشکده حفاظت و بهداشت کار",
      "دانشکده صنایع مخابرات ایران (شیراز)",
      "دانشکده علمی کاربردی پست و مخابرات",
      "دانشکده فنی انقلاب اسلامی",
      "دانشکده فنی پسران نیشابور",
      "دانشکده فنی دکتر شریعتی",
      "دانشکده فنی ولیعصر (عج) تهران",
      "دانشکده فنی ملاصدرا رامسر",
      "دانشکده فنی مهندسی ثامن الحجج (ع) مشهد",
      "دانشکده فنی مهندسی شهید محمد منتظری مشهد",
      "دانشکده فنی مهندسی علی‌آباد کتول",
      "دانشکده فنی شهید مهاجر",
      "پژوهشکده محیط زیست و توسعه پایدار",
      "دانشکده کشاورزی شهید رجایی نیشابور",
      "دانشکده کشاورزی و دامپروری تربت جام",
      "دانشکده و آموزشکده دختران آمل",
      "مرکز آموزش عالی تربیت مربی عقیدتی سیاسی",
      "مرکز آموزش عالی فنی الغدیر زنجان",
      "مرکز بین‌المللی علوم و تکنولوژی پیشرفته و علوم محیطی کرمان",
      "دانشگاه آزاد اسلامی واحد تهران مرکزی",
      "دانشگاه آزاد اسلامی واحد تهران جنوب",
      "دانشگاه آزاد اسلامی واحد مشهد",
      "دانشگاه آزاد اسلامی واحد کازرون",
      "دانشگاه آزاد اسلامی واحد زرقان",
      "دانشگاه آزاد اسلامی واحد ممقان",
      "دانشگاه آزاد اسلامی واحد خدابنده",
      "دانشگاه آزاد اسلامی واحد جیرفت",
      "دانشگاه آزاد اسلامی واحد رباط کریم",
      "دانشگاه آزاد اسلامی ابرکوه",
      "دانشگاه آزاد اسلامی اوز (لارستان)",
      "دانشگاه آزاد اسلامی اقلید",
      "دانشگاه آزاد اسلامی آزادشهر",
      "دانشگاه آزاد اسلامی آشتیان",
      "دانشگاه آزاد اسلامی واحد آیت‌الله آملی",
      "دانشگاه آزاد اسلامی ابهر",
      "دانشگاه آزاد اسلامی اراک",
      "دانشگاه آزاد اسلامی اردبیل",
      "دانشگاه آزاد اسلامی ارومیه",
      "دانشگاه آزاد اسلامی ارسنجان",
      "دانشگاه آزاد اسلامی خوراسگان",
      "دانشگاه آزاد اسلامی اسفراین",
      "دانشگاه آزاد اسلامی اسکو",
      "دانشگاه آزاد اسلامی اسلامشهر",
      "دانشگاه آزاد اسلامی الیگودرز",
      "دانشگاه آزاد اسلامی امیدیه",
      "دانشگاه آزاد اسلامی اهر",
      "دانشگاه آزاد اسلامی واحد میانه",
      "دانشگاه آزاد اسلامی اهواز",
      "دانشگاه آزاد اسلامی ایذه",
      "دانشگاه آزاد اسلامی ایلام",
      "دانشگاه آزاد اسلامی بابل",
      "دانشگاه آزاد اسلامی بندرانزلی",
      "دانشگاه آزاد اسلامی بجنورد",
      "دانشگاه آزاد اسلامی واحد بردسکن",
      "دانشگاه آزاد اسلامی بروجرد",
      "دانشگاه آزاد اسلامی بندرگز",
      "دانشگاه آزاد اسلامی بوئین زهرا",
      "دانشگاه آزاد اسلامی بهارستان",
      "دانشگاه آزاد اسلامی بهبهان",
      "دانشگاه آزاد اسلامی بهشهر",
      "دانشگاه آزاد اسلامی بیرجند",
      "دانشگاه آزاد اسلامی پردیس",
      "دانشگاه آزاد اسلامی پارس‌آباد",
      "دانشگاه آزاد اسلامی پرند",
      "دانشگاه آزاد اسلامی تاکستان",
      "دانشگاه آزاد اسلامی تبریز",
      "دانشگاه آزاد اسلامی تربت حیدریه",
      "دانشگاه آزاد اسلامی تفرش",
      "دانشگاه آزاد اسلامی تنکابن",
      "دانشگاه آزاد اسلامی تویسرکان",
      "دانشگاه آزاد اسلامی واحد پزشکی تهران",
      "دانشگاه آزاد اسلامی واحد تهران مرکزی",
      "دانشگاه آزاد اسلامی واحد تهران غرب",
      "دانشگاه آزاد اسلامی واحد تهران شرق",
      "دانشگاه آزاد اسلامی واحد تهران شمال",
      "دانشگاه آزاد اسلامی واحد علوم و تحقیقات تهران",
      "دانشگاه آزاد اسلامی واحد تهران جنوب",
      "دانشگاه آزاد اسلامی واحد تیران",
      "دانشگاه آزاد اسلامی واحد چالوس",
      "دانشگاه آزاد اسلامی خرم‌آباد",
      "دانشگاه آزاد اسلامی واحد خوی",
      "دانشگاه آزاد اسلامی دامغان",
      "دانشگاه آزاد اسلامی دزفول",
      "دانشگاه آزاد اسلامی دماوند",
      "دانشگاه آزاد اسلامی دولت‌آباد",
      "دانشگاه آزاد اسلامی دورود",
      "دانشگاه آزاد اسلامی دهاقان",
      "دانشگاه آزاد اسلامی دهلران",
      "دانشگاه آزاد اسلامی رامهرمز",
      "دانشگاه آزاد اسلامی رودهن",
      "دانشگاه آزاد اسلامی زنجان",
      "دانشگاه آزاد اسلامی ساری",
      "دانشگاه آزاد اسلامی ساوه",
      "دانشگاه آزاد اسلامی سبزوار",
      "دانشگاه آزاد اسلامی سپیدان",
      "دانشگاه آزاد اسلامی سروستان",
      "دانشگاه آزاد اسلامی سلماس",
      "دانشگاه آزاد اسلامی سمنان",
      "دانشگاه آزاد اسلامی سنقر",
      "دانشگاه آزاد اسلامی سنندج",
      "دانشگاه آزاد اسلامی شاهرود",
      "دانشگاه آزاد اسلامی شبستر",
      "دانشگاه آزاد اسلامی شوشتر",
      "دانشگاه آزاد اسلامی شهرری",
      "دانشگاه آزاد اسلامی شهرضا",
      "دانشگاه آزاد اسلامی شهر مجلسی",
      "دانشگاه آزاد اسلامی شهر قدس",
      "دانشگاه آزاد اسلامی شیراز",
      "دانشگاه آزاد اسلامی شیروان",
      "دانشگاه آزاد اسلامی علی‌آباد کتول",
      "دانشگاه آزاد اسلامی فسا",
      "دانشگاه آزاد اسلامی فردوس",
      "دانشگاه آزاد اسلامی فیروزکوه",
      "دانشگاه آزاد اسلامی قائمشهر",
      "دانشگاه آزاد اسلامی قزوین",
      "دانشگاه آزاد اسلامی واحد قم",
      "دانشگاه آزاد اسلامی کاشان",
      "دانشگاه آزاد اسلامی کازرون",
      "دانشگاه آزاد اسلامی کرج",
      "دانشگاه آزاد اسلامی کرمان",
      "دانشگاه آزاد اسلامی کرمانشاه",
      "دانشگاه آزاد اسلامی گرگان",
      "دانشگاه آزاد اسلامی واحد گرمسار",
      "دانشگاه آزاد اسلامی واحد گناباد",
      "دانشگاه آزاد اسلامی لاهیجان",
      "دانشگاه آزاد اسلامی ماکو",
      "دانشگاه آزاد اسلامی مرودشت",
      "دانشگاه آزاد اسلامی واحد ملایر",
      "دانشگاه آزاد اسلامی واحد ملارد",
      "دانشگاه آزاد اسلامی مسجدسلیمان",
      "دانشگاه آزاد اسلامی مشهد",
      "دانشگاه آزاد اسلامی واحد مها باد",
      "دانشگاه آزاد اسلامی مهدی‌شهر",
      "دانشگاه آزاد اسلامی میبد",
      "دانشگاه آزاد اسلامی نجف آباد",
      "دانشگاه آزاد اسلامی مرکز بین‌المللی خلیج فارس",
      "دانشگاه آزاد اسلامی نراق",
      "دانشگاه آزاد اسلامی واحد نهاوند",
      "دانشگاه آزاد اسلامی واحد نور",
      "دانشگاه آزاد اسلامی واحد نوشهر",
      "دانشگاه آزاد اسلامی نی‌ریز",
      "دانشگاه آزاد اسلامی نیشابور",
      "دانشگاه آزاد اسلامی ورامین و پیشوا",
      "دانشگاه آزاد اسلامی واحد هرند",
      "دانشگاه آزاد اسلامی همدان",
      "دانشگاه آزاد اسلامی گنبدکاووس",
      "دانشگاه آزاد اسلامی هیدج",
      "دانشگاه آزاد اسلامی واحد یاسوج",
      "دانشگاه آزاد اسلامی واحد یزد",
      "دانشگاه آزاد اسلامی واحد جویبار",
      "دانشکده سما واحد ممقان",
      "دانشکده سما واحد اهواز",
      "دانشکده سما واحد ملایر",
      "دانشکده سما واحد اقلید",
      "دانشکده سما واحد مها باد",
      "دانشکده سما واحد اردبیل (سما)",
      "دانشکده سما واحد ارسنجان (سما)",
      "دانشکده سما واحد پاکدشت (سما)",
      "دانشکده سما واحد تهران (سما)",
      "دانشکده سما واحد خرم‌آباد (سما)",
      "دانشکده سما واحد یاسوج (سما)",
      "دانشکده سما واحد بندرعباس (سما)",
      "دانشکده سما واحد علی‌آباد کتول (سما)",
      "دانشکده سما واحد رودهن (سما)",
      "دانشکده سما واحد ساوه (سما)",
      "آموزشکده سما واحد سمنان",
      "آموزشکده سما واحد سیاهکل (سما)",
      "دانشکده سما واحد نیشابور (سما)",
      "دانشکده سما واحد کازرون (سما)",
      "دانشگاه سما واحد خمین",
      "دانشکده سما آمل ۲",
      "آزمونهای سراسری",
      "سازمان سنجش آموزش کشور",
      "وزارت علوم، تحقیقات و فناوری",
      "مؤسسه آموزش عالی خاتم",
      "مؤسسه آموزش عالی غیاث الدین جمشید کاشانی(آبیک)",
      "مؤسسه آموزش عالی ادیبان",
      "مؤسسه آموزش عالی علوم و توسعه آریا",
      "مؤسسه آموزش عالی غیرانتفاعی تعالی",
      "مؤسسه آموزش عالی طلوع مهر قم",
      "دانشکده آمار و انفورماتیک سازمان سنجش آموزش کشور",
      "دانشگاه ادیان و مذاهب",
      "دانشگاه غیرانتفاعی هیمن مهاباد",
      "دانشکده اصول الدین",
      "دانشگاه رایانه‌ای تهران",
      "دانشکده علوم حدیث",
      "دانشکده معارف قرآنی اصفهان",
      "دانشگاه باقرالعلوم",
      "دانشگاه بین‌المللی امام رضا",
      "دانشگاه شیخ بهایی",
      "دانشگاه شمال",
      "دانشگاه علم و فرهنگ",
      "دانشگاه علوم و فنون مازندران",
      "دانشگاه علامه محدث نوری",
      "دانشگاه مفید",
      "مؤسسه آموزش عالی ناصر خسرو- اتوبان ساوه - تهران کیلومتر 6",
      "مؤسسه آموزش عالی میزان تبریز",
      "مؤسسه آموزش عالی آرادان",
      "مؤسسه آموزش عالی ایوانکی",
      "مرکز آموزش عالی شرکت داده پردازی ایران",
      "مؤسسه آموزش عالی آبا(آبیک)",
      "مؤسسه آموزش عالی غیاث الدین جمشید کاشانی(آبیک)",
      "مؤسسه آموزش عالی آمل - آمل",
      "مؤسسه آموزش عالی ادیب مازندران",
      "مؤسسه آموزش عالی ادیبان یزد",
      "مؤسسه آموزش عالی شرق گلستان",
      "مؤسسه آموزش عالی شمس گنبدکاووس",
      "مؤسسه آموزش عالی اسرار مشهد",
      "مؤسسه آموزش عالی اقبال لاهوری مشهد",
      "مؤسسه آموزش عالی باختر ایلام",
      "مؤسسه آموزش عالی سفیر دانش ایلام",
      "مؤسسه آموزش عالی بعثت کرمان",
      "مؤسسه آموزش عالی بهار مشهد",
      "مؤسسه آموزش عالی پارسا بابلسر",
      "مؤسسه آموزش عالی پارسیان",
      "مؤسسه آموزش عالی پارس رضوی گناباد",
      "مؤسسه آموزش عالی پویا",
      "مؤسسه آموزش عالی پیام گلپایگان",
      "مؤسسه آموزش عالی تابران",
      "مؤسسه آموزش عالی توسعه دانش سنندج",
      "مؤسسه آموزش عالی ثامن نیشابور",
      "مؤسسه آموزش عالی جابر بن حیان-رشت",
      "مؤسسه آموزش عالی جامی",
      "مؤسسهٔ آموزش عالی جهاد دانشگاهی خوزستان",
      "مؤسسه آموزش عالی جهاد دانشگاهی استان یزد",
      "مؤسسه آموزش عالی حکمت قم",
      "مؤسسه آموزش عالی خاوران مشهد",
      "مؤسسه آموزش عالی خراسان",
      "مؤسسه آموزش عالی خرد گرایان مطهر مشهد",
      "مؤسسه آموزش عالی خیام مشهد",
      "مؤسسه آموزش عالی دارالعلم یزد",
      "مؤسسه آموزش عالی دانش پژوهان",
      "مؤسسه آموزش عالی دانشوران تبریز",
      "مؤسسه آموزش عالی رجا قزوین",
      "مؤسسه آموزش عالی رسام",
      "مؤسسه آموزش عالی راه دانش بابل",
      "مؤسسه آموزش عالی رشدیه -تبریز",
      "مؤسسه آموزش عالی روزبه زنجان",
      "مؤسسه آموزش عالی روزبهان ساری",
      "مؤسسه آموزش عالی ساریان ساری",
      "مؤسسه آموزش عالی سبلان اردبیل",
      "مؤسسه آموزش عالی سبحان نیشابور",
      "دانشگاه صنعتی سجاد مشهد",
      "مؤسسه آموزش عالی سبز آمل",
      "مؤسسه آموزش عالی سپهر اصفهان",
      "مؤسسه آموزش عالی سلمان(مشهد)",
      "مؤسسه آموزش عالی سوره",
      "مؤسسه آموزش عالی سینا کاشان",
      "مؤسسه آموزش عالی شاهرود",
      "مؤسسه آموزش عالی شهاب دانش قم",
      "مؤسسه آموزش عالی شهریار آستارا",
      "مؤسسه آموزش عالی شهید اشرفی اصفهانی",
      "مؤسسه آموزش عالی صدرالمتألهین (صدرا)",
      "مؤسسه آموزش عالی صفاهان",
      "مؤسسه آموزش عالی صنعتی فولاد",
      "مؤسسه آموزش عالی صنعتی مازندران",
      "مؤسسه آموزش عالی طبرستان چالوس",
      "مؤسسه آموزش عالی پویندگان دانش چالوس",
      "مؤسسه آموزش عالی طبری بابل",
      "مؤسسه آموزش عالی عبدالرحمان صوفی رازی زنجان",
      "مؤسسه آموزش عالی عطار مشهد",
      "مؤسسه آموزش عالی علامه جعفری رفسنجان",
      "مؤسسه آموزش عالی علامه دهخدا قزوین",
      "مؤسسه آموزش عالی علامه فیض کاشانی",
      "مؤسسه آموزش عالی علوم و فناوری آریان بابل",
      "مؤسسه آموزش عالی صالحان قایم شهر",
      "مؤسسه آموزش عالی علوم و فناوری سپاهان",
      "مؤسسه آموزش عالی غزالی - قزوین",
      "مؤسسه آموزش عالی فردوس",
      "مؤسسه آموزش عالی فنی کرمان",
      "مؤسسه آموزش عالی کار",
      "موسسه آموزش عالی کاوش",
      "مؤسسه آموزش عالی کنسرواتوار تهران",
      "مؤسسه آموزش عالی کسری رامسر",
      "مؤسسه آموزش عالی کومش سمنان",
      "مؤسسه آموزش عالی لامعی گرگانی",
      "مؤسسه آموزش عالی مازیار رویان",
      "مؤسسه آموزش عالی مهرگان محلات",
      "مؤسسه آموزش عالی مهد علم کاشان",
      "مؤسسه آموزش عالی مهر آستان-آستانه اشرفیه-رشت",
      "مؤسسه آموزش عالی مهر اراک",
      "مؤسسه آموزش عالی میرداماد گرگان",
      "مؤسسه آموزش عالی نبی اکرم تبریز",
      "مؤسسه آموزش عالی نجف آباد",
      "مؤسسه آموزش عالی نورطوبی",
      "مؤسسه آموزش عالی نهاوند",
      "مؤسسه آموزش عالی هدف -ساری",
      "مؤسسه آموزش عالی هرمزان",
      "مؤسسه آموزش عالی هنر کمال الملک نوشهر",
      "مؤسسه آموزش عالی دیلمان",
      "مؤسسه آموزش عالی غیرانتفاعی کارون",
      "مؤسسه آموزش عالی غیرانتفاعی گنجنامه همدان",
      "مؤسسه آموزشی و پژوهشی امام خمینی(ره)",
      "دانشگاه پیام نور",
      "دانشگاه پیام نور",
      "دانشگاه پیام نور",
      "دانشگاه پیام نور ممقان",
      "دانشگاه پیام نور قم",
      "دانشگاه پیام نور بوشهر",
      "دانشگاه پیام نور برازجان",
      "دانشگاه پیام نور دیلم",
      "دانشگاه پیام نور کنگان",
      "دانشگاه پیام نور گناوه",
      "دانشگاه پیام نور دیر",
      "دانشگاه پیام نور جم",
      "دانشگاه بین‌المللی پیام نور عسلویه",
      "دانشگاه پیام نور خورموج",
      "دانشگاه پیام نوراهرم",
      "دانشگاه پیام نور شبانکاره",
      "دانشگاه پیام نور اوز",
      "دانشگاه پیام نوراقلید",
      "دانشگاه پیام نور ابهر",
      "دانشگاه پیام نور اردبیل",
      "دانشگاه پیام نور اردکان",
      "دانشگاه پیام نور اسفراین",
      "دانشگاه پیام نور ایلام",
      "دانشگاه پیام نور بردسکن",
      "دانشگاه پیام نور بوانات",
      "دانشگاه پیام نور بیرجند",
      "دانشگاه پیام نور تبریز",
      "دانشگاه پیام نور تربت حیدریه",
      "دانشگاه پیام نور خرمدره",
      "دانشگاه پیام نور جیرفت",
      "دانشگاه پیام نور خوانسار",
      "دانشگاه پیام نور دزفول",
      "دانشگاه پیام نور دهلران",
      "دانشگاه پیام نور رامیان",
      "دانشگاه پیام نور زنجان",
      "دانشگاه پیام نور ساوه",
      "دانشگاه پیام نور سروستان",
      "دانشگاه پیام نور سمیرم",
      "دانشگاه پیام نور سلطانیه",
      "دانشگاه پیام نور شبستر",
      "دانشگاه پیام نور شهریار",
      "دانشگاه پیام نور شیراز",
      "دانشگاه پیام نور کازرون",
      "دانشگاه پیام نور طارم زنجان",
      "دانشگاه پیام نور فیروزآباد",
      "دانشگاه پیام نور قزوین",
      "دانشگاه پیام نور قم",
      "دانشگاه پیام نور کرج",
      "دانشگاه پیام نور گرگان",
      "دانشگاه پیام نور گرمسار",
      "دانشگاه پیام نور گلپایگان",
      "دانشگاه پیام نور گناباد",
      "دانشگاه پیام نور گنبد",
      "دانشگاه پیام نور لالجین",
      "دانشگاه پیام نور واحد مهدی‌شهر",
      "دانشگاه پیام نور نیشابور",
      "دانشگاه پیام نور مشهد",
      "دانشگاه پیام نور نجف‌آباد",
      "دانشگاه پیام نور هشترود",
      "دانشگاه پیام نور رودهن",
      "دانشگاه پیام نور ساری",
      "دانشگاه پیام نور شوش",
      "دانشگاه پیام نور رینه",
      "دانشگاه پیام نور آمل",
      "دانشگاه پیام نور بانه",
      "جامع علمی کاربردی",
      "وزارت علوم، تحقیقات و فناوری",
      "دانشگاه جامع علمی کاربردی",
      "دانشگاه جامع علمی کاربردی آذربایجان شرقی",
      "دانشگاه جامع علمی کاربردی آذربایجان غربی",
      "دانشگاه جامع علمی کاربردی اردبیل",
      "دانشگاه جامع علمی کاربردی اصفهان",
      "دانشگاه جامع علمی کاربردی بوشهر",
      "دانشگاه جامع علمی کاربردی گروه صنعتی گلرنگ - تهران",
      "دانشگاه جامع علمی کاربردی چهارمحال بختیاری",
      "دانشگاه جامع علمی کاربردی خراسان جنوبی",
      "دانشگاه جامع علمی کاربردی خراسان رضوی",
      "دانشگاه جامع علمی کاربردی خراسان شمالی",
      "دانشگاه جامع علمی کاربردی خوزستان",
      "دانشگاه جامع علمی کاربردی زنجان",
      "دانشگاه جامع علمی کاربردی سیستان و بلوچستان",
      "دانشگاه جامع علمی کاربردی فارس",
      "دانشگاه جامع علمی کاربردی قم",
      "دانشگاه جامع علمی کاربردی علوم و فنون قزوین",
      "دانشگاه جامع علمی کاربردی کردستان",
      "دانشگاه جامع علمی کاربردی کرمان",
      "دانشگاه جامع علمی کاربردی کرمانشاه",
      "دانشگاه جامع علمی کاربردی کهگیلویه و بویراحمد",
      "دانشگاه جامع علمی کاربردی کیش",
      "دانشگاه جامع علمی کاربردی گلستان",
      "دانشگاه جامع علمی کاربردی گیلان",
      "دانشگاه جامع علمی کاربردی لرستان",
      "دانشگاه جامع علمی کاربردی مازندران",
      "دانشگاه جامع علمی کاربردی مرکزی",
      "دانشگاه جامع علمی کاربردی مهدی‌شهر",
      "دانشگاه جامع علمی کاربردی هرمزگان",
      "دانشگاه جامع علمی کاربردی ملایر",
      "دانشگاه جامع علمی کاربردی همدان",
      "دانشگاه جامع علمی کاربردی نیشابور",
      "دانشگاه جامع علمی کاربردی یزد",
      "مرکز علمی کاربردی گنبد کاووس",
      "مرکز علمی کاربردی پارس‌آباد",
    ];
  }

  @override
  List<String> get viewsOn {
    return const ["منفی", "بسیار منفی", "مثبت", "خنثی", "توافقی"];
  }

  @override
  List<String> get worldviews {
    return const [
      "کفر",
      "لاادری گری",
      "خداپرستی",
      "یهودیت",
      "کاتولیک",
      "همه خدایی",
      "آیین کنفوسیوس",
      "انسان گرایی سکولار",
    ];
  }
}
