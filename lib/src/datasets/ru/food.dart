import 'package:imitatio/src/datasets/models/food.dart';

class RuFoodData extends FoodDataset {
  @override
  List<String> get dishes {
    return const [
      "Бефстроганов классический",
      "Борщ без картошки с фасолью",
      "Борщ с грибами",
      "Вареники",
      "Винегрет",
      "Голубцы с перловкой и грибами",
      "Грузди соленые",
      "Докторская колбаса",
      "Духовые пирожки с картофелем и луком",
      "Жареные пирожки «Лапоточки»",
      "Заливное из семги",
      "Квашеная капуста по-кубански",
      "Квашеная капуста, тушенная с мясом и картофелем",
      "Кишка",
      "Кулебяка с судаком",
      "Ленивые голубцы с курицей",
      "Огурцы бочковые на зиму в банках",
      "Окрошка",
      "Окрошка на курином бульоне",
      "Окрошка на минералке",
      "Пельмени с индейкой",
      "Помидоры, соленые в банках как бочковые",
      "Рассольник вегетарианский",
      "Ряпушка по-карельски",
      "Салат «Грибное лукошко» с опятами",
      "Салат оливье",
      "Сало",
      "Сандвич с селедкой, майонезом и соленым огурцом",
      "Свекольник на кефире",
      "Свинушки жареные",
      "Сельдь",
      "Солянка",
      "Стейк из семги в духовке",
      "Тонкие блины с черемшой",
      "Уха из ряпушки",
      "Холодец",
    ];
  }

  @override
  List<String> get drinks {
    return const [
      "Абсент",
      "Авиация",
      "Аквавит",
      "Александр",
      "Американо",
      "Арак",
      "Арманьяк",
      "Архи",
      "Б-52",
      "Бакарди",
      "Бальзамы",
      "Бамбузе",
      "Баракуда",
      "Белая леди",
      "Беллини",
      "Билк",
      "Брамбл",
      "Бренди",
      "Брют",
      "В постели",
      "Вампиро",
      "Венецианские брызги",
      "Вермут",
      "Веспер",
      "Вино",
      "Виски",
      "Виски сауэр",
      "Водки",
      "Граппа",
      "Грязный мартини",
      "Дайкири",
      "Дерби",
      "Джин",
      "Джин Физ",
      "Джон Коллинз",
      "Жёлтая Птица",
      "Золотая Мечта",
      "Игристые вина",
      "Кагор",
      "Казино",
      "Кайпиринья",
      "Кальвадос",
      "Камикадзе",
      "Кашаса",
      "Кизлярка",
      "Кир",
      "Киршвассер",
      "Кловер Клаб",
      "Коньяк",
      "Космополитен",
      "Кофе по-ирландски",
      "Кровавая Мэри",
      "Крёстная мать",
      "Крёстный отец",
      "Куба либре",
      "Кузнечик",
      "Кумыс",
      "Лемон Дроп Мартини",
      "Ликёры",
      "Лицо ангела",
      "Лонг-Айленд",
      "Лошадиная шея",
      "Мадера",
      "Маи Таи",
      "Малага",
      "Манхэттен",
      "Маотай",
      "Маргарита",
      "Марсала",
      "Мартини",
      "Метакса",
      "Мимоза",
      "Мускат",
      "Мэри Пикфорд",
      "Настойки",
      "Негрони",
      "Немецкая водка",
      "Новое вино",
      "Обезьяньи гланды",
      "Олд фешен",
      "Отвёртка",
      "Перри",
      "Пиво",
      "Писко Сауэр",
      "Плантаторский пунш",
      "Плодовые вина",
      "Польская водка",
      "Портвейн",
      "Порто-флип",
      "Рай",
      "Ракы",
      "Рамос физз",
      "Ржавый гвоздь",
      "Ром",
      "Русская водка",
      "Русский весенний пунш",
      "Сазерак",
      "Сайдкар",
      "Саке",
      "Самогон",
      "Сато",
      "Сидр",
      "Сливовица",
      "Смокинг",
      "Старка",
      "Стингер",
      "Тарасун",
      "Текила",
      "Токай",
      "Томми маргарита",
      "Тутовка",
      "Тьма и буря",
      "Узо",
      "Французский мартини",
      "Французский связной",
      "Френч 75",
      "Ханшина",
      "Харви Волбенгер",
      "Хемингуэй спешл",
      "Херес",
      "Хурэмгэ",
      "Цинар",
      "Чача",
      "Чёрный русский",
      "Шампанское",
      "Шампань",
      "Шнапс",
      "Эспрессо мартини",
      "Ягати",
    ];
  }

  @override
  List<String> get fruits {
    return const [
      "Абиу",
      "Абрикос",
      "Авокадо",
      "Айва",
      "Аки",
      "Алиберция",
      "Алыча",
      "Амбарелла",
      "Американский абрикос",
      "Американский орех",
      "Ананас",
      "Аннона горная",
      "Аннона колючая",
      "Аннона сетчатая",
      "Аннона черимола",
      "Аннона чешуйчатая",
      "Антильский крыжовник",
      "Апельсин",
      "Арабика",
      "Араза",
      "Арахис",
      "Арбуз обыкновенный",
      "Астрокариум колючий",
      "Атимойя",
      "Африканский колючий огурец",
      "Африканский тамаринд",
      "Бакау",
      "Баклажан",
      "Балия",
      "Бананы",
      "Баобаб",
      "Барбадин (Большая гранадилла)",
      "Барбадосская вишня",
      "Бархатное яблоко",
      "Баэль",
      "Белая сапота",
      "Бергамот",
      "Билимби",
      "Бирсонима",
      "Блигия вкусная",
      "Большой змеиный фрукт",
      "Боярышник",
      "Бразильский орех",
      "Бычье сердце",
      "Вампи",
      "Вангерия",
      "Ваниль",
      "Виноград",
      "Воаванга",
      "Водяное(восковое) яблоко",
      "Гандария",
      "Генипа",
      "Гибискус съедобный",
      "Гнетум гнемон",
      "Голубиная слива",
      "Голубой квандонг",
      "Горлянка",
      "Горький огурец",
      "Гранадилла (Маракуйа)",
      "Гранадилла большая (Барбадин)",
      "Гранадилла сладкая",
      "Гранат",
      "Грейпфрут",
      "Грумичама",
      "Груша",
      "Гуайява земляничная",
      "Гуайява коста-риканская",
      "Гуайява красная",
      "Гуайява обыкновенная",
      "Гуарана",
      "Давидсония",
      "Дамские пальчики",
      "Деревянное яблоко",
      "Десертный квандонг",
      "Джекфрут",
      "Древесная калебаса",
      "Дуку",
      "Дуриан",
      "Дуриан цибетиновый",
      "Дынная груша",
      "Дынное дерево",
      "Дыня обыкновенная",
      "Евгения одноцветковая",
      "Жаботикаба",
      "Звездчатое яблоко",
      "Земляничная груша",
      "Земляничный томат",
      "Земляной орех",
      "Зизифус мавританский",
      "Золотая слива",
      "Золотистый апельсин",
      "Индийские бобы",
      "Индийский инжир",
      "Индийский миндаль",
      "Индийское розовое яблоко",
      "Инжир",
      "Кабачки",
      "Каинито",
      "Какао",
      "Кактус инжировый",
      "Каламондин",
      "Калебаса",
      "Канариум яйцевидный",
      "Капский крыжовник",
      "Карамбола",
      "Кас",
      "Квини",
      "Квислендский орех",
      "Кепель",
      "Кетамбилла",
      "Кивано",
      "Киви",
      "Китайская калебаса",
      "Клементин",
      "Кокколоба ягодоносная",
      "Кокос",
      "Корилла",
      "Кофейные деревья",
      "Кранжи",
      "Кумкват овальный",
      "Купуасу",
      "Курбарил",
      "Кустовой горошек",
      "Лайм настоящий",
      "Лангсат",
      "Лансиум домашний",
      "Леуцена светлоголовчатая",
      "Либерика",
      "Ликания",
      "Лимон грубокожистый",
      "Лимон обыкновенный",
      "Лимон Мейера",
      "Лимонная осина",
      "Личи",
      "Лобия",
      "Ложный мангустан",
      "Лох узколистный",
      "Лукума",
      "Луло",
      "Люффа остроребристая",
      "Маболо",
      "Мадагаскарская слива",
      "Макадамия цельнолистная",
      "Малабарский апельсин",
      "Малайское яблоко",
      "Малуко",
      "Мальпигия гранатолистная",
      "Маммея американская",
      "Мамончилло (Лайм испанский)",
      "Манго благоухающее",
      "Манго великолепное",
      "Манго индийское",
      "Манго резко пахнущее",
      "Мангостан",
      "Мангустан",
      "Мандарин",
      "Манилкара",
      "Маракуйя",
      "Мармеладный плод",
      "Марула",
      "Мауриция извилистая",
      "Маш",
      "Мексиканская земляная вишня",
      "Мексиканский огурец",
      "Мелинжо",
      "Моква",
      "Момбин желтый",
      "Момбин красный",
      "Момордика",
      "Моринда",
      "Мунду",
      "Мускатный орех",
      "Мушмула японская",
      "Наранхилла",
      "Ням-ням",
      "Обезьяний хлеб",
      "Огурец",
      "Огуречное дерево",
      "Орех кешью",
      "Пальма катеху",
      "Пальма кокосовая",
      "Пальма масличная африканская",
      "Пальма персиковая",
      "Пальчиковый лайм",
      "Папайя",
      "Папайя горная",
      "Папеда",
      "Паприка",
      "Пара-гуайява",
      "Паркия красивая",
      "Пассифлора съедобная",
      "Пекуи",
      "Пепино",
      "Перец",
      "Перец кайенский",
      "Перец стручковый",
      "Персик",
      "Перуанская вишня",
      "Питайя",
      "Питомба",
      "Пиши",
      "Помело",
      "Померанец",
      "Помидор",
      "Помпельмус",
      "Понцирус (несъедобный плод)",
      "Приморский виноград",
      "Путерия",
      "Пуласан",
      "Ракум-салакка",
      "Рамбай",
      "Рамбутан",
      "Робуста",
      "Розовое яблоко",
      "Роллиния слизистая",
      "Салакка",
      "Салакка скученная",
      "Саламандровое дерево",
      "Сантол",
      "Саподилла",
      "Сатсума",
      "Сахарное яблоко",
      "Сахарный горошек",
      "Свечное дерево",
      "Свити",
      "Сизигиум аквеум",
      "Сизигиум малаккский",
      "Сизигиум ямбоза",
      "Сингапурский миндаль",
      "Слива",
      "Слива какаду",
      "Слоновье яблоко",
      "Сметанное яблоко",
      "Сонсоя",
      "Соя",
      "Спаржевая фасоль",
      "Страстоцвет",
      "Суринамская вишня",
      "Съедобный таитянский орех",
      "Таитянское яблоко",
      "Такако",
      "Тамарилло",
      "Тамаринд",
      "Танжерин",
      "Терминалия катаппа",
      "Томат настоящий",
      "Томатное дерево",
      "Тукума",
      "Тупа",
      "Тыквенное дерево",
      "Фейхоа",
      "Ферония лимонная",
      "Ферония слоновая",
      "Физалис земляничный",
      "Филиппинское розовое яблоко",
      "Филлантус кислый",
      "Финик",
      "Флакурция",
      "Хлебное дерево",
      "Хлебные бобы",
      "Хурма восточная",
      "Цейлонский крыжовник",
      "Циклантера",
      "Цуккини",
      "Чайот",
      "Чампедак",
      "Черешня",
      "Черимойя",
      "Черная гуайява",
      "Черная сапота",
      "Черная хурма",
      "Черный тамаринд",
      "Чили",
      "Чилибуха колючая",
      "Чупа",
      "Шоколадное дерево",
      "Яблоко",
      "Яблоко-кажу",
      "Яботикаба",
      "Ямайская вишня",
      "Ятоба",
    ];
  }

  @override
  List<String> get spices {
    return const [
      "Анис",
      "Анис звездчатый",
      "Аир",
      "Ажгон",
      "Асафетида",
      "Аджван",
      "Айован душистый",
      "Ангелика китайская",
      "Аннато",
      "Амчур",
      "Аругула",
      "Асам",
      "Асперула",
      "Асмаргок",
      "Абиш",
      "Бадьян",
      "Базилик",
      "Барбарис",
      "Богородская трава",
      "Бедренец",
      "Бораго",
      "Бархатцы",
      "Блошница",
      "Бурачник",
      "Буниум",
      "Буркун",
      "Ваниль",
      "Василек душистый",
      "Гравилат",
      "Галгант",
      "Гаир",
      "Гвоздика",
      "Гвоздичный корень",
      "Гибискус",
      "Горчица",
      "Гамем",
      "Глистник",
      "Гусеничник",
      "Девясил",
      "Дудник",
      "Душица",
      "Дудочник (дудчатая трава)",
      "Дягиль",
      "Дандур",
      "Драконоголовник",
      "Дакар",
      "Донник",
      "Женьшень",
      "Жадобник",
      "Зира",
      "Заборина",
      "Золотой корень",
      "Зверобой",
      "Зверобой синий",
      "Зубровка",
      "Змееголовник",
      "Зоря",
      "Зимозелень",
      "Иссоп",
      "Имбирь",
      "Имеретинский шафран",
      "Индау",
      "Имли",
      "Илан",
      "Калган",
      "Кассия",
      "Коровник",
      "Кориандр",
      "Корица",
      "Колюрия",
      "Кинза",
      "Куркума",
      "Каперсы",
      "Кайенский перец",
      "Кардамон",
      "Калмус",
      "Кервель",
      "Купырь",
      "Кипрей",
      "Кресс-салат",
      "Кунжут",
      "Каффир-лайм",
      "Китайский корень",
      "Калинджи",
      "Камнеломка",
      "Капуцин",
      "Кумин",
      "Калонги",
      "Ложный шафран",
      "Лаванда",
      "Лавровый лист",
      "Любисток",
      "Лимонник",
      "Лакрица",
      "Лофант анисовый",
      "Мелисса турецкая",
      "Маригольд",
      "Мацердушка",
      "Майоран",
      "Мак",
      "Многоколосник фенхельный",
      "Мускатный орех",
      "Мелисса",
      "Можжевельник",
      "Милодух",
      "Мята",
      "Метхи",
      "Материнка",
      "Нигелла",
      "Настурция",
      "Омбало",
      "Орегано",
      "Огуречная трава",
      "Подлесник",
      "Портулак",
      "Перчик боровой",
      "Пажитник",
      "Пастернак",
      "Петрушка",
      "Паприка",
      "Пижма",
      "Порошок манго",
      "Рута",
      "Розмарин",
      "Римский кориандр",
      "Реган",
      "Рукола",
      "Рокет",
      "Сизигиум",
      "Синявка",
      "Сальвия",
      "Сумах",
      "Синий зверобой",
      "Сабельник",
      "Снедок",
      "Солодка",
      "Сельдерей",
      "Санчо",
      "Сафлор",
      "Сампалок",
      "Седана",
      "Тулси",
      "Тмин",
      "Тамаринд",
      "Тимьян",
      "Тархун",
      "Трилистник",
      "Укроп",
      "Уцхо-сунели",
      "Ферула вонючая",
      "Фенугрек",
      "Фенхель",
      "Хрен",
      "Хинг",
      "Хильбе",
      "Царская трава",
      "Цитронелла",
      "Чернушка",
      "Чабрец",
      "Чабер",
      "Черный тмин",
      "Чаман",
      "Черная соль",
      "Шалфей",
      "Шафран",
      "Шамбала",
      "Шабрий",
      "Шавлий",
      "Эрука",
      "Эстрагон",
      "Эльсгольция",
      "Ямайский перец",
      "Ясменник душистый",
      "Явер",
      "Анис",
      "Анис звездчатый",
      "Аир",
      "Ажгон",
      "Асафетида",
      "Аджван",
      "Айован душистый",
      "Ангелика китайская",
      "Аннато",
      "Амчур",
      "Аругула",
      "Асам",
      "Асперула",
      "Асмаргок",
      "Абиш",
      "Бадьян",
      "Базилик",
      "Барбарис",
      "Богородская трава",
      "Бедренец",
      "Бораго",
      "Бархатцы",
      "Блошница",
      "Бурачник",
      "Буниум",
      "Буркун",
      "Ваниль",
      "Василек душистый",
      "Гравилат",
      "Галгант",
      "Гаир",
      "Гвоздика",
      "Гвоздичный корень",
      "Гибискус",
      "Горчица",
      "Гамем",
      "Глистник",
      "Гусеничник",
      "Девясил",
      "Дудник",
      "Душица",
      "Дудочник (дудчатая трава)",
      "Дягиль",
      "Дандур",
      "Драконоголовник",
      "Дакар",
      "Донник",
      "Женьшень",
      "Жадобник",
      "Зира",
      "Заборина",
      "Золотой корень",
      "Зверобой",
      "Зверобой синий",
      "Зубровка",
      "Змееголовник",
      "Зоря",
      "Зимозелень",
      "Иссоп",
      "Имбирь",
      "Имеретинский шафран",
      "Индау",
      "Имли",
      "Илан",
      "Калган",
      "Кассия",
      "Коровник",
      "Кориандр",
      "Корица",
      "Колюрия",
      "Кинза",
      "Куркума",
      "Каперсы",
      "Кайенский перец",
      "Кардамон",
      "Калмус",
      "Кервель",
      "Купырь",
      "Кипрей",
      "Кресс-салат",
      "Кунжут",
      "Каффир-лайм",
      "Китайский корень",
      "Калинджи",
      "Камнеломка",
      "Капуцин",
      "Кумин",
      "Калонги",
      "Ложный шафран",
      "Лаванда",
      "Лавровый лист",
      "Любисток",
      "Лимонник",
      "Лакрица",
      "Лофант анисовый",
      "Мелисса турецкая",
      "Маригольд",
      "Мацердушка",
      "Майоран",
      "Мак",
      "Многоколосник фенхельный",
      "Мускатный орех",
      "Мелисса",
      "Можжевельник",
      "Милодух",
      "Мята",
      "Метхи",
      "Материнка",
      "Нигелла",
      "Настурция",
      "Омбало",
      "Орегано",
      "Огуречная трава",
      "Подлесник",
      "Портулак",
      "Перчик боровой",
      "Пажитник",
      "Пастернак",
      "Петрушка",
      "Паприка",
      "Пижма",
      "Порошок манго",
      "Рута",
      "Розмарин",
      "Римский кориандр",
      "Реган",
      "Рукола",
      "Рокет",
      "Сизигиум",
      "Синявка",
      "Сальвия",
      "Сумах",
      "Синий зверобой",
      "Сабельник",
      "Снедок",
      "Солодка",
      "Сельдерей",
      "Санчо",
      "Сафлор",
      "Сампалок",
      "Седана",
      "Тулси",
      "Тмин",
      "Тамаринд",
      "Тимьян",
      "Тархун",
      "Трилистник",
      "Укроп",
      "Уцхо-сунели",
      "Ферула вонючая",
      "Фенугрек",
      "Фенхель",
      "Хрен",
      "Хинг",
      "Хильбе",
      "Царская трава",
      "Цитронелла",
      "Чернушка",
      "Чабрец",
      "Чабер",
      "Черный тмин",
      "Чаман",
      "Черная соль",
      "Шалфей",
      "Шафран",
      "Шамбала",
      "Шабрий",
      "Шавлий",
      "Эрука",
      "Эстрагон",
      "Эльсгольция",
      "Ямайский перец",
      "Ясменник душистый",
      "Явер",
    ];
  }

  @override
  List<String> get vegetables {
    return const [
      "Кольраби",
      "Брокколи",
      "Брюссельская капуста",
      "Савойская капуста",
      "Пекинская капуста",
      "Цветная капуста",
      "Мангольд",
      "Петрушка",
      "Укроп",
      "Базилик",
      "Майоран",
      "Чабер",
      "Мята",
      "Мелисса",
      "Душица",
      "Эстрагон",
      "Щавель",
      "Шпинат",
      "Фенхель",
      "Любисток",
      "Кресс-салат",
      "Эндивий",
      "Латук",
      "Помидор",
      "Перец овощной",
      "Баклажан",
      "Сараха съедобная",
      "Огурец",
      "Кабачок",
      "Патиссон",
      "Тыква",
      "Чайот",
      "Китайская горькая тыква",
      "Лагенария",
      "Восковая тыква",
      "Циклантера",
      "Люффа",
      "Трихозант",
      "Антильский огурец",
      "Мелотрия шершавая",
      "Тладианта сомнительная",
      "Артишок",
      "Горох",
      "Фасоль",
      "Вигна спаржевая",
      "Крылатые бобы",
      "Картофель",
      "Морковь",
      "Свекла",
      "Редька",
      "Редис",
      "Хрен",
      "Сельдерей",
      "Пастернак",
      "Батат",
      "Ямс",
      "Маниок",
      "Таро",
      "Якон",
      "Топинамбур",
      "Репа",
      "Брюква",
      "Хикама",
      "Китайский артишок",
      "Скорцонера испанская",
      "Козлобородник пореелистный",
      "Уллюко клубненосный",
      "Арракача",
      "Мака перуанская",
      "Лук",
      "Лук-шалот",
      "Лук-порей",
      "Лук-батун",
      "Лук-слизун",
      "Лук душистый",
      "Чеснок",
      "Спаржа",
      "Ревень",
      "Щавель",
      "Бамия",
      "Капуста пак-чой",
      "Капуста романеско",
      "Кукуруза",
      "Лук репчатый",
      "Момордика",
      "Топинамбур",
    ];
  }
}
