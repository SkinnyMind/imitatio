import 'package:imitatio/src/datasets/models/finance.dart';

class UkFinanceData extends FinanceDataset {
  @override
  List<String> get banks {
    return const [
      "Абанк",
      "Аваль",
      "Альфа-Банк",
      "Альянс",
      "Асвіо Банк",
      "Банк Восток",
      "Банк Грант",
      "Банк Дніпро",
      "Банк Ідея Банк",
      "Банк Кредит Дніпро",
      "Банк Михайлівський",
      "Банк Пивденкомбанк",
      "Банк Поділля",
      "Банк Ренесанс Капітал",
      "Банк Січ",
      "Банк Українська Фінансова Група",
      "Банк Фінанси та Кредит",
      "Банк Хоум Кредит",
      "БАНК-КЛІРИНГОВИХ ТЕХНОЛОГІЙ",
      "БАНКОВА КОЛЕГІЯ",
      "БАНКОВИЙ ДІМ",
      "БАНКОВСЬКИЙ ЦЕНТР КРЕДИТУ",
      "БОНУС БАНК",
      "БУДІВЕЛЬНИЙ КОМЕРЦІЙНИЙ БАНК",
      "ВЕСТА",
      "ГРАНІТ БАНК",
      "ДЕМЕТРА",
      "ДНІСТЕР",
      "ЕНЕРГОБАНК",
      "ЕРС Банк",
      "ІНВЕСТ БАНК",
      "КИЇВ",
      "МЕГАБАНК",
      "МІНБАНК",
      "НАТІОНАЛЬНИЙ КРЕДИТ",
      "ОЩАДБАНК",
      "ПРАВЕКС-БАНК",
      "ПРИВАТБАНК",
      "ПУМБ",
      "Райффайзен Банк Аваль",
      "Укргазбанк",
      "Укрексімбанк",
      "Укрсиббанк",
      "Укрсоцбанк",
      "Унікредит банк",
    ];
  }

  @override
  List<String> get companies {
    return const [
      "Нафтогаз",
      "Метінвест",
      "ДП Енергоринок",
      "Укрзалізниця",
      "ІСД",
      "ДТЕК",
      "АрселорМіттал Кривий Ріг",
      "Укрнафта",
      "Донецьксталь",
      "Укртатнафта",
      "НАЕК Енергоатом",
      "Запоріжсталь",
      "Fozzy Group",
      "WOG",
      "ОСТХЕМ Україна",
      "Кернел",
      "АТБ-Маркет",
      "ТНК-BP Коммерс",
      "Ferrexpo",
      "Мегаполіс",
      "Інтерпайп",
      "Київстар",
      "Галнафтогаз",
      "Епіцентр-К",
      "Газтек",
      "Енергостандарт",
      "ПГЗК",
      "Азовмаш",
      "ROSHEN",
      "Газпром збут Україна",
      "METRO Cash & Carry",
      "Миронхлібпродукт",
      "Vodafone Україна",
      "Укренергоконсалтінг",
      "VS Energy",
      "Євраз",
      "БаДМ",
      "Нібулон",
      "Фокстрот",
      "БНК-Україна",
      "Центренерго",
      "Укртелеком",
      "НЗФ",
      "Cargill Україна",
      "Оптима Фарм",
      "Лукойл Україна",
      "JTI Україна",
      "КВЗ",
      "Лемтранс",
      "УГМК",
      "Альтком",
      "Samsung Electronics",
      "Хліб Інвестбуд",
      "Мотор Січ",
      "Дніпроспецсталь",
      "Богдан",
      "АероСвіт",
      "ДЕМЗ",
      "Philip Morris Україна",
      "Procter & Gamble Україна",
      "Альянс Ойл Україна",
      "Фуршет",
      "Bunge Україна",
      "Конті",
      "Comfy",
      "Амстор",
      "Альба Україна",
      "Серна",
      "Imperial Tobacco Україна",
      "Рітейл Груп",
      "Global Spirits",
      "Авангард",
      "Баядера холдинг",
      "Nestle",
      "КЗРК",
      "УкрМет",
      "Kraft Foods Україна",
      "Auchan Україна",
      "Toyota Україна",
      "ТД Щедро",
      "Альянс Холдинг",
      "Дніпровагонмаш",
      "SUN InBev Україна",
      "Porsche Україна",
      "ДІС",
      "Авіас",
      "ПАТ«Запоріжтрансформатор»",
      "НОРД",
      "Оболонь",
      "Стаханівський вагонзавод",
      "Карпатнафтохім",
      "Астарта-Київ",
      "Керамет",
      "MTI",
      "Славутич",
      "Віннер Імпортс",
      "МГЗ",
      "LG Electronics Україна",
      "БРСМ-Нафта",
      "Таврія В",
      "МАУ",
      "Паралель",
      "АВК",
      "Ельдорадо",
      "Закарпаття-нафтопродукт-Мукачево",
      "Астеліт",
      "УкрАВТО",
      "Харківобленерго",
      "SLAV AG",
      "Ерідон",
      "НЕК Укренерго",
      "ДніпроАзот",
      "НКМЗ",
      "Сумське НПО ім. Фрунзе",
      "Нова лінія",
      "Креатив Груп",
      "Кока-Кола Беверіджиз Україна",
      "Henkel Україна",
      "ЕКО-маркет",
      "Омега-Автопоставка",
      "Караван",
      "КВВМПУ",
      "Альтіс Холдинг",
      "Рутекс",
      "Укрвторчермет",
      "Регіон",
      "AES Україна",
      "Атолл Холдинг",
      "Retal Industries Україна",
      "Молочний Альянс",
      "ЄвроХім-Україна",
      "Alfred C. Toepfer Україна",
      "Нафтогазодобування",
      "British American Tobacco Україна",
      "Мрія Агрохолдинг",
      "Київміськбуд",
      "Крименерго",
      "Rompetrol Україна",
      "Київський КПК",
      "Nissan Motor Україна",
      "Фра-М",
      "Вента ЛТД",
      "Агроімпорт",
      "Технополіс",
      "Дельта Вілмар СНД",
      "Шахта ім.Засядько",
      "Кримська горілчана компанія",
      "ДонбасІнформЗвязокСервіс",
      "Louis Dreyfus Україна",
      "Піонер-Трейд",
      "ПГНК",
      "McDonald’s Україна",
      "НВК Гірничі машини",
      "Noble Resources Україна",
      "Nemiroff",
      "Луганськтепловоз",
      "Євротек",
      "Сингента",
      "Агромарс",
      "НІКО-Україна",
      "Індустріал групп",
      "Renault Україна",
      "МегаМаркет",
      "StarLightMedia",
      "Оліяр",
      "Атан-Крим",
      "Watsons",
      "Oriflame Cosmetics Україна",
      "АВ Метал Груп",
      "РКТК",
      "Фармак",
      "Сентравіс Продакшн Юкрейн",
      "Техноторг",
      "Ліа",
      "IDS Group",
      "Савсервіс-мова",
      "СКТБ «Комплекс»",
      "Кроно-Україна",
      "Вікант",
      "Надія",
      "LOreal Україна",
      "Bayer Україна",
      "Mars Україна",
      "Росава",
      "Євроцемент груп-Україна",
      "Кременчуцький сталеливарний завод",
      "Інком",
      "HeidelbergCement Україна",
      "SOCAR Україна",
      "Knauf Україна",
      "Енергомашспецсталь",
      "Комінмет",
      "Союзпромінвестдонбас",
      "Турбоатом",
      "Sanofi Україна",
      "Avon Cosmetics Україна",
      "Українські аграрні інвестиції",
      "Coal Energy",
      "Novus",
      "ПАККО Холдинг",
      "Billa-Україна",
      "Укрпластик",
      "Дизельний завод",
      "Стальканат-Сілур",
      "Adidas Україна",
      "Albedo Tech",
      "Яготинський маслозавод",
      "Майкрософт Україна",
      "Amway Україна",
      "Coca-Cola Україна",
      "B.A.T.-Україна",
      "Danone-Юнімілк",
      "Metro Cash & Carry Україна",
    ];
  }

  @override
  List<String> companyTypes({required bool isAbbr}) {
    return switch (isAbbr) {
      true => const ["ПАТ", "ПрАТ", "ТОВ", "ТДВ", "КТ", "ДАТ", "ПТ"],
      false => const [
        "Публічне акціонерне товариство",
        "Приватне акціонерне товариство",
        "Товариство з обмеженою відповідальністю",
        "Товариство з додатковою відповідальністю",
        "Командитне товариство",
        "Державне акціонерне товариство",
        "Повне товариство",
      ],
    };
  }

  @override
  String get currencyCode => "UAH";

  @override
  String get currencySymbol => "₴";
}
