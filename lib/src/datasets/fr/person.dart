import 'package:imitatio/src/datasets/models/person.dart';
import 'package:imitatio/src/enums.dart';

class FrPersonData extends PersonDataset {
  @override
  List<String> get academicDegrees {
    return const [
      "Licence",
      "Master",
      "Doctorat",
    ];
  }

  @override
  List<String> get genders {
    return const ["Masculin", "Féminin"];
  }

  @override
  List<String> get languages {
    return const [
      "Chinois mandarin",
      "Espagnol",
      "Anglais",
      "Bengali",
      "Hindi",
      "Portugais",
      "Russe",
      "Japonais",
      "Allemand",
      "Chinois wu",
      "Javanais",
      "Coréen",
      "Français",
      "Vietnamien",
      "Telugu",
      "Chinois cantonais",
      "Marathi",
      "Tamoul",
      "Turc",
      "Ourdou",
      "Chinois min",
      "Chinois jinyu",
      "Goudjarati",
      "Polonais",
      "Arabe égyptien",
      "Ukrainien",
      "Italien",
      "Chinois xiang",
      "Malayalam",
      "Chinois hakka",
      "Kannada",
      "Oriya",
      "Pendjabi de l’ouest",
      "Sunda",
      "Pendjabi de l’est",
      "Roumain",
      "Bhojpouri",
      "Azerbaïdjanais",
      "Farsi",
      "Maithili",
      "Haoussa",
      "Arabe algérien",
      "Birman",
      "Serbe",
      "Chinois gan",
      "Awadhi",
      "Thaï",
      "Néerlandais",
      "Yorouba",
      "Sindhi",
      "Arabe marocain",
      "Arabe saidi",
      "Ouzbek",
      "Malais",
      "Amharique",
      "Indonésien",
      "Ibo",
      "Filipino",
      "Népali",
      "Arabe soudanais",
      "Saraiki",
      "Visayan",
      "Arabe bedawi",
      "Thaï du nord-est",
      "Assamais",
      "Hongrois",
      "Chittagonian",
      "Arabe mésopotam.",
      "Madourais",
      "Singhalais",
      "Haryanvi",
      "Marwari",
      "Tchèque",
      "Grec",
      "Magahi",
      "Chhattisgarhi",
      "Deccan",
      "Chinois min bei",
      "Biélorusse",
      "Zhuang",
      "Chinois mandarin",
      "Espagnol",
      "Anglais",
      "Bengali",
      "Hindi",
      "Portugais",
      "Russe",
      "Japonais",
      "Allemand",
      "Chinois wu",
      "Javanais",
      "Coréen",
      "Français",
      "Vietnamien",
      "Telugu",
      "Chinois cantonais",
      "Marathi",
      "Tamoul",
      "Turc",
      "Ourdou",
      "Chinois min",
      "Chinois jinyu",
      "Goudjarati",
      "Polonais",
      "Arabe égyptien",
      "Ukrainien",
      "Italien",
      "Chinois xiang",
      "Malayalam",
      "Chinois hakka",
      "Kannada",
      "Oriya",
      "Pendjabi de l’ouest",
      "Sunda",
      "Pendjabi de l’est",
      "Roumain",
      "Bhojpouri",
      "Azerbaïdjanais",
      "Farsi",
      "Maithili",
      "Haoussa",
      "Arabe algérien",
      "Birman",
      "Serbe",
      "Chinois gan",
      "Awadhi",
      "Thaï",
      "Néerlandais",
      "Yorouba",
      "Sindhi",
      "Arabe marocain",
      "Arabe saidi",
      "Ouzbek",
      "Malais",
      "Amharique",
      "Indonésien",
      "Ibo",
      "Filipino",
      "Népali",
      "Arabe soudanais",
      "Saraiki",
      "Visayan",
      "Arabe bedawi",
      "Thaï du nord-est",
      "Assamais",
      "Hongrois",
      "Chittagonian",
      "Arabe mésopotam.",
      "Madourais",
      "Singhalais",
      "Haryanvi",
      "Marwari",
      "Tchèque",
      "Grec",
      "Magahi",
      "Chhattisgarhi",
      "Deccan",
      "Chinois min bei",
      "Biélorusse",
      "Zhuang",
    ];
  }

  @override
  List<String> names(Gender gender) {
    return switch (gender) {
      Gender.female => const [
          "Aaliyah",
          "Abigael",
          "Adriana",
          "Adèle",
          "Agathe",
          "Aicha",
          "Albane",
          "Alessia",
          "Alexandra",
          "Alexia",
          "Aleyna",
          "Alia",
          "Alice",
          "Alicia",
          "Alienor",
          "Aline",
          "Alix",
          "Aliya",
          "Aliyah",
          "Alizee",
          "Alya",
          "Alycia",
          "Alyssa",
          "Alyssia",
          "Amandine",
          "Ambre",
          "Ambrine",
          "Amel",
          "Amelia",
          "Amina",
          "Aminata",
          "Amira",
          "Amy",
          "Amélie",
          "Ana",
          "Anae",
          "Anaelle",
          "Anastasia",
          "Anaïs",
          "Andrea",
          "Angela",
          "Angelina",
          "Angeline",
          "Angèle",
          "Anissa",
          "Anna",
          "Annabelle",
          "Annaelle",
          "Anouk",
          "Apolline",
          "Ashley",
          "Asma",
          "Assia",
          "Assya",
          "Astrid",
          "Athénaïs",
          "Audrey",
          "Augustine",
          "Aurore",
          "Aurélie",
          "Ava",
          "Awa",
          "Axelle",
          "Aya",
          "Berenice",
          "Bertille",
          "Blanche",
          "Cali",
          "Calie",
          "Camelia",
          "Camille",
          "Candice",
          "Capucine",
          "Carla",
          "Carmen",
          "Caroline",
          "Cassandra",
          "Cassandre",
          "Cassie",
          "Cecilia",
          "Celia",
          "Celya",
          "Chahinez",
          "Chaima",
          "Charlene",
          "Charlie",
          "Charline",
          "Charlotte",
          "Chayma",
          "Chiara",
          "Chloé",
          "Claire",
          "Clara",
          "Clarisse",
          "Clea",
          "Clelia",
          "Clementine",
          "Cleo",
          "Clotilde",
          "Cloé",
          "Clémence",
          "Coline",
          "Constance",
          "Coralie",
          "Cyrielle",
          "Cécile",
          "Céleste",
          "Célestine",
          "Céline",
          "Daphné",
          "Deborah",
          "Diana",
          "Diane",
          "Dina",
          "Dounia",
          "Eden",
          "Ela",
          "Elea",
          "Eleana",
          "Elena",
          "Elia",
          "Elif",
          "Elina",
          "Eline",
          "Elisa",
          "Elissa",
          "Ella",
          "Eloane",
          "Elona",
          "Elora",
          "Elsa",
          "Elya",
          "Elyna",
          "Elyne",
          "Eléonore",
          "Ema",
          "Emeline",
          "Emie",
          "Emily",
          "Emma",
          "Emmanuelle",
          "Emmie",
          "Emmy",
          "Emna",
          "Emy",
          "Enola",
          "Enora",
          "Erine",
          "Estelle",
          "Esther",
          "Eugénie",
          "Eva",
          "Eve",
          "Eya",
          "Fanny",
          "Fantine",
          "Farah",
          "Fatima",
          "Fatma",
          "Fatoumata",
          "Faustine",
          "Flavie",
          "Fleur",
          "Flora",
          "Flore",
          "Florine",
          "Gabriella",
          "Gabrielle",
          "Garance",
          "Gaëlle",
          "Giulia",
          "Grace",
          "Gwenaëlle",
          "Gwendoline",
          "Hafsa",
          "Hajar",
          "Halima",
          "Hana",
          "Hanae",
          "Hanna",
          "Hannah",
          "Harriet",
          "Hawa",
          "Helena",
          "Hiba",
          "Hind",
          "Honor",
          "Hortense",
          "Héloïse",
          "Hélène",
          "Ibtissem",
          "Ilana",
          "Iliana",
          "Ilona",
          "Ilyana",
          "Imane",
          "Imen",
          "Imene",
          "Inaya",
          "Inès",
          "Iris",
          "Isabeau",
          "Isaure",
          "Isis",
          "Isra",
          "Ivette",
          "Izia",
          "Jacqueline",
          "Jade",
          "Jaimie",
          "Jana",
          "Janel",
          "Janelle",
          "Janna",
          "Jasmine",
          "Jeanne",
          "Jenna",
          "Jessica",
          "Jihane",
          "Joana",
          "Johanna",
          "Jolie",
          "Joséphine",
          "Joy",
          "Joyce",
          "Judith",
          "Julia",
          "Julie",
          "Juliet",
          "Juliette",
          "Juline",
          "Justine",
          "Kadence",
          "Kahina",
          "Kaina",
          "Kalet",
          "Kamelia",
          "Kayla",
          "Kayna",
          "Kelia",
          "Kelly",
          "Kelya",
          "Kenza",
          "Kessy",
          "Khadija",
          "Kiara",
          "Kim",
          "Kimberley",
          "Kordell",
          "Kyara",
          "Kyrielle",
          "L'Oréal",
          "Laetitia",
          "Lalie",
          "Laly",
          "Lamia",
          "Lana",
          "Lara",
          "Larue",
          "Laura",
          "Laure",
          "Lauren",
          "Laurine",
          "Laverne",
          "Layana",
          "Leala",
          "Leana",
          "Leane",
          "Leanne",
          "Leelou",
          "Leia",
          "Leila",
          "Leina",
          "Lena",
          "Leona",
          "Leontyne",
          "Lesh",
          "Leyna",
          "Lia",
          "Liana",
          "Lila",
          "Lilas",
          "Lili",
          "Lili-Rose",
          "Lilia",
          "Lilie",
          "Lilly",
          "Lilou",
          "Lilwenn",
          "Lily",
          "Lily-Rose",
          "Lilya",
          "Lina",
          "Linda",
          "Lindsay",
          "Line",
          "Lisa",
          "Lise",
          "Lisette",
          "Lisle",
          "Lison",
          "Livia",
          "Liya",
          "Loane",
          "Loanne",
          "Lois",
          "Lola",
          "Lorena",
          "Loring",
          "Lou",
          "Lou-Ann",
          "Lou-Anne",
          "Louane",
          "Louann",
          "Louanne",
          "Louisa",
          "Louise",
          "Louison",
          "Louna",
          "Luce",
          "Lucetta",
          "Lucia",
          "Lucie",
          "Lucile",
          "Lucille",
          "Lucy",
          "Ludivine",
          "Luna",
          "Lunette",
          "Lya",
          "Lyana",
          "Lydia",
          "Lyla",
          "Lyle",
          "Lylia",
          "Lylou",
          "Lyna",
          "Léa",
          "Léonie",
          "Madeleine",
          "Mae",
          "Maelia",
          "Maelie",
          "Maeline",
          "Maelly",
          "Maellys",
          "Maely",
          "Maelyne",
          "Maelyss",
          "Maeva",
          "Maia",
          "Maily",
          "Mailys",
          "Maimouna",
          "Maina",
          "Maine",
          "Maissa",
          "Maissane",
          "Maiwenn",
          "Malak",
          "Malia",
          "Mallory",
          "Manel",
          "Manelle",
          "Manon",
          "Mardi",
          "Margaux",
          "Margo",
          "Margot",
          "Maria",
          "Mariam",
          "Marianne",
          "Maribel",
          "Marie",
          "Marilou",
          "Marina",
          "Marine",
          "Marion",
          "Marvelle",
          "Marwa",
          "Maryam",
          "Marylou",
          "Mathilde",
          "Maud",
          "Maureen",
          "Mauve",
          "Mavis",
          "Maya",
          "Maylis",
          "Mayssa",
          "Maëlle",
          "Maëlys",
          "Meissa",
          "Melia",
          "Melina",
          "Melinda",
          "Melisenda",
          "Mellina",
          "Melodie",
          "Melody",
          "Melyna",
          "Melyne",
          "Merane",
          "Meriem",
          "Merla",
          "Merle",
          "Merlyn",
          "Meryem",
          "Meryl",
          "Mia",
          "Miette",
          "Mignon",
          "Mila",
          "Milla",
          "Mina",
          "Mona",
          "Morgane",
          "Mya",
          "Myriam",
          "Mélanie",
          "Méline",
          "Mélissa",
          "Nada",
          "Naelle",
          "Naia",
          "Naila",
          "Naima",
          "Nais",
          "Naomi",
          "Naomie",
          "Nawel",
          "Naya",
          "Nayla",
          "Neila",
          "Nelia",
          "Nell",
          "Nesrine",
          "Neyla",
          "Nina",
          "Ninon",
          "Nisa",
          "Noa",
          "Noelie",
          "Noeline",
          "Nola",
          "Nora",
          "Norah",
          "Nour",
          "Noémie",
          "Océane",
          "Olivia",
          "Ophélie",
          "Oriane",
          "Orlane",
          "Ornella",
          "Paloma",
          "Paola",
          "Pauline",
          "Perrine",
          "Philippine",
          "Philomène",
          "Prune",
          "Pénélope",
          "Rachel",
          "Rahma",
          "Rania",
          "Raphaëlle",
          "Romane",
          "Romy",
          "Rosalie",
          "Rose",
          "Roxane",
          "Rébecca",
          "Sabrina",
          "Sacha",
          "Safa",
          "Safia",
          "Safiya",
          "Sakina",
          "Salma",
          "Salomé",
          "Samia",
          "Sana",
          "Sara",
          "Sarah",
          "Sasha",
          "Savannah",
          "Selena",
          "Selene",
          "Selma",
          "Serena",
          "Shaima",
          "Shaina",
          "Shana",
          "Shanna",
          "Shayna",
          "Sherine",
          "Sidonie",
          "Sirine",
          "Sixtine",
          "Sofia",
          "Soline",
          "Solène",
          "Sonia",
          "Sophia",
          "Sophie",
          "Soraya",
          "Soukaina",
          "Soumaya",
          "Stella",
          "Suzanne",
          "Suzie",
          "Syrine",
          "Taina",
          "Talia",
          "Tamara",
          "Tania",
          "Tatiana",
          "Tess",
          "Tessa",
          "Thais",
          "Thalia",
          "Thea",
          "Tiffany",
          "Tina",
          "Tiphaine",
          "Valentine",
          "Vanessa",
          "Victoire",
          "Victoria",
          "Violette",
          "Wendy",
          "Yael",
          "Yaelle",
          "Yasmina",
          "Yasmine",
          "Youna",
          "Yousra",
          "Yuna",
          "Zahra",
          "Zelie",
          "Zia",
          "Zoé",
          "Élisabeth",
          "Élise",
          "Élodie",
          "Éloïse",
          "Émilie",
        ],
      Gender.male => const [
          "Aaron",
          "Abdallah",
          "Abdoulaye",
          "Abel",
          "Achille",
          "Adam",
          "Adama",
          "Adame",
          "Adel",
          "Adem",
          "Adil",
          "Adrian",
          "Adrien",
          "Ahmed",
          "Aidan",
          "Aksel",
          "Alan",
          "Alban",
          "Albin",
          "Aleron",
          "Alessandro",
          "Alessio",
          "Alex",
          "Alexandre",
          "Alexis",
          "Alexy",
          "Algernon",
          "Ali",
          "Alix",
          "Allan",
          "Alois",
          "Amadou",
          "Amaury",
          "Ambroise",
          "Ames",
          "Amiens",
          "Amin",
          "Amine",
          "Amir",
          "Anael",
          "Anas",
          "Anatole",
          "Andrea",
          "Andreas",
          "Andy",
          "Ange",
          "Angel",
          "Angelo",
          "Anis",
          "Anthony",
          "Antoine",
          "Anton",
          "Antonin",
          "Antonio",
          "Antony",
          "Aramis",
          "Arda",
          "Armand",
          "Arnaud",
          "Arsène",
          "Arthur",
          "Aubin",
          "Audric",
          "Auguste",
          "Augustin",
          "Aurélien",
          "Axel",
          "Ayman",
          "Aymane",
          "Aymen",
          "Aymeric",
          "Ayoub",
          "Badis",
          "Baptiste",
          "Basile",
          "Bastien",
          "Beau",
          "Benjamin",
          "Benoit",
          "Bilal",
          "Bilel",
          "Blaise",
          "Bonar",
          "Boniface",
          "Boyce",
          "Brahim",
          "Brandon",
          "Brayan",
          "Briar",
          "Bruce",
          "Bryan",
          "Byron",
          "Cabal",
          "Cade",
          "Calvin",
          "Cameron",
          "Camil",
          "Camille",
          "Campbell",
          "Canaan",
          "Canon",
          "Carolus",
          "Carvell",
          "Cedric",
          "Celian",
          "Chahine",
          "Chainey",
          "Charles",
          "Charlie",
          "Charly",
          "Chevy",
          "Chris",
          "Christopher",
          "Ciel",
          "Circinus",
          "Clovis",
          "Clément",
          "Colar",
          "Colin",
          "Colville",
          "Corentin",
          "Courvoisier",
          "Coyne",
          "Curtis",
          "Cyprien",
          "Cyril",
          "Célestin",
          "César",
          "Côme",
          "Dabney",
          "Damien",
          "Daniel",
          "Dany",
          "Dashiell",
          "David",
          "Dax",
          "Delano",
          "Demont",
          "Denis",
          "Destin",
          "Didier",
          "Diego",
          "Dimitri",
          "Dior",
          "Djibril",
          "Donatien",
          "Donovan",
          "Dorian",
          "Dylan",
          "Eden",
          "Edgar",
          "Elias",
          "Elijah",
          "Elio",
          "Eliot",
          "Eliott",
          "Elliot",
          "Eloan",
          "Eloi",
          "Elouan",
          "Elyas",
          "Elyes",
          "Emir",
          "Emmanuel",
          "Enes",
          "Enzo",
          "Eren",
          "Eric",
          "Erwan",
          "Erwann",
          "Esteban",
          "Etan",
          "Ethan",
          "Ethann",
          "Etienne",
          "Evan",
          "Evann",
          "Ewan",
          "Ewen",
          "Ewenn",
          "Ezio",
          "Fabien",
          "Fabio",
          "Fabron",
          "Fares",
          "Ferrand",
          "Flavien",
          "Flavio",
          "Florent",
          "Florian",
          "Franchot",
          "Francois",
          "François",
          "Frasier",
          "Félix",
          "Gabin",
          "Gabriel",
          "Gage",
          "Garen",
          "Gareth",
          "Garland",
          "Garner",
          "Garrison",
          "Gaspard",
          "Gaston",
          "Gauthier",
          "Gautier",
          "Gaylord",
          "Gaétan",
          "Gaël",
          "Gerald",
          "Germain",
          "Geron",
          "Gianni",
          "Giles",
          "Giovanni",
          "Glanville",
          "Gower",
          "Granger",
          "Grant",
          "Granville",
          "Gregory",
          "Grégoire",
          "Guilhem",
          "Guillaume",
          "Gustave",
          "Guy",
          "Hackett",
          "Hadrien",
          "Hamza",
          "Hector",
          "Hello",
          "Henri",
          "Hippolyte",
          "Holland",
          "Honore",
          "Hugo",
          "Iban",
          "Ibrahim",
          "Ibrahima",
          "Idris",
          "Idriss",
          "Ilan",
          "Ilhan",
          "Ilian",
          "Ilias",
          "Ilies",
          "Ilyan",
          "Ilyas",
          "Ilyes",
          "Ilyess",
          "Imran",
          "Imrane",
          "Isaac",
          "Ismael",
          "Ismail",
          "Issa",
          "Issam",
          "Ivan",
          "Iyad",
          "Iyed",
          "Jacques",
          "Jaden",
          "James",
          "Jason",
          "Jasper",
          "Javier",
          "Jawad",
          "Jayson",
          "Jean",
          "Jean-Baptiste",
          "Jemond",
          "Jeremy",
          "Jesper",
          "Jessim",
          "Jessy",
          "Jibril",
          "Jimmy",
          "Joachim",
          "Joan",
          "Johan",
          "Johann",
          "Jonas",
          "Jonathan",
          "Jordan",
          "Joris",
          "Joseph",
          "Joshua",
          "Josué",
          "Jules",
          "Julian",
          "Julien",
          "Justin",
          "Jérémie",
          "Kais",
          "Kamil",
          "Karim",
          "Karl",
          "Kelian",
          "Kelyan",
          "Kenan",
          "Kenji",
          "Kenny",
          "Kenzo",
          "Kevin",
          "Keziah",
          "Khalil",
          "Kilian",
          "Killian",
          "Kilyan",
          "Kylian",
          "Kyllian",
          "Labaron",
          "Lance",
          "Landry",
          "Laramie",
          "Laron",
          "Larrimore",
          "Launcelot",
          "Laurent",
          "Leandro",
          "Leggett",
          "Lenny",
          "Leny",
          "Lenzo",
          "Leverett",
          "Liam",
          "Lilian",
          "Lino",
          "Lionel",
          "Lisandro",
          "Livio",
          "Loan",
          "Loann",
          "Logan",
          "Lohan",
          "Lois",
          "Lorenzo",
          "Loris",
          "Lou",
          "Louca",
          "Loucas",
          "Louis",
          "Louison",
          "Louka",
          "Loukas",
          "Loïc",
          "Lubin",
          "Luc",
          "Luca",
          "Lucas",
          "Lucien",
          "Ludovic",
          "Luis",
          "Luka",
          "Lukas",
          "Lyam",
          "Léandre",
          "Léo",
          "Léon",
          "Léonard",
          "Léopold",
          "Maceo",
          "Mae",
          "Mahamadou",
          "Mahdi",
          "Mahe",
          "Malik",
          "Mallory",
          "Malo",
          "Malone",
          "Mamadou",
          "Marc",
          "Marceau",
          "Marcel",
          "Marco",
          "Marcus",
          "Mardian",
          "Marin",
          "Marius",
          "Marley",
          "Marlon",
          "Marouane",
          "Marshall",
          "Martin",
          "Marvel",
          "Marvin",
          "Marwan",
          "Marwane",
          "Mason",
          "Mateo",
          "Matheo",
          "Mathias",
          "Mathieu",
          "Mathis",
          "Mathys",
          "Matis",
          "Matisse",
          "Matt",
          "Matteo",
          "Mattheo",
          "Matthew",
          "Matthias",
          "Matthieu",
          "Matys",
          "Max",
          "Maxence",
          "Maxime",
          "Maximilien",
          "Mayron",
          "Maël",
          "Mehdi",
          "Melvin",
          "Melvyn",
          "Merlin",
          "Merrill",
          "Michael",
          "Michel",
          "Mickael",
          "Miguel",
          "Mike",
          "Milan",
          "Milo",
          "Mohamed",
          "Mohamed-Ali",
          "Mohamed-Amine",
          "Mohammed",
          "Montague",
          "Montgomery",
          "Montrell",
          "Montrose",
          "Morgan",
          "Mortimer",
          "Moussa",
          "Muhammed",
          "Mylan",
          "Nabil",
          "Nadir",
          "Nael",
          "Nahel",
          "Nahil",
          "Nail",
          "Naim",
          "Nassim",
          "Natan",
          "Nathael",
          "Nathan",
          "Nathanael",
          "Nazaire",
          "Nelson",
          "Neo",
          "Neville",
          "Nicolas",
          "Nils",
          "Nino",
          "Noa",
          "Noah",
          "Noam",
          "Noan",
          "Noel",
          "Noha",
          "Noham",
          "Nohan",
          "Nohlan",
          "Nolan",
          "Nolann",
          "Nolhan",
          "Noé",
          "Octave",
          "Olivier",
          "Omar",
          "Oscar",
          "Ousmane",
          "Owen",
          "Pablo",
          "Paco",
          "Pacome",
          "Paolo",
          "Paul",
          "Philippe",
          "Pierre",
          "Quentin",
          "Rafael",
          "Raphaël",
          "Rayan",
          "Rayane",
          "Reda",
          "Riyad",
          "Robin",
          "Romain",
          "Roman",
          "Romeo",
          "Ronan",
          "Ruben",
          "Rudy",
          "Ryad",
          "Ryan",
          "Rémi",
          "Rémy",
          "Sabri",
          "Sacha",
          "Said",
          "Salim",
          "Sam",
          "Sami",
          "Samuel",
          "Samy",
          "Sandro",
          "Sasha",
          "Sean",
          "Selim",
          "Selyan",
          "Sevan",
          "Simeon",
          "Simon",
          "Soan",
          "Sofiane",
          "Sohan",
          "Solal",
          "Soren",
          "Souleymane",
          "Stan",
          "Stanislas",
          "Steven",
          "Stéphane",
          "Swan",
          "Swann",
          "Sébastien",
          "Tao",
          "Teddy",
          "Teo",
          "Thibaud",
          "Thibault",
          "Thibaut",
          "Thimeo",
          "Thomas",
          "Théo",
          "Théodore",
          "Théophile",
          "Tiago",
          "Tidiane",
          "Tilio",
          "Tim",
          "Timeo",
          "Timote",
          "Timothe",
          "Timothée",
          "Tino",
          "Titouan",
          "Tom",
          "Tony",
          "Tristan",
          "Tymeo",
          "Tyron",
          "Ugo",
          "Ulysse",
          "Vadim",
          "Valentin",
          "Vianney",
          "Victor",
          "Vincent",
          "Virgile",
          "Wael",
          "Walid",
          "Warren",
          "Wassim",
          "Wesley",
          "William",
          "Wissem",
          "Wyatt",
          "Xavier",
          "Yacine",
          "Yael",
          "Yanis",
          "Yann",
          "Yannis",
          "Yasser",
          "Yassin",
          "Yassine",
          "Ylan",
          "Ylann",
          "Yoan",
          "Yoann",
          "Yohan",
          "Yohann",
          "Youcef",
          "Younes",
          "Youssef",
          "Yusuf",
          "Zacharie",
          "Zakaria",
          "Édouard",
          "Élie",
          "Émeric",
          "Émile",
          "Émilien",
          "Étienne",
        ],
    };
  }

  @override
  List<String> nationalities(Gender gender) {
    return switch (gender) {
      Gender.female => const [
          'Afghane'
              'Algérienne'
              'Américaine'
              'Anglaise'
              'Australienne'
              'Autrichienne'
              'Belge'
              'Brésilienne'
              'Canadienne',
          'Chinoise',
          'Danoise',
          'Égyptienne',
          'Espagnole',
          'Finnoise',
          'Française',
          'Allemande',
          'Grecque',
          'Indienne',
          'Irlandaise',
          'Italienne',
          'Japonaise',
          'Libanaise',
          'Mexicaine',
          'Néerlandaise',
          'Norvégienne',
          'Néo-Zélandaise',
          'Pakistanais',
          'Polonaise',
          'Portugaise',
          'Russe',
          'Suédoise',
          'Suisse',
          'Turque',
          'Amérindienne',
        ],
      Gender.male => const [
          'Afghan',
          'Algérien',
          'Américain',
          'Anglais',
          'Australien',
          'Autrichien',
          'Belge',
          'Brésilien',
          'Canadien',
          'Chinois',
          'Danois',
          'Égyptien',
          'Espagnol',
          'Finnois',
          'Français',
          'Allemand',
          'Grec',
          'Indien',
          'Irlandais',
          'Italien',
          'Japonais',
          'Libanais',
          'Mexicain',
          'Néerlandais',
          'Norvégien',
          'Néo-Zélandais',
          'Pakistanaise',
          'Polonais',
          'Portugais',
          'Russe',
          'Suédois',
          'Suisse',
          'Turc',
          'Amérindien',
        ],
    };
  }

  @override
  List<String> get occupations {
    return const [
      "Acheteur d’espaces publicitaires",
      "Acheteur en centrale d’achat",
      "Acheteur en produits agroalimentaires",
      "Acheteur en produits horticoles",
      "Acheteur industriel",
      "Acousticien du bâtiment",
      "Actrice",
      "Actuaire",
      "Administrateur dans la fonction publique européenne",
      "Administrateur de bases de données",
      "Administrateur de réseaux",
      "Administrateur des ventes",
      "Agent",
      "Agent d'accueil en office de tourisme",
      "Agent de déchetterie",
      "Agent de développement local",
      "Agent de planning aérien (ou agent de planification)",
      "Agent de recouvrement du Trésor",
      "Agent d’accueil",
      "Agent général",
      "Agent immobilier",
      "Agriculteur / Paysan",
      "Agriculteur bio",
      "Aide médico-psychologique",
      "Aide-soignant(e)",
      "Aiguilleur du ciel",
      "Ambassadeur du tri",
      "Ambassadeur ou consul",
      "Analyste de crédit",
      "Analyste et contrôleur des risques financiers",
      "Analyste social",
      "Anesthésiste",
      "Réanimateur",
      "Animateur de formation",
      "Animateur de patrimoine",
      "Animateur des activités physiques",
      "Animateur en écotourisme",
      "Animateur nature",
      "Animateur radio-télé",
      "Animateur socioculturel",
      "Animateur sportif",
      "Animateur touristique",
      "Animateur-éducateur environnement",
      "Aquaculteur",
      "Archéologue",
      "Architecte",
      "Architecte d’intérieur",
      "Architecte en environnement",
      "Architecte en systèmes d’information",
      "Aromaticien/ Aromaticienne",
      "Assistant comptable",
      "Assistant de service social",
      "Assistant export",
      "Assistant manager",
      "Astronaute",
      "Astronome (ou astrophysicien)",
      "Attaché commercial",
      "Attaché de presse",
      "Audioprothésiste",
      "Auditeur financier",
      "Auxiliaire de puériculture",
      "Avocat",
      "Bibliothécaire",
      "Bibliothécaire jeunesse",
      "Bijoutier-joaillier",
      "Biochimiste",
      "Biologiste",
      "Biologiste médical",
      "Biostatisticien",
      "Botaniste",
      "Boucher",
      "Boulanger",
      "Campus manager",
      "Cartographe",
      "Céramiste",
      "Chanteur",
      "Chargé de clientèle dans la banque",
      "Chargé de clientèle en banque/assurance",
      "Chargé de communication en environnement",
      "Chargé de communication événementielle",
      "Chargé de mission Agenda 21",
      "Chargé de mission ENR (énergies renouvelables)",
      "Chargé de pharmaco-vigilance",
      "Chargé des relations publiques",
      "Chargé du recrutement",
      "Chargé du suivi des programmes humanitaires",
      "Chargé d’affaires",
      "Chargé d’études commerciales",
      "Chef d'escale",
      "Chef de cabine",
      "Chef de caisse",
      "Chef de chantier",
      "Chef de cuisine",
      "Chef de mission dans l’humanitaire",
      "Chef de produit",
      "Chef de projet démantèlement nucléaire",
      "Chef de projet informatique",
      "Chef de projet photovoltaïque",
      "Chef de projet sites et sols pollués",
      "Chef de projet urbain",
      "Chef de pub en agence",
      "Chef de rang",
      "Chef de rayon",
      "Chef d’exploitation d’unité de récupération",
      "Chef opérateur",
      "Chef réceptionniste",
      "Chercheur en sciences et en industries",
      "Chirurgien",
      "Dentiste",
      "Climatologue",
      "Clown",
      "Com and Ben",
      "Comédien",
      "Commandant de bord",
      "Commercial en énergies renouvelables",
      "Commissaire aux comptes",
      "Commissaire de police",
      "Commissaire-priseur",
      "Community manager",
      "Comptable",
      "Concepteur de produits touristiques",
      "Concepteur-réalisateur de loisirs durables",
      "Concepteur-réalisateur multimédia",
      "Concepteur-rédacteur",
      "Conducteur de travaux",
      "Conférencier de musée et de monument",
      "Conseil en lobbying",
      "Conseiller agricole",
      "Conseiller conjugal et familial",
      "Conseiller d'orientation-psychologue",
      "Conseiller en agriculture durable",
      "Conseiller en économie sociale et familiale",
      "Conseiller en environnement",
      "Conseiller en immobilier d’entreprise",
      "Conseiller en insertion sociale et professionnelle",
      "Conseiller en maîtrise d'énergie",
      "Conseiller en management",
      "Conseiller en organisation",
      "Conseiller en prévention des risques psychosociaux",
      "Conseiller principal d'éducation",
      "Conseiller-vendeur en agence de voyages",
      "Conservateur de musée",
      "Conservateur de réserve naturelle",
      "Constructeur bois",
      "Consultant en green IT",
      "Consultant en recrutement",
      "Conteur",
      "Contrôleur budgétaire",
      "Contrôleur de gestion",
      "Costumier",
      "Courtier",
      "Créateur de parfums",
      "Décorateur-scénographe",
      "Demographe",
      "Designer industriel",
      "Designer sonore",
      "Designer textile",
      "Dessinateur industriel",
      "Dessinateur-projeteur en bâtiment",
      "Développeur de produits bancaires",
      "Développeur de projets en énergies renouvelables",
      "Développeur informatique",
      "Développeur(se)",
      "Diagnostiqueur immobilier",
      "Diététicien",
      "Directeur artistique",
      "Directeur d'équipements sportifs",
      "Directeur d'établissement",
      "Directeur d'office de tourisme",
      "Directeur d'une salle de spectacle",
      "Directeur de centre de vacances ou de loisirs",
      "Directeur de création",
      "Directeur de restaurant",
      "Directeur des achats",
      "Directeur des opérations",
      "Directeur des ressources humaines",
      "Directeur du marketing",
      "Directeur d’enseigne",
      "Documentaliste",
      "Domoticien",
      "Ébéniste",
      "Éco-concepteur",
      "Éco-développeur",
      "Économe",
      "Économiste de la construction",
      "Écotoxicologue",
      "Éditeur",
      "Éducateur de jeunes enfants",
      "Éducateur de la Protection judiciaire de la jeunesse",
      "Éducateur spécialisé",
      "Éducateur technique spécialisé",
      "Éducatrice de jeunes enfants",
      "Éducatrice spécialisée",
      "Électrotechnicien",
      "Enseignant à l'étranger",
      "Enseignant chercheur",
      "Entraîneur sportif",
      "Ethnologue",
      "Éthologiste",
      "Expert automobile",
      "Expert bilan carbone",
      "Expert immobilier",
      "Expert incendies et risques divers",
      "Expert(e) en sécurité informatique",
      "Expert-comptable",
      "Game designer",
      "Garde du littoral",
      "Garde-chasse / Garde pêche",
      "Garde-moniteur",
      "Gendarme",
      "Généticien",
      "Géologue",
      "Géologue en environnement",
      "Géomaticien/ Géomaticienne",
      "Géomètre-topographe",
      "Gestionnaire de carrière",
      "Gestionnaire de paie",
      "Gestionnaire de patrimoine",
      "Grand reporter",
      "Graphiste multimédia",
      "Guide accompagnateur",
      "Guide interprète",
      "Gynécologue",
      "Herboriste",
      "Horticulteur",
      "Hôtesse de l'air ou steward",
      "Huissier de justice",
      "Hydrobiologiste",
      "Hydrogéologue",
      "Hydrogéologue",
      "Iconographe",
      "Illustrateur",
      "Infirmier",
      "Infirmier d’entreprise",
      "Infirmier sapeur-pompier",
      "Infirmière",
      "Infographiste de presse",
      "Ingénieur",
      "Ingénieur en gestion des risques industriels",
      "Ingénieur agronome",
      "Ingénieur automatisme-instrumentation",
      "Ingénieur calcul et appui scientifique",
      "Ingénieur chimiste",
      "Ingénieur d'études dans l'automobile",
      "Ingénieur de fabrication",
      "Ingénieur du son",
      "Ingénieur d’études dans l'industrie",
      "Ingénieur d’études en aéronautique",
      "Ingénieur en efficacité énergétique des bâtiments",
      "Ingénieur en environnement",
      "Ingénieur en mécanique",
      "Ingénieur en méthanisation",
      "Ingénieur en recherche clinique",
      "Ingénieur méthodes",
      "Ingénieur numéricien",
      "Ingénieur procédés",
      "Ingénieur procédés en environnement",
      "Ingénieur qualité ou qualiticien",
      "Ingénieur système",
      "Ingénieur télécoms",
      "Ingénieur(e) agroalimentaire",
      "Inspecteur comptable",
      "Inspecteur des Finances",
      "Inspecteur des impôts",
      "Inspecteur du travail",
      "Installateur de panneaux solaires photovoltaïques",
      "Interprète",
      "Journaliste",
      "Journaliste d’entreprise",
      "Journaliste radio en studio",
      "Journaliste reporter d’images (JRI)",
      "Journaliste spécialisé en environnement",
      "Juge aux affaires familiales",
      "Juge d'application des peines (JAP)",
      "Juge d'instruction",
      "Juge des enfants",
      "Juriste d'entreprise",
      "Juriste/avocat spécialisé en droit des ENR",
      "Libraire",
      "Lieutenant de police",
      "Lieutenant de pompiers",
      "Logisticien",
      "Logisticien dans l'humanitaire",
      "Ludothécaire",
      "Maître d'hôtel",
      "Maître d’hôtel",
      "Maître nageur sauveteur",
      "Manager dans la restauration rapide",
      "Manipulateur en électroradiologie",
      "Maquettiste",
      "Marionnettiste",
      "Masseur-kinésithérapeute",
      "Mathématicien / Mathématicienne",
      "Médecin de l'Education nationale",
      "Médecin du travail",
      "Médecin généraliste",
      "Médecin légiste",
      "Média-planneur",
      "Médiateur",
      "Médiateur culturel",
      "Médiateur en environnement",
      "Merchandiser",
      "Metteur en scène",
      "Microtechnicien",
      "Mixeur",
      "Modéliste",
      "Moniteur de ski",
      "Moniteur d’équitation",
      "Moniteur-éducateur",
      "Monteur d'éolienne",
      "Muséographe",
      "Musicien",
      "Océanologue",
      "Officier de l'armée",
      "Officier de la marine marchande",
      "Officier de l’armée de l’air",
      "Officier marinier ou officier de la Marine nationale",
      "Opérateur back office",
      "Opérateur de marché CO2",
      "Opérateur de marché en CO2",
      "Opérateur middle office",
      "Ophtalmologue",
      "Opticien-lunetier",
      "Ornithologue",
      "Orthophoniste",
      "Orthoprothésiste",
      "Orthoptiste",
      "Ostéopathe",
      "Oto-rhino-laryngologiste",
      "Paysagiste",
      "Pédiatre",
      "Pédicure-podologue",
      "Pédologue",
      "Pédopsychiatre",
      "Pharmacien d'officine",
      "Photographe",
      "Pilote de ligne",
      "Pilote d’hélicoptère",
      "Policier",
      "Producteur télé",
      "Prof de maths",
      "Professeur agrégé",
      "Professeur de lycée professionnel",
      "Professeur des écoles",
      "Professeur des écoles spécialisées",
      "Professeur documentaliste",
      "Professeur d’EPS (Éducation physique et sportive)",
      "Professeur en collège ou en lycée",
      "Prothésiste dentaire",
      "Psychanalyste",
      "Psychiatre",
      "Psychologue",
      "Psychologue scolaire",
      "Psychomotricien",
      "Puéricultrice",
      "Réalisateur",
      "Régisseur de spectacles",
      "Reporter",
      "Reporter radio",
      "Responsable de la sécurité et risques des systèmes d’information",
      "Responsable commercial recyclage",
      "Responsable d'eau potable ou d’assainissement",
      "Responsable de bases de données",
      "Responsable de centres de services éco-industriels",
      "Responsable de formation",
      "Responsable de la communication interne",
      "Responsable de la promotion des ventes",
      "Responsable de projet biodiversité",
      "Responsable de relais assistantes maternelles",
      "Responsable du bilan environnemental",
      "Responsable du e-business",
      "Responsable du marketing direct",
      "Responsable du référencement",
      "Responsable d’ordonnancement",
      "Responsable d’une station d’épuration",
      "Responsable environnement de projets ferroviaires",
      "Responsable HSE (hygiène-sécurité-environnement)",
      "Responsable SAV (Service après-vente)",
      "Restaurateur de livres anciens",
      "Restaurateur d’œuvres d’art",
      "Sage-femme",
      "Sapeur-pompier",
      "Scénariste",
      "Scripte",
      "Secrétaire administratif ou de chancellerie",
      "Secrétaire aux Affaires étrangères",
      "Sociologue",
      "Sommelier",
      "Spécialiste en biotechnologies",
      "Sportif professionnel",
      "Statisticen / Statisticienne",
      "Statisticien",
      "Styliste",
      "Substitut du procureur",
      "Technicien biocarburants",
      "Technicien conseiller en agrobiologie",
      "Technicien d'analyses biomédicales",
      "Technicien de laboratoire d’analyse des eaux",
      "Technicien de maintenance",
      "Technicien de mesure de la pollution",
      "Technicien de rivière",
      "Technicien du bâtiment en énergies renouvelables",
      "Technicien du génie rural",
      "Technicien du réemploi",
      "Technicien du traitement des eaux",
      "Technicien en automatismes",
      "Technicien en traitement d'eau potable",
      "Technicien en traitement des déchets",
      "Technicien forestier",
      "Technicien gestionnaire d’espaces naturels",
      "Technicien mesure pollution",
      "Technicien paysagiste",
      "Technicien qualité automobile",
      "Technicien traitement des eaux",
      "Technicien(ne) d’intervention",
      "Technicien-conseil en agriculture biologique",
      "Technico-commercial",
      "Technico-commercial en agrofournitures",
      "Testeur",
      "Trader",
      "Traducteur",
      "Traffic manager",
      "Vendeur automobile",
      "Vendeur d’espaces publicitaires",
      "Vétérinaire",
      "Vétérinaire équin",
      "Visiteur médical",
      "Viticulteur",
      "Volcanologue",
      "Volcanologue",
      "Webmaster",
      "Zoologiste",
    ];
  }

  @override
  List<String> get politicalViews {
    return const [
      "Anarchiste",
      "Libéral",
      "Communiste",
      "Socialiste",
      "Conservateur",
      "Libertaire",
    ];
  }

  @override
  List<String> surnames(Gender gender) {
    return switch (gender) {
      _ => const [
          "Abadie",
          "Adam",
          "Adams",
          "Ahmed",
          "Alain",
          "Alarie",
          "Albert",
          "Alexandre",
          "Alix",
          "Allain",
          "Allaire",
          "Allard",
          "Allemand",
          "Allen",
          "Alonso",
          "Alvarez",
          "Alves",
          "Amyot",
          "Anctil",
          "Anderson",
          "Andre",
          "Andrieu",
          "Andrieux",
          "Angers",
          "Antoine",
          "April",
          "Arbour",
          "Arcand",
          "Archambault",
          "Armand",
          "Arnaud",
          "Arnould",
          "Arnoux",
          "Arsenault",
          "Asselin",
          "Astier",
          "Aubert",
          "Aubin",
          "Aubry",
          "Aubut",
          "Aubé",
          "Auclair",
          "Audet",
          "Audy",
          "Auffret",
          "Auge",
          "Auger",
          "Auvray",
          "Avril",
          "Awashish",
          "Ayotte",
          "Babin",
          "Bachand",
          "Bachelet",
          "Bacon",
          "Baillargeon",
          "Bailleul",
          "Bailly",
          "Banville",
          "Barbe",
          "Barbeau",
          "Barbier",
          "Barbot",
          "Bardet",
          "Bardin",
          "Baribeau",
          "Baril",
          "Baron",
          "Barraud",
          "Barre",
          "Barreau",
          "Barret",
          "Barrette",
          "Barriere",
          "Barthelemy",
          "Basset",
          "Bastien",
          "Bataille",
          "Baud",
          "Baudet",
          "Baudin",
          "Baudoin",
          "Baudouin",
          "Baudry",
          "Bauer",
          "Bayle",
          "Bazin",
          "Bazinet",
          "Beau",
          "Beaucage",
          "Beauchamp",
          "Beauchemin",
          "Beauchesne",
          "Beaudet",
          "Beaudin",
          "Beaudoin",
          "Beaudry",
          "Beaufils",
          "Beaulieu",
          "Beaumier",
          "Beaumont",
          "Beaupré",
          "Beauregard",
          "Beausoleil",
          "Beauséjour",
          "Beauvais",
          "Beck",
          "Becker",
          "Begue",
          "Beguin",
          "Belhumeur",
          "Belin",
          "Bellanger",
          "Bellavance",
          "Belleau",
          "Bellec",
          "Bellefleur",
          "Bellemare",
          "Bellerose",
          "Bellet",
          "Belley",
          "Belzile",
          "Ben",
          "Benard",
          "Benoist",
          "Benoit",
          "Benoît",
          "Berard",
          "Berger",
          "Bergeron",
          "Bergevin",
          "Bernard",
          "Bernatchez",
          "Bernier",
          "Berry",
          "Berthe",
          "Berthelot",
          "Berthet",
          "Berthiaume",
          "Berthier",
          "Bertin",
          "Bertrand",
          "Besnard",
          "Besse",
          "Bessette",
          "Besson",
          "Bianchi",
          "Bibeau",
          "Bidault",
          "Bigot",
          "Bigras",
          "Billard",
          "Billet",
          "Billon",
          "Billy",
          "Bilodeau",
          "Binet",
          "Binette",
          "Biron",
          "Bisaillon",
          "Bisson",
          "Bissonnette",
          "Blackburn",
          "Blain",
          "Blais",
          "Blaise",
          "Blanc",
          "Blanchard",
          "Blanchet",
          "Blanchette",
          "Blandin",
          "Bleau",
          "Blin",
          "Blondeau",
          "Blondel",
          "Blondin",
          "Blot",
          "Blouin",
          "Bo",
          "Bob",
          "Bocquet",
          "Bodin",
          "Boilard",
          "Boileau",
          "Boily",
          "Bois",
          "Boisclair",
          "Boisjoli",
          "Boisseau",
          "Boisson",
          "Boissonneault",
          "Boisvert",
          "Boivin",
          "Bolduc",
          "Bon",
          "Bond",
          "Bonhomme",
          "Bonin",
          "Bonnard",
          "Bonnaud",
          "Bonneau",
          "Bonnefoy",
          "Bonnet",
          "Bonnin",
          "Bontemps",
          "Bordeleau",
          "Bordes",
          "Bossard",
          "Bossé",
          "Bouchard",
          "Bouche",
          "Boucher",
          "Bouchet",
          "Bouchez",
          "Boudet",
          "Boudreau",
          "Bouffard",
          "Bougie",
          "Bouillon",
          "Boulanger",
          "Boulay",
          "Boulet",
          "Boulianne",
          "Bouquet",
          "Bour",
          "Bourassa",
          "Bourbeau",
          "Bourbon",
          "Bourbonnais",
          "Bourdages",
          "Bourdeau",
          "Bourdin",
          "Bourdon",
          "Bourgault",
          "Bourgeois",
          "Bourget",
          "Bourgoin",
          "Bourguignon",
          "Bourque",
          "Bousquet",
          "Boutet",
          "Bouthillier",
          "Boutin",
          "Bouton",
          "Bouvet",
          "Bouvier",
          "Boyer",
          "Brassard",
          "Brault",
          "Braun",
          "Brazeau",
          "Bresson",
          "Bret",
          "Breton",
          "Briand",
          "Brien",
          "Briere",
          "Brisebois",
          "Brisset",
          "Brissette",
          "Brisson",
          "Brière",
          "Brochard",
          "Brochu",
          "Brodeur",
          "Brossard",
          "Brosseau",
          "Brouillard",
          "Brouillette",
          "Brousseau",
          "Brown",
          "Bru",
          "Brun",
          "Bruneau",
          "Brunel",
          "Brunelle",
          "Brunet",
          "Bruno",
          "Bruyere",
          "Buisson",
          "Bujold",
          "Bureau",
          "Burel",
          "Busque",
          "Bussières",
          "Busson",
          "Béchard",
          "Bédard",
          "Bégin",
          "Bélair",
          "Béland",
          "Bélanger",
          "Bélisle",
          "Béliveau",
          "Bénard",
          "Bérard",
          "Bérubé",
          "Cabana",
          "Cadieux",
          "Cadiou",
          "Cadorette",
          "Caillaud",
          "Caillet",
          "Caissy",
          "Calvet",
          "Calvez",
          "Cameron",
          "Campbell",
          "Campeau",
          "Camus",
          "Cano",
          "Cantin",
          "Canuel",
          "Caouette",
          "Capelle",
          "Carbonneau",
          "Cardin",
          "Cardinal",
          "Cardon",
          "Carignan",
          "Cariou",
          "Carle",
          "Carlier",
          "Caron",
          "Carpentier",
          "Carre",
          "Carrere",
          "Carrier",
          "Carriere",
          "Carrière",
          "Cartier",
          "Carton",
          "Carvalho",
          "Casanova",
          "Castel",
          "Castonguay",
          "Cauchon",
          "Cayer",
          "Chabert",
          "Chabot",
          "Chagnon",
          "Chalifoux",
          "Chamberland",
          "Chambon",
          "Champagne",
          "Champion",
          "Champoux",
          "Chan",
          "Chapdelaine",
          "Chapelle",
          "Chapuis",
          "Chaput",
          "Charbonneau",
          "Charbonnier",
          "Chardon",
          "Charest",
          "Charette",
          "Charland",
          "Charlebois",
          "Charles",
          "Charlet",
          "Charlier",
          "Charlot",
          "Charpentier",
          "Charrier",
          "Charron",
          "Chartier",
          "Charton",
          "Chartrand",
          "Chassé",
          "Chateau",
          "Chatelain",
          "Chauveau",
          "Chauvet",
          "Chauvin",
          "Chayer",
          "Chemin",
          "Chen",
          "Chene",
          "Cheron",
          "Chevalier",
          "Chevallier",
          "Chevrier",
          "Chiasson",
          "Chicoine",
          "Chiron",
          "Choinière",
          "Chollet",
          "Chopin",
          "Choquette",
          "Chouinard",
          "Chretien",
          "Christophe",
          "Chrétien",
          "Chénard",
          "Chénier",
          "Clarke",
          "Claude",
          "Claudel",
          "Claveau",
          "Clement",
          "Clerc",
          "Clermont",
          "Cliche",
          "Cloarec",
          "Cloutier",
          "Clément",
          "Cochet",
          "Coderre",
          "Cohen",
          "Colas",
          "Colin",
          "Collard",
          "Collet",
          "Collignon",
          "Collin",
          "Collins",
          "Combe",
          "Combes",
          "Comeau",
          "Comte",
          "Comtois",
          "Constant",
          "Constantineau",
          "Conte",
          "Corbeil",
          "Corbin",
          "Cordier",
          "Cormier",
          "Cornet",
          "Cornu",
          "Corre",
          "Corriveau",
          "Cossette",
          "Costa",
          "Coste",
          "Cottin",
          "Couderc",
          "Coudert",
          "Couillard",
          "Coulombe",
          "Coulon",
          "Courchesne",
          "Cournoyer",
          "Courtemanche",
          "Courtin",
          "Courtois",
          "Cousin",
          "Cousineau",
          "Coutant",
          "Coutu",
          "Couture",
          "Couturier",
          "Crepin",
          "Crevier",
          "Cros",
          "Croteau",
          "Crouzet",
          "Crépeau",
          "Crête",
          "Cusson",
          "Cuvelier",
          "Cyr",
          "Côté",
          "Da Costa",
          "Da Silva",
          "Dagenais",
          "Daigle",
          "Daigneault",
          "Dallaire",
          "Dalpé",
          "Damours",
          "Daneau",
          "Daniel",
          "Danis",
          "Dansereau",
          "Daoust",
          "Daraiche",
          "Darras",
          "Dasilva",
          "Dastous",
          "Dauphin",
          "Daviault",
          "David",
          "Davis",
          "De",
          "De Oliveira",
          "De Sousa",
          "Deblois",
          "Debray",
          "Delage",
          "Delahaye",
          "Delamare",
          "Delannoy",
          "Delaporte",
          "Delarue",
          "Delattre",
          "Delaunay",
          "Delcroix",
          "Delhaye",
          "Delisle",
          "Delmas",
          "Delorme",
          "Delâge",
          "Demange",
          "Demers",
          "Deneault",
          "Denis",
          "Deraspe",
          "Derrien",
          "Desaulniers",
          "Desautels",
          "Desbiens",
          "Desbois",
          "Descamps",
          "Deschamps",
          "Deschênes",
          "Descôteaux",
          "Desfossés",
          "Desgagné",
          "Deshaies",
          "Deshayes",
          "Desjardins",
          "Deslauriers",
          "Desmarais",
          "Desmeules",
          "Desnoyers",
          "Desormeaux",
          "Despres",
          "Després",
          "Desrochers",
          "Desroches",
          "Desrosiers",
          "Dessureault",
          "Devaux",
          "Deveault",
          "Deville",
          "Devos",
          "Diamond",
          "Dias",
          "Diaz",
          "Didier",
          "Dion",
          "Dionne",
          "Diot",
          "Diotte",
          "Doiron",
          "Dore",
          "Dorion",
          "Dorval",
          "Doré",
          "Dos Santos",
          "Dostie",
          "Doucet",
          "Doyle",
          "Doyon",
          "Drapeau",
          "Drolet",
          "Drouet",
          "Drouin",
          "Dubeau",
          "Dubois",
          "Dubos",
          "Dubost",
          "Dubourg",
          "Dubreuil",
          "Dubuc",
          "Dubus",
          "Dubé",
          "Ducharme",
          "Duchemin",
          "Duchene",
          "Duchesne",
          "Duchesneau",
          "Duclos",
          "Ducrocq",
          "Dufort",
          "Dufour",
          "Dufresne",
          "Dugas",
          "Duguay",
          "Duhaime",
          "Duhamel",
          "Dujardin",
          "Dulude",
          "Dumais",
          "Dumas",
          "Dumont",
          "Dumortier",
          "Dumouchel",
          "Dumoulin",
          "Dunn",
          "Dupin",
          "Dupond",
          "Dupont",
          "Dupras",
          "Duprat",
          "Dupre",
          "Dupré",
          "Dupuis",
          "Dupuy",
          "Dupéré",
          "Duquesne",
          "Duquette",
          "Durand",
          "Durant",
          "Duret",
          "Durieux",
          "Duriez",
          "Durocher",
          "Dussault",
          "Dutertre",
          "Duval",
          "Décarie",
          "Déry",
          "Désilets",
          "Déziel",
          "Esnault",
          "Esteve",
          "Etienne",
          "Evrard",
          "Fabre",
          "Fafard",
          "Faivre",
          "Falardeau",
          "Faubert",
          "Faucher",
          "Faure",
          "Fauteux",
          "Favier",
          "Favre",
          "Favreau",
          "Fayolle",
          "Fecteau",
          "Felix",
          "Ferland",
          "Fernandes",
          "Fernandez",
          "Ferrand",
          "Ferrari",
          "Ferre",
          "Ferreira",
          "Ferrer",
          "Ferrier",
          "Ferron",
          "Ferry",
          "Fevre",
          "Fevrier",
          "Fievet",
          "Filiatrault",
          "Fillion",
          "Fischer",
          "Fiset",
          "Flamand",
          "Flament",
          "Fleurant",
          "Fleury",
          "Floch",
          "Foisy",
          "Fontaine",
          "Forcier",
          "Forest",
          "Forestier",
          "Forget",
          "Forgues",
          "Fort",
          "Fortier",
          "Fortin",
          "Foucault",
          "Foucher",
          "Foulon",
          "Fouquet",
          "Fourcade",
          "Fournier",
          "Fradette",
          "Fraisse",
          "Franco",
          "Francoeur",
          "Francois",
          "François",
          "Frappier",
          "Fraser",
          "Fremont",
          "Frenette",
          "Frey",
          "Frigon",
          "Froment",
          "Fréchette",
          "Gabriel",
          "Gagnon",
          "Gagné",
          "Gaillard",
          "Galarneau",
          "Galipeau",
          "Gallais",
          "Galland",
          "Gallant",
          "Gallet",
          "Gallois",
          "Gamache",
          "Garand",
          "Garceau",
          "Garcia",
          "Gareau",
          "Garin",
          "Gariépy",
          "Garneau",
          "Garnier",
          "Garon",
          "Garreau",
          "Gascon",
          "Gasnier",
          "Gaudet",
          "Gaudin",
          "Gaudreault",
          "Gaulin",
          "Gaumond",
          "Gauthier",
          "Gautier",
          "Gauvin",
          "Gauvreau",
          "Gay",
          "Geffroy",
          "Gemme",
          "Gendreau",
          "Gendron",
          "Genest",
          "Genet",
          "Genin",
          "Gentil",
          "Geoffroy",
          "George",
          "Georges",
          "Gerard",
          "Germain",
          "Gervais",
          "Giasson",
          "Gibert",
          "Gicquel",
          "Gignac",
          "Giguère",
          "Gil",
          "Gilbert",
          "Gill",
          "Gilles",
          "Gillet",
          "Gillot",
          "Gimenez",
          "Gingras",
          "Girard",
          "Giraud",
          "Girault",
          "Girouard",
          "Giroux",
          "Gobeil",
          "Gobert",
          "Godard",
          "Godart",
          "Godbout",
          "Godefroy",
          "Godet",
          "Godin",
          "Gomes",
          "Gomez",
          "Gonthier",
          "Gonzales",
          "Gonzalez",
          "Gonçalves",
          "Gordon",
          "Gosselin",
          "Gosset",
          "Goudreau",
          "Gougeon",
          "Gouin",
          "Goujon",
          "Goulet",
          "Goupil",
          "Goyer",
          "Goyette",
          "Grand",
          "Grandjean",
          "Grange",
          "Granger",
          "Granier",
          "Grant",
          "Gras",
          "Gratton",
          "Gravel",
          "Gregoire",
          "Grenier",
          "Grenon",
          "Grimard",
          "Grimaud",
          "Groleau",
          "Grondin",
          "Gros",
          "Grosjean",
          "Gross",
          "Groulx",
          "Grégoire",
          "Guay",
          "Guegan",
          "Gueguen",
          "Guerin",
          "Guertin",
          "Guery",
          "Guibert",
          "Guichard",
          "Guignard",
          "Guilbaud",
          "Guilbault",
          "Guilbert",
          "Guillard",
          "Guillaume",
          "Guillemette",
          "Guillemin",
          "Guillemot",
          "Guillet",
          "Guillon",
          "Guillot",
          "Guillou",
          "Guimond",
          "Guindon",
          "Guiraud",
          "Guitton",
          "Guy",
          "Guyard",
          "Guyon",
          "Guyot",
          "Guénette",
          "Guérard",
          "Guérette",
          "Guérin",
          "Gélinas",
          "Généreux",
          "Haché",
          "Hains",
          "Hallé",
          "Hamel",
          "Hamelin",
          "Hamilton",
          "Hamon",
          "Hardouin",
          "Hardy",
          "Harnois",
          "Harrisson",
          "Harvey",
          "Hebert",
          "Hennequin",
          "Henri",
          "Henry",
          "Herault",
          "Hernandez",
          "Herve",
          "Hervieux",
          "Hoarau",
          "Hoffmann",
          "Hogue",
          "Honore",
          "Houde",
          "Houle",
          "Huard",
          "Hubert",
          "Hudon",
          "Hue",
          "Huet",
          "Hugues",
          "Huguet",
          "Humbert",
          "Huot",
          "Hurtubise",
          "Husson",
          "Huynh",
          "Hébert",
          "Hélie",
          "Hénault",
          "Héroux",
          "Hétu",
          "Imbeault",
          "Imbert",
          "Isabelle",
          "Jacob",
          "Jacquemin",
          "Jacques",
          "Jacquet",
          "Jacquin",
          "Jacquot",
          "Jalbert",
          "James",
          "Jamet",
          "Jan",
          "Janin",
          "Janvier",
          "Jaouen",
          "Jardin",
          "Jarry",
          "Jean",
          "Jeanne",
          "Jegou",
          "Jetté",
          "Jobin",
          "Jodoin",
          "Johnson",
          "Jolicoeur",
          "Jolin",
          "Jolivet",
          "Jolly",
          "Joly",
          "Jomphe",
          "Joncas",
          "Jones",
          "Joseph",
          "Josse",
          "Jouan",
          "Joubert",
          "Jourdain",
          "Jourdan",
          "Jouve",
          "Joyal",
          "Julien",
          "Jullien",
          "Juneau",
          "Jung",
          "Juteau",
          "Jutras",
          "Kaur",
          "Keller",
          "Kelly",
          "Khan",
          "Kieffer",
          "King",
          "Kirouac",
          "Klein",
          "Labat",
          "Labbe",
          "Labbé",
          "Labelle",
          "Laberge",
          "Labonté",
          "Laborde",
          "Labranche",
          "Labrecque",
          "Labrie",
          "Lacasse",
          "Lacaze",
          "Lacelle",
          "Lacerte",
          "Lachance",
          "Lachapelle",
          "Lacharité",
          "Lachaîne",
          "Lacombe",
          "Lacoste",
          "Lacour",
          "Lacoursière",
          "Lacroix",
          "Ladouceur",
          "Laferrière",
          "Laflamme",
          "Lafleur",
          "Lafon",
          "Lafond",
          "Lafont",
          "Lafontaine",
          "Laforest",
          "Lafortune",
          "Laframboise",
          "Lafrance",
          "Lafrenière",
          "Lagacé",
          "Lagarde",
          "Lagrange",
          "Lahaie",
          "Laine",
          "Lajeunesse",
          "Lajoie",
          "Lalancette",
          "Lalande",
          "Lalanne",
          "Laliberté",
          "Lallemand",
          "Lallement",
          "Lalonde",
          "Lam",
          "Lamarche",
          "Lamarre",
          "Lambert",
          "Lamontagne",
          "Lamothe",
          "Lamotte",
          "Lamour",
          "Lamoureux",
          "Lampron",
          "Lamy",
          "Lanctôt",
          "Landais",
          "Landry",
          "Lang",
          "Langelier",
          "Langevin",
          "Langlais",
          "Langlois",
          "Lanthier",
          "Lapalme",
          "Laperrière",
          "Lapeyre",
          "Lapierre",
          "Laplante",
          "Lapointe",
          "Laporte",
          "Laprise",
          "Laramée",
          "Larcher",
          "Larivière",
          "Laroche",
          "Larochelle",
          "Larocque",
          "Larose",
          "Larouche",
          "Larrivée",
          "Larue",
          "Lassalle",
          "Lasserre",
          "Latour",
          "Latreille",
          "Latulippe",
          "Launay",
          "Laurent",
          "Laurin",
          "Lauzier",
          "Lauzon",
          "Laval",
          "Lavallée",
          "Lavaud",
          "Laverdière",
          "Lavergne",
          "Lavigne",
          "Laviolette",
          "Lavoie",
          "Le",
          "Le Bars",
          "Le Berre",
          "Le Bihan",
          "Le Borgne",
          "Le Bras",
          "Le Breton",
          "Le Bris",
          "Le Corre",
          "Le Floch",
          "Le Gal",
          "Le Gall",
          "Le Goff",
          "Le Guen",
          "Le Meur",
          "Le Roux",
          "Le Roy",
          "Lebas",
          "Lebeau",
          "Lebel",
          "Leblanc",
          "Leblond",
          "Leboeuf",
          "Lebon",
          "Lebreton",
          "Lebrun",
          "Leclerc",
          "Leclercq",
          "Leclere",
          "Lecocq",
          "Lecoeur",
          "Lecompte",
          "Lecomte",
          "Leconte",
          "Lecoq",
          "Lecours",
          "Lecuyer",
          "Ledoux",
          "Leduc",
          "Lee",
          "Lefebvre",
          "Lefeuvre",
          "Lefevre",
          "Lefort",
          "Lefranc",
          "Lefrancois",
          "Lefrançois",
          "Legaré",
          "Legault",
          "Legay",
          "Legendre",
          "Leger",
          "Legrand",
          "Legras",
          "Legros",
          "Lehoux",
          "Lejeune",
          "Leleu",
          "Lelievre",
          "Lelièvre",
          "Lelong",
          "Leloup",
          "Lemaire",
          "Lemaitre",
          "Lemarchand",
          "Lemay",
          "Lemelin",
          "Lemercier",
          "Lemieux",
          "Lemire",
          "Lemoine",
          "Lemonnier",
          "Lenoir",
          "Leon",
          "Leonard",
          "Lepage",
          "Lepine",
          "Lepretre",
          "Leray",
          "Leriche",
          "Leroux",
          "Leroy",
          "Lesage",
          "Lespérance",
          "Lessard",
          "Lesueur",
          "Letellier",
          "Letendre",
          "Levac",
          "Levasseur",
          "Leveque",
          "Levy",
          "Lewis",
          "Lheureux",
          "Lhomme",
          "Li",
          "Lienard",
          "Limoges",
          "Lirette",
          "Lizotte",
          "Loiseau",
          "Loisel",
          "Loiselle",
          "Loison",
          "Lombard",
          "Longpré",
          "Lopes",
          "Lopez",
          "Loranger",
          "Lord",
          "Lortie",
          "Louis",
          "Louvet",
          "Loyer",
          "Lucas",
          "Lupien",
          "Lussier",
          "Ly",
          "Lécuyer",
          "Léger",
          "Léonard",
          "Lépine",
          "Létourneau",
          "Léveillé",
          "Lévesque",
          "Lévy",
          "Mace",
          "Madec",
          "Madore",
          "Magnan",
          "Magne",
          "Magnier",
          "Magnin",
          "Mahe",
          "Maheux",
          "Mahieu",
          "Mailhot",
          "Maillard",
          "Maillet",
          "Maillot",
          "Mailloux",
          "Maillé",
          "Mainville",
          "Maire",
          "Maisonneuve",
          "Maitre",
          "Major",
          "Malenfant",
          "Mallet",
          "Mallette",
          "Malo",
          "Maltais",
          "Mangin",
          "Manseau",
          "Marais",
          "Marc",
          "Marceau",
          "Marcel",
          "Marchal",
          "Marchand",
          "Marcil",
          "Marcotte",
          "Marcoux",
          "Marechal",
          "Marie",
          "Marier",
          "Marin",
          "Marion",
          "Mark",
          "Marleau",
          "Marois",
          "Marques",
          "Marquet",
          "Marquis",
          "Marteau",
          "Martel",
          "Martin",
          "Martineau",
          "Martinet",
          "Martinez",
          "Martins",
          "Marty",
          "Mary",
          "Mas",
          "Masse",
          "Massicotte",
          "Masson",
          "Massé",
          "Mathieu",
          "Mathis",
          "Matte",
          "Mauger",
          "Maurel",
          "Maurice",
          "Maurin",
          "Maury",
          "Mayer",
          "McDonald",
          "McDuff",
          "McKenzie",
          "McLean",
          "Meilleur",
          "Meloche",
          "Menard",
          "Mendes",
          "Mercier",
          "Merle",
          "Merlin",
          "Messier",
          "Metais",
          "Metayer",
          "Meunier",
          "Meyer",
          "Michaud",
          "Michaux",
          "Michel",
          "Michon",
          "Migneault",
          "Mignot",
          "Mille",
          "Miller",
          "Millet",
          "Millette",
          "Millot",
          "Milot",
          "Miquel",
          "Mireault",
          "Miron",
          "Mitchell",
          "Moi",
          "Moine",
          "Moisan",
          "Molina",
          "Monette",
          "Mongeau",
          "Mongrain",
          "Monier",
          "Monin",
          "Monnet",
          "Monnier",
          "Montagne",
          "Monteil",
          "Montminy",
          "Montpetit",
          "Montreuil",
          "Moore",
          "Mora",
          "Morais",
          "Morand",
          "Moreau",
          "Morel",
          "Morency",
          "Moreno",
          "Moret",
          "Morice",
          "Morin",
          "Morissette",
          "Morneau",
          "Morvan",
          "Motte",
          "Moulin",
          "Mounier",
          "Mouton",
          "Muller",
          "Munger",
          "Munier",
          "Munoz",
          "Murphy",
          "Murray",
          "Mélançon",
          "Ménard",
          "Méthot",
          "Métivier",
          "Nadeau",
          "Nadon",
          "Nantel",
          "Nault",
          "Navarro",
          "Nedelec",
          "Neveu",
          "Nguyen",
          "Nicol",
          "Nicolas",
          "Nicolle",
          "Noel",
          "Nogues",
          "Nolet",
          "Nolin",
          "Normand",
          "Normandeau",
          "Normandin",
          "Noël",
          "Néron",
          "Oger",
          "Olive",
          "Olivier",
          "Ollivier",
          "Otis",
          "Ouellet",
          "Ouimet",
          "Page",
          "Pageau",
          "Pages",
          "Pagé",
          "Paiement",
          "Paillard",
          "Pain",
          "Painchaud",
          "Papin",
          "Papineau",
          "Paquet",
          "Paquette",
          "Paquin",
          "Paradis",
          "Parent",
          "Parenteau",
          "Paris",
          "Parisot",
          "Parmentier",
          "Paré",
          "Pascal",
          "Pasquet",
          "Pasquier",
          "Pastor",
          "Patel",
          "Patenaude",
          "Patry",
          "Paul",
          "Payen",
          "Payet",
          "Payette",
          "Pearson",
          "Pedneault",
          "Pelchat",
          "Pelissier",
          "Pellerin",
          "Pelletier",
          "Peltier",
          "Pepin",
          "Pereira",
          "Peres",
          "Perez",
          "Perin",
          "Pernot",
          "Peron",
          "Perras",
          "Perreault",
          "Perret",
          "Perrier",
          "Perrin",
          "Perron",
          "Perrot",
          "Peter",
          "Petit",
          "Petitjean",
          "Peyre",
          "Pham",
          "Phaneuf",
          "Pharand",
          "Philippe",
          "Philippon",
          "Philippot",
          "Picard",
          "Pichard",
          "Pichette",
          "Pichon",
          "Piché",
          "Picot",
          "Pierre",
          "Pierron",
          "Pigeon",
          "Pillet",
          "Pilon",
          "Pilote",
          "Pinard",
          "Pineau",
          "Pineault",
          "Pinel",
          "Pinette",
          "Pinson",
          "Pinsonneault",
          "Pinto",
          "Piquet",
          "Pires",
          "Pitre",
          "Plamondon",
          "Plante",
          "Plouffe",
          "Plourde",
          "Poirier",
          "Poirot",
          "Poissant",
          "Poisson",
          "Poitras",
          "Poliquin",
          "Pollet",
          "Pomerleau",
          "Pommier",
          "Poncet",
          "Pons",
          "Portal",
          "Porte",
          "Portier",
          "Pothier",
          "Potier",
          "Pottier",
          "Potvin",
          "Poudrier",
          "Pouget",
          "Poulain",
          "Poulet",
          "Poulin",
          "Pouliot",
          "Pouliquen",
          "Prat",
          "Pratte",
          "Prevost",
          "Prevot",
          "Prieur",
          "Prigent",
          "Primeau",
          "Prince",
          "Pronovost",
          "Prost",
          "Proulx",
          "Proust",
          "Provencher",
          "Provost",
          "Prudhomme",
          "Pruneau",
          "Pruvost",
          "Prévost",
          "Puech",
          "Pujol",
          "Péloquin",
          "Pépin",
          "Quenneville",
          "Quentin",
          "Quere",
          "Quesnel",
          "Quintal",
          "Quirion",
          "Quévillon",
          "Racette",
          "Racicot",
          "Racine",
          "Raffin",
          "Ragot",
          "Rainville",
          "Ramos",
          "Rancourt",
          "Ranger",
          "Raoul",
          "Ratté",
          "Rault",
          "Raymond",
          "Raynal",
          "Raynaud",
          "Reboul",
          "Regnier",
          "Reid",
          "Remond",
          "Remy",
          "Renard",
          "Renaud",
          "Renaudin",
          "Renault",
          "Renou",
          "René",
          "Revel",
          "Rey",
          "Reynaud",
          "Rhéault",
          "Rhéaume",
          "Ribeiro",
          "Ricard",
          "Richard",
          "Richer",
          "Riendeau",
          "Rigal",
          "Rigaud",
          "Ringuette",
          "Rio",
          "Riopel",
          "Riou",
          "Rioux",
          "Rivard",
          "Riverin",
          "Rivest",
          "Rivet",
          "Riviere",
          "Roberge",
          "Robert",
          "Roberts",
          "Robertson",
          "Robichaud",
          "Robidoux",
          "Robillard",
          "Robin",
          "Robinet",
          "Robinson",
          "Robitaille",
          "Roch",
          "Roche",
          "Rochefort",
          "Rocheleau",
          "Rocher",
          "Rochette",
          "Rochon",
          "Rodier",
          "Rodrigue",
          "Rodrigues",
          "Rodriguez",
          "Roger",
          "Roland",
          "Rolland",
          "Rollet",
          "Rollin",
          "Romain",
          "Romero",
          "Rondeau",
          "Roques",
          "Rose",
          "Ross",
          "Rossi",
          "Rossignol",
          "Roth",
          "Rougier",
          "Rouillard",
          "Rouleau",
          "Rousseau",
          "Roussel",
          "Rousselle",
          "Rousset",
          "Roussy",
          "Routhier",
          "Roux",
          "Rouxel",
          "Rouyer",
          "Roy",
          "Royer",
          "Ruel",
          "Ruest",
          "Ruiz",
          "Rémillard",
          "Sabatier",
          "Sabourin",
          "Salaun",
          "Salle",
          "Salles",
          "Salmon",
          "Salomon",
          "Samson",
          "Sanchez",
          "Santerre",
          "Santos",
          "Sarrazin",
          "Saucier",
          "Saulnier",
          "Saunier",
          "Sauriol",
          "Sauvage",
          "Sauvageau",
          "Sauvé",
          "Savage",
          "Savard",
          "Savary",
          "Savoie",
          "Schaeffer",
          "Schmidt",
          "Schmitt",
          "Schneider",
          "Schwartz",
          "Scott",
          "Seguin",
          "Sellier",
          "Senechal",
          "Sergent",
          "Sergerie",
          "Serra",
          "Serrano",
          "Serre",
          "Serres",
          "Sicard",
          "Sigouin",
          "Simard",
          "Simon",
          "Simoneau",
          "Simonin",
          "Simonnet",
          "Singh",
          "Sirois",
          "Smith",
          "Soler",
          "Soucy",
          "Soulie",
          "Soulier",
          "Soulières",
          "St-Amand",
          "St-Amour",
          "St-Arnaud",
          "St-Cyr",
          "St-Denis",
          "St-Gelais",
          "St-Georges",
          "St-Germain",
          "St-Hilaire",
          "St-Jacques",
          "St-Jean",
          "St-Laurent",
          "St-Louis",
          "St-Martin",
          "St-Onge",
          "St-Pierre",
          "Ste-Marie",
          "Stephan",
          "Stewart",
          "Surprenant",
          "Sylvain",
          "Sylvestre",
          "Séguin",
          "Sénécal",
          "Sénéchal",
          "Sévigny",
          "Taillefer",
          "Taillon",
          "Talbot",
          "Tanguay",
          "Tanguy",
          "Tardif",
          "Tardy",
          "Tassé",
          "Tavernier",
          "Taylor",
          "Teixeira",
          "Tellier",
          "Terrier",
          "Tessier",
          "Texier",
          "Teyssier",
          "Thebault",
          "Therrien",
          "Thery",
          "Thevenet",
          "Thevenin",
          "Thibault",
          "Thibodeau",
          "Thiebaut",
          "Thierry",
          "Thiery",
          "Thiffault",
          "Thivierge",
          "Thomas",
          "Thompson",
          "Thuillier",
          "Théberge",
          "Théorêt",
          "Thériault",
          "Théroux",
          "Tison",
          "Tisserand",
          "Tissier",
          "Tissot",
          "Tixier",
          "Torres",
          "Touchette",
          "Toupin",
          "Tourigny",
          "Tournier",
          "Tousignant",
          "Toussaint",
          "Trahan",
          "Tran",
          "Tremblay",
          "Trottier",
          "Truchon",
          "Trudeau",
          "Trudel",
          "Trépanier",
          "Turbide",
          "Turcotte",
          "Turgeon",
          "Turmel",
          "Turpin",
          "Tétreault",
          "Vacher",
          "Vachon",
          "Vaillancourt",
          "Vaillant",
          "Valcourt",
          "Valentin",
          "Valette",
          "Valiquette",
          "Vallee",
          "Vallet",
          "Vallières",
          "Vallée",
          "Valois",
          "Vandal",
          "Vanier",
          "Vannier",
          "Varin",
          "Vasseur",
          "Veillette",
          "Veilleux",
          "Venne",
          "Verdier",
          "Verger",
          "Vermette",
          "Vernet",
          "Vernier",
          "Veron",
          "Verreault",
          "Verret",
          "Verrier",
          "Verville",
          "Vial",
          "Viard",
          "Viau",
          "Viaud",
          "Vidal",
          "Viens",
          "Vigier",
          "Vignal",
          "Vigneault",
          "Vigneron",
          "Vignon",
          "Vigouroux",
          "Viguier",
          "Vilain",
          "Villain",
          "Villard",
          "Villeneuve",
          "Villette",
          "Vincent",
          "Vinet",
          "Vivier",
          "Voisin",
          "Voyer",
          "Vézina",
          "Wagner",
          "Walker",
          "Walter",
          "Weber",
          "Weiss",
          "White",
          "Williams",
          "Wilson",
          "Wolff",
          "Wong",
          "Xx",
          "Young",
          "Zimmermann",
          "Émond",
          "Éthier",
        ],
    };
  }

  @override
  List<String> get telephoneFormats {
    return const [
      "01########",
      "02########",
      "03########",
      "04########",
      "05########",
      "06########",
      "07########",
      "+33 1########",
      "+33 2########",
      "+33 3########",
      "+33 4########",
      "+33 5########",
      "+33 6########",
      "+33 7########",
    ];
  }

  @override
  List<String> titles({required Gender gender, required TitleType titleType}) {
    return switch (gender) {
      Gender.female => titleType == TitleType.academic
          ? const ["PhD", "Dr.", "Prof."]
          : const ["Mme.", "Madame"],
      Gender.male => titleType == TitleType.academic
          ? const ["PhD", "Dr.", "Prof."]
          : const ["M.", "Mr."],
    };
  }

  @override
  List<String> get universities {
    return const [
      "École Normale Supérieure",
      "École Polytechnique",
      "Pierre and Marie Curie University",
      "Paris-Sud University",
      "CentraleSupélec",
      "École Normale Supérieure de Lyon",
      "Paris Descartes University",
      "Paris Diderot University – Paris 7",
      "Mines ParisTech",
      "Aix-Marseille University",
      "University of Bordeaux",
      "Federal University of Toulouse Midi-Pyrénées",
      "University of Strasbourg",
      "Claude Bernard University Lyon 1",
      "École des Ponts ParisTech",
      "Montpellier University",
      "Paris-Sorbonne University – Paris 4",
      "École Normale Supérieure de Cachan",
      "University of Nantes",
      "University of Nice Sophia Antipolis",
      "Panthéon-Sorbonne University – Paris 1",
      "University of Lille",
      "National Institute of Applied Sciences of Lyon (INSA Lyon)",
      "University of Rennes 1",
      "Université Bourgogne Franche-Comté (UBFC)",
      "École Centrale de Lyon",
      "Université d’Aix-Marseille",
      "Aix-Marseille Université",
      "Université d’Amiens",
      "Université de Picardie Jules-Verne",
      "Université d’Angers",
      "Université des Antilles",
      "Université d'Artois",
      "Université d'Avignon",
      "Université d’Avignon et des Pays de Vaucluse",
      "Université de Besançon",
      "Université de Franche-Comté",
      "Université de Bordeaux",
      "Université de Bordeaux-III",
      "Université Bordeaux-Montaigne",
      "Université de Brest",
      "Université de Bretagne-Occidentale",
      "Université de Bretagne-Sud",
      "Université de Caen",
      "Université Caen-Normandie",
      "Université de Chambéry",
      "Université de Savoie",
      "Université de Clermont-Ferrand-I",
      "Université d'Auvergne",
      "Université de Clermont-Ferrand-II",
      "Université Blaise-Pascal",
      "Université de Corse",
      "Université de Corse-Pascal-Paoli",
      "Université de Dijon",
      "Université de Bourgogne",
      "Université Grenoble-Alpes",
      "Université de Guyane",
      "Université de La Réunion",
      "Université de La Rochelle",
      "Université du Havre",
      "Université du Mans",
      "Université du Maine",
      "Université Lille-I",
      "Université Lille-II",
      "Université Lille-III",
      "Université Charles-de-Gaulle",
      "Université de Limoges",
      "Université du Littoral",
      "Université du Littoral-Côte-d’Opale",
      "Université de Lyon-I",
      "Université Claude-Bernard",
      "Université de Lyon-II",
      "Université Lumière-Lyon-II",
      "Université de Lyon-III",
      "Université Jean-Moulin-Lyon-III",
      "Université de Montpellier",
      "Université Paul-Valéry-Montpellier",
      "Université Paul-Valéry",
      "Université de Mulhouse",
      "Université de Haute-Alsace",
      "Université de Nantes",
      "Université de Nice",
      "Université Nice Sophia Antipolis",
      "Université de Nîmes",
      "Université de la Nouvelle-Calédonie",
      "Université d’Orléans",
      "Université de Paris-I",
      "Université Panthéon-Sorbonne",
      "Université de Paris-II",
      "Université Panthéon-Assas",
      "Université de Paris-III",
      "Université Sorbonne-Nouvelle",
      "Université de Paris-IV",
      "Université Paris-Sorbonne",
      "Université de Paris-V",
      "Université Paris-Descartes",
      "Université de Paris-VI",
      "Université Pierre-et-Marie-Curie",
      "Université de Paris-VII",
      "Université Paris-Diderot",
      "Université de Paris-VIII",
      "Université Vincennes-Saint-Denis",
      "Université de Paris-X",
      "Université Paris-Ouest-Nanterre-La Défense",
      "Université de Paris-XI",
      "Université Paris-Sud",
      "Université de Paris-XII",
      "Université Paris-Est-Créteil-Val-de-Marne",
      "Université de Paris-XIII",
      "Université Paris-XIII-Nord",
      "Université de Marne-la-Vallée",
      "Université Paris-Est Marne-la-Vallée",
      "Université de Cergy-Pontoise",
      "Université d'Évry-Val-d'Essonne",
      "Université de Versailles",
      "Université de Versailles-Saint-Quentin-en-Yvelines",
      "Université de Pau",
      "Université de Pau et des Pays de l’Adour",
      "Université de Perpignan",
      "Université de Poitiers",
      "Université de la Polynésie française",
      "Université de Reims",
      "Université de Reims-Champagne-Ardenne",
      "Université de Rennes-I",
      "Université de Rennes-II",
      "Université Rennes-II-Haute-Bretagne",
      "Université de Rouen",
      "Université de Saint-Étienne",
      "Université Jean-Monnet-Saint-Étienne",
      "Université de Strasbourg",
      "Université de Toulon",
      "Université de Toulouse-I",
      "Université Toulouse-I-Capitole",
      "Université de Toulouse-II",
      "Université Toulouse-Jean-Jaurès",
      "Université de Toulouse-III",
      "Université Toulouse-III-Paul-Sabatier",
      "Institut national polytechnique de Toulouse",
      "Université de Tours",
      "Université François-Rabelais",
      "Université de Valenciennes",
      "Université de Valenciennes et du Hainaut-Cambrésis",
    ];
  }

  @override
  List<String> get viewsOn {
    return const [
      "Négatif",
      "Positif",
      "Neutre",
      "Indécis",
    ];
  }

  @override
  List<String> get worldviews {
    return const [
      "Athéisme",
      "Agnosticisme",
      "Catholicisme",
      "Déisme",
      "Panthéisme",
      "Confucianisme",
      "Humanisme séculier",
    ];
  }
}
