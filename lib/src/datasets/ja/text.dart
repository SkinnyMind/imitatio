// It's ok to have long strings in that file
// ignore_for_file: lines_longer_than_80_chars

import 'package:imitatio/src/datasets/models/text.dart';

class JaTextData extends TextDataset {
  @override
  List<String> alphabet({required bool isLowerCase}) {
    return switch (isLowerCase) {
      true => const [
        "ア",
        "イ",
        "ウ",
        "エ",
        "オ",
        "カ",
        "キ",
        "ク",
        "ケ",
        "コ",
        "サ",
        "シ",
        "ス",
        "セ",
        "ソ",
        "タ",
        "チ",
        "ツ",
        "テ",
        "ト",
        "ナ",
        "ニ",
        "ヌ",
        "ネ",
        "ノ",
        "ハ",
        "ヒ",
        "フ",
        "ヘ",
        "ホ",
        "マ",
        "ミ",
        "ム",
        "メ",
        "モ",
        "ヤ",
        "ユ",
        "ヨ",
        "ラ",
        "リ",
        "ル",
        "レ",
        "ロ",
        "ワ",
        "ヲ",
        "ン",
      ],
      false => const [
        "あ",
        "い",
        "う",
        "え",
        "お",
        "か",
        "き",
        "く",
        "け",
        "こ",
        "さ",
        "し",
        "す",
        "せ",
        "そ",
        "た",
        "ち",
        "つ",
        "て",
        "と",
        "な",
        "に",
        "ぬ",
        "ね",
        "の",
        "は",
        "ひ",
        "ふ",
        "へ",
        "ほ",
        "ま",
        "み",
        "む",
        "め",
        "も",
        "や",
        "ゆ",
        "よ",
        "ら",
        "り",
        "る",
        "れ",
        "ろ",
        "わ",
        "を",
        "ん",
      ],
    };
  }

  @override
  List<String> get answers {
    return const ["はい", "いいえ", "多分"];
  }

  @override
  List<String> get colors {
    return const [
      "赤",
      "赤錆色",
      "赤丹",
      "茜色",
      "赤紫",
      "曙色",
      "浅蘇芳",
      "小豆色",
      "洗朱",
      "暗紅色",
      "今様",
      "薄紅梅",
      "海老色・葡萄色",
      "臙脂・燕脂",
      "鉛丹",
      "黄丹",
      "オールドローズ",
      "カーマイン",
      "韓紅・唐紅",
      "土器色",
      "金赤",
      "銀朱",
      "紅梅",
      "サーモンピンク",
      "桜色",
      "錆色",
      "珊瑚色",
      "シグナルレッド",
      "赤銅色",
      "朱色",
      "猩々緋",
      "深紅・真紅",
      "蘇芳",
      "スカーレット",
      "ストロベリー",
      "石竹色",
      "赭",
      "退紅",
      "代赭色",
      "丹",
      "躑躅色",
      "鴇色",
      "鳶色",
      "丹",
      "バーミリオン",
      "バーントシェンナ",
      "薔薇色",
      "ピーチ",
      "緋色",
      "ピンク",
      "深緋",
      "紅赤",
      "紅色",
      "ベビーピンク",
      "ベンガラ色",
      "牡丹色",
      "ボルドー",
      "マゼンタ",
      "真赭",
      "桃色",
      "煉瓦色",
      "ローズ",
      "ローズピンク",
      "ワインレッド",
      "菖蒲色",
      "薄花色",
      "薄花桜",
      "江戸紫",
      "オーキッド",
      "桔梗色",
      "滅紫",
      "濃色",
      "古代紫",
      "紺桔梗",
      "紫紺",
      "菖蒲色",
      "菫色",
      "鉄紺",
      "茄子紺",
      "バイオレット",
      "鳩羽色",
      "花色",
      "パンジー",
      "藤色",
      "藤納戸",
      "二藍",
      "紫",
      "モーブ",
      "ライラック",
      "ラベンダー",
      "藍色",
      "藍鉄色",
      "青",
      "青褐",
      "浅葱・浅黄",
      "浅縹",
      "薄縹",
      "ウルトラマリン",
      "御納戸色",
      "褐色",
      "褐返し",
      "群青色",
      "濃藍",
      "コバルトブルー",
      "紺",
      "紺藍",
      "紺青",
      "紺瑠璃",
      "サックスブルー",
      "錆浅葱",
      "シアン",
      "新橋色",
      "スカイブルー",
      "青藍",
      "セルリアンブルー",
      "空色",
      "ターコイスブルー",
      "露草色",
      "鉄納戸",
      "ナイルブルー",
      "納戸色",
      "ネービーブルー",
      "縹色",
      "ピーコックブルー",
      "秘色",
      "ブルー",
      "ベビーブルー",
      "マリンブルー",
      "水浅葱",
      "水色",
      "ミッドナイトブルー",
      "ライトブルー",
      "瑠璃色",
      "イエローオーカー",
      "鬱金",
      "黄土色",
      "カーキ",
      "カナリヤ",
      "芥子色・辛子色",
      "黄",
      "生成",
      "生蘗色・黄膚色",
      "梔子",
      "クリーム色",
      "クロムイエロー",
      "黄金色",
      "卵色",
      "淡黄色",
      "蒲公英色",
      "中黄",
      "鳥の子色",
      "菜の花色",
      "ネープルスイエロー",
      "練色",
      "向日葵色",
      "ブロンド",
      "ベージュ",
      "山吹色",
      "緑黄色",
      "レグホーン",
      "レモン色",
      "亜麻色",
      "飴色",
      "洗柿",
      "杏色・杏子色",
      "アンバー",
      "薄柿色",
      "梅染",
      "葡萄茶・海老茶",
      "オレンジ色",
      "柿色",
      "褐色",
      "蒲色・樺色",
      "蒲茶・樺茶",
      "枯茶・唐茶",
      "枯色",
      "萱草色",
      "黄枯茶・黄唐茶",
      "狐色",
      "黄橡",
      "伽羅色",
      "銀煤竹",
      "金茶",
      "朽葉",
      "栗色",
      "栗梅",
      "胡桃色",
      "黒茶",
      "香色",
      "柑子色",
      "焦茶",
      "ココア色",
      "琥珀色",
      "媚茶",
      "小麦色",
      "コルク色",
      "煤竹色",
      "赤褐色",
      "セピア",
      "仙斎茶",
      "橙色",
      "タン",
      "茶色",
      "茶褐色",
      "丁子色",
      "丁子茶",
      "チョコレート色",
      "肉色",
      "肉桂色",
      "バーントアンバー",
      "バーガンディー",
      "黄櫨色・櫨色",
      "肌色",
      "バフ",
      "人色",
      "檜皮色",
      "ブラウン",
      "紅鬱金",
      "マルーン",
      "蜜柑色",
      "木蘭",
      "駱駝色",
      "藍海松茶",
      "青竹色",
      "青丹",
      "青緑",
      "浅緑",
      "油色",
      "鶯色",
      "鶯茶",
      "薄緑",
      "エメラルドグリーン",
      "オリーブ",
      "オリーブグリーン",
      "オリーブドラブ",
      "革色",
      "麹塵",
      "黄緑",
      "草色",
      "国防色",
      "苔色",
      "コバルトグリーン",
      "青磁色",
      "千草色",
      "鉄色",
      "常磐色",
      "木賊色・砥草色",
      "苗色",
      "生壁色",
      "ピーコックグリーン",
      "白緑",
      "ビリジャン",
      "鶸色",
      "鶸茶",
      "深緑",
      "ボトルグリーン",
      "抹茶色",
      "松葉色",
      "マラカイトグリーン",
      "緑色",
      "海松色",
      "萌葱・萌黄",
      "モスグリーン",
      "柳色",
      "山鳩色",
      "リーフグリーン",
      "利休色",
      "緑青色",
      "若草色",
      "若葉色",
      "若緑",
      "藍鼠",
      "アイボリー",
      "青鈍",
      "薄墨色",
      "薄鈍色",
      "薄鼠",
      "空五倍子色",
      "灰白色",
      "銀鼠",
      "グレー",
      "黒",
      "黒橡",
      "濃鼠",
      "錆鼠",
      "紫黒色",
      "漆黒",
      "シルバーグレー",
      "白",
      "スカイグレー",
      "スチールグレイ",
      "砂色",
      "スノーホワイト",
      "墨色",
      "スレートグレイ",
      "象牙色",
      "チャコールグレー",
      "茶鼠",
      "鉛色",
      "鈍色",
      "乳白色",
      "鼠色",
      "パールグレー",
      "灰色",
      "葡萄鼠",
      "利休鼠",
      "蝋色",
    ];
  }

  @override
  List<String> get levels {
    return const ["低い", "適当な", "高い", "非常な"];
  }

  @override
  List<String> get quotes {
    return const [
      "自分らしく生きなさい。他人の真似なんかしなくていい。",
      "3ヶ月間、それまでの人生を通して背負い過ぎたものを下ろし、「自分はどう生きたいのか」「何が好きなのか」と、自分の本心に向き合い続けました。そんな自問自答の末、最後に残ったのが「書家」という道だったのです。書家になると決断した瞬間、心がスッと軽くなったのを今でも鮮明に覚えています。",
      "もしあなたが私を嫌いでも、あなたに私は変えられないわ。だって私は私だもの。",
      "他人に何を言われようと関係ない。私は私。",
      "私は私であることを恥じない、恥じたこともない。",
      "パーフェクトでないこと。それが人間であることの証しであり、同時にその人固有の魅力にも、つながることなのです。",
      "自分らしく生きることができない人には次なる道は開けない。",
      "道に迷ったら、自分に自問自答すればいいんですよ、僕の様に。私はこう思ってる、でも人はこう言う、でも私はこう思ってるので行きますという事でいい。皆がこう言うから、ちょっと止めとこうは、僕の人生だけで言うと損したことばかりです。",
      "求められたら応えるべきだとは思うけど、そこで自分がなくなってしまったら意味がない。",
      "やなことは忘れるに限る。おかげでぼくはいつも幸せなんだ。",
      "自分らしく、思ったままを言葉にしていいんだよ。だってあれこれとやかく言う人を大事にする必要はないんだし、君が大事にしたいと思う人はあれこれとやかく言わないもんさ。",
      "人生は公平じゃない。そのことに慣れた方が良い。",
      "他人の言葉を心底気にしていたら、もう何年も前に自殺しているわ。",
      "あんまり誰かを崇拝するのは、自分の自由を失うことだよ。",
      "私も常に自信とコンプレックスの狭間にいます。けれども、それは他人と自分とを比較することで生じるものではなく、自分の中の理想との関係によるものでしかありません。",
      "女は40を過ぎて始めておもしろくなる。",
      "これが浜崎あゆみですということをキチンとまっすぐ言える人間であり続けなきゃいけない。",
      "私にとっての自由は歌の中にある。だから右手でマイクを持つ。そして愛を左手にね。",
      "生きがいとは、自分を徹底的に大事にすることから始まる。",
      "自分が好きなこと、確信が持てることに従って生きる。それは他人に媚びない生き方です。世の中のすべてから愛されようといった、虫のよいことは考えず、異端を恐れずに進む。それが私の原点です。",
      "私の人生はたくさんのドラマでいっぱいだから、自分が可愛く見えるかどうかなんて気にしている暇はないの。ジムに行くのは好きじゃないし、美味しい食べ物とワインを楽しむ方が好き。",
      "何かに興味を持っていれば、新しい興味を探す必要がない。向こうからこっちにやってきます。純粋に一つのことに打ち込めば、必ず違う何かにつながっていきます。",
      "全てはコインの表と裏。これだけの幸せを手に入れたら、それを失う怖さも手にしている。常に表裏一体じゃん。ってことを自分で意識できていたら、幸せをもっと幸せに感じられるし、もっと大切にしようって思える。",
      "時の流れや雰囲気に応じる「時流」でなく、自分の器や正直な気持ちで動く「自流」で生きろ。",
      "絶えずあなたを何者かに変えようとする世界の中で、自分らしくあり続けること。それがもっとも素晴らしい偉業である。",
      "外見を色々気にするよりも、たくさん傷ついて中身を磨いて、そうしたら滲み出てくるものがある。それが魅力というもの。",
      "悩みや苦しみは、自分が本当にやりたいこと、もっと自分らしく生きることに気づかせてくれる贈り物。",
      "私は自分の価値観で生きています。色々な事を言われているのは知っていますけれども。それによって私の価値観や生き方を変えるつもりはありません。例えその事によって誰からも好かれないとしても構わないのです。",
      "時には好きなだけ、わがままになってみればいい。",
      "やってやれないわけはない。やらずにできるわけがない。",
      "私は自分を信じています。",
      "人生は複雑じゃない。私たちの方が複雑だ。人生はシンプルで、シンプルなことが正しいことなんだ。",
      "自分が一番好きなことをやりましょう。お金はその後です。",
      "何も仕事で必ず自己実現しなければならないということはない。例え「私らしくない仕事」に就いていてもトータルで「私らしい人生」であればそれでいいのではないだろうか。",
      "いつのまにか自分らしさを見失い、学校や家庭や職場で求められる偽りの自己像に合わせつまらない自分になってしまう。あなたがつまらなければ、友達だって仕事だってつまらないに決まっている。",
      "自分らしくあれ。他の人の席はすでに埋まっているのだから。",
      "正しかろうが間違っていようが自分らしく生きよ。安易に服従してしまう臆病者よりずっと立派だ。",
      "人生は、全ての闘いに勝つことではない。大切なのは意味のある闘いに勝つということだ。",
      "悩むことは、自分らしさを見つけていく作業。",
      "遊ぶって、人生をやっていくためには絶対必要よ。",
      "あなたの心が正しいと思うことをしなさい。どっちにしたって批判されるのだから。",
      "多くの人が自分の可能性よりずっと低い次元で生きている。それは常に自分らしさを他人に手渡しているからだ。",
      "自分らしさとは求めるものではなく、あとから振り返ったときに初めて気づくものです。",
      "どのような状況になろうとも人間には一つだけ自由が残されている。 それはどう行動するかだ。",
      "バラはひまわりになれないし、ひまわりはバラにはなれない。花はそれぞれが独自に美しいのと同じで、女性もそれぞれに美しいの。みんな自分の個性を大切にするべきよ。",
      "他人を自分に同調させようなどと望むのは、そもそも馬鹿げた話。",
      "他人と自分を比べて優越感や劣等感を持つのはくだらない他人志向。大切なのは自己志向。自分が満足して自分が納得すればいい。",
      "強固なアイデンティティーを確立しなければいけない時代なんだよ。",
      "越えた先に自分らしさがある。",
      "力を抜いて「自分らしさ」から始めよう。",
      "自分らしさねぇ…。 実はあまり好きじゃなくて、'自分らしさ' というものが。 自分らしさ、という自己防衛みたいな感じがするからですかね。 自分とかなくても、もっと自由で、もっと捉えどころがなくて、もっと可能性があった方がいいなって思うんです。",
      "そもそも「自分を探しにいく」という行為自体が不思議なんですよね。自分を探すって「探しに行って見つかるもんじゃないのが自分」だったりするじゃないですか。私も「自分らしさ」なんて無いもん。",
      "他の誰かになりたがることは、自分らしさの無駄遣いだ。",
      "自分らしさを書き換えるには1.ぶっ飛んだ夢を持つ2.あこがれの群れに飛び込む3.すでにうまくいったふりをして生きる。",
    ];
  }

  @override
  List<String> get texts {
    return const [
      "徳川家の当主が正二位内大臣兼右大将に叙任され、征夷大将軍に任じられて、260余りの武家大名と主従関係を結び、彼らを統率するという制度は、1600年代後半までに確立された。",
      "その将軍の政府を「幕府」、臣従している大名家を「藩」、さらに両者が複合した権力の体制を「幕藩体制」と一般に呼んでいる。",
      "ただし、「幕府」及び「藩」の語は幕末期に広く使用され、現在も歴史用語として定着しているものの、江戸時代を通じて使用されていた訳ではない。",
      "それまでは、将軍の政府は「公儀」「公辺」などと漠然と呼ばれていた。",
      "幕府の始期及び終期については諸説あるが、征夷大将軍の任官時期に着目する場合には、家康がはじめて将軍職に任じられた1603年3月24日（慶長8年2月12日）から、いわゆる王政復古の大号令によって15代将軍徳川慶喜の将軍職辞任が勅許され、併せて幕府の廃止が宣言された1868年1月3日（慶応3年12月9日）までとなる。",
      "終期には他にも1867年11月9日（慶応3年10月14日）に15代将軍徳川慶喜が大政奉還を行った時、1868年5月3日（慶応4年/明治元年4月11日）の江戸開城とする説もある。",
      "徳川将軍が実質的に日本を支配した、この260年あまりの期間を一般に「江戸時代」と呼ぶ。",
      "江戸幕府は日本の歴史上、鎌倉幕府及び室町幕府に続く最後の武家政権である。",
      "幕府の支配体制は幕藩体制と呼ばれ、将軍の政府である幕府と、将軍と主従関係を結んだ大名の政府である藩で構成されていた。",
      "将軍は大名に対して朱印状を与えてその知行を保障し、大名は当該知行内において独自に統治を行う権限を一定程度有した。",
      "なお、「藩」の語が公称として用いられるようになったのは明治時代のことで、公文書では「領」「領分」、あるいは「領知」などが使用された。",
      "公称としての藩は、1868年（明治元年）に公布された政体書によって設けられ、1871年（明治4年）の廃藩置県によって廃止された。",
      "江戸幕府の支配においては、将軍と大名の主従関係を確認するための軍役として、各藩大名に対して参勤交代や、築城・治水工事などの手伝普請が課せられた。",
      "初代家康、3代家光、5代綱吉、8代吉宗、11代家斉など、親政を行ったとされる将軍も存在するが、基本的に政治の多くの部分は老中を初めとする幕閣に委ねられた。",
      "権力の集中を避けるため主要な役職は複数名が配置され、一か月交代で政務を担当する月番制を導入し、重要な決定は合議を原則とした。",
      "常置の最高職である老中及び臨時に置かれる大老、その補佐役である若年寄は譜代大名から選任され、大目付・三奉行（寺社奉行・町奉行・勘定奉行）等の要職には譜代あるいは旗本が充てられて実務を担った。",
      "幕府組織は後期にはその全貌の把握が困難であるほど巨大化・複雑化し、幕末には老中の月番制を廃止して、国内事務・会計・外国・陸軍・海軍の各総裁を専務する等の改革が行われた。",
      "幕府は「公儀」として国内全体の統治を行うとともに、自らも1大名として領分（天領・御領）を支配し、京都所司代、大坂城代、遠国奉行、郡代・代官などの地方官を設置した。",
      "江戸城（えどじょう）は、武蔵国豊嶋郡江戸（現在の東京都千代田区千代田）にあった日本の城である。",
      "江戸時代においては江城（こうじょう）という呼び名が一般的だったと言われ、また千代田城（ちよだじょう）とも呼ばれる。",
      "江戸城は麹町台地の東端に、扇谷上杉氏の家臣太田道灌が築いた平山城である。",
      "近世に徳川氏によって段階的に改修された結果、総構周囲約4里と、日本最大の面積の城郭になった。",
      "徳川家康が江戸城に入城した後は徳川家の居城、江戸幕府の開幕後は幕府の政庁となる。",
      "明治維新後の東京奠都で宮城（きゅうじょう）となった。",
      "以後は吹上庭園が御所、旧江戸城西ノ丸が宮殿の敷地となっている。",
      "その東側にある旧江戸城の中心部である本丸・二ノ丸と三ノ丸の跡は皇居東御苑として開放されている。",
      "南東側の皇居外苑と北側の北の丸公園は常時開放され、それらの外側は一般に利用できる土地になっている。",
      "国の特別史跡に指定されている。",
    ];
  }

  @override
  List<String> get words {
    return const [
      "1st",
      "2LDK",
      "2欠け",
      "2気筒",
      "2行割",
      "8日ぶり",
      "AIRくまモン",
      "Fab4",
      "ICPCN憲章",
      "ICT教育",
      "IF鋼",
      "IOC注文",
      "ISBNコード",
      "IS志願",
      "I字2回ターン",
      "LINE乗っ取り",
      "LINE外し",
      "LINE桜",
      "LINE疲れ",
      "MYキッチン",
      "TCR再構成",
      "TNJ女子",
      "TNP",
      "TPE48",
      "TTP",
      "Tm値",
      "Twitterアイコン",
      "Twitter民",
      "Tカード",
      "Web価格",
      "Web男子",
      "Y字路",
      "〆日",
      "ああいった",
      "ああだこうだ",
      "ああでもないこうでもない",
      "ああなる",
      "ああ言えばこう言う",
      "あいがけ",
      "あいじょうおとこ",
      "いいえ、ケフィアです",
      "いいこと尽くめ",
      "いいぞ",
      "いいつつ",
      "いい例",
      "いい加減",
      "いい加減にしなさい",
      "いい女",
      "いい得も知れない",
      "いい思いをする",
      "いい格好しい",
      "いい歳して",
      "いい歳をして",
      "いい線いっている",
      "いい質問ですねぇ",
      "うおさお",
      "おいくら万円",
      "お天道さま",
      "お姉系タレント",
      "お姉言葉",
      "お家騒動",
      "お後がよろしいようで",
      "お揚げさん",
      "お改め",
      "お明かし料",
      "お熱い",
      "お相子",
      "お相手探し",
      "お維",
      "お義姉さん",
      "お集まり",
      "お預け",
      "お預け入れ",
      "おｋ",
      "きいたような口",
      "きうす",
      "きうり封じ",
      "こう言っちゃ何だが",
      "さい帯血",
      "しぇしぇしぇ",
      "し得ない",
      "し得る",
      "すい臓ガン",
      "そういう訳で",
      "そう言うこと",
      "そう言えば",
      "そう言や",
      "っ付きことば",
      "っ付き言葉",
      "ついうっかり",
      "ついでだから",
      "ついには",
      "て言うか",
      "ぬかった",
      "ぬかるみ",
      "ぬけぬけと",
      "ねぎソース",
      "ねぎ味噌",
      "ねぎ抜き",
      "はいからうどん",
      "ひいおじいさん",
      "ひいおばあさん",
      "ひいひいおじいさん",
      "ひいひいおばあさん",
      "まえあつ",
      "みかじめ料",
      "みかん缶",
      "むき出し",
      "もうちょい右",
      "もうメロメロ",
      "もう一つ",
      "もう一回",
      "もう一息",
      "もう一踏ん張り",
      "やいかがし",
      "やいくさし",
      "やがて哀しき",
      "ゆいP",
      "よいれい",
      "よい年末を",
      "よい所",
      "よい目が出る",
      "ろうと胸",
      "わかる人にはわかる",
      "アイアンウッド",
      "アイアンストマック",
      "アイキャッチ",
      "アイクリーム",
      "アイシス",
      "イイ人止まり",
      "イイ大人",
      "イイ女",
      "イイ感じ",
      "イイ男",
      "ウィキペデア",
      "ウィズバス",
      "ウィダーinゼリー",
      "ウィドウメーカー",
      "ウィリアムおうじ",
      "ウィリアム王子",
      "ウィルス",
      "ウィルスと細菌の違い",
      "ウィルス性胃腸炎",
      "ウィンウィン",
      "ウィングバック",
      "ウィンテージ",
      "ウィンナー",
      "ウィンナーコーヒー",
      "ウィンナーシュニッツェル",
      "ウィークポイント",
      "ウィーニング",
      "ウィービング",
      "ウイナーズブラケット",
      "ウイニング",
      "ウイルス",
      "ウイルヘルム・レントゲン",
      "ウイーニング",
      "ウェアラブル",
      "ウェアラブルカメラ",
      "ウェアラブルディスプレイ",
      "ウェイ",
      "ウェイウェイ",
      "ウェイツァイ",
      "ウェイトを占めている",
      "ウェイトドール",
      "ウェイトベア",
      "ウェイトボルト",
      "ウェイパー",
      "ウェザーガールズ",
      "ウェストバッグ",
      "ウェストポーチ",
      "ウェッジソール",
      "ウェブ男子",
      "ウェルター級",
      "ウェルダー",
      "ウェーイ",
      "ウエスタンブーツ",
      "ウエストを絞る",
      "ウエットシート",
      "ウエルター級",
      "ウォッカトニック",
      "ウォッカベース",
      "ウォッシュアウト",
      "ウォーキングシューズ",
      "ウォークイン",
      "ウォーターサーバー",
      "ウォータースライダー",
      "ウォーターディスペンサー",
      "ウォーターローディング",
      "ウォーミングアップ",
      "ウォームアップ",
      "エア",
      "エアK",
      "エアシャワー",
      "エアバースト",
      "エアブロー",
      "エアボンボン",
      "エアリアルヨガ",
      "エアリプライ",
      "エアロビ",
      "エアロビクスダンス",
      "エアーK",
      "エアーバンド",
      "エアーブロー",
      "エア充",
      "カイ2乗",
      "キウイジュース",
      "クアッガ",
      "クアッド",
      "クアッハ",
      "クィディッチ",
      "クイックスローイン",
      "クイックトス",
      "クイックモッパー",
      "クイック攻撃",
      "クイーンサイズ・ベッド",
      "ケアメン",
      "ケア用品",
      "コアなファン",
      "コアエリア",
      "コアゾーン",
      "コア事業",
      "コア機械受注",
      "コア機械受注統計",
      "コア預金",
      "コインシャワー",
      "サイアンドコー",
      "サイギャップ",
      "シアッチ・ミーケロルム",
      "シアット・ミーケロルム",
      "シアツ・メエケロルム",
      "シアトル系コーヒー",
      "シアト・メエケロルム",
      "シア充",
      "シェアオフィス",
      "シェイク・ウマル・カーン",
      "シェイプアップグッズ",
      "シェビ大砂丘",
      "シェビ砂丘",
      "シェフサラダ",
      "シェールオイル",
      "シェールガス田",
      "シエスパ",
      "スイカジュース",
      "スイッチオンヌ",
      "タイアップ曲",
      "タイガーロープ",
      "チアミン硝化物",
      "チェキ会",
      "チェキ撮影会",
      "チェックインタイム",
      "チェックメイト",
      "チェック機能",
      "チェック牌",
      "チェヌンキブ",
      "チェリスト",
      "チェリーボム",
      "チェルノブイリ級",
      "チェンジエージェント",
      "チェンジニア",
      "チェンジングボード",
      "チェーンスモーカー",
      "ツアーガイド",
      "ツイッター歴",
      "ツイッター警部",
      "ツイドラ",
      "ツイ消し",
      "ツイ減",
      "ツイ飲み",
      "ティウンティウン",
      "ティグ溶接",
      "ティグ溶接法",
      "ティックチャート",
      "ティッピングポイント",
      "ティップ",
      "ティップシェーパー",
      "ティプトロニック",
      "ティーアールエフダイエット",
      "ティーカッププードル",
      "ティーカード",
      "ティーサービス",
      "ティーブレイク",
      "ティーンエイジ",
      "トイサイダー村",
      "トイレタリー",
      "トイレットブレーク",
      "トイレ用擬音装置",
      "トゥイフイフウマオ",
      "トゥキック",
      "トゥドゥ",
      "トゥルパ",
      "トゥルーエンド",
      "トゥーキック",
      "ナイスアレンジ",
      "ナイスダーツ",
      "ナイストン",
      "ナイスハット",
      "ナイスバッティング",
      "ナイター中継",
      "ナイトクルージング",
      "ナイトツアー",
      "ナイトパック",
      "ナイトライド",
      "ニアショア",
      "ニアピンコンテスト",
      "ニキビ跡",
      "ヌガー",
      "ヌクマム",
      "ヌクモリティ",
      "ネイキッドヨガ",
      "ネイティブチェック",
      "ネイティブデータ",
      "ネイリスト",
      "ネイルスクール",
      "ネイル用品",
      "ネオサンプーン",
      "ネオジムヤグレーザー",
      "ネオニコチノイド系",
      "ネオニコ系",
      "ネカフェ",
      "ネガキャン",
      "ネガティブ",
      "ネガティブエクイティ",
      "ネガティブガンマ",
      "ネガティブキャンペーン",
      "ネガティブコントロール",
      "ネガティブシンキング",
      "ネガポ",
      "ネガポ辞典",
      "ネギダク",
      "ネコバス",
      "ノアの大洪水",
      "ノアゼット",
      "ノキ男",
      "ハアハア",
      "ハイイロタチヨタカ",
      "ハイウェスト",
      "ハイエナ系男子",
      "ハイオフ",
      "ハイカットスニーカー",
      "ハイクラス求人",
      "ハイクリア",
      "ハイグレード",
      "ハイゲーム",
      "ハイコンテクスト文化",
      "ハイシリーズ",
      "ハイジニーナ",
      "ハイスペック",
      "ヒアリング調査",
      "ヒアルロン酸注入",
      "ヒエラルキー型組織",
      "ファイアパンチ",
      "ファイアーポリッシュ",
      "ファイナルステージ",
      "ファイブカウント",
      "ファイブセットマッチ",
      "ファイブドア",
      "ファイブナンバー",
      "ファイヤーポリッシュ",
      "ファインセーブ",
      "ファインディング",
      "ファウルスロー",
      "ファクトファインデイング",
      "ファクトリーリファービッシュ品",
      "ファスティング",
      "ファスティングダイエット",
      "ファストトラッキング",
      "ファストフート",
      "ファストリ",
      "ファッキチ",
      "ファックサイン",
      "ファッシュン",
      "ファッション",
      "ファッションアイテム",
      "ファッションオタク",
      "ファッションスタイル",
      "ファッションセンス",
      "ファッションリーダー",
      "ファッション感覚",
      "ファットヘッドフィッシュ",
      "ファビュラス",
      "ファビョる",
      "ファビョン",
      "ファブフォー",
      "ファボ爆",
      "ファボ魔",
      "ファミチキ",
      "ファミチョコ",
      "ファミマ",
      "ファミリマート",
      "ファミリーカー",
      "ファミレス",
      "ファミレス御三家",
      "ファラ王",
      "ファルファッレ",
      "ファルール",
      "ファンカスト",
      "ファンカストーディアル",
      "ファンコミュニティ",
      "ファンシーショット",
      "ファンデーションフレーム",
      "ファンドアナリスト",
      "ファンパイ",
      "ファンミ",
      "ファンミーティング",
      "ファンモン電車",
      "ファン心理",
      "ファン食い",
      "ファースト",
      "ファーストインパクト",
      "ファーストタックラー",
      "ファーストタックル",
      "ファーストチョイス",
      "ファーストライン",
      "ファームプライス",
      "ファールライン",
      "ヘアエクステンション",
      "ヘアカラー",
      "ヘアコンタクト",
      "ヘアゴム",
      "ヘアサロン",
      "ヘアスタイリングウォーター",
      "ヘアマニ",
      "ヘアーエクステンション",
      "ヘアーサロン",
      "ホイッパー",
      "マイクロインプラント",
      "マイクロウェル",
      "マイクロサッカード",
      "マイクロミニピッグ",
      "マイシューズ",
      "マイダーツ",
      "マイチョコ",
      "マイナー-チェンジ",
      "マイナースポーツ",
      "マイナー契約",
      "マイバッグ",
      "マイボトル",
      "マイボール",
      "マイルドなメロン熊",
      "マイルドヤンキー",
      "マインドコントロール",
      "マウンティング女子",
      "マウンテンライオン",
      "ミイラ取りがミイラになる",
      "ミカファンギン",
      "ムカつく",
      "ムカムカ",
      "ムキになる",
      "メイクのノリ",
      "メイクぼん",
      "メイク本",
      "メイソンジャー",
      "メイドインPRC",
      "メイドカフェ",
      "メイド喫茶",
      "ヤオラー",
      "ヤキが回る",
      "ユイルスブリム",
      "ヨイショ",
      "ヨイトマケ",
      "ヨイトマケの唄",
      "ライイングオンザボール",
      "ライジングカット",
      "ライジングカットボール",
      "ライスト",
      "ライトアップ",
      "ライトノベル",
      "ライトフライ級",
      "ライトヘビー級",
      "ライバル会社",
      "ライバル店",
      "ライバー",
      "ライフスタイリスト",
      "ライフプラン",
      "ライフワーク",
      "ライメックス",
      "ラインナップ",
      "ラウド",
      "ラウドロック",
      "ラウド系",
      "ラウンドアバウト",
      "ラウンドアームブロー",
      "ラウンドザクロック",
      "ラウンドスコア",
      "ラウンドマストシステム",
      "ラウンドリミット",
      "ラウンドロビントーナメント",
      "ラガール",
      "ラク",
      "ラクイラ地震",
      "リア",
      "リアカノ",
      "リアカレ",
      "リアル",
      "リアルに",
      "リアルアカウント",
      "リアル書店",
      "リア充",
      "リア充爆発しろ",
      "リア友",
      "リア垢",
      "リインカネーション",
      "リオ五輪",
      "リクスバンク",
      "リヴィジョン",
      "ルイス酸",
      "ルチャドール",
      "ルテオスキリン",
      "ルテオリン",
      "ルヌラ",
      "ルネサス",
      "ルンブリクスルベルス",
      "ルーザーズブラケット",
      "ルーティンワーク",
      "ルーティーン",
      "ルート営業",
      "ルート案内",
      "ルームシェアリング",
      "ルームフレグランス",
      "ルール無用",
      "ルー・ゲーリック病",
      "ルー語",
      "レアアイテム",
      "レアアース泥",
      "レアカード",
      "レアシュガー",
      "レアーアース",
      "レア物",
      "レイアウトフリー",
      "レイアップ",
      "レイコー",
      "レイテ島",
      "レイトタックル",
      "ロイノーゼ",
      "ロイホ",
      "ロイヤルオックスフォードシャツ",
      "ロイヤルベビー",
      "ロイヤルワラント",
      "ワイじろ",
      "ワイアードカフェ",
      "ワイガヤ",
      "ワイドセレクション",
      "ワイドパンツ",
      "ワイハ",
      "ワイプアウト",
      "ワイプイン",
      "ワイプ映像",
      "ワイルドなメロン熊",
      "ワイルド発言",
      "ワインが開く",
      "ワインず",
      "ワインを開く",
      "ワイン酢",
      "ワイ談",
      "ワカチコ",
      "ヲコト点",
      "ヲタ",
      "ヲタアニメ",
      "ヲタク",
      "ヲタ切り",
      "ヲタ芸",
      "ヲチ",
      "ヲチスレ",
      "ヲーイング",
      "三日月形",
      "三日月状",
      "三重県警",
      "上から目線",
      "上で",
      "上のせ価格",
      "上ぶれ",
      "上島システム",
      "上野のシンシン",
      "下医",
      "不安がる",
      "不安が募る",
      "不安が残る",
      "不安にかられる",
      "不安視",
      "並行作業",
      "並記",
      "主机",
      "主腐",
      "主限",
      "二位タイ",
      "二家立直",
      "二月の誕生石",
      "二月札",
      "二極分化",
      "二極化",
      "二脚鳥居支柱",
      "二貫盛り",
      "二階派",
      "仁王立ち",
      "仕入れ単価",
      "仕入先",
      "他愛の無い",
      "付いて来る",
      "付いて行く",
      "令閨",
      "企画力",
      "企画書",
      "企画者",
      "会員制",
      "会員制サークル",
      "会員制ホテル",
      "似合わない",
      "低カロリー",
      "低位破水",
      "低価格",
      "低圧線",
      "低圧電線",
      "低学歴",
      "低機能",
      "体位ドレナージ",
      "体位交換",
      "体位排痰法",
      "体幹トレ",
      "体当たり",
      "体当りの演技",
      "体感型脱出ゲーム",
      "体液",
      "体系化",
      "体系立てて",
      "体育会系",
      "体育服",
      "体育着",
      "余韻に浸る",
      "你好",
      "併記",
      "併設施設",
      "例によって",
      "例に漏れず",
      "例外措置",
      "例外無く",
      "例外規定",
      "俳句甲子園",
      "催行",
      "債券利回り",
      "優位性",
      "優秀の美",
      "優秀賞",
      "優越権",
      "光GENJI",
      "光GENJI SUPER 5",
      "光と影の日々",
      "光の祭典",
      "光る所がない",
      "光コヒーレンストモグラフィー",
      "光干渉断層計",
      "光陰矢のごとし",
      "八個師団",
      "八尾空港",
      "八重の桜",
      "八重山石亀",
      "公九牌",
      "公安委員会遵守事項違反",
      "内々定",
      "内偵捜査",
      "内反膝",
      "内在化",
      "内外情勢調査会",
      "内夫",
      "内定コレクター",
      "内定契約書",
      "内定承諾書",
      "内定拘束",
      "内定者",
      "内定長者",
      "内戦下",
      "内斜視",
      "内省的",
      "内線番号",
      "内職斡旋所",
      "内臓疾患",
      "内藤ホライゾン",
      "内藤武敏",
      "内製",
      "内製化",
      "内見",
      "内規",
      "内辞",
      "内通者",
      "内進生",
      "内部設計",
      "内閣セキュリティセンター",
      "内閣退陣",
      "再加速",
      "再可決",
      "再啓",
      "再委託",
      "再教育",
      "再校",
      "再校正",
      "再構成",
      "再現レシピ",
      "再現実験",
      "再確認",
      "再結成",
      "再議決",
      "再起不能",
      "再開発計画",
      "冥土の土産",
      "冶金家",
      "冷えた視線",
      "冷凍イチゴ",
      "冷凍パン",
      "冷凍ミカン",
      "冷凍室",
      "冷凍米飯",
      "冷凍野菜",
      "冷却化",
      "冷却室",
      "冷却水",
      "冷暖房機",
      "冷水域",
      "冷温水",
      "冷笑的",
      "冷蔵室",
      "冷製ソース",
      "冷製料理",
      "冷酷無慙",
      "冷酷無残",
      "冷静沈着",
      "凄艶な",
      "分からず屋",
      "分からず終い",
      "分からない",
      "分かりかねます",
      "分かりかねる",
      "分かりにくい",
      "分かりました",
      "分かりやすく",
      "分かり合う",
      "分かり辛い",
      "分かれ道",
      "分ち合い",
      "分りません",
      "別れさせ工作",
      "利休形割り箸",
      "利活用",
      "利益の搾取",
      "利益を失う",
      "利益享受者",
      "利益供与",
      "利益処分",
      "利益操作",
      "利益留保性引当金",
      "前売券",
      "前尾派",
      "前株",
      "前祝",
      "前髪クネ男",
      "剥き物",
      "剥き胡麻",
      "剥き身",
      "創刊号",
      "創刊誌",
      "創意工夫",
      "創業者",
      "力うどん",
      "力及ばず",
      "労いの言葉",
      "労せず",
      "労をねぎらう",
      "労を厭わず",
      "労を多とする",
      "労を惜しまず",
      "労を惜しまない",
      "労働報酬",
      "労働形態",
      "労働意欲",
      "労働所得",
      "労働環境",
      "労働者数",
      "労働衛生",
      "労働衛生学",
      "労働部門",
      "労多くして",
      "労災認定",
      "匂い立つような",
      "包合",
      "包含する",
      "包囲網",
      "包括",
      "包括一罪",
      "包括協定",
      "包括支援",
      "包括的",
      "包材",
      "十一",
      "十一はねけん",
      "十一一二四",
      "十一三本",
      "十一喰付",
      "十一四三",
      "十一手四",
      "十一時の方向",
      "十一枚麻雀",
      "十一立三本",
      "卵うどん",
      "厚意",
      "参りました",
      "参ります",
      "友好国",
      "友好的",
      "友好関係",
      "双丘",
      "可々",
      "司々",
      "合鍵",
      "合鴨農法",
      "名伯楽",
      "名刺ホルダー",
      "名刺交換",
      "名地検",
      "名宛て",
      "名宛人",
      "名実共に",
      "名店の味",
      "名投手",
      "名文家",
      "名物に美味いもの無し",
      "名物先生",
      "名物教師",
      "名状し難い",
      "名産品",
      "名目上",
      "名詞化する",
      "向かって右",
      "向かって左",
      "向き不向き",
      "吸い地",
      "吸い込まれる",
      "吸込仕事率",
      "味覇",
      "味覚の秋",
      "味覚狩り",
      "命じられる",
      "命名の儀",
      "命脈が尽きる",
      "命運が尽きる",
      "命運を握る",
      "咆哮",
      "和歌山県警",
      "和解離婚",
      "哀愁を帯びる",
      "哀感が漂う",
      "哀感を込める",
      "哀艶",
      "唯一の",
      "唯一不動",
      "唯一無二の",
      "啄む",
      "問いかけ",
      "問いただす",
      "問い直す",
      "問合わせ先",
      "喪明け",
      "喪飯",
      "喫",
      "喰いタン",
      "喰い伸ばす",
      "喰い流す",
      "喰い直し",
      "営業キャッシュフローマージン",
      "営業キャッシュフロー当期純利益率",
      "営業サイド",
      "営業スタンス",
      "営業スマイル",
      "営業ポリシー",
      "営業努力",
      "営業収益",
      "営業方針",
      "営業日",
      "営業活動",
      "営業益",
      "営業費用",
      "営業部隊",
      "嚢胞性腎腫瘤",
      "嚢胞腎",
      "四時の方向",
      "四時ラー",
      "四時間足",
      "地下に眠る",
      "地下駐車場",
      "地位の向上",
      "地域主権",
      "地域別",
      "地域史",
      "地域密着型サービス",
      "地域差",
      "地域性",
      "地域的",
      "坩堝",
      "坩堝と化す",
      "垂直離着陸機",
      "埋伏歯",
      "埋却",
      "埋却処分",
      "埋蔵域",
      "埋設管路",
      "基幹システム",
      "基幹電力系統",
      "報告会",
      "塞翁が馬",
      "壮観図",
      "夕下風",
      "夕御飯",
      "夜間営業",
      "夜間外出禁止令",
      "夜間走行",
      "夜闇",
      "大儀そうに",
      "大局的",
      "大局的に見る",
      "大局的見地",
      "大義名文",
      "大過なく",
      "大雨時行",
      "奉仕者",
      "奉献酒",
      "奉謝",
      "奉賛金",
      "好い気味",
      "好い線",
      "好い線行っている",
      "好い線行ってる",
      "好い鴨",
      "好印象",
      "妄想が膨らむ",
      "妄想に耽る",
      "妄想をかき立てる",
      "妄想力",
      "妄想彼氏",
      "妄撮",
      "妄言リレー",
      "妖艶な",
      "威圧感",
      "威圧的",
      "季刊誌",
      "定位置",
      "定員外乗車",
      "定員稼働率",
      "定期健診",
      "定期点検",
      "定期的に",
      "定期的実施",
      "定期購入",
      "定期購読",
      "定格電圧",
      "容器栽培",
      "容易ではない",
      "容易ならざる",
      "容易化",
      "容易性",
      "寝押し",
      "寝落ち",
      "寝起きドッキリ",
      "寝込みを襲う",
      "寝首をかく",
      "対11",
      "対3",
      "対7",
      "対外債務",
      "対外投資",
      "対外折衝",
      "対外株式投資",
      "対外純債務",
      "対外純資産",
      "対外負債",
      "対外資産",
      "対子カブリ",
      "対子筋",
      "対子落とし",
      "対家",
      "対応している",
      "対応付ける",
      "対応力",
      "対応策",
      "対極的",
      "対概念",
      "対死",
      "小さな巨人",
      "小さな政府",
      "小一時間",
      "小悪党",
      "小悪魔",
      "小悪魔クソビッチ",
      "小悪魔的",
      "居空き",
      "差上げます",
      "差異化",
      "市営住宅",
      "帝王切開後経膣分娩",
      "幣所",
      "平均年収",
      "平均法",
      "平均降水量",
      "平均風速",
      "平城遷都",
      "平安京遷都",
      "平安遷都",
      "平日昼顔妻",
      "平滑化",
      "平滑性",
      "平行トス",
      "平行ピン",
      "平行世界",
      "平行棒",
      "平行棒種目",
      "平行線に終わる",
      "平身低頭",
      "平身低頭する",
      "平静を装う",
      "幸せのパンケーキMagia di farina",
      "幸せを呼ぶ",
      "幸せ太り",
      "幸運巻",
      "幽暗な",
      "幾何学模様",
      "幾何学的な",
      "幾何的",
      "広域地方公共団体",
      "広域暴力団",
      "広域自治体",
      "広域運営",
      "広域避難場",
      "広域避難場所",
      "序でに",
      "廃",
      "廃人",
      "廃家電",
      "廃棄処分",
      "廃棄品",
      "廃棄物受入",
      "廃課金",
      "弄して",
      "弄舌癖",
      "弊局",
      "弊校",
      "弊社",
      "弊職",
      "弊舎",
      "弊誌",
      "弊院",
      "引いたら負け",
      "強いて言うなら",
      "強いられた",
      "強いられる",
      "強い肴",
      "強肴",
      "影響ないレベル",
      "影響を及ぼす",
      "待機室",
      "後逸",
      "御家",
      "御家芸",
      "御引見",
      "御明かし料",
      "御買い得",
      "思案する",
      "思案の外",
      "思案六法",
      "性依存症",
      "性格の不一致",
      "性格考課",
      "性格障害",
      "怪奇現象",
      "怪演",
      "恋しがる",
      "恋に落ちる",
      "恋の悩み",
      "恋バナ",
      "恋心を抱く",
      "恋活",
      "恋煩い",
      "恋金橋",
      "恣意的な解釈",
      "悔いが残る",
      "悠久の時",
      "想起させる",
      "惹かれ合う",
      "愛がある",
      "愛がない",
      "愛され",
      "愛されキャラ",
      "愛され系",
      "愛し合う",
      "愛唱",
      "愛国主義",
      "愛国主義者",
      "愛国心",
      "愛国無罪",
      "愛好者",
      "愛嬌を振りまく",
      "愛嬌毛",
      "愛犬",
      "愛玩用",
      "愛盛り",
      "愛飲家",
      "慰安旅行",
      "慶",
      "慶休",
      "慶應ボーイ",
      "憂いごと",
      "憂国の士",
      "憂国の情",
      "憂鬱感",
      "成果を収める",
      "成果を挙げる",
      "成果物",
      "戴冠式",
      "手厚く",
      "手垢に塗れる",
      "手垢のついた",
      "手垢塗れ",
      "手汗",
      "手足を伸ばす",
      "抜かりない",
      "抜かりなく",
      "抜か六",
      "抜き伐り",
      "抜き打ちテスト",
      "抜き打ち検査",
      "抜き技",
      "抜けるような青空",
      "抜けセー",
      "抜け参り",
      "抜け感",
      "抜け毛",
      "抜け番",
      "抜け目の無い",
      "抜型",
      "拝金主義",
      "拝金主義者",
      "拝顔の栄",
      "拝顔の栄に浴する",
      "拝鬼",
      "拭い切れない",
      "拭い難い",
      "拭えない",
      "挂甲の武人",
      "挨拶文",
      "捕安",
      "掃海母艦",
      "排気マニホールド",
      "排水性アスファルト",
      "排除命令",
      "排除措置命令",
      "控えの間",
      "控えめ",
      "控えゲラ",
      "控え選手",
      "控室",
      "推測の域を出ない",
      "推牌",
      "推薦書",
      "推計震度分布図",
      "提供する",
      "提供主",
      "提供先",
      "提供曲",
      "提供者",
      "提案書",
      "携行",
      "携行タンク",
      "携行弾数",
      "携行浄水器",
      "携行食",
      "操業停止",
      "操業利益",
      "操業損失",
      "支",
      "支援の",
      "支援物資",
      "支援金",
      "改易",
      "改貫",
      "改貫札",
      "改軌",
      "改革会派",
      "放棄呼",
      "放火容疑",
      "放火魔",
      "放血",
      "政活費",
      "政界人",
      "政界再編",
      "政界引退",
      "故意負け",
      "敗残兵",
      "敗血性ショック",
      "敗血症性ショック",
      "敬意の念",
      "敬意を払う",
      "敬意を表す",
      "敬意を表する",
      "敬虔な",
      "斃死",
      "斉一性",
      "斉一的",
      "新国立",
      "新島",
      "新潟県警",
      "方位神",
      "方合",
      "方向修正",
      "方向性",
      "方向音痴",
      "方式",
      "方眼北",
      "方眼罫",
      "方言チャート",
      "旗艦店",
      "旗艦製品",
      "既往歴",
      "日が浅い",
      "日出ずる処",
      "日脚伸ぶ立春の候",
      "日足が伸びる",
      "早急に",
      "早慶",
      "早慶上智",
      "早慶上理",
      "早期新生児",
      "早期治療",
      "早期発見",
      "早期退職",
      "早期退院",
      "明々白々たる",
      "明意",
      "明敏な",
      "明晰な",
      "明治時代",
      "明確化",
      "明細票",
      "明青立法中",
      "昔噺",
      "映画化",
      "映画鑑賞",
      "景気づけ",
      "景気の腰折れ",
      "景気動向",
      "景況感",
      "最下位",
      "最下層",
      "最下段",
      "最下部",
      "最外壁",
      "最大手",
      "最奥部",
      "最強",
      "最強種",
      "最恵価格",
      "最高学府",
      "最高幹部",
      "最高水準",
      "最高級",
      "最高経営責任者",
      "最高裁",
      "最高速度",
      "最高額",
      "有効打突",
      "有効期限",
      "有効活用",
      "有印",
      "有史以来",
      "有害化学物質",
      "有害無益",
      "有害食品",
      "有形財産",
      "有意義な",
      "有感地震",
      "有期制",
      "有期懲役",
      "有権代理",
      "有機イオン交換体",
      "有益な",
      "有益性",
      "有茎種",
      "有資格者",
      "有限会社",
      "朔日饅頭",
      "朗読劇",
      "期間限定",
      "未受付",
      "未回答",
      "未完全",
      "未完治",
      "未確認情報",
      "未確認生物",
      "未科学",
      "未解決問題",
      "未開の地",
      "未開拓分野",
      "未開拓地",
      "本意無し",
      "来園者",
      "来塔者",
      "来孫",
      "来客攻め",
      "来山者",
      "来年中",
      "来年以降",
      "来年度",
      "来庁者",
      "来所者",
      "来月中",
      "来村者",
      "来校者",
      "来港者",
      "来町者",
      "来観者",
      "来訪者",
      "来賓者",
      "来週中",
      "枚挙に暇がない",
      "枚葉",
      "枚葉機",
      "柊鰯",
      "柳の下の2匹目のどじょうを狙う",
      "栄枯盛衰",
      "栄華を極める",
      "根こそぎ",
      "根暗",
      "桑間濮上",
      "梨園の妻",
      "棺かけ",
      "植松メソッド",
      "楽オク",
      "楽易",
      "機会に恵まれる",
      "機械株",
      "機械銘柄",
      "機械関連株",
      "機械関連銘柄",
      "檜花粉症",
      "正大関",
      "正確な",
      "正確無比",
      "歳月を経る",
      "歳祝い",
      "残りコンクリート",
      "母さま",
      "毎々格別のお引き立て",
      "毎年",
      "毎年度",
      "毎年恒例",
      "毎度あり",
      "毎時間",
      "比較",
      "比較的軽微",
      "毛穴ケア",
      "毛頭ない",
      "気いつける",
      "気おつける",
      "気がある",
      "気が気",
      "気が置けない",
      "気が置ける",
      "気合が入る",
      "気後れ",
      "気後れする",
      "気懸かり",
      "気懸り",
      "水上スポーツ",
      "水上バイク転落事故",
      "水上パレード",
      "水上自転車",
      "水上観光",
      "水中ゴーグル",
      "水中スポーツ",
      "水位線",
      "水晶ケーキ",
      "水晶煮",
      "水泳着",
      "水火の苦しみ",
      "水産加工品",
      "水産株",
      "水産銘柄",
      "水産関連株",
      "水産関連銘柄",
      "水素水サーバー",
      "水道料金",
      "水道浄水場",
      "水酸化ナトリウム水溶液",
      "水酸化ナトリュウム",
      "水際線",
      "氷魚",
      "永久劣後ローン",
      "永久就職",
      "永久歯列期",
      "沸立つような",
      "治金",
      "法外な",
      "法悦に浸る",
      "法改正",
      "洋型墓石",
      "流刑地",
      "流浪人剣心",
      "浪費家",
      "浮いた話",
      "浮いている",
      "浮いてまて",
      "浮いて待て",
      "海外の反応",
      "海外セレブ",
      "海外出張",
      "海外戦略",
      "海外流出",
      "海外短期筋",
      "海外組",
      "海外航空券",
      "海外送金",
      "海外邦人",
      "海岸部",
      "海運株",
      "海運銘柄",
      "海運関連株",
      "海運関連銘柄",
      "海関",
      "消え入りそうな",
      "消え入りたい",
      "涙活",
      "涙腺崩壊",
      "涙腺決壊",
      "添いかねる",
      "添い寝カフェ",
      "添い寝シーツ",
      "添い寝ボイス",
      "湧いてくる",
      "溶岩マウンド",
      "溶解性",
      "溶解炉",
      "溺",
      "漏斗胸",
      "漏洩",
      "濃い味",
      "濃い顔族",
      "濃厚牛乳",
      "濃灰色",
      "濃艶な",
      "火病",
      "火病る",
      "灰かぶり姫",
      "灰色立夜鷹",
      "灰被り姫",
      "災害のサイクル",
      "災害サイクル",
      "災害地名",
      "災害大国",
      "炊事兵",
      "炊事場",
      "無一文",
      "無回答",
      "無塩マーガリン",
      "無希釈",
      "無影灯",
      "無意識な行動",
      "無感情",
      "無期懲役刑",
      "無機イオン交換体",
      "無機質な",
      "無気力感",
      "無気力試合",
      "無為に",
      "無益な争い",
      "無益な殺生",
      "無確認",
      "無縁仏",
      "無菌包装米飯",
      "無菌性髄膜炎",
      "無菌性髄膜炎症候群",
      "無観客試合",
      "無記名アンケート",
      "無記名式",
      "無記名裏書",
      "無許可",
      "無許可営業",
      "無課金",
      "無軌条",
      "無軌道な若者",
      "無軌道に走る",
      "無軌道ドライブ",
      "無過失",
      "無音テロップ",
      "無響",
      "焼きしめる",
      "焼きもちを焼く",
      "焼き加減",
      "焼き氷",
      "焼き漬け",
      "焼き物代わり",
      "焼き締める",
      "焼き霜",
      "焼入れ焼き戻し",
      "焼物",
      "焼肉チェーン",
      "焼肉トング",
      "焼肉店",
      "煮え切らない態度",
      "煮え湯を飲まされる",
      "煮切り味醂",
      "煮合わせ",
      "爽快感",
      "狼狽気味",
      "猛スピード",
      "猛反発",
      "猛威を振るう",
      "猛攻",
      "猛暑の候",
      "猛暑日",
      "猛火に包まれる",
      "猛省を促す",
      "猫ちぐら",
      "猫った",
      "猫の額ほど",
      "猫も跨いで通る",
      "猫カフェ",
      "猫バスにのってジブリの森へ",
      "猫ホイホイ",
      "猫撫で声",
      "猫水",
      "猫義義",
      "猫耳",
      "猫被り",
      "猫足バスタブ",
      "猫避けペットボトル",
      "理学士",
      "理解がある",
      "理解し難い",
      "理解に苦しむ",
      "理解のある",
      "理解を求める",
      "理解を深める",
      "理解を示す",
      "理解を絶する",
      "理解を阻む",
      "理解不能",
      "理解者",
      "瑠璃煮",
      "瑠璃茄子",
      "生化学的妊娠",
      "生活に困って",
      "生活に窮する",
      "生活パターン",
      "生活保護費",
      "生活品",
      "生活困窮者",
      "生活基盤",
      "生活必需品",
      "生活感",
      "生活手段",
      "生活援助",
      "生活相談員",
      "生活臭",
      "生活苦",
      "生活防水",
      "生育地",
      "生還者",
      "用意せよ",
      "用意周到",
      "由緒ある",
      "甲斐あって",
      "甲斐がある",
      "甲斐があるというものだ",
      "甲斐が無い",
      "甲斐甲斐しく",
      "申しつかる",
      "申しつけ",
      "申し上げます",
      "申し分なく",
      "申し分の無い",
      "申し分無い",
      "申し訳ありません",
      "申し訳が立たない",
      "申し訳ない",
      "申し訳御座いません",
      "申し訳程度",
      "痒いところに手が届く",
      "皮下腫瘍",
      "盗人猛々しい",
      "盗人算",
      "盛会裏",
      "盛会裡",
      "盛夏の候",
      "目新しさ",
      "目荒し",
      "盲点を突く",
      "盲目的に",
      "相加",
      "相家",
      "相容れる",
      "相小手面",
      "相性占い",
      "相性抜群",
      "相討ちに終わる",
      "相違点",
      "相関性",
      "相関関連",
      "真上",
      "真烏賊",
      "着あわせ",
      "着合わせ",
      "着替え台",
      "矢面に立たされる",
      "矢飛白",
      "知恵を絞る",
      "知恵詣り",
      "知恵貰い",
      "示威行為",
      "礼に始まり礼に終わる",
      "礼をいう",
      "礼儀正しい",
      "礼儀知らず",
      "礼節を欠く",
      "礼節を重んじる",
      "礼節を重んずる",
      "禊月",
      "禍々",
      "秀でた",
      "私怨を晴らす",
      "空カン",
      "空キレイ",
      "空中いす",
      "空中イス",
      "空中回廊",
      "空中散歩",
      "空中栽培",
      "空中椅子",
      "空中機動",
      "空姐",
      "空想上",
      "空槓",
      "空気が凍る",
      "空気が固まる",
      "空気を読む",
      "空気圧",
      "空気嫁",
      "空気感",
      "空狐",
      "空疎な",
      "空自警戒機",
      "空虚な",
      "空虚感",
      "空行為",
      "空車重量",
      "空運株",
      "空運銘柄",
      "空運関連株",
      "空運関連銘柄",
      "空間放射線量",
      "空間除菌",
      "竊鉄の疑い",
      "箒星",
      "籠城",
      "籠城戦",
      "粋がり",
      "粗悪な",
      "粗悪品",
      "精一杯",
      "糠に釘",
      "糠床",
      "納付書",
      "納入先",
      "納刀",
      "納品先",
      "納采の儀",
      "素甘",
      "累積利益",
      "累積損失",
      "累積警告",
      "細工麩",
      "終の住処",
      "終の栖",
      "経営主体",
      "経営会議",
      "経営努力",
      "経営外利益",
      "経営外損失",
      "経営方針",
      "経営概況",
      "経営陣",
      "経営難",
      "経血",
      "経過報告",
      "経過措置",
      "経過時間",
      "経験を踏む",
      "経験知",
      "経験者",
      "経験豊富",
      "絵画展",
      "網羅される",
      "網羅的",
      "総勘定元帳",
      "総括",
      "総括的",
      "総毛立つ",
      "総決算",
      "縫い合わす",
      "縫腋袍",
      "羅患",
      "羅患率",
      "羊羮",
      "美味しいところ",
      "美味しいところを持って行く",
      "美味しい話には裏がある",
      "美味しそう",
      "翻牌",
      "老い先短い",
      "老人福祉施設",
      "老兵は死なずただ去り行くのみ",
      "老婆の休日",
      "老婆心ながら",
      "老後破産",
      "老頭牌",
      "老骨に鞭打つ",
      "耐久試験",
      "耐圧性",
      "耐荷重",
      "耐越水堤防",
      "耕うん機",
      "肉包子",
      "肺カルチノイド",
      "肺ガン",
      "肺出血",
      "背景",
      "背景画像",
      "背景色",
      "能力主義",
      "能力向上",
      "能力考課",
      "能動的",
      "能天気な",
      "能無しの口叩き",
      "脱ぎ出す",
      "脳みそを絞る",
      "脳ドック",
      "脳内お花畑",
      "脳内内定",
      "脳内内定者",
      "脳内花畑",
      "脳力",
      "脳年齢",
      "脳挫滅",
      "脳損傷",
      "脳漿",
      "脳科学",
      "脳筋",
      "脳血流",
      "脳血管",
      "脳裏に浮かぶ",
      "脳裏をよぎる",
      "脳裡に浮かぶ",
      "膵島移植",
      "膵腫瘍",
      "膵臓腫瘍",
      "臈長けた",
      "臈長ける",
      "興亜",
      "良いお年を",
      "良いから",
      "良いことずくめ",
      "良いこと尽くめ",
      "良いところのお嬢さん",
      "良いとこ取り",
      "良いのか",
      "良い人",
      "良い人止まり",
      "良い天気",
      "良い思い",
      "良い悪い",
      "良い所",
      "良い方向",
      "良い方向で",
      "良い方向に向かう",
      "良い方法",
      "良い格好しい",
      "良い歳こいて",
      "良い気になる",
      "良い気味",
      "良い点",
      "良い知らせ",
      "芳志",
      "芳香を放つ",
      "若く見える",
      "若さ故の過ち",
      "若手有望株",
      "若手芸人",
      "若手議員",
      "若気の至り",
      "若番",
      "若番側",
      "若者の何とか離れ",
      "若者層",
      "若者言葉",
      "若返り",
      "苦い経験",
      "苦い薬",
      "苦い言葉",
      "苦い顔",
      "苦み走った",
      "苦手意識",
      "苦玉",
      "苦肝",
      "苦虫を噛み潰したような",
      "苦虫を噛み潰す",
      "英会話スクール",
      "英会話教室",
      "英国国民投票",
      "英国民投票",
      "英国王",
      "英国王室",
      "英国紳士",
      "英学会",
      "英気を養う",
      "英語コンプレックス",
      "英語公用語化",
      "英議会",
      "英銀",
      "荊冠",
      "萌芽のみぎり",
      "萌芽の候",
      "萌芽期",
      "落伍者",
      "落札値",
      "落札者",
      "落石の恐れあり",
      "葬儀会社",
      "蒙が啓かれる",
      "蒼穹",
      "蔑ろ",
      "蔑ろにする",
      "虐げられた",
      "虐げられる",
      "蛍光活性化セルソーティング",
      "蜜柑ジュース",
      "融資契約",
      "血合い肉",
      "衛検済マーク",
      "衰退期",
      "衰退産業",
      "補員",
      "製函",
      "製菓業",
      "要援護者",
      "要網",
      "見えざる",
      "見えない",
      "見えない努力",
      "見える仕組み",
      "見え消し",
      "見かけ",
      "見かけによらず",
      "見かねる",
      "見受けられない",
      "見受けられる",
      "見合わせる",
      "見合わない",
      "見当たらない",
      "見当たり捜査",
      "見得を切る",
      "見掛け倒し",
      "見終わる",
      "規格外",
      "規格外野菜",
      "言いがかり",
      "言いたげ",
      "言い出しっ屁",
      "言い尽くせない",
      "言い得て",
      "言い掛かりをつける",
      "言い換えれば",
      "言い繕う",
      "計画倒れ",
      "計画停電",
      "計画的",
      "計画的犯行",
      "計画答申書",
      "記憶に新しい",
      "記憶消去",
      "訪韓",
      "試合に勝って勝負に負けた",
      "試合に負けて勝負に勝った",
      "試合巧者",
      "試合形式",
      "誘拐犯人",
      "誠意がない",
      "警戒感",
      "警音器吹鳴義務違反",
      "豊胸エクササイズ",
      "貴意に沿う",
      "貴館",
      "買いかぶり過ぎ",
      "買いかぶる",
      "買い上げ",
      "買い被り過ぎ",
      "買い集める",
      "買入れ",
      "贔屓にする",
      "起因する",
      "身内贔屓",
      "身洗い",
      "軒を連ねる",
      "載荷",
      "農ガール",
      "農メン",
      "農林株",
      "農林水産",
      "農林被害",
      "農林銘柄",
      "農林関連株",
      "農林関連銘柄",
      "農水産物",
      "農産品",
      "農畜産業",
      "迎えに行く",
      "迎える正月",
      "迎え梅雨",
      "迎え火",
      "近いうちに",
      "近いうちに解散",
      "近い内",
      "近い将来",
      "近い筋",
      "近づく",
      "近寄り難い",
      "近間",
      "近頃",
      "迷子になる",
      "迷子紐",
      "迷彩服",
      "迷彩柄",
      "迷言",
      "追々",
      "追いかけっこ",
      "追いオリーブオイル",
      "追いコン",
      "追い掛けられる",
      "追い掛ける",
      "追い掛け立直",
      "追い越し違反",
      "追い風に乗る",
      "追い鰹",
      "追儺",
      "追切り",
      "追加",
      "追加公演",
      "追加合格",
      "追加工",
      "追加手配",
      "追加注文",
      "追加緩和",
      "追加証拠金",
      "追善供養",
      "追尾撮影",
      "追弔",
      "追従者",
      "追悼",
      "追悼式",
      "追悼式典",
      "追悼慰霊式",
      "追突事故",
      "追肴",
      "追討ちを掛ける",
      "追証",
      "追走劇",
      "追越禁止",
      "追越車線",
      "追跡",
      "追跡番号",
      "追跡調査",
      "追随を許さない",
      "退き口",
      "退回服貿",
      "退学勧告",
      "退居",
      "退屈凌ぎ",
      "退院時共同指導",
      "退院要求",
      "送行会",
      "逃がさない",
      "遂行する",
      "遅延型アレルギー",
      "遅延型フードアレルギー",
      "遅延型食物アレルギー",
      "遅延証明書",
      "遊休地",
      "遊技業",
      "遊技機",
      "遊撃戦",
      "邁進",
      "邁進する",
      "配信先",
      "配偶者弔慰金",
      "配合物",
      "配架",
      "配活",
      "配管トンネル",
      "配給会社",
      "配給原点",
      "配荷",
      "酔客",
      "酔象",
      "酔象調伏",
      "酩酊者",
      "酸橙",
      "野営地",
      "野外フェス",
      "野外席",
      "野外撮影",
      "野外炊飯",
      "野球投げ",
      "野球漫画",
      "野球賭博",
      "鈴かすてら",
      "銘品",
      "銘打って",
      "銘版",
      "銘銘の",
      "鋭意",
      "鋭意努力",
      "閉創",
      "閉塞感",
      "閉架式",
      "閉栓",
      "閉鎖空間",
      "閉鎖骨折",
      "開架図書館",
      "開架式",
      "開花ピーク",
      "開花時期",
      "開花期間",
      "開花状況",
      "開運巻き寿司",
      "降雨状況",
      "陸サーファー",
      "陸上十種",
      "陸曹長",
      "陸海空",
      "陸自ヘリ",
      "陸運株",
      "陸運銘柄",
      "陸運関連株",
      "陸運関連銘柄",
      "陽イオン交換樹脂",
      "陽圧",
      "陽気な",
      "際限なく",
      "離活",
      "離開",
      "離隔距離",
      "霊異",
      "霊長類最強女子",
      "霊験あらたか",
      "霊験いやちこ",
      "露悪的",
      "露韓",
      "青果市場",
      "青灰色",
      "青雲の志",
      "非アクティブ",
      "非オタ",
      "非上場",
      "非可逆性",
      "非可逆的",
      "非活性化",
      "非観血的",
      "非課税扱い",
      "非貨幣性資産",
      "非関連多角化",
      "頂牌",
      "額賀派",
      "願い事",
      "願ったり",
      "願わくば",
      "類の無い",
      "類を見ない",
      "類似の",
      "類似性",
      "類似語",
      "類友",
      "類型化",
      "飜牌",
      "食いそびれる",
      "食いっ逸れる",
      "食い下げ",
      "食い仕掛け",
      "食い付く",
      "食い入るように",
      "食い縛る",
      "食い込み",
      "食扶持",
      "飢え死",
      "飼育係",
      "飼育放棄",
      "養育放棄",
      "騒音運転等",
      "高位破水",
      "高圧的",
      "高圧的な態度",
      "高圧的態度",
      "高圧線",
      "高圧電線",
      "魅かれる",
      "魅入られる",
      "魔油",
      "魚すき",
      "魚上氷",
      "魴",
      "鯉の滝登り",
      "鯉口を切る",
      "鯉菓子",
      "鱫",
      "鵺のような",
      "鵺的",
      "鶏口牛後",
      "麗春の候",
      "黄医生",
      "黄油",
      "黄色いダイヤ",
      "黄色地",
    ];
  }
}
