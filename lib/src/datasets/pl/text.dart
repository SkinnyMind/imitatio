// ignore_for_file: lines_longer_than_80_chars

import 'package:imitatio/src/datasets/models/text.dart';

class PlTextData extends TextDataset {
  @override
  List<String> alphabet({required bool isLowerCase}) {
    return switch (isLowerCase) {
      true => const [
          "a",
          "ą",
          "b",
          "c",
          "ć",
          "d",
          "e",
          "ę",
          "f",
          "g",
          "h",
          "i",
          "j",
          "k",
          "l",
          "ł",
          "m",
          "n",
          "ń",
          "o",
          "ó",
          "p",
          "r",
          "s",
          "ś",
          "t",
          "u",
          "w",
          "y",
          "z",
          "ź",
          "ż",
        ],
      false => const [
          "A",
          "Ą",
          "B",
          "C",
          "Ć",
          "D",
          "E",
          "Ę",
          "F",
          "G",
          "H",
          "I",
          "J",
          "K",
          "L",
          "Ł",
          "M",
          "N",
          "Ń",
          "O",
          "Ó",
          "P",
          "R",
          "S",
          "Ś",
          "T",
          "U",
          "W",
          "Y",
          "Z",
          "Ź",
          "Ż",
        ],
    };
  }

  @override
  List<String> get answers {
    return const ["Tak", "Nie", "Może"];
  }

  @override
  List<String> get colors {
    return const [
      "Amarantowy",
      "Biały",
      "Brązowy",
      "Czarny",
      "Czerwony",
      "Fioletowy",
      "Granatowy",
      "Hebanowy",
      "Indygo",
      "Jaśminowy",
      "Karmazynowy",
      "Kremowy",
      "Liliowy",
      "Łososiowy",
      "Malachitowy",
      "Niebieski",
      "Oliwkowy",
      "Pomarańczowy",
      "Purpurowy",
      "Różowy",
      "Srebrny",
      "Szary",
      "Szafirowy",
      "Turkusowy",
      "Ultramaryna",
      "Wrzosowy",
      "Złoty",
      "Zielony",
      "Żółty",
    ];
  }

  @override
  List<String> get levels {
    return const [
      "niski",
      "średni",
      "wysoki",
      "bardzo wysoki",
      "ekstremalny",
      "krytyczny",
    ];
  }

  @override
  List<String> get quotes {
    return const [
      "Grzegorz Brzęczyszczykiewicz, Chrząszczyżewoszyce, powiat Łękołody",
      "Nas? Bohaterów? Prądem?!",
      "Ciemność, widzę ciemność, ciemność widzę.",
      "Czy można stąd zadzwonić na miasto? Pytałem się o coś! Małżonka się niepokoi!",
      "Dzień dobry, zastałem Jolkę?",
      "Hej! Nasi tu byli!",
      "Miano nas obudzić bez względu na sytuację polityczną, co mnie obchodzi jakaś tam wasza wojna?",
      "Nie wytrzymam! Permanentna inwigilacja! Nie wytrzymam",
      "Nieźleście nas urządziły, siostry!",
      "Ciemność, ja ciemność widzę!",
      "Parówkowym skrytożercom mówimy stanowcze: NIE!",
      "Sąd sądem, ale sprawiedliwość musi być po naszej stronie.",
      "Ja rozumiem, że wam jest zimno, ale jak jest zima, to musi być zimno!",
      "Przyszłem wcześniej, gdyż nie miałem co robić.",
      "Oczko mu się odlepiło. Temu misiu.",
      "Bardzo niedobre dialogi są. Proszę pana. W ogóle brak akcji jest. Nic się nie dzieje.",
      "Wiktorio, nie kop pana, bo się spocisz.",
      "Z twarzy podobny zupełnie do nikogo.",
      "Konie łby pospuszczać. Czy konie mnie słyszą?!",
      "A ja, jeżely pan pozwoly… z przyjemnościom.",
      "Nie mieszajmy myślowo dwóch różnych systemów walutowych. Nie bądźmy peweksami!",
      "Podstawowe zasady higieny są takie, że jak się wchodzi, to się puka, a pan wchodzi bez pukania z brudną ścierką.",
      "Urodziłem się w Małkini w 1937 roku w lipcu. Znaczy, w połowie lipca. Właściwie w drugiej połowie lipca. Dokładnie 17 lipca.",
      "Pytania są tendencyjne.",
      "Na każdym zebraniu jest taka sytuacja, że ktoś musi zacząć pierwszy.",
      "A w filmie polskim, proszę pana, to jest tak: nuda… Nic się nie dzieje, proszę pana. Nic. Taka, proszę pana… Dialogi niedobre… Bardzo niedobre dialogi są. W ogóle brak akcji jest. Nic się nie dzieje.",
      "Przejdźmy od słów do czynów. Chciałem powiedzieć kilka słów.",
      "Mój mąż? Mój mąż z zawodu jest dyrektorem!",
      "Co ty myślisz, cwaniaczku?! Że z piątego przykazania możesz sobie zrobić spółkę z ograniczoną odpowiedzialnością?!",
      "Tu Ryba, wzywam cię, Akwarium.",
      "Pan komisarz po oddaniu... jedenastu ostrzegawczych strzałów, odpowiedział ogniem. Mógł zabić, a trafił tylko w kolano.",
      "Nie chcem, ale muszem.",
      "O take Polskę zawsze walczyłem.",
      "Odpowiem wymijająco wprost.",
      "Są plusy dodatnie i plusy ujemne.",
      "W każdym społeczeństwie potrzebne są dwie nogi.",
      "Zdrowie wasze w gardło nasze!",
      "Cyfry składają się w liczby, liczby składają się w dziesiątki, dziesiątki składają się w setki, setki składają się w litry",
      "Wszystko jest piękne, ale kiedyś się kończy i potem na scenę wychodzę ja.",
      "Dobry konferansjer jest jak biustonosz - z pozoru niepotrzebny, ale podtrzymuje całość.",
      "Niby każdy wie, że flaszka kiedyś się kończy, ale zawsze jest to zaskoczenie.",
      "Nie sztuka zrobić szpagat, sztuka to jeszcze wstać",
      "Są na niebie i ziemi rzeczy, o których nie śniło się filozofom, a większość z nich jest w Polsce.",
      "Witam państwa w tej oto pięknej, klimatyzowanej sali krakowskiego klubu Rotunda!",
      "Do moich czasów były dwa sposoby, by zafrapować ludzi na scenie: forma bądź treść. Ja wprowadziłem trzeci – brak jednego i drugiego.",
      "W moim wieku różnica między przodem a tyłem jest minimalna.",
      "Jak jest ochota, to i pies kota wyłomota.",
      "Oczywiście. Prawa ręka, to jest ta, niech pan to sobie raz na zawsze zapamięta, prawa ręka to jest ta jak się śpi na brzuchu to od ściany.",
      "U nas ściany się trzymają tylko dzięki zdjęciom z pielgrzymek.",
    ];
  }

  @override
  List<String> get texts {
    return const [
      "Abstrakcją w programowaniu nazywamy pewnego rodzaju uproszczenie rozpatrywanego problemu, polegające na ograniczeniu zakresu cech manipulowanych obiektów wyłącznie do cech kluczowych dla algorytmu, a jednocześnie niezależnych od implementacji.",
      "C++ został zaprojektowany przez Bjarne Stroustrupa jako rozszerzenie języka C o obiektowe mechanizmy abstrakcji danych i silną statyczną kontrolę typów.",
      "C++ – język programowania ogólnego przeznaczenia.",
      "Cel stosowania abstrakcji jest dwojaki: ułatwienie rozwiązania problemu i zwiększenie jego ogólności.",
      "Charakteryzuje się wysoką wydajnością kodu wynikowego, bezpośrednim dostępem do zasobów sprzętowych i funkcji systemowych, łatwością tworzenia i korzystania z bibliotek (napisanych w C++, C lub innych językach), niezależnością od konkretnej platformy sprzętowej lub systemowej (co gwarantuje wysoką przenośność kodów źródłowych) oraz niewielkim środowiskiem uruchomieniowym.",
      "Część współbieżna bazuje na teoretycznym modelu znanym jako Actor model.",
      "Dla obiektów specjalizowanych nie trzeba redefiniować całej funkcjonalności, lecz tylko tę, której nie ma obiekt ogólniejszy.",
      "Dla przykładu, dany program komputerowy może przetwarzać różne zbiory obiektów, jednak wszystkie one mają wspólną cechę - w pewnych warunkach można je traktować jako listę o następujących operacjach: pierwszy(), ostatni() i następny() oraz predykacie czy_ostatni().",
      "Dlatego dla potrzeb fragmentów zajmujących się przetwarzaniem list, wygodnie jest wprowadzić abstrakcję iteratora, który będzie udostępniał te właśnie operacje.",
      "Ericsson i kilka innych firm oferują komercyjną pomoc dla Erlanga.",
      "Erlang został upubliczniony wraz z kodem źródłowym przez firmę Ericsson, aby zapewnić jego niezależność od pojedynczej firmy i zwiększyć popularność języka.",
      "Erlang – język programowania zaprojektowany z myślą o zastosowaniach współbieżnych, a także środowisko uruchomieniowe dla aplikacji w nim napisanych.",
      "Gdy dostęp do wszystkich pól danej klasy jest możliwy wyłącznie poprzez metody, lub inaczej mówiąc: gdy wszystkie pola w klasie znajdują się w sekcji prywatnej lub chronionej, to taką hermetyzację nazywa się hermetyzacją pełną.",
      "Haskell był początkowo intensywnie rozwijany wokół ośrodka University of Glasgow, popularny kompilator tego języka to Glasgow Haskell Compiler (GHC) kompilujący szybki kod maszynowy porównywalny w szybkości wykonania do kodów z GCC (ok. 1,3 razy wolniejszy niż C).",
      "Haskell – czysto funkcyjny język programowania nazwany na cześć Haskella Curry'ego.",
      "Hermetyzacja (inna używana nazwa to enkapsulacja, ang. encapsulation) – jedno z założeń programowania obiektowego.",
      "Hermetyzacja polega na ukrywaniu pewnych danych składowych lub metod obiektów danej klasy tak, aby były one dostępne tylko metodom wewnętrznym danej klasy lub funkcjom zaprzyjaźnionym.",
      "Java jest językiem tworzenia programów źródłowych kompilowanych do kodu bajtowego, czyli postaci wykonywanej przez maszynę wirtualną.",
      "Java – obiektowy język programowania stworzony przez grupę roboczą pod kierunkiem Jamesa Goslinga z firmy Sun Microsystems.",
      "Jego podstawowe koncepcje zostały przejęte z języka Smalltalk (maszyna wirtualna, zarządzanie pamięcią) oraz z języka C++ (duża część składni i słów kluczowych).",
      "Jeśli dzieje się to w czasie działania programu, to nazywa się to późnym wiązaniem lub wiązaniem dynamicznym.",
      "Jeżeli klasa ma za zadanie np. gromadzić zbiór liczb podawanych do niej pojedynczo a następnie, w dowolnym momencie, zwrócić maksimum tej kolekcji, to interfejs w żaden sposób nie narzuca programiście klasy konkretnego sposobu wykonania tej implementacji.",
      "Język C++ jest językiem wieloparadygmatowym.",
      "Język C++ ze względu na bardzo rozbudowaną składnię oraz surowe wymogi w zakresie wydajności kodu stanowi duże wyzwanie dla twórców kompilatorów.",
      "Język cechuje się silnym typowaniem.",
      "Język jest rozprowadzany razem z bibliotekami (m.in. implementacją CORBA, SNMP, HTTP, FTP, TCP/IP, IIOP) oraz rozproszoną bazą danych czasu rzeczywistego (Mnesia) całość jest znana jako Open Telecom Platform (OTP), ponieważ jest najczęściej wykorzystywana w zastosowaniach telekomunikacyjnych.",
      "Język programowania – zbiór zasad określających, kiedy ciąg symboli tworzy program komputerowy oraz jakie obliczenia opisuje.",
      "Język został zaprojektowany pod kątem tworzenia rozproszonych systemów wymagających długotrwałej pracy oraz odporności na awarie.",
      "Język został zaprojektowany przez Joe Armstronga w 1986 roku pracującego w firmie Ericsson.",
      "Języki programowania (w szczególności języki obiektowe) mogą oferować specjalne narzędzia do tworzenia pewnych typów abstrakcji, jednak pojęcie to nie zależy w żaden sposób od konkretnego języka.",
      "Każde uogólnienie problemu mające na celu wyizolowanie jego kluczowych aspektów jest abstrakcją.",
      "Na przykładzie języka C++, klasa może to wewnętrznie wykonywać w oparciu o składową typu std::set jak również std::vector, który każdorazowo przed zwróceniem wartości maksymalnej będzie poddawany sortowaniu.",
      "Największym atutem programowania, projektowania oraz analizy obiektowej jest zgodność takiego podejścia z rzeczywistością – mózg ludzki jest w naturalny sposób najlepiej przystosowany do takiego podejścia przy przetwarzaniu informacji.",
      "Nazwa Erlang została nadana na cześć A. K. Erlanga, choć często bywa również interpretowana jako „Ericsson Language”.",
      "Niektóre języki udostępniają bardziej statyczne (w trakcie kompilacji) rozwiązania polimorfizmu – na przykład szablony i przeciążanie operatorów w C++.",
      "Obiektowy program komputerowy wyrażony jest jako zbiór takich obiektów, komunikujących się pomiędzy sobą w celu wykonywania zadań.",
      "Obsługuje mechanizm hot-swappingu pozwalający na aktualizację kodu aplikacji bez jej zatrzymywania.",
      "Od strony użytkownika klasy sposób implementacji nie powinien być szczególnie interesujący, a co ważniejsze, nie powinien wpływać na sposób używania klasy.",
      "Odzwierciedlają one wspólne cechy obiektów.",
      "Oznacza to, że można w nim stosować jednocześnie różne style programowania, w tym programowanie proceduralne, obiektowe, generyczne, jak również programować na poziomie asemblera.",
      "Początkowo był własnościowym narzędziem tej firmy, lecz w roku 1998 wraz z implementacją i bibliotekami, stał się oprogramowaniem open source.",
      "Podejście to różni się od tradycyjnego programowania proceduralnego, gdzie dane i procedury nie są ze sobą bezpośrednio związane.",
      "Podstawowym obszarem jego zastosowań są aplikacje i systemy operacyjne.",
      "Porządkuje i wspomaga polimorfizm i enkapsulację dzięki umożliwieniu definiowania i tworzenia specjalizowanych obiektów na podstawie bardziej ogólnych.",
      "Posiadając zdefiniowany interfejs programista klasy ma pełną swobodę implementacji mechanizmu, w jaki klasa będzie wykonywała swoje zdanie.",
      "Programowanie funkcyjne – filozofia i metodyka programowania będąca odmianą programowania deklaratywnego, w której funkcje należą do wartości podstawowych, a nacisk kładzie się na wartościowanie (często rekurencyjnych) funkcji, a nie na wykonywanie poleceń.",
      "Programowanie obiektowe (ang. object-oriented programming) – paradygmat programowania, w którym programy definiuje się za pomocą obiektów – elementów łączących stan (czyli dane, nazywane najczęściej polami) i zachowanie (czyli procedury, tu: metody).",
      "Programowanie obiektowe ma ułatwić pisanie, konserwację i wielokrotne użycie programów lub ich fragmentów.",
      "Python – język programowania wysokiego poziomu ogólnego przeznaczenia, o rozbudowanym pakiecie bibliotek standardowych, którego ideą przewodnią jest czytelność i klarowność kodu źródłowego",
      "Referencje i kolekcje obiektów mogą dotyczyć obiektów różnego typu, a wywołanie metody dla referencji spowoduje zachowanie odpowiednie dla pełnego typu obiektu wywoływanego.",
      "Ruby (wym. /ˈru:bi/) – interpretowany, w pełni obiektowy i dynamicznie typowany język programowania stworzony w 1995 roku przez Yukihiro Matsumoto.",
      "Sekwencyjny podzbiór Erlanga jest językiem funkcyjnym z wartościowaniem zachłannym, jednokrotnym przypisaniem oraz dynamicznym typowaniem.",
      "Umożliwia abstrakcję danych oraz stosowanie kilku paradygmatów programowania: proceduralnego, obiektowego i generycznego.",
      "W języku Java deklaracja interfejsu realizującego tę abstrakcję mogłaby wyglądać następująco.",
      "W języku angielskim ruby oznacza rubin.",
      "W tym sensie abstrakcja jest odmianą formalizmu matematycznego.",
      "W typowym przypadku powstają grupy obiektów zwane klasami, oraz grupy klas zwane drzewami.",
      "Zachowanie zgodności z językiem C na poziomie kodu źródłowego pozostaje jednym z podstawowych celów projektowych kolejnych standardów języka.",
    ];
  }

  @override
  List<String> get words {
    return const [
      "absolutnie",
      "aby",
      "abym",
      "abyś",
      "abyśmy",
      "ach",
      "adam",
      "adres",
      "adwokat",
      "agencja",
      "agencji",
      "agent",
      "agenta",
      "agentem",
      "agentów",
      "aha",
      "akcja",
      "akcji",
      "akt",
      "akta",
      "akurat",
      "alarm",
      "albo",
      "ale",
      "ależ",
      "alfa",
      "alkohol",
      "amen",
      "amerykański",
      "angielski",
      "ani",
      "anioł",
      "aparat",
      "aresztowany",
      "aresztować",
      "armia",
      "armii",
      "armię",
      "artykuł",
      "atak",
      "ataku",
      "auta",
      "auto",
      "autobus",
      "autobusu",
      "aż",
      "babci",
      "babcia",
      "babciu",
      "baby",
      "badania",
      "badanie",
      "badań",
      "bal",
      "banda",
      "bani",
      "bania",
      "bank",
      "banku",
      "bar",
      "bardziej",
      "bardzo",
      "baru",
      "barze",
      "baw",
      "bawi",
      "bawisz",
      "bawić",
      "baza",
      "bazie",
      "bazy",
      "bać",
      "bałagan",
      "bałem",
      "be",
      "bedzie",
      "bez",
      "beze",
      "bezpieczeństwa",
      "bezpieczeństwo",
      "bezpieczna",
      "bezpieczne",
      "bezpieczni",
      "bezpiecznie",
      "bezpieczny",
      "bezpośrednio",
      "białe",
      "białego",
      "biały",
      "białych",
      "białym",
      "biedny",
      "biegiem",
      "biegnij",
      "bierz",
      "bierze",
      "bierzemy",
      "bierzesz",
      "big",
      "bije",
      "bilet",
      "bilety",
      "bingo",
      "biorą",
      "biorąc",
      "biorę",
      "bitwy",
      "biura",
      "biurku",
      "biuro",
      "biurze",
      "biznes",
      "bić",
      "bla",
      "blisko",
      "bliżej",
      "blond",
      "blondynka",
      "bo",
      "boga",
      "bogaty",
      "bogiem",
      "bogowie",
      "bogu",
      "bogów",
      "bohater",
      "bohatera",
      "bohaterem",
      "boi",
      "boisz",
      "boją",
      "boję",
      "bok",
      "boku",
      "bolało",
      "boli",
      "bomba",
      "bomby",
      "bombę",
      "bond",
      "boską",
      "boże",
      "brachu",
      "braci",
      "bracia",
      "bracie",
      "brad",
      "brak",
      "brakowało",
      "brakuje",
      "bramę",
      "brat",
      "brata",
      "bratem",
      "brawo",
      "brać",
      "brał",
      "broni",
      "bronią",
      "bronić",
      "broń",
      "brzmi",
      "brzuch",
      "budynek",
      "budynku",
      "bum",
      "burza",
      "but",
      "butelkę",
      "buty",
      "butów",
      "by",
      "bycia",
      "bycie",
      "byle",
      "byli",
      "byliście",
      "byliśmy",
      "bym",
      "bywa",
      "być",
      "był",
      "była",
      "byłaby",
      "byłam",
      "byłaś",
      "byłby",
      "byłbym",
      "byłbyś",
      "byłem",
      "byłeś",
      "było",
      "byłoby",
      "były",
      "byś",
      "byście",
      "byśmy",
      "bzdur",
      "bzdura",
      "bzdury",
      "bóg",
      "bój",
      "ból",
      "bólu",
      "bądź",
      "bądźcie",
      "będzie",
      "będziecie",
      "będziemy",
      "będziesz",
      "będą",
      "będąc",
      "będę",
      "błagam",
      "błąd",
      "błędy",
      "cała",
      "całe",
      "całego",
      "całej",
      "całkiem",
      "całkowicie",
      "całość",
      "cały",
      "całym",
      "całą",
      "cel",
      "cele",
      "celem",
      "celi",
      "celu",
      "cena",
      "centrum",
      "centów",
      "ceny",
      "cenę",
      "chce",
      "chcecie",
      "chcemy",
      "chcesz",
      "chciał",
      "chciała",
      "chciałabym",
      "chciałabyś",
      "chciałam",
      "chciałaś",
      "chciałby",
      "chciałbym",
      "chciałbyś",
      "chciałem",
      "chciałeś",
      "chcieli",
      "chcielibyśmy",
      "chcieliśmy",
      "chcieć",
      "chcą",
      "chcę",
      "chleb",
      "chociaż",
      "chodzi",
      "chodzisz",
      "chodzić",
      "chodził",
      "chodziło",
      "chodzą",
      "chodzę",
      "chodź",
      "chodźcie",
      "chodźmy",
      "chora",
      "chore",
      "choroba",
      "choroby",
      "chorobę",
      "chory",
      "choć",
      "choćby",
      "chronić",
      "chryste",
      "chrystusa",
      "chwila",
      "chwile",
      "chwileczkę",
      "chwili",
      "chwilkę",
      "chwilą",
      "chwilę",
      "chyba",
      "chętnie",
      "chłopak",
      "chłopaka",
      "chłopaki",
      "chłopakiem",
      "chłopaków",
      "chłopca",
      "chłopcem",
      "chłopcy",
      "chłopcze",
      "chłopców",
      "chłopie",
      "ci",
      "ciasto",
      "ciał",
      "ciała",
      "ciało",
      "cicho",
      "ciebie",
      "ciekawe",
      "ciele",
      "ciemno",
      "ciemności",
      "ciepło",
      "cierpi",
      "cierpię",
      "cieszy",
      "cieszyć",
      "cieszę",
      "cień",
      "cios",
      "cisza",
      "ciuchy",
      "ciągle",
      "ciągu",
      "ciąży",
      "cię",
      "cięcie",
      "ciężarówki",
      "ciężka",
      "ciężki",
      "ciężkie",
      "ciężko",
      "ciśnienie",
      "cm",
      "co",
      "codziennie",
      "cofnij",
      "cofnąć",
      "cokolwiek",
      "com",
      "coraz",
      "cos",
      "coś",
      "cud",
      "cudem",
      "cudowna",
      "cudowne",
      "cudownie",
      "cudowny",
      "czarna",
      "czarne",
      "czarnego",
      "czarny",
      "czarnych",
      "czarnym",
      "czarną",
      "czas",
      "czasach",
      "czasami",
      "czasem",
      "czasie",
      "czasu",
      "czasy",
      "czasów",
      "czego",
      "czegoś",
      "czek",
      "czeka",
      "czekaj",
      "czekajcie",
      "czekają",
      "czekam",
      "czekamy",
      "czekasz",
      "czekać",
      "czekał",
      "czekałem",
      "czemu",
      "czerwone",
      "czerwony",
      "cześć",
      "czoła",
      "czołem",
      "czoło",
      "czterech",
      "cztery",
      "czuje",
      "czujesz",
      "czuję",
      "czuć",
      "czuł",
      "czułam",
      "czułem",
      "czwartek",
      "czy",
      "czyli",
      "czym",
      "czymś",
      "czyni",
      "czynienia",
      "czysta",
      "czyste",
      "czysto",
      "czysty",
      "czytam",
      "czytasz",
      "czytać",
      "czytałem",
      "czyż",
      "czyżby",
      "często",
      "części",
      "częściej",
      "częścią",
      "część",
      "członkiem",
      "członków",
      "człowiek",
      "człowieka",
      "człowiekiem",
      "człowieku",
      "córka",
      "córki",
      "córką",
      "córkę",
      "cóż",
      "da",
      "dach",
      "dachu",
      "dadzą",
      "daj",
      "dajcie",
      "daje",
      "dajesz",
      "dają",
      "daję",
      "dala",
      "dalej",
      "daleka",
      "daleko",
      "dali",
      "dam",
      "damy",
      "dan",
      "dane",
      "daniel",
      "danny",
      "danych",
      "dar",
      "darmo",
      "dasz",
      "data",
      "dawaj",
      "dawać",
      "dawna",
      "dawno",
      "dać",
      "dał",
      "dała",
      "dałam",
      "dałem",
      "dałeś",
      "dało",
      "dbam",
      "decyzja",
      "decyzje",
      "decyzji",
      "decyzję",
      "delikatnie",
      "derek",
      "deszcz",
      "detektyw",
      "detektywie",
      "diabeł",
      "diabli",
      "diabła",
      "dla",
      "dlaczego",
      "dlatego",
      "dna",
      "dni",
      "dnia",
      "dnie",
      "dniu",
      "dno",
      "do",
      "dobra",
      "dobranoc",
      "dobre",
      "dobrego",
      "dobrej",
      "dobro",
      "dobry",
      "dobrych",
      "dobrym",
      "dobrze",
      "dobrzy",
      "dobrą",
      "doceniam",
      "doczekać",
      "dodatkowe",
      "dodać",
      "dojdzie",
      "dojść",
      "dokonać",
      "dokończyć",
      "doktor",
      "doktora",
      "doktorku",
      "doktorze",
      "dokumenty",
      "dokąd",
      "dokładnie",
      "dolarów",
      "dolców",
      "dole",
      "dom",
      "domem",
      "domu",
      "domy",
      "domyślam",
      "domów",
      "dookoła",
      "dopasowane",
      "dopiero",
      "doprawdy",
      "dopóki",
      "dorwać",
      "doskonale",
      "doskonałe",
      "doskonały",
      "dostaje",
      "dostajesz",
      "dostaję",
      "dostaliśmy",
      "dostanie",
      "dostaniemy",
      "dostaniesz",
      "dostanę",
      "dostarczyć",
      "dostać",
      "dostał",
      "dostała",
      "dostałam",
      "dostałaś",
      "dostałem",
      "dostałeś",
      "dostęp",
      "dostępu",
      "dosyć",
      "doszło",
      "dosłownie",
      "dotknąć",
      "dotrze",
      "dotrzemy",
      "dotrzeć",
      "dotyczy",
      "dotykaj",
      "dotykać",
      "dotąd",
      "dowie",
      "dowiedział",
      "dowiedziałem",
      "dowiedzieć",
      "dowiem",
      "dowiemy",
      "dowiesz",
      "dowodu",
      "dowody",
      "dowodzi",
      "dowodów",
      "dowód",
      "dowódca",
      "dołączyć",
      "doświadczenia",
      "doświadczenie",
      "dość",
      "dr",
      "draniu",
      "drań",
      "drinka",
      "drobne",
      "drodze",
      "droga",
      "drogi",
      "drogie",
      "drogą",
      "drogę",
      "druga",
      "drugi",
      "drugie",
      "drugiego",
      "drugiej",
      "drugim",
      "drugą",
      "drużyna",
      "drużynie",
      "drużyny",
      "drzewa",
      "drzewo",
      "drzwi",
      "drzwiach",
      "drzwiami",
      "duch",
      "ducha",
      "duchy",
      "dumna",
      "dumny",
      "dusza",
      "dusze",
      "duszy",
      "duszę",
      "duża",
      "duże",
      "dużego",
      "dużo",
      "duży",
      "dużym",
      "dużą",
      "dwa",
      "dwadzieścia",
      "dwaj",
      "dwie",
      "dwoje",
      "dwoma",
      "dwóch",
      "dym",
      "dyrektor",
      "dziadek",
      "dziadka",
      "dziadku",
      "działa",
      "działają",
      "działania",
      "działać",
      "działo",
      "dzieci",
      "dzieciak",
      "dzieciaka",
      "dzieciaki",
      "dzieciaku",
      "dzieciaków",
      "dziecinko",
      "dzieciom",
      "dziecka",
      "dzieckiem",
      "dziecko",
      "dziecku",
      "dzieje",
      "dzielić",
      "dzielnicy",
      "dziennie",
      "dziesięciu",
      "dziesięć",
      "dziewczyn",
      "dziewczyna",
      "dziewczynie",
      "dziewczynka",
      "dziewczynki",
      "dziewczynką",
      "dziewczynkę",
      "dziewczyno",
      "dziewczyny",
      "dziewczyną",
      "dziewczynę",
      "dziewczęta",
      "dziewięć",
      "dziećmi",
      "dzieła",
      "dzieło",
      "dzień",
      "dzisiaj",
      "dzisiejszego",
      "dzisiejszy",
      "dziury",
      "dziurę",
      "dziwna",
      "dziwne",
      "dziwnego",
      "dziwnie",
      "dziwny",
      "dzięki",
      "dziękuje",
      "dziękujemy",
      "dziękuję",
      "dziś",
      "dzwoni",
      "dzwonisz",
      "dzwonić",
      "dzwonię",
      "dzwonił",
      "dzwoniła",
      "dzwoniłem",
      "dzwoń",
      "dół",
      "dłoni",
      "dłonie",
      "dłoń",
      "dług",
      "długa",
      "długi",
      "długie",
      "długo",
      "dłużej",
      "dźwięk",
      "dżungli",
      "ego",
      "ej",
      "ekipa",
      "ekstra",
      "emerytura",
      "emeryturę",
      "energii",
      "europejski",
      "faceci",
      "facet",
      "faceta",
      "facetem",
      "facetów",
      "fajna",
      "fajne",
      "fajnie",
      "fajny",
      "fakt",
      "fakty",
      "faktycznie",
      "fantastycznie",
      "film",
      "filmie",
      "filmu",
      "filmy",
      "filmów",
      "firma",
      "firmie",
      "firmy",
      "firmę",
      "floty",
      "form",
      "forma",
      "formie",
      "forsa",
      "forsy",
      "forsę",
      "frank",
      "fred",
      "funtów",
      "gada",
      "gadaj",
      "gadasz",
      "gadać",
      "gapisz",
      "garażu",
      "gardło",
      "garnitur",
      "gary",
      "garść",
      "gaz",
      "gazety",
      "gazu",
      "gdy",
      "gdyby",
      "gdybym",
      "gdybyś",
      "gdybyśmy",
      "gdyż",
      "gdzie",
      "gdziekolwiek",
      "gdzieś",
      "gejem",
      "generale",
      "generał",
      "generała",
      "geniuszem",
      "getto",
      "glina",
      "gliny",
      "gliną",
      "gniew",
      "go",
      "godzin",
      "godzina",
      "godzinie",
      "godziny",
      "godzinę",
      "gorsze",
      "gorzej",
      "gorąca",
      "gorące",
      "gorąco",
      "gotowa",
      "gotowe",
      "gotowi",
      "gotowy",
      "gotów",
      "gości",
      "gościa",
      "goście",
      "gościem",
      "gościu",
      "gość",
      "gra",
      "grają",
      "gram",
      "gramy",
      "grane",
      "granicy",
      "granicę",
      "grasz",
      "gratulacje",
      "gratuluję",
      "grać",
      "grał",
      "grałem",
      "grozi",
      "gruby",
      "grupa",
      "grupy",
      "grupę",
      "gry",
      "grze",
      "grzeczny",
      "grę",
      "guzik",
      "gwiazd",
      "gwiazda",
      "gwiazdy",
      "gwiazdą",
      "góra",
      "góry",
      "górze",
      "górę",
      "głodna",
      "głodny",
      "głodu",
      "głos",
      "głosu",
      "głosy",
      "głowa",
      "głowie",
      "głowy",
      "głową",
      "głowę",
      "głośniej",
      "głośno",
      "głupi",
      "głupia",
      "głupie",
      "głupiego",
      "głupio",
      "głównej",
      "głównie",
      "główny",
      "głównym",
      "głęboko",
      "ha",
      "halo",
      "hasło",
      "hałas",
      "hej",
      "helikopter",
      "herbaty",
      "herbatę",
      "historia",
      "historie",
      "historii",
      "historię",
      "hmm",
      "honor",
      "hotel",
      "hotelu",
      "humoru",
      "ich",
      "idealna",
      "idealne",
      "idealnie",
      "idealny",
      "idzie",
      "idziecie",
      "idziemy",
      "idziesz",
      "idą",
      "idę",
      "idź",
      "idźcie",
      "ile",
      "ilość",
      "ilu",
      "im",
      "imienia",
      "imieniem",
      "imieniu",
      "imię",
      "imperium",
      "impreza",
      "imprezę",
      "inaczej",
      "indziej",
      "info",
      "informacje",
      "informacji",
      "informacją",
      "informację",
      "inna",
      "inne",
      "innego",
      "innej",
      "inni",
      "inny",
      "innych",
      "innym",
      "innymi",
      "inną",
      "inspektorze",
      "interes",
      "interesach",
      "interesuje",
      "interesujące",
      "interesy",
      "interesów",
      "istnieje",
      "istnieją",
      "iść",
      "iż",
      "ja",
      "jacy",
      "jacyś",
      "jadą",
      "jadę",
      "jaj",
      "jaja",
      "jajka",
      "jak",
      "jaka",
      "jakaś",
      "jakby",
      "jakbym",
      "jakbyś",
      "jaki",
      "jakich",
      "jakichś",
      "jakie",
      "jakiego",
      "jakiegoś",
      "jakiej",
      "jakiejś",
      "jakieś",
      "jakim",
      "jakimś",
      "jakiś",
      "jako",
      "jakoś",
      "jakość",
      "jaką",
      "jakąś",
      "jasna",
      "jasne",
      "jasno",
      "jazda",
      "jazdy",
      "je",
      "jechać",
      "jeden",
      "jedna",
      "jednak",
      "jednakże",
      "jednego",
      "jednej",
      "jedno",
      "jednocześnie",
      "jednostka",
      "jednostki",
      "jednym",
      "jedną",
      "jedyna",
      "jedyne",
      "jedynie",
      "jedyny",
      "jedynym",
      "jedyną",
      "jedz",
      "jedzenia",
      "jedzenie",
      "jedzie",
      "jedziemy",
      "jedziesz",
      "jedź",
      "jedźmy",
      "jego",
      "jej",
      "jem",
      "jemu",
      "jest",
      "jestem",
      "jesteś",
      "jesteście",
      "jesteśmy",
      "jeszcze",
      "jezu",
      "jezus",
      "jezusa",
      "jeśli",
      "jeść",
      "jeździ",
      "jeździć",
      "jeżeli",
      "junior",
      "jutra",
      "jutro",
      "już",
      "ją",
      "język",
      "języka",
      "kamera",
      "kamery",
      "kamerę",
      "kamień",
      "kanał",
      "kapelusz",
      "kapitan",
      "kapitana",
      "kapitanem",
      "kapitanie",
      "kara",
      "karetkę",
      "karierę",
      "karta",
      "karty",
      "kartę",
      "karę",
      "kasa",
      "kasy",
      "kasę",
      "kawa",
      "kawał",
      "kawałek",
      "kawałki",
      "kawałków",
      "kawy",
      "kawę",
      "kazał",
      "kazałem",
      "każ",
      "każda",
      "każde",
      "każdego",
      "każdej",
      "każdemu",
      "każdy",
      "każdym",
      "każdą",
      "każe",
      "każę",
      "kiedy",
      "kiedykolwiek",
      "kiedyś",
      "kiepski",
      "kiepsko",
      "kierowca",
      "kieruje",
      "kierunku",
      "kieszeni",
      "kilka",
      "kilku",
      "kilo",
      "kilometrów",
      "kim",
      "kimkolwiek",
      "kimś",
      "kina",
      "kinomania",
      "klasa",
      "klasie",
      "klasy",
      "klatce",
      "klient",
      "klienta",
      "klientów",
      "klub",
      "klubie",
      "klubu",
      "klucz",
      "klucze",
      "kluczyki",
      "km",
      "kobiecie",
      "kobiet",
      "kobieta",
      "kobietami",
      "kobiety",
      "kobietą",
      "kobietę",
      "kocha",
      "kochają",
      "kocham",
      "kochamy",
      "kochana",
      "kochanie",
      "kochany",
      "kochasz",
      "kochać",
      "kochał",
      "kochałem",
      "kod",
      "kogo",
      "kogoś",
      "kolacja",
      "kolacji",
      "kolację",
      "kolana",
      "koledzy",
      "kolega",
      "kolego",
      "kolei",
      "kolej",
      "kolejna",
      "kolejne",
      "kolejnego",
      "kolejny",
      "kolejną",
      "kolesia",
      "koleś",
      "kolor",
      "komandorze",
      "kompletnie",
      "komputer",
      "komputera",
      "komu",
      "komuś",
      "komórki",
      "komórkę",
      "koncert",
      "konia",
      "konie",
      "koniec",
      "konieczne",
      "konta",
      "kontakt",
      "kontaktu",
      "konto",
      "kontrakt",
      "kontrola",
      "kontroli",
      "kontrolować",
      "kontrolą",
      "kontrolę",
      "kontynuować",
      "kopię",
      "korekta",
      "koszmar",
      "kosztuje",
      "koszulę",
      "kot",
      "kota",
      "kotku",
      "koło",
      "koń",
      "końca",
      "końcu",
      "kończy",
      "kończyć",
      "kości",
      "kościele",
      "kościoła",
      "kościół",
      "kość",
      "kradzież",
      "kraj",
      "kraju",
      "krew",
      "krok",
      "krwi",
      "krwią",
      "krzesło",
      "krzyczeć",
      "krzywdy",
      "król",
      "króla",
      "królem",
      "królestwo",
      "królowa",
      "królowej",
      "królową",
      "krótkie",
      "krótko",
      "kręci",
      "ksiądz",
      "książce",
      "książek",
      "książka",
      "książki",
      "książkę",
      "książę",
      "księcia",
      "księżniczka",
      "księżyc",
      "księżyca",
      "kto",
      "ktokolwiek",
      "ktoś",
      "która",
      "które",
      "którego",
      "któregoś",
      "której",
      "któremu",
      "który",
      "których",
      "którym",
      "którymi",
      "któryś",
      "którzy",
      "którą",
      "kuchni",
      "kula",
      "kule",
      "kuli",
      "kumpel",
      "kumpla",
      "kumple",
      "kumplem",
      "kup",
      "kupa",
      "kupić",
      "kupię",
      "kupił",
      "kupiłem",
      "kupy",
      "kupę",
      "kurczaka",
      "kurcze",
      "kurczę",
      "kurde",
      "kurs",
      "kuzyn",
      "kwestia",
      "kwiaty",
      "kółko",
      "kłamać",
      "kłamie",
      "kłamiesz",
      "kłamstwa",
      "kłamstwo",
      "kłopot",
      "kłopoty",
      "kłopotów",
      "la",
      "laboratorium",
      "lada",
      "lady",
      "las",
      "laska",
      "laski",
      "laskę",
      "lasu",
      "lat",
      "lata",
      "latach",
      "latać",
      "lato",
      "leci",
      "lecieć",
      "lecimy",
      "lecz",
      "lecę",
      "ledwie",
      "ledwo",
      "legenda",
      "lekarstwa",
      "lekarstwo",
      "lekarz",
      "lekarza",
      "lekarze",
      "lekarzem",
      "lekcje",
      "lekcji",
      "leki",
      "lekko",
      "lepiej",
      "lepsza",
      "lepsze",
      "lepszego",
      "lepszy",
      "lepszym",
      "lepszą",
      "lesie",
      "lewej",
      "lewo",
      "leć",
      "leżeć",
      "leży",
      "liceum",
      "licha",
      "liczba",
      "liczby",
      "liczy",
      "liczyć",
      "liczę",
      "linia",
      "linie",
      "linii",
      "linię",
      "lisa",
      "list",
      "lista",
      "listy",
      "listę",
      "litości",
      "litość",
      "liście",
      "lodu",
      "lody",
      "lord",
      "los",
      "lot",
      "lotnisko",
      "lotnisku",
      "lotu",
      "lub",
      "lubi",
      "lubisz",
      "lubią",
      "lubię",
      "lubił",
      "lubiłem",
      "lud",
      "ludzi",
      "ludziach",
      "ludzie",
      "ludziom",
      "ludzkie",
      "ludzkiej",
      "ludzkości",
      "ludźmi",
      "lód",
      "ma",
      "macie",
      "magazyn",
      "magii",
      "major",
      "majorze",
      "mają",
      "mając",
      "mam",
      "mama",
      "mamie",
      "mamo",
      "mamusia",
      "mamusiu",
      "mamy",
      "mamą",
      "mamę",
      "mapę",
      "martw",
      "martwa",
      "martwe",
      "martwego",
      "martwi",
      "martwisz",
      "martwić",
      "martwię",
      "martwy",
      "martwych",
      "marty",
      "mary",
      "marzenia",
      "marzenie",
      "maskę",
      "masz",
      "maszyna",
      "maszyny",
      "matce",
      "materiał",
      "matka",
      "matki",
      "matko",
      "matką",
      "matkę",
      "mać",
      "mała",
      "małe",
      "małego",
      "małej",
      "mało",
      "mały",
      "małych",
      "małym",
      "małą",
      "małżeństwa",
      "małżeństwem",
      "małżeństwo",
      "mecz",
      "meczu",
      "mego",
      "metra",
      "metrów",
      "mi",
      "miasta",
      "miasto",
      "miał",
      "miała",
      "miałabym",
      "miałam",
      "miałaś",
      "miałby",
      "miałbym",
      "miałbyś",
      "miałem",
      "miałeś",
      "miało",
      "miały",
      "miecz",
      "miej",
      "miejmy",
      "miejsc",
      "miejsca",
      "miejscach",
      "miejsce",
      "miejscem",
      "miejscu",
      "mieli",
      "mieliście",
      "mieliśmy",
      "miesiąc",
      "miesiąca",
      "miesiące",
      "miesiącu",
      "miesięcy",
      "mieszka",
      "mieszkam",
      "mieszkania",
      "mieszkanie",
      "mieszkaniu",
      "mieszkasz",
      "mieszkać",
      "mieszkał",
      "miewa",
      "mieć",
      "mieście",
      "mil",
      "mile",
      "mili",
      "milion",
      "miliona",
      "miliony",
      "milionów",
      "mimo",
      "minie",
      "minut",
      "minuty",
      "minutę",
      "minął",
      "minęło",
      "misja",
      "misji",
      "misję",
      "mistrz",
      "mistrza",
      "mistrzem",
      "mistrzu",
      "między",
      "mięsa",
      "mięso",
      "miła",
      "miłe",
      "miłego",
      "miłej",
      "miło",
      "miłości",
      "miłością",
      "miłość",
      "miły",
      "mleko",
      "mnie",
      "mniej",
      "mnóstwo",
      "mną",
      "moc",
      "mocniej",
      "mocno",
      "mocy",
      "model",
      "mogli",
      "mogliby",
      "moglibyśmy",
      "mogliśmy",
      "mogą",
      "mogę",
      "mogła",
      "mogłaby",
      "mogłabym",
      "mogłabyś",
      "mogłam",
      "mogłaś",
      "mogłem",
      "mogłeś",
      "mogło",
      "mogłoby",
      "mogły",
      "moi",
      "moich",
      "moim",
      "moimi",
      "moja",
      "moje",
      "mojego",
      "mojej",
      "mojemu",
      "moją",
      "momencie",
      "moment",
      "momentu",
      "morderca",
      "mordercy",
      "mordercą",
      "morderstwa",
      "morderstwo",
      "morza",
      "morze",
      "morzu",
      "most",
      "motelu",
      "mowa",
      "mowy",
      "może",
      "możecie",
      "możemy",
      "możesz",
      "możliwe",
      "możliwości",
      "możliwość",
      "można",
      "mu",
      "musi",
      "musiał",
      "musiała",
      "musiałam",
      "musiałaś",
      "musiałem",
      "musiałeś",
      "musiało",
      "musicie",
      "musieli",
      "musieliśmy",
      "musimy",
      "musisz",
      "muszą",
      "muszę",
      "muzeum",
      "muzyka",
      "muzyki",
      "muzykę",
      "my",
      "mylisz",
      "mylę",
      "myśl",
      "myślał",
      "myślała",
      "myślałam",
      "myślałaś",
      "myślałem",
      "myślałeś",
      "myśleli",
      "myśleliśmy",
      "myśleć",
      "myśli",
      "myślicie",
      "myślimy",
      "myślisz",
      "myślą",
      "myślę",
      "móc",
      "mógł",
      "mógłby",
      "mógłbym",
      "mógłbyś",
      "mój",
      "mów",
      "mówi",
      "mówicie",
      "mówili",
      "mówimy",
      "mówisz",
      "mówią",
      "mówiąc",
      "mówić",
      "mówię",
      "mówił",
      "mówiła",
      "mówiłam",
      "mówiłaś",
      "mówiłem",
      "mówiłeś",
      "mózg",
      "mózgu",
      "mądry",
      "mąż",
      "męża",
      "mężczyzn",
      "mężczyzna",
      "mężczyzny",
      "mężczyzną",
      "mężczyznę",
      "mężczyźni",
      "mężem",
      "młoda",
      "młodego",
      "młody",
      "młodych",
      "młodzi",
      "młodą",
      "na",
      "nad",
      "nadaje",
      "nadal",
      "nadchodzi",
      "nadchodzą",
      "nadejdzie",
      "nadszedł",
      "nadziei",
      "nadzieja",
      "nadzieje",
      "nadzieję",
      "nagle",
      "nagranie",
      "nagrodę",
      "najbardziej",
      "najgorsze",
      "najgorszy",
      "najlepiej",
      "najlepsza",
      "najlepsze",
      "najlepszego",
      "najlepszy",
      "najlepszych",
      "najlepszym",
      "najlepszą",
      "najmniej",
      "najpierw",
      "najszybciej",
      "najważniejsze",
      "największe",
      "największy",
      "największym",
      "największą",
      "najwyraźniej",
      "najwyższy",
      "nakaz",
      "należy",
      "należą",
      "nam",
      "nami",
      "namierzyć",
      "napad",
      "napijesz",
      "napisane",
      "napisać",
      "napisał",
      "napisałem",
      "napisy",
      "napisów",
      "napić",
      "naprawdę",
      "naprawić",
      "naprzód",
      "nareszcie",
      "narkotyki",
      "narkotyków",
      "nas",
      "nasi",
      "nastroju",
      "nastąpi",
      "następna",
      "następne",
      "następnego",
      "następnej",
      "następnie",
      "następny",
      "następnym",
      "następną",
      "nasz",
      "nasza",
      "nasze",
      "naszego",
      "naszej",
      "naszych",
      "naszym",
      "naszymi",
      "naszą",
      "naturalnie",
      "natury",
      "natychmiast",
      "nauczysz",
      "nauczyć",
      "nauczył",
      "nauczyłem",
      "nauczyłeś",
      "nauka",
      "nauki",
      "nawet",
      "nawzajem",
      "nazwa",
      "nazwać",
      "nazwał",
      "nazwiska",
      "nazwisko",
      "nazwę",
      "nazywa",
      "nazywają",
      "nazywam",
      "nazywamy",
      "nazywasz",
      "nazywać",
      "nazywał",
      "nerwy",
      "niby",
      "nic",
      "nich",
      "niczego",
      "niczym",
      "nie",
      "nieba",
      "niebezpieczeństwie",
      "niebezpieczeństwo",
      "niebezpieczna",
      "niebezpieczne",
      "niebezpieczny",
      "niebie",
      "niebieski",
      "niebieskie",
      "niebo",
      "niech",
      "nieco",
      "niedaleko",
      "niedawno",
      "niedobrze",
      "niedorzeczne",
      "niedzielę",
      "niedługo",
      "niego",
      "niej",
      "niektóre",
      "niektórych",
      "niektórzy",
      "nielegalne",
      "niemal",
      "niemożliwe",
      "nienawidzi",
      "nienawidzisz",
      "nienawidzę",
      "nieprawda",
      "nieprawdaż",
      "niesamowicie",
      "niesamowita",
      "niesamowite",
      "niesamowity",
      "niespodzianka",
      "niespodziankę",
      "niestety",
      "nieważne",
      "niewiarygodne",
      "niewiele",
      "niewinny",
      "niezbyt",
      "niezupełnie",
      "niezwykle",
      "niezwykłe",
      "niezła",
      "niezłe",
      "niezły",
      "niezłą",
      "nieźle",
      "nigdy",
      "nigdzie",
      "nikim",
      "nikogo",
      "nikomu",
      "nikt",
      "nim",
      "nimi",
      "nisko",
      "nią",
      "niż",
      "niżej",
      "no",
      "noc",
      "nocy",
      "nocą",
      "noga",
      "nogach",
      "nogami",
      "nogi",
      "nogę",
      "normalne",
      "normalnie",
      "normalny",
      "nos",
      "nosa",
      "nosi",
      "nosisz",
      "nosić",
      "noszę",
      "nowa",
      "nowe",
      "nowego",
      "nowej",
      "nowo",
      "nowy",
      "nowych",
      "nowym",
      "nową",
      "nożem",
      "nr",
      "numer",
      "numeru",
      "numery",
      "nóg",
      "nóż",
      "oba",
      "obaj",
      "obaw",
      "obawiam",
      "obchodzi",
      "obcy",
      "obcych",
      "obecnie",
      "obecności",
      "obecność",
      "obejrzeć",
      "obiad",
      "obie",
      "obiecaj",
      "obiecał",
      "obiecałem",
      "obiecałeś",
      "obiecuję",
      "obiekt",
      "oboje",
      "obok",
      "obowiązek",
      "obowiązki",
      "obozie",
      "obozu",
      "obraz",
      "obrazy",
      "obrony",
      "obrzydliwe",
      "obserwować",
      "obszar",
      "obu",
      "obudzić",
      "obudź",
      "oby",
      "obóz",
      "ocalić",
      "ocean",
      "och",
      "ochoty",
      "ochotę",
      "ochrona",
      "ochrony",
      "ochronę",
      "oczach",
      "oczami",
      "oczekuje",
      "oczekuję",
      "oczu",
      "oczy",
      "oczywiste",
      "oczywiście",
      "od",
      "odbiera",
      "odbierz",
      "odbiór",
      "odbiło",
      "odchodzę",
      "odchodź",
      "odciski",
      "oddaj",
      "oddam",
      "oddać",
      "oddech",
      "oddychaj",
      "oddychać",
      "oddział",
      "oddzwonię",
      "ode",
      "odebrać",
      "odejdę",
      "odejdź",
      "odejść",
      "odeszła",
      "odkąd",
      "odległość",
      "odnaleźć",
      "odnośnie",
      "odpocząć",
      "odpowiada",
      "odpowiedni",
      "odpowiedz",
      "odpowiedzi",
      "odpowiedzialność",
      "odpowiedzialny",
      "odpowiedzieć",
      "odpowiedź",
      "odpuść",
      "odrobinę",
      "odsunąć",
      "odsuń",
      "odszedł",
      "odwagi",
      "odwagę",
      "odwal",
      "odwiedzić",
      "odwiedź",
      "odwrócić",
      "odwrót",
      "odwróć",
      "odzyskać",
      "odłóż",
      "ofertę",
      "ofiar",
      "ofiara",
      "ofiary",
      "ofiarą",
      "oficer",
      "oficjalnie",
      "ogień",
      "oglądać",
      "ognia",
      "ogromne",
      "ogromny",
      "ogóle",
      "oj",
      "ojca",
      "ojcem",
      "ojciec",
      "ojcu",
      "ojcze",
      "ok",
      "oka",
      "okazało",
      "okazja",
      "okazji",
      "okazję",
      "okej",
      "okiem",
      "okna",
      "okno",
      "oko",
      "okolicy",
      "okoliczności",
      "około",
      "okres",
      "okropne",
      "okropnie",
      "okropny",
      "okręt",
      "oku",
      "okulary",
      "on",
      "ona",
      "one",
      "oni",
      "ono",
      "operacja",
      "operacji",
      "operację",
      "opieki",
      "opowiedz",
      "opowiedzieć",
      "opowiem",
      "opowieści",
      "opowieść",
      "oprócz",
      "opuścić",
      "opuścił",
      "opuść",
      "oraz",
      "organizacji",
      "osiem",
      "osiągnąć",
      "oskarżony",
      "osoba",
      "osobie",
      "osobiste",
      "osobiście",
      "osoby",
      "osobą",
      "osobę",
      "ostatni",
      "ostatnia",
      "ostatnich",
      "ostatnie",
      "ostatniego",
      "ostatniej",
      "ostatnim",
      "ostatnio",
      "ostatnią",
      "ostro",
      "ostrożnie",
      "ostrożny",
      "ostrzec",
      "ostrzegam",
      "ostrzeżenie",
      "oszalałeś",
      "osób",
      "oto",
      "otwarcie",
      "otwarte",
      "otwarty",
      "otwieraj",
      "otwierać",
      "otworzyć",
      "otworzę",
      "otwórz",
      "owszem",
      "oznacza",
      "oświadczenie",
      "pa",
      "pachnie",
      "pacjent",
      "pacjenta",
      "pacjentów",
      "pada",
      "padnij",
      "palant",
      "palce",
      "palców",
      "palec",
      "pali",
      "paliwa",
      "palić",
      "pamięci",
      "pamięta",
      "pamiętacie",
      "pamiętaj",
      "pamiętajcie",
      "pamiętam",
      "pamiętasz",
      "pamiętać",
      "pamięć",
      "pan",
      "pana",
      "panem",
      "pani",
      "panie",
      "panienki",
      "panienko",
      "panią",
      "panna",
      "panno",
      "panny",
      "panowie",
      "panu",
      "panów",
      "papier",
      "papierosa",
      "papierosy",
      "papiery",
      "para",
      "park",
      "parker",
      "parkingu",
      "parku",
      "partner",
      "partnera",
      "paru",
      "pary",
      "parę",
      "pas",
      "pasuje",
      "pasują",
      "pasy",
      "patrz",
      "patrzcie",
      "patrzeć",
      "patrzy",
      "patrzysz",
      "patrzył",
      "patrząc",
      "patrzę",
      "pańska",
      "pański",
      "pańskie",
      "pańskiego",
      "pańskiej",
      "pańskim",
      "pańską",
      "państwa",
      "państwo",
      "państwu",
      "pecha",
      "pewien",
      "pewna",
      "pewne",
      "pewnego",
      "pewnej",
      "pewni",
      "pewnie",
      "pewno",
      "pewności",
      "pewnością",
      "pewność",
      "pewny",
      "pewnych",
      "pewnym",
      "pewną",
      "pełen",
      "pełna",
      "pełne",
      "pełni",
      "pełno",
      "pełną",
      "picia",
      "piekle",
      "piekła",
      "piekło",
      "pieniądze",
      "pieniędzmi",
      "pieniędzy",
      "pieprz",
      "piersi",
      "pierwsza",
      "pierwsze",
      "pierwszego",
      "pierwszej",
      "pierwszy",
      "pierwszym",
      "pierwszą",
      "pierścień",
      "pierścionek",
      "pies",
      "pij",
      "pijany",
      "pijesz",
      "piję",
      "pilnować",
      "pilnuj",
      "pilot",
      "pilota",
      "piosenka",
      "piosenki",
      "piosenkę",
      "pisać",
      "pistolet",
      "pisze",
      "piszę",
      "piwa",
      "piwnicy",
      "piwo",
      "piątek",
      "pić",
      "pięciu",
      "piękna",
      "piękne",
      "pięknie",
      "piękny",
      "piękną",
      "piętro",
      "piętrze",
      "pięć",
      "piłka",
      "piłkę",
      "plan",
      "planecie",
      "planem",
      "planeta",
      "planety",
      "planetę",
      "planu",
      "plany",
      "planów",
      "plaży",
      "plecami",
      "plecy",
      "plotki",
      "plus",
      "po",
      "pobliżu",
      "pobrane",
      "pocałować",
      "pocałuj",
      "pocałunek",
      "pochodzi",
      "pociąg",
      "pociągu",
      "poczekaj",
      "poczekajcie",
      "poczekam",
      "poczekać",
      "poczta",
      "poczucie",
      "poczujesz",
      "poczuć",
      "poczułem",
      "początek",
      "początku",
      "pod",
      "podaj",
      "podać",
      "podczas",
      "poddać",
      "podejdź",
      "podejrzanego",
      "podejrzany",
      "podejść",
      "podjąć",
      "podnieś",
      "podnieść",
      "podoba",
      "podobają",
      "podobało",
      "podobne",
      "podobnego",
      "podobnie",
      "podobno",
      "podobny",
      "podpisać",
      "podróż",
      "podróży",
      "podstawie",
      "podzielić",
      "podziękować",
      "podłodze",
      "podłogę",
      "pogadamy",
      "pogadać",
      "pogoda",
      "pogodzić",
      "pogrzeb",
      "pojadę",
      "pojawi",
      "pojawia",
      "pojawił",
      "pojechać",
      "pojechał",
      "pojedziemy",
      "pojedziesz",
      "pojęcia",
      "pojęcie",
      "pokaz",
      "pokazać",
      "pokazał",
      "pokazuje",
      "pokaż",
      "pokaże",
      "pokażę",
      "pokoju",
      "pokonać",
      "pokój",
      "pokład",
      "pokładzie",
      "pola",
      "pole",
      "polega",
      "policja",
      "policjant",
      "policjanta",
      "policjantem",
      "policjantów",
      "policji",
      "policję",
      "polu",
      "pomaga",
      "pomagać",
      "pomimo",
      "pomiędzy",
      "pomoc",
      "pomocy",
      "pomocą",
      "pomogę",
      "pomoże",
      "pomożesz",
      "pomysł",
      "pomysły",
      "pomyłka",
      "pomyśl",
      "pomyślał",
      "pomyślałam",
      "pomyślałem",
      "pomyśleć",
      "pomyślę",
      "pomóc",
      "pomógł",
      "pomówić",
      "pomóż",
      "pomóżcie",
      "ponad",
      "poniedziałek",
      "ponieważ",
      "poniżej",
      "ponownie",
      "popatrz",
      "popatrzcie",
      "popełnił",
      "poprawki",
      "poprosić",
      "poprosił",
      "poproszę",
      "poprzez",
      "pora",
      "poradzimy",
      "poradzisz",
      "poradzić",
      "poradzę",
      "porozmawiaj",
      "porozmawiajmy",
      "porozmawiam",
      "porozmawiamy",
      "porozmawiać",
      "portfel",
      "porucznik",
      "poruczniku",
      "pory",
      "porze",
      "porządek",
      "porządku",
      "porę",
      "posiada",
      "pospiesz",
      "postaci",
      "postaraj",
      "postaram",
      "postaw",
      "postawić",
      "postać",
      "poszedł",
      "poszedłem",
      "poszli",
      "poszukać",
      "poszukiwania",
      "poszła",
      "poszłam",
      "poszło",
      "posłuchaj",
      "posłuchajcie",
      "posłuchać",
      "potem",
      "potrafi",
      "potrafisz",
      "potrafią",
      "potrafię",
      "potrwa",
      "potrzeba",
      "potrzebna",
      "potrzebne",
      "potrzebny",
      "potrzebować",
      "potrzebował",
      "potrzebowała",
      "potrzebuje",
      "potrzebujecie",
      "potrzebujemy",
      "potrzebujesz",
      "potrzebują",
      "potrzebuję",
      "potrzeby",
      "potwierdzić",
      "potwora",
      "potwór",
      "potężny",
      "poważna",
      "poważne",
      "poważnego",
      "poważnie",
      "poważny",
      "powie",
      "powiedz",
      "powiedzcie",
      "powiedzenia",
      "powiedziano",
      "powiedział",
      "powiedziała",
      "powiedziałam",
      "powiedziałaś",
      "powiedziałbym",
      "powiedziałem",
      "powiedziałeś",
      "powiedzieli",
      "powiedzieć",
      "powiedzmy",
      "powiedzą",
      "powiem",
      "powiemy",
      "powierzchni",
      "powiesz",
      "powietrza",
      "powietrze",
      "powietrzu",
      "powinien",
      "powinienem",
      "powinieneś",
      "powinna",
      "powinnam",
      "powinnaś",
      "powinni",
      "powinniście",
      "powinniśmy",
      "powinno",
      "powinny",
      "powodem",
      "powodu",
      "powoduje",
      "powody",
      "powodzenia",
      "powodów",
      "powoli",
      "powrocie",
      "powrotem",
      "powrotu",
      "powrót",
      "powstrzyma",
      "powstrzymać",
      "powtarzam",
      "powtarzać",
      "powtórz",
      "powtórzyć",
      "powód",
      "poza",
      "pozbyć",
      "poziom",
      "poziomie",
      "poznaj",
      "poznaliśmy",
      "poznać",
      "poznał",
      "poznałam",
      "poznałem",
      "poznałeś",
      "pozostaje",
      "pozostali",
      "pozostanie",
      "pozostać",
      "pozostałe",
      "pozostało",
      "pozostałych",
      "pozwala",
      "pozwolenie",
      "pozwoli",
      "pozwolisz",
      "pozwolić",
      "pozwolił",
      "pozwolę",
      "pozwól",
      "pozwólcie",
      "pozycje",
      "pozycji",
      "pozycję",
      "połowa",
      "połowie",
      "połowę",
      "położyć",
      "południe",
      "południowej",
      "południu",
      "połóż",
      "połącz",
      "połączenie",
      "połączyć",
      "pośpiesz",
      "pośród",
      "pożar",
      "pożegnać",
      "pożyczyć",
      "praca",
      "prace",
      "pracować",
      "pracował",
      "pracowała",
      "pracowałem",
      "pracowników",
      "pracuje",
      "pracujemy",
      "pracujesz",
      "pracują",
      "pracuję",
      "pracy",
      "pracą",
      "pracę",
      "pragnie",
      "pragniesz",
      "pragnę",
      "praktycznie",
      "praw",
      "prawa",
      "prawda",
      "prawdopodobnie",
      "prawdy",
      "prawdziwa",
      "prawdziwe",
      "prawdziwego",
      "prawdziwej",
      "prawdziwy",
      "prawdziwym",
      "prawdziwą",
      "prawdą",
      "prawdę",
      "prawej",
      "prawem",
      "prawie",
      "prawnik",
      "prawnika",
      "prawnikiem",
      "prawo",
      "prezent",
      "prezenty",
      "prezydencie",
      "prezydent",
      "prezydenta",
      "prezydentem",
      "problem",
      "problemem",
      "problemu",
      "problemy",
      "problemów",
      "procent",
      "proces",
      "procesu",
      "prochy",
      "profesor",
      "profesorze",
      "program",
      "programie",
      "programu",
      "projekt",
      "proponuję",
      "propozycję",
      "prosi",
      "prosimy",
      "prosić",
      "prosił",
      "prosiłem",
      "prosta",
      "proste",
      "prosto",
      "prostu",
      "prosze",
      "proszę",
      "prowadzi",
      "prowadzić",
      "prowadził",
      "prowadzą",
      "prowadzę",
      "prysznic",
      "przechodzi",
      "przecież",
      "przeciw",
      "przeciwko",
      "przeciwnie",
      "przeczucie",
      "przeczytaj",
      "przeczytać",
      "przed",
      "przede",
      "przedstawienie",
      "przedstawić",
      "przedstawię",
      "przedtem",
      "przejdzie",
      "przejdziemy",
      "przejdźmy",
      "przejmuj",
      "przejąć",
      "przejście",
      "przejść",
      "przekazać",
      "przekaż",
      "przekonany",
      "przekonać",
      "przemocy",
      "przenieść",
      "przepraszam",
      "przepraszamy",
      "przeprosiny",
      "przeprosić",
      "przeprowadzić",
      "przeraża",
      "przerwać",
      "przerwy",
      "przerwę",
      "przespać",
      "przestanie",
      "przestaniesz",
      "przestać",
      "przestał",
      "przestań",
      "przestańcie",
      "przestrzeni",
      "przestrzeń",
      "przestępstwo",
      "przeszedł",
      "przeszkadza",
      "przeszkadzam",
      "przeszkadzać",
      "przeszłości",
      "przeszłość",
      "przesłuchanie",
      "przetrwać",
      "przewagę",
      "przez",
      "przeze",
      "przeznaczenie",
      "przeżyje",
      "przeżyć",
      "przeżył",
      "przodu",
      "przy",
      "przybycie",
      "przybył",
      "przychodzi",
      "przychodzić",
      "przychodzą",
      "przyda",
      "przygotować",
      "przygotuj",
      "przygotujcie",
      "przyjaciel",
      "przyjaciela",
      "przyjaciele",
      "przyjacielem",
      "przyjacielu",
      "przyjaciół",
      "przyjaciółka",
      "przyjaciółką",
      "przyjaciółmi",
      "przyjadę",
      "przyjaźń",
      "przyjdzie",
      "przyjdziesz",
      "przyjdą",
      "przyjdę",
      "przyjdź",
      "przyjechać",
      "przyjechał",
      "przyjechała",
      "przyjechałem",
      "przyjechałeś",
      "przyjedzie",
      "przyjemności",
      "przyjemnością",
      "przyjemność",
      "przyjąć",
      "przyjąłem",
      "przyjęcia",
      "przyjęcie",
      "przyjęciu",
      "przyjść",
      "przykro",
      "przykład",
      "przynajmniej",
      "przyniesie",
      "przynieś",
      "przynieść",
      "przyniosę",
      "przyniosłem",
      "przyniósł",
      "przynosi",
      "przypadek",
      "przypadkiem",
      "przypadku",
      "przypomina",
      "przypominam",
      "przypomnieć",
      "przyprowadź",
      "przypuszczam",
      "przysięgam",
      "przystojny",
      "przyszedł",
      "przyszedłem",
      "przyszedłeś",
      "przyszli",
      "przyszliśmy",
      "przyszła",
      "przyszłam",
      "przyszłaś",
      "przyszło",
      "przyszłości",
      "przyszłość",
      "przyszły",
      "przyszłym",
      "przysłał",
      "przysługę",
      "przyznaję",
      "przyznać",
      "próba",
      "próbki",
      "próbowali",
      "próbowaliśmy",
      "próbować",
      "próbował",
      "próbowała",
      "próbowałam",
      "próbowałem",
      "próbowałeś",
      "próbuj",
      "próbuje",
      "próbujemy",
      "próbujesz",
      "próbują",
      "próbując",
      "próbuję",
      "próby",
      "próbę",
      "prąd",
      "prędkość",
      "prędzej",
      "psa",
      "psem",
      "psie",
      "psy",
      "psów",
      "ptak",
      "ptaki",
      "pudełko",
      "punkcie",
      "punkt",
      "punktu",
      "punkty",
      "punktów",
      "puste",
      "pusty",
      "pustyni",
      "puszczaj",
      "pułapka",
      "pułkownik",
      "pułkowniku",
      "puścić",
      "puść",
      "pyta",
      "pytaj",
      "pytam",
      "pytania",
      "pytanie",
      "pytasz",
      "pytać",
      "pytał",
      "pytałem",
      "pytań",
      "pójdzie",
      "pójdziemy",
      "pójdziesz",
      "pójdę",
      "pójść",
      "póki",
      "pół",
      "północ",
      "północy",
      "później",
      "późno",
      "płaci",
      "płacić",
      "płacz",
      "płacą",
      "płacę",
      "płakać",
      "płaszcz",
      "płynie",
      "rachunek",
      "rachunki",
      "racja",
      "racje",
      "racji",
      "rację",
      "raczej",
      "rada",
      "radio",
      "radości",
      "radość",
      "rady",
      "radzi",
      "radzisz",
      "radzę",
      "radę",
      "raka",
      "ramiona",
      "ramię",
      "rana",
      "randka",
      "randkę",
      "randy",
      "ranka",
      "ranny",
      "rano",
      "rany",
      "raport",
      "ratować",
      "ratunku",
      "raz",
      "razem",
      "razie",
      "razu",
      "razy",
      "reguły",
      "restauracji",
      "reszta",
      "reszty",
      "resztą",
      "resztę",
      "robert",
      "robi",
      "robicie",
      "robili",
      "robiliśmy",
      "robimy",
      "robisz",
      "robią",
      "robić",
      "robię",
      "robił",
      "robiła",
      "robiłam",
      "robiłem",
      "robiłeś",
      "robota",
      "roboty",
      "robotę",
      "rodzaj",
      "rodzaju",
      "rodzicami",
      "rodzice",
      "rodziców",
      "rodzina",
      "rodzinie",
      "rodziny",
      "rodziną",
      "rodzinę",
      "rogu",
      "rok",
      "roku",
      "roli",
      "rolę",
      "romans",
      "rosjanie",
      "rosji",
      "rozdział",
      "rozkaz",
      "rozkazy",
      "rozmawia",
      "rozmawialiśmy",
      "rozmawiam",
      "rozmawiamy",
      "rozmawiasz",
      "rozmawiać",
      "rozmawiał",
      "rozmawiałam",
      "rozmawiałem",
      "rozmawiałeś",
      "rozmiar",
      "rozmowa",
      "rozmowy",
      "rozmowę",
      "rozmów",
      "rozpocząć",
      "rozum",
      "rozumie",
      "rozumiecie",
      "rozumiem",
      "rozumiemy",
      "rozumiesz",
      "rozwiązanie",
      "rozwiązać",
      "rośnie",
      "ruch",
      "ruchu",
      "ruchy",
      "rusz",
      "rusza",
      "ruszaj",
      "ruszajcie",
      "ruszajmy",
      "ruszamy",
      "ruszać",
      "ruszyć",
      "ryba",
      "ryby",
      "ryzyko",
      "ryzykować",
      "rzadko",
      "rzecz",
      "rzeczach",
      "rzeczy",
      "rzeczywistości",
      "rzeczywiście",
      "rzeczą",
      "rzeki",
      "rzucić",
      "rzucił",
      "rzuć",
      "rzymu",
      "rząd",
      "rządu",
      "rządzi",
      "rób",
      "róbcie",
      "równie",
      "również",
      "różne",
      "różnica",
      "różnicy",
      "różnych",
      "rąk",
      "ręce",
      "ręka",
      "rękach",
      "rękami",
      "ręki",
      "ręką",
      "rękę",
      "sali",
      "sam",
      "sama",
      "same",
      "samego",
      "samej",
      "samemu",
      "sami",
      "samo",
      "samobójstwo",
      "samochodem",
      "samochodu",
      "samochody",
      "samochodzie",
      "samochodów",
      "samochód",
      "samolocie",
      "samolot",
      "samolotu",
      "samoloty",
      "samotna",
      "samotnie",
      "samotny",
      "samych",
      "samym",
      "samą",
      "scenariusz",
      "scenie",
      "scenę",
      "schowaj",
      "sekret",
      "seks",
      "seksu",
      "sekund",
      "sekundy",
      "sekundę",
      "sen",
      "senator",
      "sens",
      "sensie",
      "sensu",
      "serca",
      "serce",
      "sercem",
      "sercu",
      "serio",
      "setki",
      "siadaj",
      "siebie",
      "sieci",
      "siedem",
      "siedmiu",
      "siedzi",
      "siedział",
      "siedzieć",
      "siedzisz",
      "siedzę",
      "siedź",
      "sierżancie",
      "sierżant",
      "sieć",
      "silna",
      "silnik",
      "silniki",
      "silny",
      "siostra",
      "siostro",
      "siostry",
      "siostrą",
      "siostrę",
      "się",
      "sił",
      "siła",
      "siły",
      "siłą",
      "siłę",
      "skacz",
      "skarb",
      "skarbie",
      "sklep",
      "sklepie",
      "sklepu",
      "skok",
      "skomplikowane",
      "skontaktować",
      "skoro",
      "skorzystać",
      "skończ",
      "skończona",
      "skończone",
      "skończy",
      "skończyliśmy",
      "skończymy",
      "skończysz",
      "skończyć",
      "skończył",
      "skończyła",
      "skończyłam",
      "skończyłem",
      "skończyłeś",
      "skończyło",
      "skończę",
      "skrzydła",
      "skrzywdzić",
      "skup",
      "skupić",
      "skóra",
      "skóry",
      "skórę",
      "skąd",
      "składa",
      "smak",
      "smakuje",
      "smutne",
      "snu",
      "sny",
      "snów",
      "sobie",
      "sobotę",
      "sobą",
      "sok",
      "spacer",
      "spada",
      "spadaj",
      "spadam",
      "spadamy",
      "spadł",
      "spalić",
      "spać",
      "spał",
      "spałem",
      "specjalne",
      "specjalnego",
      "specjalnie",
      "specjalny",
      "spod",
      "spodnie",
      "spodoba",
      "spodziewałem",
      "spojrzenie",
      "spojrzeć",
      "spoko",
      "spokojnie",
      "spokojny",
      "spokoju",
      "spokój",
      "sporo",
      "sposobem",
      "sposobu",
      "sposób",
      "spotkajmy",
      "spotkaliśmy",
      "spotkamy",
      "spotkania",
      "spotkanie",
      "spotkaniu",
      "spotkać",
      "spotkał",
      "spotkałam",
      "spotkałem",
      "spotyka",
      "spotykamy",
      "spotykać",
      "spraw",
      "sprawa",
      "sprawach",
      "sprawdzam",
      "sprawdzić",
      "sprawdziłem",
      "sprawdzę",
      "sprawdź",
      "sprawdźcie",
      "sprawdźmy",
      "sprawi",
      "sprawia",
      "sprawie",
      "sprawiedliwości",
      "sprawiedliwość",
      "sprawić",
      "sprawy",
      "sprawą",
      "sprawę",
      "sprowadza",
      "sprowadzić",
      "sprzeciw",
      "sprzed",
      "sprzedaje",
      "sprzedać",
      "sprzedaż",
      "sprzęt",
      "spróbować",
      "spróbuj",
      "spróbuje",
      "spróbujemy",
      "spróbujmy",
      "spróbuję",
      "spust",
      "spytaj",
      "spytać",
      "spójrz",
      "spójrzcie",
      "spóźnienie",
      "spędzić",
      "spędziłem",
      "stacja",
      "stacji",
      "staje",
      "stali",
      "stamtąd",
      "stan",
      "stanie",
      "stanowiska",
      "stanowisko",
      "stanu",
      "stanów",
      "stanąć",
      "stara",
      "staram",
      "starczy",
      "stare",
      "starego",
      "starej",
      "stark",
      "starszy",
      "start",
      "stary",
      "starych",
      "starym",
      "starą",
      "statek",
      "statki",
      "statku",
      "stawiam",
      "stać",
      "stał",
      "stała",
      "stałem",
      "stało",
      "stań",
      "sto",
      "stoi",
      "stoisz",
      "stoję",
      "stole",
      "stolik",
      "stop",
      "stopni",
      "stopy",
      "stołu",
      "strach",
      "strachu",
      "straciliśmy",
      "stracisz",
      "stracić",
      "stracił",
      "straciłam",
      "straciłem",
      "straciłeś",
      "straszne",
      "strasznie",
      "straszny",
      "strata",
      "straty",
      "straż",
      "strażnik",
      "strażników",
      "stron",
      "strona",
      "stronie",
      "strony",
      "stronę",
      "strzał",
      "strzały",
      "strzela",
      "strzelaj",
      "strzelać",
      "strój",
      "stu",
      "studia",
      "stworzenia",
      "stworzyć",
      "stworzył",
      "styl",
      "stylu",
      "stój",
      "stóp",
      "stół",
      "stąd",
      "sukces",
      "sukienkę",
      "sumie",
      "super",
      "swe",
      "swego",
      "swej",
      "swoich",
      "swoim",
      "swoimi",
      "swoje",
      "swojego",
      "swojej",
      "swojemu",
      "swoją",
      "swych",
      "swój",
      "swą",
      "sygnał",
      "symbol",
      "syn",
      "syna",
      "synchro",
      "synem",
      "synu",
      "sypialni",
      "system",
      "systemu",
      "systemy",
      "sytuacja",
      "sytuacji",
      "sytuację",
      "szacunek",
      "szacunkiem",
      "szacunku",
      "szaleństwo",
      "szalona",
      "szalone",
      "szalony",
      "szampana",
      "szans",
      "szansa",
      "szanse",
      "szansy",
      "szansę",
      "szczególnie",
      "szczegóły",
      "szczery",
      "szczerze",
      "szczury",
      "szczyt",
      "szczęścia",
      "szczęście",
      "szczęśliwa",
      "szczęśliwego",
      "szczęśliwi",
      "szczęśliwy",
      "szef",
      "szefa",
      "szefem",
      "szefie",
      "szeryf",
      "szeryfa",
      "szeryfie",
      "sześciu",
      "sześć",
      "szkoda",
      "szkodzi",
      "szkole",
      "szkoła",
      "szkoły",
      "szkołę",
      "szlag",
      "szpital",
      "szpitala",
      "szpitalu",
      "sztuka",
      "sztuki",
      "sztukę",
      "szuka",
      "szukaj",
      "szukają",
      "szukam",
      "szukamy",
      "szukasz",
      "szukać",
      "szukał",
      "szukałem",
      "szybciej",
      "szybki",
      "szybko",
      "szyi",
      "są",
      "sąd",
      "sądu",
      "sądzi",
      "sądzie",
      "sądzisz",
      "sądziłam",
      "sądziłem",
      "sądzę",
      "sędzia",
      "słaby",
      "słodka",
      "słodki",
      "słodkie",
      "słowa",
      "słowo",
      "słońca",
      "słońce",
      "słucha",
      "słuchaj",
      "słuchajcie",
      "słucham",
      "słuchasz",
      "słuchać",
      "słuchu",
      "słusznie",
      "służbie",
      "służby",
      "służy",
      "służyć",
      "słychać",
      "słyszał",
      "słyszałam",
      "słyszałaś",
      "słyszałem",
      "słyszałeś",
      "słyszeliście",
      "słyszeliśmy",
      "słyszeć",
      "słyszy",
      "słyszycie",
      "słyszysz",
      "słyszę",
      "słów",
      "ta",
      "tacie",
      "tacy",
      "tajemnica",
      "tajemnicy",
      "tajne",
      "tak",
      "taka",
      "taki",
      "takich",
      "takie",
      "takiego",
      "takiej",
      "takim",
      "takimi",
      "taksówkę",
      "taką",
      "także",
      "talent",
      "tam",
      "tamtego",
      "tamtej",
      "tamten",
      "tamto",
      "tamtym",
      "tamtą",
      "taniec",
      "tata",
      "tato",
      "tatuaż",
      "tatusia",
      "tatusiu",
      "tatuś",
      "taty",
      "tatą",
      "tatę",
      "tańca",
      "tańczyć",
      "taśmę",
      "te",
      "tego",
      "tej",
      "tekst",
      "telefon",
      "telefonu",
      "telefony",
      "telefonów",
      "telewizji",
      "telewizor",
      "temat",
      "temu",
      "ten",
      "teoria",
      "teraz",
      "teren",
      "terenie",
      "terry",
      "test",
      "testy",
      "tez",
      "też",
      "to",
      "toalety",
      "toast",
      "tobie",
      "tobą",
      "torby",
      "torbę",
      "towar",
      "towarzystwa",
      "towarzystwo",
      "tożsamość",
      "traci",
      "tracimy",
      "tracisz",
      "tracę",
      "trafi",
      "trafić",
      "trafił",
      "trakcie",
      "transport",
      "trening",
      "troche",
      "trochę",
      "troszkę",
      "trudne",
      "trudno",
      "trupem",
      "trwa",
      "trzeba",
      "trzech",
      "trzeci",
      "trzecia",
      "trzecie",
      "trzeciej",
      "trzy",
      "trzyma",
      "trzymaj",
      "trzymajcie",
      "trzymają",
      "trzymam",
      "trzymasz",
      "trzymać",
      "trzymał",
      "tu",
      "tunel",
      "tunelu",
      "tutaj",
      "tuż",
      "tv",
      "twardy",
      "twarz",
      "twarzy",
      "twarzą",
      "twe",
      "twego",
      "twierdzi",
      "twierdzisz",
      "twierdzą",
      "twoi",
      "twoich",
      "twoim",
      "twoimi",
      "twoja",
      "twoje",
      "twojego",
      "twojej",
      "twojemu",
      "twoją",
      "twój",
      "ty",
      "tych",
      "tydzień",
      "tygodni",
      "tygodnia",
      "tygodnie",
      "tygodniu",
      "tyle",
      "tyler",
      "tylko",
      "tylu",
      "tym",
      "tymczasem",
      "tymi",
      "typ",
      "typu",
      "tysiąc",
      "tysiące",
      "tysięcy",
      "tytuł",
      "tył",
      "tyłu",
      "tą",
      "tę",
      "tędy",
      "tęsknię",
      "tłum",
      "tłumaczenie",
      "ubrania",
      "ubranie",
      "uciec",
      "ucieczki",
      "ucieka",
      "uciekaj",
      "uciekajcie",
      "uciekać",
      "uciekł",
      "uciekła",
      "uczucia",
      "uczucie",
      "uczuć",
      "uczy",
      "uczyć",
      "uda",
      "udaje",
      "udawaj",
      "udawać",
      "udać",
      "udało",
      "uderz",
      "uderzenie",
      "uderzyć",
      "uderzył",
      "udowodnić",
      "udział",
      "udziału",
      "ufam",
      "ufasz",
      "ufać",
      "ukradł",
      "ukraść",
      "ukrywa",
      "ukryć",
      "układ",
      "ulicach",
      "ulicy",
      "ulicę",
      "ulubione",
      "ulubiony",
      "umarł",
      "umarła",
      "umie",
      "umiejętności",
      "umiem",
      "umiera",
      "umierają",
      "umieram",
      "umierać",
      "umiesz",
      "umieścić",
      "umowa",
      "umowy",
      "umowę",
      "umrze",
      "umrzesz",
      "umrzeć",
      "umrę",
      "umysł",
      "umysłu",
      "uniknąć",
      "upewnij",
      "upewnić",
      "uratować",
      "uratował",
      "urocze",
      "uroczy",
      "urodzin",
      "urodziny",
      "urządzenie",
      "usiądź",
      "usiąść",
      "uspokoić",
      "uspokój",
      "ust",
      "usta",
      "ustach",
      "ustalić",
      "usunąć",
      "uszy",
      "usług",
      "usłyszałem",
      "usłyszeć",
      "utrzymać",
      "uwaga",
      "uwagi",
      "uwagę",
      "uważa",
      "uważaj",
      "uważajcie",
      "uważają",
      "uważam",
      "uważasz",
      "uważać",
      "uważnie",
      "uwielbia",
      "uwielbiam",
      "uwierz",
      "uwierzysz",
      "uwierzyć",
      "uwolnić",
      "uśmiech",
      "użyj",
      "używa",
      "używać",
      "użyć",
      "użył",
      "wagi",
      "wakacje",
      "wal",
      "walce",
      "walcz",
      "walczy",
      "walczyć",
      "walczył",
      "walizkę",
      "walka",
      "walki",
      "walkę",
      "wam",
      "wami",
      "wariat",
      "wart",
      "warta",
      "warte",
      "warto",
      "wartości",
      "warunki",
      "was",
      "wasz",
      "wasza",
      "wasze",
      "waszego",
      "waszej",
      "waszych",
      "waszym",
      "waszą",
      "waż",
      "ważna",
      "ważne",
      "ważnego",
      "ważniejsze",
      "ważny",
      "wbrew",
      "wcale",
      "wchodzi",
      "wchodzimy",
      "wchodzisz",
      "wchodzić",
      "wchodzę",
      "wchodź",
      "wciąż",
      "wcześnie",
      "wcześniej",
      "wczoraj",
      "wdzięczny",
      "we",
      "według",
      "wejdzie",
      "wejdziesz",
      "wejdź",
      "wejścia",
      "wejście",
      "wejść",
      "wersji",
      "wesołych",
      "wewnątrz",
      "wezmę",
      "wezwać",
      "wezwij",
      "weź",
      "weźcie",
      "weźmie",
      "weźmiemy",
      "weźmiesz",
      "wiadomo",
      "wiadomości",
      "wiadomość",
      "wiary",
      "wiarę",
      "wiatr",
      "widać",
      "wideo",
      "widok",
      "widzenia",
      "widzi",
      "widział",
      "widziała",
      "widziałam",
      "widziałaś",
      "widziałem",
      "widziałeś",
      "widzicie",
      "widzieli",
      "widzieliście",
      "widzieliśmy",
      "widzieć",
      "widzimy",
      "widzisz",
      "widzą",
      "widzę",
      "wie",
      "wiec",
      "wiecie",
      "wiecznie",
      "wieczora",
      "wieczorem",
      "wieczoru",
      "wieczór",
      "wiedział",
      "wiedziała",
      "wiedziałam",
      "wiedziałaś",
      "wiedziałem",
      "wiedziałeś",
      "wiedzieli",
      "wiedzieliśmy",
      "wiedzieć",
      "wiedzy",
      "wiedzą",
      "wiedząc",
      "wiek",
      "wieki",
      "wieku",
      "wiele",
      "wielka",
      "wielki",
      "wielkich",
      "wielkie",
      "wielkiego",
      "wielkiej",
      "wielkim",
      "wielkości",
      "wielką",
      "wielu",
      "wiem",
      "wiemy",
      "wierz",
      "wierzy",
      "wierzysz",
      "wierzyć",
      "wierzą",
      "wierzę",
      "wiesz",
      "wieści",
      "wieży",
      "wina",
      "winien",
      "winna",
      "winny",
      "wino",
      "winy",
      "wioski",
      "wirus",
      "wirusa",
      "wisi",
      "witaj",
      "witajcie",
      "witam",
      "witamy",
      "wizja",
      "wizytę",
      "więc",
      "więcej",
      "większa",
      "większe",
      "większego",
      "większości",
      "większość",
      "większy",
      "większą",
      "więzienia",
      "więzienie",
      "więzieniu",
      "więźniów",
      "wkrótce",
      "wobec",
      "woda",
      "wody",
      "wodzie",
      "wodą",
      "wodę",
      "wojna",
      "wojnie",
      "wojny",
      "wojnę",
      "wojska",
      "wojsko",
      "wokół",
      "wolałbym",
      "woli",
      "wolisz",
      "wolna",
      "wolne",
      "wolnego",
      "wolno",
      "wolności",
      "wolność",
      "wolny",
      "wolę",
      "wozu",
      "wpadł",
      "wpadłem",
      "wpaść",
      "wprost",
      "wprowadzić",
      "wpływ",
      "wraca",
      "wracaj",
      "wracajcie",
      "wracajmy",
      "wracam",
      "wracamy",
      "wracasz",
      "wracać",
      "wraz",
      "wrażenie",
      "wrażeniem",
      "wreszcie",
      "wroga",
      "wrogiem",
      "wrogów",
      "wrota",
      "września",
      "wróci",
      "wrócili",
      "wrócimy",
      "wrócisz",
      "wrócić",
      "wrócił",
      "wróciła",
      "wróciłam",
      "wróciłem",
      "wróciłeś",
      "wrócą",
      "wrócę",
      "wróg",
      "wróć",
      "wręcz",
      "wschód",
      "wsiadaj",
      "wskakuj",
      "wskazuje",
      "wspaniale",
      "wspaniała",
      "wspaniałe",
      "wspaniały",
      "wspaniałym",
      "wspaniałą",
      "wsparcia",
      "wsparcie",
      "wspomnienia",
      "wspólnego",
      "wstawaj",
      "wstawać",
      "wstać",
      "wstań",
      "wstrzymać",
      "wstyd",
      "wszedł",
      "wszelki",
      "wszelkie",
      "wszyscy",
      "wszystkich",
      "wszystkie",
      "wszystkiego",
      "wszystkim",
      "wszystkimi",
      "wszystko",
      "wszędzie",
      "wtedy",
      "wtorek",
      "wujek",
      "wujka",
      "wujku",
      "wy",
      "wybacz",
      "wybaczcie",
      "wybaczy",
      "wybaczyć",
      "wybiera",
      "wybieram",
      "wybierasz",
      "wybierz",
      "wyboru",
      "wybrać",
      "wybrał",
      "wybuch",
      "wybuchnie",
      "wybór",
      "wychodzi",
      "wychodzimy",
      "wychodzisz",
      "wychodzić",
      "wychodzę",
      "wychodź",
      "wycieczkę",
      "wyciągnij",
      "wyciągnąć",
      "wycofać",
      "wydaje",
      "wydajesz",
      "wydarzy",
      "wydarzyło",
      "wydawało",
      "wydać",
      "wydał",
      "wydostać",
      "wydział",
      "wydziału",
      "wygląd",
      "wygląda",
      "wyglądają",
      "wyglądam",
      "wyglądasz",
      "wyglądać",
      "wyglądał",
      "wyglądała",
      "wyglądało",
      "wygodnie",
      "wygra",
      "wygrać",
      "wygrał",
      "wygrałem",
      "wygrałeś",
      "wygrywa",
      "wyjaśnia",
      "wyjaśnić",
      "wyjaśnię",
      "wyjdzie",
      "wyjdziemy",
      "wyjdziesz",
      "wyjdę",
      "wyjdź",
      "wyjechać",
      "wyjechał",
      "wyjeżdżam",
      "wyjątkiem",
      "wyjątkowo",
      "wyjątkowy",
      "wyjścia",
      "wyjście",
      "wyjść",
      "wykonać",
      "wykorzystać",
      "wyluzuj",
      "wymaga",
      "wymiar",
      "wymyślić",
      "wynik",
      "wyniki",
      "wynocha",
      "wynoś",
      "wyobrazić",
      "wyobraź",
      "wyobrażam",
      "wyobrażasz",
      "wypadek",
      "wypadku",
      "wyprawiasz",
      "wypuścić",
      "wypuść",
      "wyraźnie",
      "wyrok",
      "wyrwać",
      "wyrzucić",
      "wyrzuć",
      "wysiadaj",
      "wysoki",
      "wysokie",
      "wysoko",
      "wysokości",
      "wysokość",
      "wyspa",
      "wyspie",
      "wyspy",
      "wyspę",
      "wystarczająco",
      "wystarczy",
      "występ",
      "wyszedł",
      "wyszedłem",
      "wyszła",
      "wyszło",
      "wysłać",
      "wysłał",
      "wysłałem",
      "wytłumaczyć",
      "wywiad",
      "wywiadu",
      "wyzwanie",
      "wyłaź",
      "wyłącz",
      "wyłącznie",
      "wyłączyć",
      "wyścig",
      "wyślij",
      "wyślę",
      "wyżej",
      "wzajemnie",
      "wzdłuż",
      "względu",
      "wziąć",
      "wziął",
      "wziąłem",
      "wziąłeś",
      "wzięli",
      "wzięła",
      "wzrok",
      "wzywa",
      "wóz",
      "wątpienia",
      "wątpię",
      "wątpliwości",
      "władze",
      "władzy",
      "władzę",
      "własne",
      "własnego",
      "własnej",
      "własny",
      "własnych",
      "własnym",
      "własną",
      "właściciel",
      "właścicielem",
      "właściwe",
      "właściwie",
      "właśnie",
      "właź",
      "włosy",
      "włosów",
      "włożyć",
      "włóż",
      "włącz",
      "włączyć",
      "wściekły",
      "wśród",
      "za",
      "zaatakował",
      "zabawa",
      "zabawić",
      "zabawne",
      "zabawny",
      "zabawy",
      "zabawę",
      "zabicie",
      "zabiera",
      "zabieraj",
      "zabieram",
      "zabierz",
      "zabierzcie",
      "zabierze",
      "zabierzemy",
      "zabierzesz",
      "zabij",
      "zabija",
      "zabijać",
      "zabije",
      "zabijesz",
      "zabiją",
      "zabiję",
      "zabili",
      "zabiorę",
      "zabity",
      "zabić",
      "zabił",
      "zabiła",
      "zabiłem",
      "zabiłeś",
      "zabrali",
      "zabrać",
      "zabrał",
      "zabrała",
      "zabójca",
      "zabójcy",
      "zabójcą",
      "zabójstwa",
      "zabójstwo",
      "zachowanie",
      "zachować",
      "zachowuje",
      "zachowujesz",
      "zachowywać",
      "zachód",
      "zaczekaj",
      "zaczekajcie",
      "zaczekać",
      "zacznie",
      "zaczniemy",
      "zaczniesz",
      "zacznij",
      "zacznijmy",
      "zaczną",
      "zacznę",
      "zaczyna",
      "zaczynaj",
      "zaczynają",
      "zaczynam",
      "zaczynamy",
      "zaczynasz",
      "zaczynać",
      "zacząć",
      "zaczął",
      "zacząłem",
      "zaczęli",
      "zaczęliśmy",
      "zaczęła",
      "zaczęłam",
      "zaczęło",
      "zaczęły",
      "zadania",
      "zadanie",
      "zadaniem",
      "zadać",
      "zadowolony",
      "zadziała",
      "zadzwoni",
      "zadzwonić",
      "zadzwonię",
      "zadzwonił",
      "zadzwoń",
      "zaginął",
      "zagrać",
      "zagrożenie",
      "zainteresowany",
      "zajmie",
      "zajmiemy",
      "zajmij",
      "zajmuje",
      "zajmujesz",
      "zajmuję",
      "zajmę",
      "zająć",
      "zajął",
      "zajęcia",
      "zajęta",
      "zajęty",
      "zajęło",
      "zakochany",
      "zakończenie",
      "zakończyć",
      "zakupy",
      "zakład",
      "zakładam",
      "zaledwie",
      "zależało",
      "zależy",
      "zamek",
      "zamian",
      "zamiar",
      "zamiaru",
      "zamiast",
      "zamienić",
      "zamierza",
      "zamierzam",
      "zamierzasz",
      "zamknij",
      "zamknijcie",
      "zamknięta",
      "zamknięte",
      "zamknięty",
      "zamknąć",
      "zamku",
      "zanim",
      "zapach",
      "zapewne",
      "zapewniam",
      "zapewnić",
      "zapomniał",
      "zapomniałam",
      "zapomniałem",
      "zapomniałeś",
      "zapomnieć",
      "zapomnij",
      "zapomnę",
      "zapraszam",
      "zaprosić",
      "zaproszenie",
      "zapytaj",
      "zapytam",
      "zapytać",
      "zapłaci",
      "zapłacisz",
      "zapłacić",
      "zapłacił",
      "zapłacę",
      "zaraz",
      "zarobić",
      "zasad",
      "zasada",
      "zasady",
      "zasadzie",
      "zasilanie",
      "zasięgu",
      "zaskoczony",
      "zasnąć",
      "zastanawiam",
      "zastanawiałem",
      "zastrzel",
      "zastrzelić",
      "zastrzelił",
      "zaszczyt",
      "zasługuje",
      "zatem",
      "zatrzyma",
      "zatrzymaj",
      "zatrzymać",
      "zatrzymał",
      "zaufaj",
      "zaufania",
      "zaufanie",
      "zaufać",
      "zauważył",
      "zauważyłem",
      "zauważyłeś",
      "zawiera",
      "zawsze",
      "zazdrosny",
      "zazwyczaj",
      "załatw",
      "załatwione",
      "załatwić",
      "załatwię",
      "załatwił",
      "załoga",
      "załogi",
      "założyć",
      "założę",
      "załóż",
      "zbadać",
      "zbieg",
      "zbliża",
      "zbrodni",
      "zbyt",
      "zdaje",
      "zdajesz",
      "zdania",
      "zdanie",
      "zdaniem",
      "zdarza",
      "zdarzyło",
      "zdecydowanie",
      "zdejmij",
      "zdenerwowany",
      "zdjąć",
      "zdjęcia",
      "zdjęcie",
      "zdjęć",
      "zdobyć",
      "zdolności",
      "zdolny",
      "zdrowia",
      "zdrowie",
      "zdrowy",
      "ze",
      "zebrać",
      "zeby",
      "zechce",
      "zechcesz",
      "zegarek",
      "zejdź",
      "zejść",
      "zemsty",
      "zero",
      "zespołu",
      "zespół",
      "zeszłej",
      "zeszłym",
      "zewnątrz",
      "zeznania",
      "zgadnij",
      "zgadnę",
      "zgadza",
      "zgadzam",
      "zginie",
      "zginiemy",
      "zginiesz",
      "zginąć",
      "zginął",
      "zginęli",
      "zginęła",
      "zginęło",
      "zgoda",
      "zgodnie",
      "zgodzić",
      "zgodził",
      "zgodę",
      "zgubiłem",
      "zgłosić",
      "zgłoś",
      "zielone",
      "zielony",
      "ziemi",
      "ziemia",
      "ziemią",
      "ziemię",
      "zimna",
      "zimno",
      "zjednoczonych",
      "zjedz",
      "zjemy",
      "zjeść",
      "zmarł",
      "zmarła",
      "zmian",
      "zmiana",
      "zmiany",
      "zmianę",
      "zmieni",
      "zmienia",
      "zmieniają",
      "zmieniać",
      "zmienić",
      "zmienił",
      "zmieniła",
      "zmieniłem",
      "zmieniłeś",
      "zmieniło",
      "zmierza",
      "zmusić",
      "zmęczona",
      "zmęczony",
      "zna",
      "znacie",
      "znaczenia",
      "znaczenie",
      "znacznie",
      "znaczy",
      "znaczyć",
      "znajduje",
      "znajdzie",
      "znajdziecie",
      "znajdziemy",
      "znajdziesz",
      "znajdą",
      "znajdę",
      "znajdź",
      "znajdźcie",
      "znajomych",
      "znają",
      "znak",
      "znaki",
      "znalazł",
      "znalazła",
      "znalazłam",
      "znalazłaś",
      "znalazłem",
      "znalazłeś",
      "znaleziono",
      "znaleźli",
      "znaleźliście",
      "znaleźliśmy",
      "znaleźć",
      "znam",
      "znamy",
      "znany",
      "znasz",
      "znać",
      "znał",
      "znałem",
      "znieść",
      "zniknął",
      "zniknęła",
      "zniszczyć",
      "znoszę",
      "znowu",
      "znów",
      "zobacz",
      "zobaczenia",
      "zobaczmy",
      "zobaczy",
      "zobaczymy",
      "zobaczysz",
      "zobaczyć",
      "zobaczył",
      "zobaczyłam",
      "zobaczyłem",
      "zobaczą",
      "zobaczę",
      "zoo",
      "zostaje",
      "zostajesz",
      "zostaję",
      "zostali",
      "zostaliśmy",
      "zostanie",
      "zostaniemy",
      "zostaniesz",
      "zostaną",
      "zostanę",
      "zostaw",
      "zostawcie",
      "zostawiaj",
      "zostawisz",
      "zostawić",
      "zostawię",
      "zostawił",
      "zostawiła",
      "zostawiłem",
      "zostawiłeś",
      "zostać",
      "został",
      "została",
      "zostałam",
      "zostałem",
      "zostałeś",
      "zostało",
      "zostały",
      "zostań",
      "zostańcie",
      "zresztą",
      "zrobi",
      "zrobicie",
      "zrobienia",
      "zrobili",
      "zrobiliście",
      "zrobiliśmy",
      "zrobimy",
      "zrobione",
      "zrobisz",
      "zrobią",
      "zrobić",
      "zrobię",
      "zrobił",
      "zrobiła",
      "zrobiłam",
      "zrobiłaś",
      "zrobiłbym",
      "zrobiłem",
      "zrobiłeś",
      "zrobiło",
      "zrozum",
      "zrozumiano",
      "zrozumiał",
      "zrozumiałam",
      "zrozumiałem",
      "zrozumiałeś",
      "zrozumie",
      "zrozumiesz",
      "zrozumieć",
      "zrób",
      "zróbcie",
      "zróbmy",
      "zupełnie",
      "zwariowałeś",
      "zwierząt",
      "zwierzę",
      "zwierzęta",
      "związane",
      "związek",
      "związku",
      "zwolnij",
      "zwolniony",
      "zwolnić",
      "zwrócić",
      "zwycięstwo",
      "zwykle",
      "zwykła",
      "zwykłe",
      "zwykły",
      "zwykłym",
      "zwłaszcza",
      "zwłoki",
      "zęby",
      "zębów",
      "zła",
      "złamać",
      "złap",
      "złapać",
      "złapał",
      "złapią",
      "złaź",
      "złe",
      "złego",
      "zło",
      "złota",
      "złotko",
      "złoto",
      "złożyć",
      "zły",
      "złych",
      "złym",
      "złą",
      "ćwiczenia",
      "ładna",
      "ładne",
      "ładnie",
      "ładny",
      "ładunek",
      "łap",
      "łapy",
      "łatwe",
      "łatwiej",
      "łatwo",
      "łazience",
      "łazienki",
      "łeb",
      "łodzi",
      "łódź",
      "łóżka",
      "łóżko",
      "łóżku",
      "łączy",
      "ścianie",
      "ściany",
      "ścianę",
      "ściśle",
      "ślad",
      "śladu",
      "ślady",
      "śladów",
      "śledztwo",
      "ślepy",
      "ślub",
      "ślubie",
      "ślubu",
      "śmiało",
      "śmieci",
      "śmierci",
      "śmiercią",
      "śmierdzi",
      "śmierć",
      "śmiesz",
      "śmieszne",
      "śmieszny",
      "śniadanie",
      "śnie",
      "śnieg",
      "śpi",
      "śpiewać",
      "śpij",
      "śpisz",
      "środek",
      "środka",
      "środki",
      "środku",
      "świadek",
      "świadka",
      "świadkiem",
      "świadków",
      "świat",
      "świata",
      "światem",
      "światu",
      "światła",
      "światło",
      "świecie",
      "świetle",
      "świetna",
      "świetne",
      "świetnie",
      "świetny",
      "świetnym",
      "świetną",
      "świeże",
      "świąt",
      "święta",
      "świętego",
      "święty",
      "źle",
      "źli",
      "źródła",
      "źródło",
      "żaden",
      "żadna",
      "żadne",
      "żadnego",
      "żadnej",
      "żadnych",
      "żadnym",
      "żal",
      "żarcie",
      "żart",
      "żartowałem",
      "żartujesz",
      "żartuję",
      "żarty",
      "żałuję",
      "że",
      "żeby",
      "żebym",
      "żebyś",
      "żebyście",
      "żebyśmy",
      "żegnaj",
      "żona",
      "żonaty",
      "żonie",
      "żony",
      "żoną",
      "żonę",
      "żołnierz",
      "żołnierze",
      "żołnierzem",
      "żołnierzu",
      "żołnierzy",
      "życia",
      "życie",
      "życiem",
      "życiu",
      "życzenie",
      "życzę",
      "żyje",
      "żyjemy",
      "żyjesz",
      "żyją",
      "żyję",
      "żywcem",
      "żywego",
      "żywo",
      "żywy",
      "żyć",
      "żył",
      "żyła",
    ];
  }
}
