import 'package:imitatio/src/datasets/models/food.dart';

class PlFoodData extends FoodDataset {
  @override
  List<String> get dishes {
    return const [
      "Babka żółtkowa",
      "Baranina",
      "Barszcz",
      "Barszcz biały",
      "Bigos",
      "Bliny gryczane",
      "Bryjka",
      "Bryndza",
      "Budyń",
      "Bundz",
      "Cebulniaczki",
      "Cepeliny",
      "Chałka",
      "Chleb biebrzański",
      "Chłodnik",
      "Czernina",
      "Czosnianka",
      "Fasolka z migdałami",
      "Faworki (chrusty)",
      "Flaki or flaczki",
      "Golonka",
      "Gołka",
      "Gołąbki",
      "Grochówka",
      "Grzyby po żmudzku",
      "Gulasz",
      "Kabanosy",
      "Kaczka z jabłkami",
      "Kapusta kiszona",
      "Kapusta zasmażana",
      "Kapuśniak",
      "Karkówka",
      "Karkówka",
      "Kartacz",
      "Kartoflanka",
      "Kartofle gotowane",
      "Kasza gryczana",
      "Kasza gryczana ze skwarkami",
      "Kaszanka",
      "Kaszanka",
      "Kawior z bakłażana",
      "Kiełbasa",
      "Kiełbasa",
      "Kiełbasa Biała",
      "Kiełbasa Parówkowa",
      "Kiełbasa Szynkowa",
      "Kisiel",
      "Kiszka ziemniaczana",
      "Klopsiki",
      "Kopytka",
      "Kopytka",
      "Korycinski",
      "Kotlet mielony",
      "Kotlet schabowy",
      "Kreple z lejka",
      "Krupnik",
      "Krówki",
      "Kugiel ze skwarkami",
      "Kurczak pieczony po wiejsku",
      "Kutia",
      "Kwaśnica",
      "Makowiec",
      "Makowiec",
      "Mazurek",
      "Melszpejz zaparzany z jabłek",
      "Mizeria",
      "Naleśniki",
      "Ogórek kiszony",
      "Ogórek konserwowy",
      "Okoń smażony, w zalewie octowej",
      "Oscypek",
      "Pasztecik szczeciński",
      "Pańska skórka",
      "Pieczarki marynowane",
      "Pieczeń cielęca",
      "Pieczeń wiedźmy",
      "Pieczeń wieprzowa z winem",
      "Pieczeń z mięsa mielonego",
      "Pierniki",
      "Pierogi",
      "Pischinger chocolate oblaten cake",
      "Placki kartoflane = ''",
      "Polędwiczki wołowe",
      "Ppieczarkowa",
      "Proziaki",
      "Przysmak piwny",
      "Pyzy",
      "Pączek",
      "Rolada z kurczakiem i pieczarkami",
      "Rolada z mięsa mielonego z pieczarkami",
      "Rosół",
      "Ruskie pierogi",
      "Ryba smażona",
      "Sałatka burakowa (buraczki)",
      "Sałatka ogórkowa",
      "Sałatka warzywana (sałatka jarzynowa)",
      "Sałatka wiosenna",
      "Sałatka z boczkiem",
      "Sałatka z kartofli (sałatka ziemniaczana)",
      "Sałatka z krewetek",
      "Sałatka",
      "Schab faszerowany",
      "Sernik",
      "Strudel jabłkowy",
      "Surówka z białej kapusty",
      "Surówka z marchewki",
      "Surówka",
      "Szodo",
      "Sękacz",
      "Ślepe ryby",
      "Tort ziemniaczany",
      "Twaróg",
      "Tłuczone ziemniaki",
      "Wołowina pieczona",
      "Ziemniaczane",
      "Zrazy",
      "Zrazy wołyńskie",
      "Zrazy zawijane",
      "Zucielki",
      "Zupa borowikowa",
      "Zupa buraczkowa",
      "Zupa grzybowa",
      "Zupa jarzynowa",
      "Zupa ogórkowa",
      "Zupa pomidorowa",
      "Zupa szczawiowa",
      "Ćwikła",
      "Łosoś",
      "Śliwowica łącka",
      "Śmietana",
      "Żeberka wieprzowe po żmudzku",
      "Żeberka wędzone",
      "Żentyca",
      "Żur",
      "Żurek",
    ];
  }

  @override
  List<String> get drinks {
    return const [
      "Absynt",
      "Ajerkoniak",
      "Alasz",
      "Amaretto",
      "Anyżówka",
      "Aperitif",
      "Arak",
      "Archi",
      "Armagnac",
      "Atafia",
      "Bimber",
      "Brandy",
      "Burbon",
      "Calvados",
      "Colonche",
      "Cytrynówka",
      "Dereniówka",
      "Genever",
      "Gin",
      "Goldwasser",
      "Grappa",
      "Koniak",
      "Kumys",
      "Likiery",
      "Long drink",
      "Malibu",
      "Malinówka",
      "Miód pitny",
      "Nalewka z wiśni",
      "Orzechówka",
      "Piołunówka",
      "Piwo",
      "Piwo ciemne",
      "Piwo jasne",
      "Podpiwek",
      "Porter",
      "Pálinka",
      "Rakija",
      "Rum",
      "Rumtopf",
      "Sake",
      "Sambuca",
      "Schnapps",
      "Siwucha",
      "Smorodinówka",
      "Tequila",
      "Whisky",
      "Winiak",
      "Wiśniówka",
      "Wódka",
      "Śliwowica",
      "Żubrówka",
    ];
  }

  @override
  List<String> get fruits {
    return const [
      "Agrest",
      "Ananas",
      "Arbuz",
      "Aronia",
      "Banan",
      "Borówka",
      "Brzoskwinia",
      "Cytryna",
      "Czereśnia",
      "Durian",
      "Flaszowiec",
      "Grejpfrut",
      "Gruszka",
      "Jabłko",
      "Jagoda kamczacka",
      "Jagoda",
      "Jeżyna",
      "Kaki",
      "Karambola",
      "Kasztan jadalny",
      "Kiwi",
      "Kokos",
      "Kumkwat",
      "Malina",
      "Malinojeżyna",
      "Mandarynka",
      "Mango",
      "Melon",
      "Morela",
      "Pigwa",
      "Pomarańcza",
      "Porzeczka",
      "Poziomka",
      "Renkloda",
      "Truskawka",
      "Winogrono",
      "Wiśnia",
      "Śliwka",
      "Żurawina",
    ];
  }

  @override
  List<String> get spices {
    return const [
      "Anyż",
      "Anyżek",
      "Bazylia",
      "Chili",
      "Chrzan",
      "Cynamon",
      "Czarnuszka",
      "Czosnek",
      "Cząber",
      "Estragon",
      "Gałka muszkatołowa",
      "Gorczyca",
      "Goździki",
      "Hyzop lekarski",
      "Imbir",
      "Jałowiec",
      "Kminek",
      "Kolendra",
      "Koper",
      "Koperek",
      "Kozieradka",
      "Kurkuma",
      "Liście laurowe",
      "Lubczyk",
      "Macierzanka",
      "Majeranek",
      "Melisa",
      "Mięta",
      "Natka pietruszki",
      "Oregano",
      "Papryka",
      "Pieprz",
      "Rozmaryn",
      "Rzeżucha",
      "Szafran",
      "Szałwia",
      "Szczypiorek",
      "Tymianek",
      "Ziele angielskie",
      "Zioła prowansalskie",
    ];
  }

  @override
  List<String> get vegetables {
    return const [
      "Bakłażan",
      "Batat",
      "Bobik",
      "Botwina",
      "Brokuł",
      "Brukiew",
      "Brukselka",
      "Burak",
      "Bób",
      "Cebula",
      "Chrzan",
      "Ciecierzyca",
      "Cukinia",
      "Cykoria",
      "Czosnek",
      "Czyściec bulwiasty",
      "Dynia",
      "Endywia",
      "Fasola",
      "Fasolka szparagowa",
      "Fenkuł",
      "Groch",
      "Groch włoski",
      "Ignam",
      "Jarmuż",
      "Kabaczek",
      "Kalafior",
      "Kalarepa",
      "Kapar",
      "Kapusta",
      "Kapusta pekińska",
      "Karczoch",
      "Kartofel",
      "Kiwano",
      "Kolczoch",
      "Koper",
      "Koper włoski",
      "Kukurydza",
      "Maniok",
      "Marchew",
      "Marchew czarna",
      "Miechunka",
      "Oberżyna",
      "Ogórek",
      "Oliwka",
      "Papryka",
      "Papryka cayenne",
      "Papryka chili",
      "Pasternak",
      "Patat",
      "Patison",
      "Peperoni",
      "Pieprzyca siewna",
      "Pietruszka",
      "Pochrzyn",
      "Pomidor",
      "Por",
      "Portulaka",
      "Pyra",
      "Rabarbar",
      "Roszponka",
      "Rukola",
      "Rzepa",
      "Rzeżucha",
      "Rzodkiewka",
      "Salsefia",
      "Sałata",
      "Sałata dębolistna",
      "Sałata rzymska",
      "Seler",
      "Skorzonera",
      "Soczewica",
      "Soja",
      "Szalotka",
      "Szczaw",
      "Szczypior",
      "Szparag",
      "Szpinak",
      "Wasabi",
      "Ziemniak",
    ];
  }
}
