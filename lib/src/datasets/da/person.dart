import 'package:imitatio/src/datasets/models/person.dart';
import 'package:imitatio/src/enums.dart';

class DaPersonData extends PersonDataset {
  @override
  List<String> get academicDegrees {
    return const [
      "Cand.oecon.",
      "Cand.polit.",
      "Cand.psychol.",
      "Cand.scient.",
      "Cand.scient.pol.",
      "Cand.med.",
      "Civiløkonom",
    ];
  }

  @override
  List<String> get genders {
    return const ["Kvinde", "Han", "Neutral"];
  }

  @override
  List<String> get languages {
    return const [
      "Abkhasisk",
      "Afar",
      "Afrikaans",
      "Akan",
      "Albansk",
      "Amharisk",
      "Arabisk",
      "Aragonisk",
      "Armensk",
      "Aserbajdsjansk",
      "Assamesisk",
      "Avarisk",
      "Avestisk",
      "Aymara",
      "Bambara",
      "Basjkirsk",
      "Baskisk",
      "Bengali",
      "Bihari",
      "Bislama",
      "Bosnisk",
      "Bretonsk",
      "Bulgarsk",
      "Burmesisk",
      "Chamorro",
      "Chichewa",
      "Congolesisk",
      "Cree",
      "Dansk",
      "Dhivehi",
      "Dzongkha",
      "Engelsk",
      "Esperanto",
      "Estisk",
      "Ewe",
      "Fijisk",
      "Finsk",
      "Fransk",
      "Frisisk",
      "Fulfulde",
      "Færøsk",
      "Galicisk",
      "Gassisk",
      "Georgisk",
      "Gikuyu",
      "Græsk",
      "Guaraní",
      "Gujarati",
      "Haitisk kreolsk",
      "Hausa",
      "Hebraisk",
      "Herero",
      "Hindi",
      "Hiri motu",
      "Hollandsk",
      "Hviderussisk",
      "Ibo",
      "Ido",
      "Indonesisk",
      "Interlingue",
      "Inuittisk",
      "Inupiak",
      "Irsk",
      "Islandsk",
      "Italiensk",
      "Japansk",
      "Javanesisk",
      "Jiddisch",
      "Kalaallisut",
      "Kannada",
      "Kanuri",
      "Kasakhisk",
      "Kashmirisk",
      "Katalansk",
      "Khmer",
      "Kinesisk",
      "Kinyarwanda",
      "Kirgisisk",
      "Kirkeslavisk",
      "Kirundi",
      "Komi",
      "Koreansk",
      "Kornisk",
      "Korsikansk",
      "Kroatisk",
      "Kurdisk",
      "Kwanyama",
      "Laotisk",
      "Latin",
      "Lettisk",
      "Letzeburgsk",
      "Limburgisk",
      "Lingala",
      "Litauisk",
      "Luba-Katanga",
      "Luganda",
      "Makedonsk",
      "Malajisk",
      "Malayalam",
      "Maltesisk",
      "Mansk",
      "Maoriski",
      "Marathi",
      "Marshallesisk",
      "Moldovisk",
      "Mongolsk",
      "Naurisk",
      "Navajo",
      "Ndonga",
      "Nepali",
      "Nord-ndebele",
      "Nordsamisk",
      "Norsk",
      "Norsk Bokmål",
      "Nynorsk",
      "Occitansk",
      "Ojibwa",
      "Oriya",
      "Oromo",
      "Ossetisk",
      "Pali",
      "Pashto",
      "Persisk",
      "Polsk",
      "Portugisisk",
      "Punjabi",
      "Quechua",
      "Rumænsk",
      "Russisk",
      "Rætoromansk",
      "Samoansk",
      "Sango",
      "Sanskrit",
      "Sardisk",
      "Serbisk",
      "Serbokroatisk",
      "Sesotho",
      "Shona",
      "Sindhi",
      "Singalesisk",
      "Skotsk gælisk",
      "Slovakisk",
      "Slovensk",
      "Somali",
      "Spansk",
      "Sundanesisk",
      "Svensk",
      "Swahili",
      "Swati",
      "Syd-ndebele",
      "Tadtjikkisk",
      "Tagalog",
      "Tahitisk",
      "Tamilsk",
      "Tatarsk",
      "Telugu",
      "Thai",
      "Tibetansk",
      "Tigrinya",
      "Tjekkisk",
      "Tjetjensk",
      "Tonganesisk",
      "Tsjuvansk",
      "Tsonga",
      "Tswana",
      "Turkmensk",
      "Twi",
      "Tyrkisk",
      "Tysk",
      "Uighur",
      "Ukrainsk",
      "Ungarsk",
      "Urdu",
      "Usbekisk",
      "Vallonsk",
      "Venda",
      "Vietnamesisk",
      "Volapük",
      "Walisisk",
      "Wolof",
      "Xhosa",
      "Yi",
      "Yoruba",
      "Zhuang",
      "Zulu",
    ];
  }

  @override
  List<String> names(Gender gender) {
    return switch (gender) {
      Gender.male => const [
          "Abiorn",
          "Absalon",
          "Absolon",
          "Adolf",
          "Adrian",
          "Afkar",
          "Aggi",
          "Aghen",
          "Aghia",
          "Aghvarth",
          "Aghwith",
          "Agir",
          "Agner",
          "Agot",
          "Aki",
          "Akkar",
          "Aksel",
          "Albert",
          "Alboldus",
          "Albrecht",
          "Albricus",
          "Alexander",
          "Alf",
          "Alfkil",
          "Alfred",
          "Alfrik",
          "Alfwin",
          "Alger",
          "Algot",
          "Algut",
          "Ali",
          "Alli",
          "Alvar",
          "Alvin",
          "Amandus",
          "Ambrosius",
          "Amdi",
          "Amlethus",
          "Amundi",
          "Andreas",
          "Ani",
          "Ankarl",
          "Anker",
          "Ansgar",
          "Anund",
          "Apsolon",
          "Ari",
          "Arnbiorn",
          "Arnfast",
          "Arnger",
          "Arngerus",
          "Arni",
          "Arnketil",
          "Arno",
          "Arnold",
          "Arnsten",
          "Arnulf",
          "Arnwith",
          "Aron",
          "Asalonis",
          "Asel",
          "Asfrith",
          "Asger",
          "Asgut",
          "Asi",
          "Askil",
          "Asl",
          "Aslak",
          "Aslef",
          "Asmund",
          "Asser",
          "Asti",
          "Astrath",
          "Asulf",
          "Aswaldi",
          "Aswith",
          "Athislus",
          "Atli",
          "Atti",
          "Aughe",
          "Auti",
          "Auwe",
          "Awarth",
          "Axelen",
          "Axeln",
          "Axilen",
          "Axlan",
          "Azur",
          "Balli",
          "Barni",
          "Barth",
          "Bassi",
          "Beghel",
          "Bendix",
          "Benedikt",
          "Bengt",
          "Bent",
          "Berengarius",
          "Berner",
          "Berngerus",
          "Bernhard",
          "Bernt",
          "Bertel",
          "Biarni",
          "Biaver",
          "Biergh",
          "Billing",
          "Biorn",
          "Birger",
          "Bise",
          "Bjerne",
          "Blek",
          "Blesse",
          "Bo",
          "Bodwar",
          "Bolde",
          "Bole",
          "Bondi",
          "Boris",
          "Bosi",
          "Botmund",
          "Botolphus",
          "Botulf",
          "Botwith",
          "Bovi",
          "Boy",
          "Boye",
          "Bram",
          "Brand",
          "Brande",
          "Brawith",
          "Breide",
          "Brod",
          "Broder",
          "Brothir",
          "Brun",
          "Bruni",
          "Brunkil",
          "Brunman",
          "Bruno",
          "Bruse",
          "Bryniulf",
          "Bryti",
          "Buggi",
          "Bulle",
          "Bundi",
          "Børge",
          "Bøsi",
          "Carsten",
          "Christian",
          "Christiern",
          "Christopher",
          "Claes",
          "Claudius",
          "Claus",
          "Clement",
          "Constantin",
          "Cornelius",
          "Daniel",
          "Danske",
          "David",
          "Detlef",
          "Dierf",
          "Dines",
          "Dirch",
          "Ditlev",
          "Diuri",
          "Dome",
          "Dotta",
          "Dreng",
          "Dwerg",
          "Dyri",
          "Ege",
          "Eggert",
          "Eghil",
          "Ejler",
          "Ejvind",
          "Elef",
          "Elevus",
          "Elias",
          "Enar",
          "Enevold",
          "Engelbrecht",
          "Engli",
          "Erhardt",
          "Erik",
          "Eringisl",
          "Erland",
          "Ernbiorn",
          "Ernst",
          "Erra",
          "Esben",
          "Esbern",
          "Esbiorn",
          "Farman",
          "Farthin",
          "Fasti",
          "Fastulf",
          "Fathi",
          "Fathir",
          "Felix",
          "Fet",
          "Filippus",
          "Fin",
          "Find",
          "Finulf",
          "Finwith",
          "Fliri",
          "Folki",
          "Folkmar",
          "Folmer",
          "Fot",
          "Fredegæst",
          "Frethi",
          "Frithbiorn",
          "Frithlef",
          "Frosti",
          "Frothi",
          "Frøsten",
          "Ful",
          "Fundin",
          "Gunhwat",
          "Gunkil",
          "Gunlef",
          "Gunnar",
          "Gunni",
          "Gunnulf",
          "Gunsten",
          "Gunwald",
          "Gunward",
          "Guse",
          "Gustaf",
          "Gustav",
          "Gute",
          "Guthbiorn",
          "Guthbrand",
          "Guthfast",
          "Guthi",
          "Guthir",
          "Guthki",
          "Guthmar",
          "Guthmund",
          "Guthrik",
          "Guththorm",
          "Guzi",
          "Gyrth",
          "Gything",
          "Gæsling",
          "Gøt",
          "Gøtar",
          "Gøti",
          "Haghbarth",
          "Haghni",
          "Hake",
          "Haki",
          "Haklang",
          "Hakun",
          "Halfdan",
          "Halli",
          "Hals",
          "Halsten",
          "Halwarth",
          "Hamund",
          "Hani",
          "Harald",
          "Harek",
          "Harth",
          "Harthe",
          "Hartvig",
          "Hasse",
          "Hasten",
          "Hawarth",
          "Hefni",
          "Heggi",
          "Heino",
          "Helf",
          "Helghi",
          "Helmer",
          "Hemming",
          "Hen",
          "Henkil",
          "Henning",
          "Henrik",
          "Herbert",
          "Herbiorn",
          "Hereth",
          "Herewardus",
          "Herluf",
          "Herlugh",
          "Hermoth",
          "Herulf",
          "Herwarth",
          "Hiarni",
          "Hielm",
          "Hildulf",
          "Hilmer",
          "Hithin",
          "Hoat",
          "Hoghni",
          "Holm",
          "Holmger",
          "Holmsten",
          "Holmwith",
          "Holte",
          "Horsten",
          "Hovi",
          "Hubert",
          "Hughe",
          "Hughlek",
          "Hugo",
          "Huldrik",
          "Hwit",
          "Hwithoveth",
          "Høk",
          "Jan",
          "Jannik",
          "Jeip",
          "Jens",
          "Jep",
          "Jeppe",
          "Jes",
          "Jesper",
          "Joachim",
          "Joannes",
          "Johan",
          "Jon",
          "Jonas",
          "Jones",
          "Josef",
          "Joseph",
          "Julian",
          "Juris",
          "Jænis",
          "Jønis",
          "Jørgen",
          "Ketil",
          "Ketilbiorn",
          "Ketilløgh",
          "Kierulf",
          "Kjeld",
          "Klak",
          "Klakke",
          "Klakki",
          "Klement",
          "Klint",
          "Knag",
          "Knut",
          "Kol",
          "Koll",
          "Kolle",
          "Konrad",
          "Konstantin",
          "Kragh",
          "Kristen",
          "Kristiarn",
          "Kristoffer",
          "Krok",
          "Krum",
          "Krusa",
          "Kul",
          "Kulben",
          "Kulle",
          "Kuno",
          "Kurt",
          "Kuse",
          "Lage",
          "Lagho",
          "Larentius",
          "Lars",
          "Lasse",
          "Lau",
          "Lauge",
          "Laurentius",
          "Laurits",
          "Lave",
          "Leg",
          "Lene",
          "Leonhard",
          "Lief",
          "Lippi",
          "Litle",
          "Liutulf",
          "Lok",
          "Lokki",
          "Lorenz",
          "Lothen",
          "Lowe",
          "Ludvig",
          "Lukas",
          "Læff",
          "Markus",
          "Marten",
          "Martin",
          "Martinus",
          "Mathias",
          "Mattheus",
          "Matthias",
          "Michael",
          "Mikkel",
          "Mogens",
          "Molti",
          "Morten",
          "Mourits",
          "Muli",
          "Nakke",
          "Nanni",
          "Nase",
          "Nicolaus",
          "Niels",
          "Niglæs",
          "Niklas",
          "Nis",
          "Nisse",
          "Norman",
          "Normand",
          "Næwe",
          "Odd",
          "Odder",
          "Oddger",
          "Oddi",
          "Odstanus",
          "Ofrath",
          "Olaf",
          "Oli",
          "Oliver",
          "Oluf",
          "Orm",
          "Ormar",
          "Osmund",
          "Ostman",
          "Other",
          "Othin",
          "Othinkar",
          "Ottar",
          "Otto",
          "Ove",
          "Pa",
          "Palni",
          "Paul",
          "Paulus",
          "Peder",
          "Pelle",
          "Per",
          "Peter",
          "Petrus",
          "Philip",
          "Poul",
          "Pridbjørn",
          "Raggi",
          "Raghi",
          "Rand",
          "Randi",
          "Rani",
          "Rasmus",
          "Rathi",
          "Rathulf",
          "Rawald",
          "Raymond",
          "Regin",
          "Regner",
          "Regni",
          "Regnwald",
          "Reimer",
          "Reimund",
          "Reinhard",
          "Reinhardt",
          "Reinhold",
          "Reinholdt",
          "Rek",
          "Rether",
          "Richard",
          "Righer",
          "Rik",
          "Rikard",
          "Riki",
          "Rikulf",
          "Ring",
          "Rink",
          "Roald",
          "Roar",
          "Robert",
          "Rodulf",
          "Rother",
          "Rothger",
          "Rothmar",
          "Roulf",
          "Rudolf",
          "Rune",
          "Runi",
          "Rut",
          "Ræf",
          "Rørik",
          "Røth",
          "Røthe",
          "Sporgh",
          "Spurgh",
          "Stak",
          "Staki",
          "Stali",
          "Star",
          "Starki",
          "Stefan",
          "Steffen",
          "Sten",
          "Stener",
          "Stenkil",
          "Stig",
          "Store",
          "Stuf",
          "Stygg",
          "Styr",
          "Styrbiorn",
          "Styreman",
          "Styrkar",
          "Styrman",
          "Sulki",
          "Suni",
          "Sutere",
          "Svantepolk",
          "Sviðandi",
          "Swart",
          "Swarte",
          "Swarthofthi",
          "Swave",
          "Swavi",
          "Swen",
          "Sweni",
          "Swening",
          "Swenki",
          "Swerting",
          "Söffren",
          "Söfren",
          "Søren",
          "Súla",
          "Thorbiorn",
          "Thorbjørn",
          "Thorfast",
          "Thorfin",
          "Thorgarth",
          "Thorger",
          "Thorgisl",
          "Thorgny",
          "Thorgot",
          "Thorgrim",
          "Thori",
          "Thorir",
          "Thorkil",
          "Thorlaf",
          "Thormar",
          "Thormoth",
          "Thormund",
          "Thorn",
          "Thorsten",
          "Thorth",
          "Thorwith",
          "Throkil",
          "Thrond",
          "Throtti",
          "Thrugels",
          "Thrugils",
          "Thrugot",
          "Thure",
          "Thurgot",
          "Thuri",
          "Thurir",
          "Thyrger",
          "Thythger",
          "Thythkil",
          "Thythulf",
          "Thøger",
          "Tobbi",
          "Tobias",
          "Tofi",
          "Toki",
          "Tokki",
          "Tole",
          "Toli",
          "Tomas",
          "Tomi",
          "Torben",
          "Torlacus",
          "Tosti",
          "Toti",
          "Tovi",
          "Trana",
          "Truels",
          "Trugot",
          "Truls",
          "Tryggi",
          "Tubbi",
          "Tuki",
          "Tuli",
          "Tulir",
          "Tumi",
          "Tuni",
          "Tuvi",
          "Tyge",
          "Tyki",
          "Tøger",
          "Tønnes",
          "Túki",
          "Túlir",
          "Túvi",
          "Ubbi",
          "Udder",
          "Uddi",
          "Uffe",
          "Uffi",
          "Ufred",
          "Unøss",
          "Uthwagin",
          "Valdemar",
          "Verner",
          "Vigge",
          "Viggo",
          "Vilhelm",
          "Villads",
          "Ville",
          "Villum",
          "Vincent",
          "Volmer",
          "Vøgg",
          "Waghn",
          "Walderik",
          "Wale",
          "Walther",
          "Wari",
          "Wemund",
          "Wermund",
          "Westi",
          "Wether",
          "Wifrith",
          "Wiger",
          "Wigh",
          "Wighlek",
          "Wighman",
          "Wigil",
          "Wigot",
          "Wikar",
          "Wiking",
          "Willi",
          "Wimar",
          "Winter",
          "Wisten",
          "Withi",
          "Wrage",
          "Wraghi",
          "Wrange",
          "Wreth",
          "Wæne",
          "Wæther",
          "Øgoti",
          "Øgrim",
          "Øiar",
          "Økil",
          "Ølvir",
          "Ømund",
          "Øn",
          "Ønder",
          "Øpi",
          "Ørn",
          "Østen",
          "Øthbiorn",
          "Øthger",
          "Øthi",
          "Øthin",
          "Øthkil",
          "Øthulf",
        ],
      Gender.female => const [
          "Abelone",
          "Adele",
          "Adelheid",
          "Adhelin",
          "Agathe",
          "Agnes",
          "Agnete",
          "Alvild",
          "Alwigh",
          "Anna",
          "Anne",
          "Annika",
          "Arløgh",
          "Arnfrith",
          "Asa",
          "Asgun",
          "Aswar",
          "Barbara",
          "Beate",
          "Beatrice",
          "Benedikte",
          "Bente",
          "Berta",
          "Bertha",
          "Birgitta",
          "Birgitte",
          "Bodil",
          "Boel",
          "Boeld",
          "Bothild",
          "Bothilda",
          "Bothilde",
          "Catharina",
          "Cecilia",
          "Cecilie",
          "Christa",
          "Christina",
          "Dagmar",
          "Dagmár",
          "Dorete",
          "Dorethe",
          "Dorothea",
          "Dorte",
          "Dorthe",
          "Dota",
          "Dotir",
          "Dótta",
          "Edel",
          "Edela",
          "Elena",
          "Elin",
          "Eline",
          "Elisa",
          "Elisabet",
          "Elisabeth",
          "Elise",
          "Elizabeth",
          "Ella",
          "Ellen",
          "Ellesef",
          "Elna",
          "Else",
          "Elsebet",
          "Elsebeth",
          "Elsef",
          "Emma",
          "Esgerth",
          "Esther",
          "Estrith",
          "Ethla",
          "Etta",
          "Eva",
          "Flora",
          "Frogertha",
          "Frøgerth",
          "Gera",
          "Gerd",
          "Gerhild",
          "Gerløgh",
          "Gerthrud",
          "Gertrud",
          "Gerwar",
          "Gerwara",
          "Gisela",
          "Gisla",
          "Greta",
          "Grethe",
          "Gro",
          "Gunbiørgh",
          "Gunhild",
          "Gunløgh",
          "Gunna",
          "Gunnild",
          "Gunnløg",
          "Gunnur",
          "Guthrun",
          "Gyda",
          "Gye",
          "Gylla",
          "Gyrith",
          "Gytha",
          "Gørvel",
          "Hedvig",
          "Helena",
          "Helga",
          "Helgha",
          "Helle",
          "Helvig",
          "Henrikke",
          "Herlef",
          "Hildegard",
          "Hildigard",
          "Hillegier",
          "Hillegierd",
          "Hillevi",
          "Holgerth",
          "Holmfrith",
          "Hyldeburg",
          "Ida",
          "Ilse",
          "Inga",
          "Inger",
          "Ingertha",
          "Ingiara",
          "Ingiburgh",
          "Ingifrith",
          "Ingigerth",
          "Ingimoth",
          "Ingirith",
          "Ingiwara",
          "Iohanna",
          "Isabella",
          "Iulfrith",
          "Johanna",
          "Judith",
          "Juliana",
          "Justina",
          "Jutta",
          "Jytte",
          "Kaja",
          "Karen",
          "Karna",
          "Katharina",
          "Kathrine",
          "Katrine",
          "Ketilelf",
          "Ketilfrith",
          "Ketilløgh",
          "Kirsten",
          "Kirstine",
          "Kristina",
          "Lathgertha",
          "Lena",
          "Lene",
          "Lisbeth",
          "Lise",
          "Magdalene",
          "Magnhild",
          "Maja",
          "Malene",
          "Maren",
          "Margareta",
          "Margaretha",
          "Margrete",
          "Margrethe",
          "Maria",
          "Marianne",
          "Marie",
          "Marina",
          "Marta",
          "Martha",
          "Mathilde",
          "Merete",
          "Merethe",
          "Mette",
          "Moder",
          "Motha",
          "Neel",
          "Oda",
          "Olof",
          "Ottilie",
          "Pernille",
          "Petronilla",
          "Ragnfrith",
          "Rakel",
          "Regitze",
          "Regna",
          "Regnhild",
          "Regnild",
          "Rigmár",
          "Sanne",
          "Sidsel",
          "Sighburgh",
          "Sighni",
          "Sighrith",
          "Signe",
          "Sille",
          "Sisse",
          "Sissel",
          "Sofie",
          "Susanne",
          "Tekla",
          "Thekla",
          "Thora",
          "Thorfred",
          "Thorgard",
          "Thorgun",
          "Thorgund",
          "Thorgunn",
          "Thorhild",
          "Thorith",
          "Thorlof",
          "Thorun",
          "Thorwar",
          "Thorwe",
          "Thrugun",
          "Thura",
          "Thyri",
          "Thyrvi",
          "Thyrwi",
          "Toka",
          "Tola",
          "Toma",
          "Tonna",
          "Tova",
          "Una",
          "Ursula",
          "Valborg",
          "Vibeke",
          "Wibeke",
          "Wiburgh",
          "Wifrith",
          "Ædel",
          "Øllegård",
          "Øthgerth",
        ],
    };
  }

  @override
  List<String> nationalities(Gender gender) {
    return switch (gender) {
      _ => const [
          "Amerikansk",
          "Arabisk",
          "Bosnisk",
          "Dansk",
          "Engelsk",
          "Finsk",
          "Finsk",
          "Fransk",
          "Færøsk",
          "Hindi",
          "Islandsk",
          "Japansk",
          "Kinesisk",
          "Norsk",
          "Persisk",
          "Polsk",
          "Russisk",
          "Serbisk",
          "Spansk",
          "Svensk",
          "Thai",
          "Tyrkisk",
          "Tysk",
          "Ungarsk",
          "Urdu",
        ]
    };
  }

  @override
  List<String> get occupations {
    return const [
      "Adm. direktør",
      "Advokat",
      "Advokatfuldmægtig",
      "Aut. el installatør",
      "Avisbud",
      "Bankassistent",
      "Butiksslagter",
      "Bybuschauffør",
      "Børnepædagog",
      "Chef projektleder",
      "Cykelbud",
      "Dommer",
      "Dyrlæge",
      "Ejendomsmægler",
      "Elektriker",
      "Flymekaniker",
      "Forretningsudvikler",
      "Fotomodel",
      "Frisørarbejde",
      "Fysioterapeut",
      "Gadefejning og beslægtet arbejde",
      "Handelsskolelærer privat",
      "Informationsarbejde på kundekontaktcentre",
      "Journalistisk arbejde",
      "Journalistisk kommunikationsmedarbejder",
      "Kasseassistentarbejde og beslægtet kundebetjening",
      "Kriminalassisent",
      "Kurator",
      "Kurer",
      "Kørerlærer",
      "Ledelse af reklame og pr",
      "Maskiningeniør",
      "Matematiker",
      "Medhjælp i butik mv.",
      "Mekanikerarbejde, person- og varevogne",
      "Murer",
      "Postomdeler",
      "Pr/reklame",
      "Programmering inden for softwareudvikling",
      "Psykologarbejde",
      "Reklamedesigner",
      "Rengøring af kontorer og beboelsesområder",
      "Rutebilchauffør",
      "Salgsarbejde i kundekontaktcentre",
      "Shippingassistent",
      "Skolelærer på grundskoleniveau",
      "Skuespiller",
      "Smed",
      "Statistiker",
      "Tandlæge",
      "Telefonsælger",
      "Togfører",
      "Tømrer",
      "Valuar",
      "Vvs montør",
      "Webudvikler",
      "Økonom",
    ];
  }

  @override
  List<String> get politicalViews {
    return const [
      "Apartisk",
      "Centrist",
      "Frihedsforkæmper",
      "Kommunist",
      "Konservativ",
    ];
  }

  @override
  List<String> surnames(Gender gender) {
    return switch (gender) {
      _ => const [
          "Aaen",
          "Aagaard",
          "Aagesen",
          "Aarup",
          "Abdi",
          "Abel",
          "Abildgaard",
          "Abildtrup",
          "Abrahamsen",
          "Adamsen",
          "Agerskov",
          "Agger",
          "Aggerholm",
          "Ahmad",
          "Ahmed",
          "Akhtar",
          "Albertsen",
          "Albrechtsen",
          "Albrektsen",
          "Alexandersen",
          "Ali",
          "Als",
          "Alstrup",
          "Amstrup",
          "Andersen",
          "Anderson",
          "Andersson",
          "Andreasen",
          "Andreassen",
          "Andresen",
          "Anker",
          "Ankersen",
          "Antonsen",
          "Appel",
          "Arildsen",
          "Asmussen",
          "Astrup",
          "Auken",
          "Axelsen",
          "Bach",
          "Bachmann",
          "Bager",
          "Bagge",
          "Bagger",
          "Bak",
          "Balle",
          "Balling",
          "Balslev",
          "Bang",
          "Bank",
          "Banke",
          "Basse",
          "Bauer",
          "Baun",
          "Bay",
          "Bech",
          "Beck",
          "Becker",
          "Begum",
          "Behrens",
          "Beier",
          "Bek",
          "Bekker",
          "Bendix",
          "Bendixen",
          "Bendsen",
          "Bendtsen",
          "Bengtsen",
          "Bengtson",
          "Bengtsson",
          "Bennedsen",
          "Bennetsen",
          "Bentsen",
          "Bentzen",
          "Berg",
          "Berger",
          "Bergmann",
          "Bering",
          "Berntsen",
          "Bertelsen",
          "Berthelsen",
          "Bertram",
          "Beyer",
          "Bidstrup",
          "Bigum",
          "Bilde",
          "Bille",
          "Binderup",
          "Birch",
          "Birk",
          "Bisgaard",
          "Bitsch",
          "Bjerg",
          "Bjerre",
          "Bjerregaard",
          "Bjerring",
          "Bjerrum",
          "Bjørn",
          "Blaabjerg",
          "Bladt",
          "Bloch",
          "Blom",
          "Bock",
          "Boel",
          "Boesen",
          "Boisen",
          "Boje",
          "Bojesen",
          "Bojsen",
          "Boll",
          "Bonde",
          "Bondesen",
          "Borch",
          "Borg",
          "Bork",
          "Borre",
          "Borregaard",
          "Borup",
          "Bossen",
          "Boye",
          "Boysen",
          "Bramsen",
          "Brandstrup",
          "Brandt",
          "Brask",
          "Bredahl",
          "Breum",
          "Brinch",
          "Brink",
          "Brix",
          "Bro",
          "Broberg",
          "Broch",
          "Brodersen",
          "Broe",
          "Brogaard",
          "Bruhn",
          "Brun",
          "Bruun",
          "Bruus",
          "Brødsgaard",
          "Brøgger",
          "Brøndum",
          "Buch",
          "Bugge",
          "Buhl",
          "Bundgaard",
          "Busch",
          "Busk",
          "Butt",
          "Buur",
          "Buus",
          "Byskov",
          "Bæk",
          "Bækgaard",
          "Bærentsen",
          "Bødker",
          "Bøgh",
          "Børgesen",
          "Børsting",
          "Bülow",
          "Callesen",
          "Carlsen",
          "Carlson",
          "Carlsson",
          "Carstens",
          "Carstensen",
          "Caspersen",
          "Celik",
          "Chen",
          "Christensen",
          "Christiansen",
          "Christoffersen",
          "Christophersen",
          "Clausen",
          "Clemensen",
          "Clemmensen",
          "Conradsen",
          "Corneliussen",
          "Cramer",
          "Dahl",
          "Dahlgaard",
          "Dalby",
          "Dalgaard",
          "Dall",
          "Dalsgaard",
          "Dam",
          "Damborg",
          "Damgaard",
          "Damkjær",
          "Damm",
          "Damsgaard",
          "Danielsen",
          "Daugaard",
          "Davidsen",
          "Degn",
          "Dehn",
          "Deleuran",
          "Demant",
          "Demir",
          "Dencker",
          "Dideriksen",
          "Didriksen",
          "Dinesen",
          "Dissing",
          "Ditlevsen",
          "Djurhuus",
          "Dogan",
          "Dohn",
          "Drachmann",
          "Dreier",
          "Drejer",
          "Dreyer",
          "Due",
          "Dueholm",
          "Duelund",
          "Dupont",
          "Duus",
          "Dyhr",
          "Døssing",
          "Ebbesen",
          "Ebsen",
          "Edvardsen",
          "Egeberg",
          "Egelund",
          "Egholm",
          "Ehlers",
          "Ejlersen",
          "Elbæk",
          "Elgaard",
          "Eliasen",
          "Elkjær",
          "Ellegaard",
          "Emborg",
          "Enemark",
          "Enevoldsen",
          "Engberg",
          "Engel",
          "Engstrøm",
          "Erichsen",
          "Eriksen",
          "Eriksson",
          "Erlandsen",
          "Ernst",
          "Ernstsen",
          "Esbensen",
          "Eskesen",
          "Eskildsen",
          "Espensen",
          "Espersen",
          "Faber",
          "Fabricius",
          "Falk",
          "Falkenberg",
          "Fallesen",
          "Farah",
          "Feddersen",
          "Fenger",
          "Filtenborg",
          "Fink",
          "Fischer",
          "Fisker",
          "Flindt",
          "Fog",
          "Foged",
          "Fogh",
          "Foldager",
          "Foss",
          "Franck",
          "Frandsen",
          "Frank",
          "Frantzen",
          "Frederiksen",
          "Friedrichsen",
          "Friis",
          "Frisk",
          "From",
          "Frost",
          "Frølund",
          "Fuglsang",
          "Funch",
          "Funder",
          "Futtrup",
          "Fyhn",
          "Gaarde",
          "Gade",
          "Gadegaard",
          "Gammelgaard",
          "Geertsen",
          "Geisler",
          "Gertsen",
          "Gissel",
          "Glerup",
          "Glud",
          "Gormsen",
          "Gotfredsen",
          "Gottlieb",
          "Gram",
          "Grau",
          "Graversen",
          "Gravesen",
          "Gravgaard",
          "Green",
          "Gregersen",
          "Greve",
          "Groth",
          "Grøn",
          "Grønbech",
          "Grønborg",
          "Grønbæk",
          "Grøndahl",
          "Grønhøj",
          "Grønkjær",
          "Grønlund",
          "Grønning",
          "Guldager",
          "Guldborg",
          "Guldbrandsen",
          "Guldbæk",
          "Gundersen",
          "Gustafsson",
          "Gustavsen",
          "Gylling",
          "Haagensen",
          "Haahr",
          "Haaning",
          "Haastrup",
          "Hagen",
          "Hahn",
          "Halberg",
          "Hald",
          "Halkjær",
          "Hall",
          "Halvorsen",
          "Hamann",
          "Hammer",
          "Handberg",
          "Hansen",
          "Hansson",
          "Harbo",
          "Harboe",
          "Harder",
          "Hartmann",
          "Hartvig",
          "Hartvigsen",
          "Haslund",
          "Hassan",
          "Hassing",
          "Haugaard",
          "Hauge",
          "Have",
          "Hede",
          "Hedegaard",
          "Hegelund",
          "Heide",
          "Hein",
          "Hejlesen",
          "Helbo",
          "Hemmingsen",
          "Hendriksen",
          "Henneberg",
          "Henningsen",
          "Henriksen",
          "Hermann",
          "Hermansen",
          "Herskind",
          "Hertz",
          "Hesselberg",
          "Hessellund",
          "Hildebrandt",
          "Hinrichsen",
          "Hjelm",
          "Hjort",
          "Hjorth",
          "Hjortshøj",
          "Hoff",
          "Hoffmann",
          "Holck",
          "Holdt",
          "Holgersen",
          "Hollænder",
          "Holm",
          "Holmberg",
          "Holme",
          "Holmgaard",
          "Holst",
          "Holt",
          "Honore",
          "Honoré",
          "Hoppe",
          "Horn",
          "Hougaard",
          "Houmann",
          "Hove",
          "Hovgaard",
          "Hovmand",
          "Hussain",
          "Hussein",
          "Husted",
          "Husum",
          "Huynh",
          "Hvass",
          "Hvid",
          "Hvidberg",
          "Hviid",
          "Hyldgaard",
          "Hyllested",
          "Håkansson",
          "Høeg",
          "Høegh",
          "Høgh",
          "Høier",
          "Høj",
          "Højberg",
          "Højbjerg",
          "Højer",
          "Højgaard",
          "Højland",
          "Højlund",
          "Højrup",
          "Høng",
          "Høst",
          "Høy",
          "Høyer",
          "Ibrahim",
          "Ibsen",
          "Ipsen",
          "Iqbal",
          "Isaksen",
          "Ismail",
          "Iversen",
          "Jacobsen",
          "Jakobsen",
          "Jama",
          "Jansen",
          "Jantzen",
          "Jensby",
          "Jensen",
          "Jeppesen",
          "Jepsen",
          "Jespersen",
          "Jessen",
          "Jochumsen",
          "Joensen",
          "Johannesen",
          "Johannessen",
          "Johannsen",
          "Johansen",
          "Johansson",
          "Johnsen",
          "Johnson",
          "Jokumsen",
          "Jonassen",
          "Jonasson",
          "Jonsson",
          "Josefsen",
          "Josephsen",
          "Juel",
          "Juhl",
          "Junker",
          "Just",
          "Justesen",
          "Juul",
          "JÃ¸rgensen",
          "Jæger",
          "Jønsson",
          "Jørgensen",
          "Jürgensen",
          "Kaae",
          "Kaas",
          "Kamp",
          "Kamper",
          "Kanstrup",
          "Kappel",
          "Kara",
          "Karlsen",
          "Karlsson",
          "Karstensen",
          "Kaspersen",
          "Kastrup",
          "Kaya",
          "Kejser",
          "Keller",
          "Khan",
          "Kiilerich",
          "Kilic",
          "Kirk",
          "Kirkeby",
          "Kirkegaard",
          "Kjeldgaard",
          "Kjeldsen",
          "Kjellerup",
          "Kjer",
          "KjÃ¦r",
          "Kjær",
          "Kjærgaard",
          "Kjærsgaard",
          "Kjærulff",
          "Kjøller",
          "Klausen",
          "Klavsen",
          "Klein",
          "Klemmensen",
          "Klinge",
          "Klint",
          "Klit",
          "Klitgaard",
          "Kloster",
          "Knudsen",
          "Koch",
          "Kock",
          "Koefoed",
          "Kofod",
          "Kofoed",
          "Kok",
          "Kokholm",
          "Kold",
          "Kolding",
          "Kondrup",
          "Konradsen",
          "Kornum",
          "Korsgaard",
          "Korsholm",
          "Krabbe",
          "Krag",
          "Kragelund",
          "Kragh",
          "Kramer",
          "Krarup",
          "Krause",
          "Krebs",
          "Kring",
          "Kristensen",
          "Kristiansen",
          "Kristoffersen",
          "Krog",
          "Krogh",
          "Krogsgaard",
          "Kromann",
          "Kronborg",
          "Kruse",
          "Kryger",
          "Krøyer",
          "Krüger",
          "Kudsk",
          "Kure",
          "Kusk",
          "Kvist",
          "Kyed",
          "Køhler",
          "Ladefoged",
          "Ladegaard",
          "Lam",
          "Lambertsen",
          "Lang",
          "Langballe",
          "Lange",
          "Langhoff",
          "Langkjær",
          "Larsen",
          "Larsson",
          "Lassen",
          "Lau",
          "Lauesen",
          "Laugesen",
          "Lauridsen",
          "Lauritsen",
          "Lauritzen",
          "Laursen",
          "Lausen",
          "Lausten",
          "Laustsen",
          "Le",
          "Lehmann",
          "Lemming",
          "Lentz",
          "Lerche",
          "Leth",
          "Li",
          "Lillelund",
          "Lind",
          "Lindahl",
          "Lindberg",
          "Linde",
          "Lindegaard",
          "Lindgaard",
          "Lindgreen",
          "Lindgren",
          "Lindhardt",
          "Lindholm",
          "Lindstrøm",
          "Linnet",
          "List",
          "Lodberg",
          "Loft",
          "Lohmann",
          "Lohse",
          "Lomholt",
          "Lorentsen",
          "Lorentzen",
          "Lorenzen",
          "Ludvigsen",
          "Lund",
          "Lundberg",
          "Lunde",
          "Lundgaard",
          "Lundgreen",
          "Lundgren",
          "Lunding",
          "Lundqvist",
          "Lundsgaard",
          "Lundstrøm",
          "Lyhne",
          "Lykke",
          "Lykkegaard",
          "Lyng",
          "Lynge",
          "Lynggaard",
          "Lyngsø",
          "Lysgaard",
          "Madsen",
          "Magnussen",
          "Magnusson",
          "Mahler",
          "Mahmood",
          "Mahmoud",
          "Malik",
          "Malling",
          "Malmberg",
          "Marcher",
          "Marcussen",
          "Mark",
          "Marker",
          "Markussen",
          "Martens",
          "Martensen",
          "Martinsen",
          "Martinussen",
          "Mathiasen",
          "Mathiassen",
          "Mathiesen",
          "Matthiesen",
          "Matzen",
          "Meier",
          "Meincke",
          "Mejer",
          "Melchiorsen",
          "Meldgaard",
          "Melgaard",
          "Meyer",
          "Michaelsen",
          "Michelsen",
          "Midtgaard",
          "Mikkelsen",
          "Miller",
          "Moesgaard",
          "Mogensen",
          "Mohamad",
          "Mohamed",
          "Mohammad",
          "Mohr",
          "Mollerup",
          "Monrad",
          "Moos",
          "Morsing",
          "Mortensen",
          "Mose",
          "Mosegaard",
          "Mouridsen",
          "Mouritsen",
          "Mouritzen",
          "Munch",
          "Munck",
          "Munk",
          "Munkholm",
          "Munksgaard",
          "Mygind",
          "MÃ¸ller",
          "Månsson",
          "Mølgaard",
          "Møller",
          "Mønsted",
          "Mønster",
          "Mørch",
          "Mørck",
          "Mørk",
          "Mørup",
          "Müller",
          "Nedergaard",
          "Neergaard",
          "Neumann",
          "Nguyen",
          "Nicolaisen",
          "Nicolajsen",
          "Niebuhr",
          "Nielsen",
          "Niemann",
          "Nikolajsen",
          "Nilsson",
          "Nissen",
          "Noer",
          "Nordentoft",
          "Norup",
          "Nyborg",
          "Nygaard",
          "Nyholm",
          "Nymand",
          "Nymann",
          "NÃ¸rgaard",
          "Nøhr",
          "Nørby",
          "Nørgaard",
          "Nørgård",
          "Nørregaard",
          "Nørskov",
          "Oddershede",
          "Odgaard",
          "Offersen",
          "Ohlsen",
          "Olesen",
          "Olsen",
          "Olsson",
          "Omar",
          "Osman",
          "Otte",
          "Ottesen",
          "Otto",
          "Ottosen",
          "Outzen",
          "Overby",
          "Overgaard",
          "Ovesen",
          "Paaske",
          "Pagh",
          "Pallesen",
          "Palm",
          "Panduro",
          "Pape",
          "Paulsen",
          "Pedersen",
          "Pehrson",
          "Persson",
          "Peters",
          "Petersen",
          "Petersson",
          "Petterson",
          "Pettersson",
          "Pham",
          "Philipsen",
          "Pihl",
          "Pilegaard",
          "Pilgaard",
          "Ploug",
          "Polat",
          "Pontoppidan",
          "Post",
          "Poulsen",
          "Povlsen",
          "Præst",
          "Puggaard",
          "Qvist",
          "Raahauge",
          "Rafn",
          "Rahbek",
          "Randrup",
          "Rasch",
          "Rask",
          "Rasmussen",
          "Raun",
          "Ravn",
          "Refsgaard",
          "Reimer",
          "Riber",
          "Richter",
          "Riis",
          "Riise",
          "Rindom",
          "Ring",
          "Ringgaard",
          "Risager",
          "Ritter",
          "Rode",
          "Roed",
          "Rohde",
          "Rokkjær",
          "Rose",
          "Rosenberg",
          "Rosendahl",
          "Rosendal",
          "Rosenkilde",
          "Rossen",
          "Roth",
          "Ryberg",
          "Rye",
          "Rytter",
          "Rømer",
          "Rønn",
          "Rønne",
          "Sahin",
          "Sahl",
          "Saleh",
          "Salling",
          "Salomonsen",
          "Samuelsen",
          "Sandberg",
          "Sander",
          "Sauer",
          "Schack",
          "Schjødt",
          "Schmidt",
          "Schneider",
          "Schou",
          "Schrøder",
          "Schultz",
          "Schulz",
          "Schwartz",
          "Schütt",
          "Secher",
          "Seerup",
          "Sehested",
          "Sejersen",
          "Serup",
          "Severinsen",
          "Shah",
          "Simonsen",
          "Singh",
          "Sivertsen",
          "Skaaning",
          "Skaarup",
          "Skafte",
          "Skipper",
          "Skjødt",
          "Skou",
          "Skov",
          "Skovbjerg",
          "Skovgaard",
          "Skovsgaard",
          "Skriver",
          "Skytte",
          "Skøtt",
          "Slot",
          "Sloth",
          "Smed",
          "Smedegaard",
          "Smidt",
          "Smith",
          "Snedker",
          "Soelberg",
          "Sommer",
          "Sonne",
          "Staal",
          "Stage",
          "Stampe",
          "Steen",
          "Steenberg",
          "Steensen",
          "Stefansen",
          "Steffensen",
          "Stender",
          "Stensgaard",
          "Stentoft",
          "Stephansen",
          "Stilling",
          "Stokholm",
          "Storgaard",
          "Storm",
          "Stougaard",
          "Straarup",
          "Strand",
          "Strange",
          "Stryhn",
          "Strøm",
          "Stæhr",
          "Støvring",
          "Suhr",
          "Sunesen",
          "Svane",
          "Svendsen",
          "Svenningsen",
          "Svensson",
          "Svenstrup",
          "Sylvest",
          "SÃ¸ndergaard",
          "SÃ¸rensen",
          "Søby",
          "Søe",
          "Søgaard",
          "Sønderby",
          "Søndergaard",
          "Sønderskov",
          "Sønderup",
          "Sønnichsen",
          "Sørensen",
          "TRUE",
          "Tang",
          "Tange",
          "Tarp",
          "Terkelsen",
          "Terkildsen",
          "Termansen",
          "Terp",
          "Thaysen",
          "Therkelsen",
          "Therkildsen",
          "Thestrup",
          "Thiesen",
          "Thisted",
          "Thomasen",
          "Thomassen",
          "Thomsen",
          "Thorhauge",
          "Thorsen",
          "Thorup",
          "Thrane",
          "Thuesen",
          "Thybo",
          "Thygesen",
          "Thyssen",
          "Thøgersen",
          "Timm",
          "Timmermann",
          "Tobiasen",
          "Toft",
          "Tolstrup",
          "Tonnesen",
          "Topp",
          "Torp",
          "Tran",
          "Tranberg",
          "Troelsen",
          "Trolle",
          "Truelsen",
          "Turan",
          "Tychsen",
          "Tønder",
          "Tønnesen",
          "Tøttrup",
          "Udsen",
          "Ulrich",
          "Ulriksen",
          "Vad",
          "Valentin",
          "Vang",
          "Vangsgaard",
          "Vedel",
          "Velling",
          "Vendelbo",
          "Vest",
          "Vester",
          "Vestergaard",
          "Vestergård",
          "Viborg",
          "Vilhelmsen",
          "Villadsen",
          "Villumsen",
          "Vilstrup",
          "Vind",
          "Vinding",
          "Vinter",
          "Vinther",
          "Vistisen",
          "Vittrup",
          "Vognsen",
          "Voigt",
          "Voss",
          "Wagner",
          "Walther",
          "Wang",
          "Warming",
          "Weber",
          "Weinreich",
          "Weiss",
          "Werner",
          "West",
          "Westergaard",
          "Westermann",
          "Westh",
          "Wiberg",
          "Wichmann",
          "Wiese",
          "Wilhelmsen",
          "Willadsen",
          "Willumsen",
          "Wind",
          "Winkel",
          "Winkler",
          "Winther",
          "Witt",
          "Wolf",
          "Wolff",
          "Wollesen",
          "Worm",
          "Wulff",
          "Würtz",
          "Yde",
          "Yildirim",
          "Yildiz",
          "Yilmaz",
          "Yusuf",
          "Zachariassen",
          "Zacho",
          "Ziegler",
          "Zimmermann",
          "Ã˜stergaard",
          "Ørum",
          "Øster",
          "Østerby",
          "Østergaard",
          "Østergård",
        ],
    };
  }

  @override
  List<String> get telephoneFormats {
    return const [
      "## ## ## ##",
      "## ### ###",
      "#### ####",
      "+45 ### ### ###",
    ];
  }

  @override
  List<String> titles({required Gender gender, required TitleType titleType}) {
    return switch (gender) {
      Gender.female => titleType == TitleType.typical
          ? const ["Fru.", "Frk."]
          : const [
              "Cand.oecon.",
              "Cand.polit.",
              "Cand.psychol.",
              "Cand.scient.",
              "Cand.scient.pol.",
              "Cand.med.",
              "Civiløkonom",
            ],
      Gender.male => titleType == TitleType.typical
          ? const ["Hr."]
          : const [
              "Cand.oecon.",
              "Cand.polit.",
              "Cand.psychol.",
              "Cand.scient.",
              "Cand.scient.pol.",
              "Cand.med.",
              "Civiløkonom",
            ]
    };
  }

  @override
  List<String> get universities {
    return const [
      "Center for Elektronnanoskopi",
      "DTU Fødevareinstituttet",
      "DTU Veterinærinstituttet",
      "Danmarks Tekniske Informationscenter",
      "IT-Universitetet i København",
      "Institut for Akvatiske Ressourcer",
      "Institut for Byggeri og Anlæg",
      "Institut for Elektroteknologi",
      "Institut for Fotonik",
      "Institut for Fysik",
      "Institut for Kemi",
      "Institut for Kemiteknik",
      "Institut for Matematik og Computer Science",
      "Institut for Mekanisk Teknologi",
      "Institut for Mikro- og Nanoteknologi",
      "Institut for Rumforskning og -teknologi",
      "Institut for Systembiologi",
      "Institut for Systemer",
      "Institut for Transport",
      "Institut for Vand og Miljø",
      "Københavns Erhvervsakademi",
      "Metropolitan University College",
      "Nationallaboratoriet for bæredygtig energi",
      "University College Capital",
      "University College Lillebælt",
      "University College Sjælland",
      "University College Syd",
      "University College of Northern Danmark",
      "VIA University College",
      "Vestjylland University College",
    ];
  }

  @override
  List<String> get viewsOn {
    return const [
      "Kompromissøgende",
      "Meget negativ",
      "Negativ",
      "Neutral",
      "Positiv",
    ];
  }

  @override
  List<String> get worldviews {
    return const [
      "Agnosticism",
      "Ateisme",
      "Buddhisme",
      "Deisme",
      "Katolicisme",
      "Kungfutses lærersætninger",
      "Ortodoks",
      "Panteisme",
      "Protestantisme",
      "Sekulær Humanisme",
    ];
  }
}
