import 'package:imitatio/src/datasets/models/finance.dart';

class DaFinanceData extends FinanceDataset {
  @override
  List<String> get banks {
    return const [
      "Danske Bank",
      "Jyske Bank",
      "Sydbank",
      "Nykredit Bank",
      "Nordea Bank Danmark",
      "Arbejdernes Landsbank",
      "Sparekassen Kronjylland",
      "Sparekassen Vendsyssel",
      "Spar Nord Bank",
      "Sparekassen Sjælland-Fyn",
      "Ringkjøbing Landbobank",
      "Den Jyske Sparekasse",
      "Sparekassen Thy",
      "Sparekassen Djursland",
      "Sparekassen Balling",
      "Vestjysk Bank",
      "Sparekassen Himmerland",
      "Sparekassen Sønderjylland",
      "Sparekassen Midtjylland",
      "Sparekassen Hvetbo",
      "Sparekassen Fyn",
      "Sparekassen Salling",
      "Sparekassen Sogn og Fjordane",
      "Sparekassen Nordjylland",
      "Sparekassen Lolland",
      "Sparekassen Sørby",
      "Sparekassen Vesterbro",
      "Sparekassen Faaborg",
      "Sparekassen Løgstør",
      "Andelskassen Fælleskassen",
      "Lollands Bank",
      "Sparekassen Thyholm",
      "Andelskassen J.A.K.",
      "Den Jyske Sparekasse",
      "Andelskassen Løgumkloster",
      "Andelskassen Mors",
      "Andelskassen Arbejdernes Andelskasse",
      "Andelskassen Regstrup",
      "Sparekassen Sæby",
      "Sparekassen Hobro",
      "Sparekassen Hjørring",
      "Sparekassen Sjælland",
      "Sparekassen Væggerløse",
      "Sparekassen København",
      "Sparekassen Østjylland",
      "Andelskassen Fællesforeningen for Danmarks Brugsforeninger",
      "Sparekassen Østjylland",
      "Sparekassen Thy",
      "Lån og Spar Bank",
      "Fynske Bank",
    ];
  }

  @override
  List<String> get companies {
    return const [
      "A. Enggaard",
      "A. Enggaard Holding",
      "A.P. Møller Holding",
      "AVK",
      "Actona",
      "Andelsbevægelsen i Danmark",
      "Arkil",
      "Arp-Hansen Hotel Group",
      "BC Hospitality Group",
      "Bach Gruppen",
      "Bandholm Frugtplantage",
      "Barslund",
      "Bauer Media Danmark",
      "Behandlingscenter Tjele",
      "Beierholm",
      "Bolius",
      "Bone's",
      "Bruger:Mservices/sandkasse",
      "Brugsforening",
      "Børsinformation",
      "C.W. Obel Ejendomme",
      "COMET",
      "COMET",
      "CapNordic",
      "Cego",
      "Center for Offentlig Kompetenceudvikling",
      "Christiani & Nielsen",
      "Christiani & Nielsen",
      "Cirkus Arena",
      "Cirkus Benneweis",
      "Cirkus Krone",
      "Cirkus Kæphøj",
      "Civitas",
      "Coop Norden",
      "DEIF",
      "DHI",
      "DanID",
      "Danbyg Totalentreprise",
      "Danhostel",
      "Danhostel",
      "Danish American Prospecting Co",
      "Danish Arctic Contractors",
      "Danlamp",
      "Dansk Autohjælp Holding",
      "Dansk Netværk",
      "Dansk Retursystem",
      "Dantek Library Solutions",
      "De Danske Sømandshjem",
      "Debito inkasso",
      "Det Historiske Hus",
      "Det Kjøbenhavnske Bygge-Selskab",
      "Dica",
      "Diners Club",
      "Discovery Networks Danmark",
      "Domea",
      "E. Pihl & Søn",
      "Elite Model Management",
      "Endomondo",
      "Equinox Fitness",
      "Esbjerg Oilfield Services",
      "Eurodan-huse",
      "Eurodan-huse",
      "Europæiske ERV",
      "Faxe Kalkbrud",
      "Fitness World",
      "Fleggaard",
      "Fleggaard Holding",
      "Flügger farver",
      "ForbrugerLaboratoriet",
      "Frode Laursen",
      "Frode Laursen-Gruppen",
      "Gaia Solar",
      "Garantifonden for Indskydere og Investorer",
      "Gazellevirksomhed",
      "Godkendt Teknologisk Serviceinstitut",
      "Gosh",
      "Greens",
      "Hans & Jørgen Larsen",
      "Hasle Klinker- og Chamottestensfabrik",
      "Helnan Holding",
      "Hovedstadens Brugsforening",
      "HusCompagniet",
      "Højgaard & Schultz",
      "ILVA",
      "Implement Consulting Group",
      "Innovation Lab",
      "Integro",
      "JAK Danmark",
      "Jacoform",
      "Jebru Inspektion",
      "Jensens Bøfhus",
      "KPC BYG",
      "KPC BYG",
      "KPK Vinduer",
      "Kagerup Savværk",
      "Kampsax",
      "Kamstrup",
      "KasiGroup",
      "KasterGaardbo",
      "Kirkbi",
      "Kiwi Danmark",
      "Kooperativ",
      "L'easy",
      "LEAD Agency",
      "Landbrugs Medierne",
      "Laserdisken",
      "Lessor",
      "Lomax",
      "Louis Nielsen",
      "Loventa",
      "Lygas Nordvestsjælland",
      "M. Goldschmidt Holding",
      "M. Larsen Vognmandsfirma",
      "MBO Group",
      "MT Højgaard",
      "Making a Difference",
      "Mindshare",
      "Monberg & Thorsen",
      "Monberg & Thorsen",
      "Monstrum",
      "NKT Flexibles",
      "NKT Holding",
      "NP Hotels",
      "Netværk Danmark",
      "Nordheat",
      "Nordisk Film Lab",
      "Nordiske Medier",
      "Nordjyske Holding",
      "Normal",
      "Novo",
      "Nyt Syn",
      "PARKEN Venues",
      "Patriotisk Selskab",
      "Pedershaab",
      "Per Aarsleff",
      "Pilgrim smykker",
      "Polfoto",
      "Potagua",
      "Profil Optik",
      "RGS 90",
      "Rasmus Holbeck",
      "Rasmussen & Schiøtz",
      "Reitz Medaljer",
      "Rossini Caviar",
      "Ruko",
      "SAS Ground Handling",
      "SEAS-NVE",
      "SOS International",
      "Saabye & Lerche",
      "Satair",
      "Satcom1",
      "Service & Co",
      "Sinatur",
      "Stauning Whisky",
      "Steen & Strøm",
      "Sticks'n'Sushi",
      "Sylvester Hvid",
      "Synoptik",
      "Søstrene Grene",
      "TR Granit",
      "TTS",
      "Teknologisk Institut",
      "Teller",
      "Thiele",
      "Thisted Arbejderforening",
      "Ticket to heaven",
      "Tiger",
      "Tip Top Trafik",
      "Transatlantisk Kompagni",
      "Unicon",
      "VP Securities",
      "Vaffelbageriet",
      "Veggerby Sport & Kultur",
      "Vester Ulslev Brugsforening",
      "Vestforbrænding",
      "Waterfront Communications",
      "Wemind",
      "Wright, Thomsen & Kier",
      "Zenvo",
      "Zleep Hotels",
    ];
  }

  @override
  List<String> companyTypes({required bool isAbbr}) {
    return switch (isAbbr) {
      true => const [
        "A.M.B.A.",
        "A/S",
        "ApS",
        "F.M.B.A.",
        "G/S",
        "I/S",
        "IVS",
        "K/S",
        "P/S",
        "S.M.B.A.",
      ],
      false => const [
        "Aktieselskab",
        "Andelsselskab med begrænset ansvar",
        "Anpartsselskab",
        "Enkeltmandsvirksomhed",
        "Forening",
        "Forening med begrænset ansvar",
        "Gensidigt selskab",
        "Interessentskab",
        "Iværksætterselskab",
        "Kommanditselskab",
        "Partnerselskab or Kommanditaktieselskab",
        "Selskab med begrænset ansvar",
      ],
    };
  }

  @override
  String get currencyCode => "DKK";

  @override
  String get currencySymbol => "kr";
}
