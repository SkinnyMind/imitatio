...
void main() {
  group('Italy', () {
    ...
    test('returns fiscal code', () async {
      final maleResult = italy.fiscalCode(gender: Gender.male);
      expect(
        RegExp(r'^[A-Z]{6}\d{2}[A-EHLMPR-T][0123][0-9][A-MZ]\d{3}[A-Z]$')
            .hasMatch(maleResult),
        true,
      );
      ...
      expect(
        RegExp(r'^[A-Z]{6}\d{2}[A-EHLMPR-T][4567][0-9][A-MZ]\d{3}[A-Z]$')
            .hasMatch(femaleResult),
        true,
      );

      expect(
        seededItaly.fiscalCode(),
        equals(seededItaly.fiscalCode()),
      );
    });
  });
}
